...
| 3R | /qa | tests/ | **Oracle hardening + cross-ownership ratification** (review F2/F3/F4/F6, must land BEFORE Wave 6 B1-be): enforce config pins in `clif_golden.sh` + smoke (`CRANELISP_CAPTURE_BORROW` live hole, `CRANELISP_NO_LENIENT`, `CRANELISP_NONATOMIC_RC`); non-empty-capture guard in `dump()` (the false-green class); adopt `--no-cache` capture (verified zero golden churn) making duplicate frames a hard error; ratify /dev(backend)'s edits to `clif_golden.sh`/smoke/MANIFEST. | in-progress |
