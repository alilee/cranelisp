...
| /review | cranelisp-typecheck | change-set review | **done** — approve; 0 Blockers, 3 Important (incl. a probe-found LIVE defect), 3 Minor |
| /qa | — | MS-P7 attribution from the brief (wave assignment or attributed carry) | **done** — W7 typecheck rider (uaf, ownership MayAliasOf); Important-3 → probes SW-1/SW-2 |
...
| /review | cranelisp-backend | change-set review (incl. the §13.5 unit matrices) | **done** — approve-w/-required-fixes; 1 Blocker (0691 UAF) + 0692 regression must fix in-wave; W-B5 deferral ENDORSED |
| /dev | cranelisp-backend | 0691+0692+0695 fix change-set (pins first) | **done** — committed `d9de24c9`; 3 pins RED→GREEN, zero regressions (baseline diff), FIXMEs deleted; W4 CLOSED |
...
| /review | src/ | change-set review | **done** — approve-w/-required-fixes; no Blocker; 0698/0699/0700 filed; census gap caught (`commit_staging_to_live` unrouted — the suspected writer) |
...
| /review | cranelisp-frontend | change-set review | **done** — approve-w/-required-fixes, no Blocker; 0702 (dotted-binder axis vs §5 MUST) + 0703 + 0704 filed; 0701 fixture must land in-wave |

### W7 — Sweeps + acceptance (Phase 5 close)
| Skill | Crate | Task | Status |
|---|---|---|---|
| /design | cranelisp-typecheck | P26 full sweep + 0653 helper-classification sweep incl. rows SW-1/SW-2 (carrier acceptance; register updates) + **W3-review Important-2**: scribe the settled three-invocation harvest shape + idempotence obligations OR design the single-settlement convergence (4th window forces the /arch class ruling; cost note: per-invocation `seen` doubles mono-recheck work) | pending |
| /dev | cranelisp-typecheck | **MS-P7 fix LEADS** (ownership `MayAliasOf` projection-out; falsifiability first; flip = safety-lane ×3 + MS-P6 re-plant) → **W3-review Important-1 fix** (trait-shadow mis-dispatch at infer.rs:966 — consult the carrier verdict; "name is TRIGGER, carrier is IDENTITY") + **Important-3 extraction** (the Ruling-5 discriminator, 3→1 sites; the I-1 fix is its 4th consumer) + Minor-1 normalization spread + Minor-2 rustdoc clause → swallow-probe follow-through (F-D2-11/12 if demonstrated) → 0590 only if capacity remains | pending |
...
| W3 | /review | settlement-drain review (078d324b) | shim default | shim default | — |
| W3 | /qa | MS-P7 attribution + Important-3 swallow-sibling disposition | shim default | shim default | — |
| W3+ | /testing | rider batch: BI-H-heap + trait-shadow pin + M2-TP + swallow probes | shim default | shim default | — |
| W4 | /dev | backend consume family W-B1→B5 + F-R1 + MS-P8 | shim default | shim default | — |
| W4 | /review | consume-contract review (f9435b37) | shim default | shim default | — |
| W4 | /dev | 0691/0692/0695 fix change-set (pins-first) | shim default | shim default | — |
| W5 | /dev | src/ Track C (C1–C5) | shim default | shim default | — |
| W5 | /review | src Track C review (58ac8e46) | shim default | shim default | — |
| W6 | /dev | frontend Track D (W-D1 + audit batch + W-D2) | shim default | shim default | — |
| W6 | /review | frontend Track D review (8b2c3e20) | shim default | shim default | — |
| W6+ | /testing | cleanup batch: 0701 + 0690/m1 re-plants + 5 assertion updates | shim default | shim default | — |
...
| W7 | /review | final wave review (68cd7a96 + 7c2d5168 + 89d2f09c) | shim default | shim default | — |
| W7 | /testing | 0706 chained-face pins + cert runs (parallel) | shim default | shim default | — |
| W7 | /qa | 0706 adjudication + plan close (parallel, no-cargo) | shim default | shim default | — |
| W7 | /dev | 0693 deferral record (one file, no-cargo) | shim default | shim default | — |
| 6a | /audit | cranelisp-typecheck rotation (read-only) | shim default | shim default | — |
| 6a | /repl | full assessment + 0700 ruling | shim default | shim default | — |
| 6a | /docs | full assessment | shim default | shim default | — |
| 6a | /stdlib | full assessment | shim default | shim default | — |
| 6a | /examples | full assessment | shim default | shim default | — |
| 6a | /port | full assessment | shim default | shim default | — |
| 6b | /qa | six-FIXME disposition batch (0708/0709/0712/0713/0719/0720) | shim default | shim default | — |
| 6b | /docs | plan execution items 1–3,5 (item 4 gated) | shim default | shim default | — |
| 6b | /port | doc items 1–2 (3–4 gated) | shim default | shim default | — |
| 6b | /repl | demo beats + spec rows + full replay | shim default | shim default | — |
| 6b | /stdlib | io.cl simplification + recipe note | shim default | shim default | — |
| 6b | /examples | 29-annotations extension | shim default | shim default | — |
| 6b | /docs | item 4 (0712-unblocked) | shim default | shim default | — |
| 6b | /port | items 3–4 (rulings-unblocked) | shim default | shim default | — |
| 6b | /testing | §12 pin batch + examples reconciliation + impl-redef pin | shim default | shim default | — |
...
- 2026-07-20: **6b batch B DONE (5 proxies)**: /repl demo beats + §5.4 reader-diagnostics spec row + §15.2.2 settled, 12/12 replay green (own commit `33e813e3`); /stdlib io.cl bare-`Some` simplification verified both modes + recipe note; /examples 29-annotations corrected + space sub-test (120 both modes) + 0463 re-check appended; /docs ctor rough-edge deleted; /port ruling-sharpened framings + 0720 measurement note. **NEW /repl probe finding: `impl` REDEFINITION SILENTLY IGNORED** (confirmation prints, first impl still dispatches — 12 not 7) — two-sided semantics fork (hot-reload like defn §18 vs immutable-per-session with a misleading confirmation) = **OPEN USER QUESTION at Phase 7**; polarity-safe pin in the /testing batch (RED under both rulings). Also: let-binder reject says "definition head" (0711 family, noted).
- 2026-07-20: **6b /qa disposition batch DONE (plan §12 = durable record)**: 0712 VERIFIED FIXED e2e (boundary = partial-application only; unblocks docs+stdlib; deleted); **0719 RULED DEFECT from spec §5.1.2** (equivalence rule — no user question; acceptance bar upgrades to equivalence-twin cells; the discriminating axis is UNNAMED — /testing owns the reduction; retargeted /testing); **0720 = residual never-freed face, minimal repro found: ADT-wrapPED COW supersede tail loop leaks 2/iteration** (W4 released bare-vec params only; retargeted /testing → S115 backend one RC-release sweep with the entry-payload leak); 0709 settled by §7.1 occurrence rule (silent-accept at declaration; retargeted /testing); **0708 = LANGUAGE-NORMATIVE → retargeted /spec, S115 user framing** (§1.4.5 vs §2.3.8 pull apart at the macro-arg seam); 0713 folded into 0604 (S115 plan re-based: structural gate on merits + synthesized trigger; dead recipe demoted); examples 119→120 reconciliation = binding /testing rider. Intended new REDs when pinned: 0709 ×2, 0719 ×1, 0720 ×2 (+0708 ×1 post-ruling).
- 2026-07-20: **6a /port DONE**: exemplar correct both modes (byte-identical output; the `--link` failure was the KNOWN piecemeal-build-skew gotcha, not a defect); NO carry-shape exposure (decisive: user_bin --link exit 0 vs the chained-face 134). **0719 (/qa): the MC-X4 hold is NOT released for the exemplar** — the green battery consumes the multi-sig return immediately; the exemplar's free-var-through-bound-parameter-to-downstream-consumer DISTANCE variant still fails (now as a LOCATED carrier-gate error, progress from the old codegen leak — but blocked; a coverage-by-variants gap). **0720 (/qa): ~11.8k objects/SOLVE RC leak** in the COW solve path (linear per-solve, serial≡parallel, vs 1-object baseline floor) — residual never-freed face vs benign 0408 copy-churn, discriminators recorded. 6b = small doc wave (falsified collapse-record correction + skew Known Issue; items 3–4 gated on the /qa rulings).
- 2026-07-20: **6a /examples DONE**: all 37 green BOTH modes (exit table current; the platform-prereq `--link` note is measurement, not divergence); no carry-shape exposure (14-vecs chained COW stress-tested 70 runs); TEACHING GAP: 29-annotations "(no space)" prose contradicts the S114 ruling — 6b = extend-only (prose fix + runnable space sub-test, exit 119→120 with a /qa reconciliation handoff + the stable `/bar` message beat; 0710/0711 consumed as sequencing constraints); delta-4 example declined with rationale; 0463 deferral re-verified standing; ZERO new FIXMEs (clean).
- 2026-07-20: **6a /repl DONE** (own commits `e04f1ba6` + `31101126`): delta surface CONFORMS with excellent located diagnostics; ALL 12 demos green; **0700 RULED (TTY-qualify — experience design, /repl authority), scribed §15.2.2, deleted**; new findings: **0708 (/qa, Important — `:Type` NOT folded in macro-argument position**, opaque `def` diagnostic, layered frontend↔int shape), **0709 (/qa, Important — non-dispatchable nullary trait method no-impl leaks to codegen** — F-D2 degenerate corner), 0707 (/dev src, minor — restore count from record not re-parse). Archive rot confirmed on sample (ring4a 14 error-ish). 6b plan produced (capability-demo beats, not changelog demos).
- 2026-07-20: **6a /docs DONE**: zero drift in existing prose (all transcripts byte-fresh); gaps are positive (catalog entries for the new located errors + value-level binder-rule extension); **the `concurrency.md` "bare ctor crashes" warning NO LONGER REPRODUCES** (carrier work fixed it incidentally) — 0712 (/qa) gates the doc edit on e2e confirmation + the 0705 boundary; 0710/0711 (frontend diagnostic parity, minor). 6b plan produced.
- 2026-07-20: **6a /audit LANDED (`audits/cranelisp-typecheck-s114.md`, 4 recommendations → S115 Phase 1 disposal). ★ HEADLINE: FIXME 0590 IS A ZOMBIE** — the resolver convergence LANDED IN S110 (`5ed07d60`; TypeExprCtx live, mirrors + never-error `Named` arms DELETED), falsely re-dispositioned S113, carried by a five-agent chain with nobody opening the `refers_to` file; the "0590 + `_hkt`" S115 carry is PHANTOM WORK. Code grades strong (carrier flip = the second-time design; duplication + coverage strong; all 5 REDs correctly attributed); record layer weak (typecheck.md §§2–5 falsified; the S108 R-4 test split silently never executed — program/tests.rs now 10,576 lines; CLAUDE.md stale). Process feedback: verify-against-refers_to as the disposition first act; close-checklist FIXME-vs-§Delivered consistency.
- 2026-07-20: **USER: Phase 5 close APPROVED; Phase 6 = FULL pass; 0699/0703/0704 carry to S115 Phase 1 agreed; 0700 lands in Phase 6.** Advanced to Phase 6a. Batch 1 dispatched: /audit (cranelisp-typecheck rotation, read-only) + /repl (incl. 0700 ruling) + /docs. Batch 2 (/stdlib, /examples, /port) follows to moderate binary contention.
- 2026-07-20: **PHASE 5 CLOSE-OUT COMPLETE** (commits `87ed0194` + `3cdd285c`). 0706 chained-face pins landed RED ×2 + green control (probe-verified 2/2 each); MS-P7 scope-narrowed everywhere; family adjudicated S115 with the binding family-grain invariant + design-first rule; 0693 deferred with re-anchored trigger; 0706 deleted (pins are the record). **Certification: stable-REDs-exact = 5 identical across runs; named flap {0694 nullary} — RED 1-of-4 runs → routes to an S115 root-cause row per the recorded condition.** Suite at close: **5148 run / 5143 passed / 5 failed / 1 skipped**. ALL 31 inherited S113 carries FLIPPED; the 5 stable REDs are all NEW probe-discovered defects pinned this sprint, each S115-attributed with owner + class + fix constraints. Phase 5 conclusion presented to user.
- 2026-07-20: **W7 FINAL REVIEW: APPROVE-W/-REQUIRED-FIXES.** All eight priorities confirmed sound (trait-shadow carrier reads at the RIGHT spans + probed post-scope-pop; F-D2-11 no latent surfacing; 0689 fence airtight — 16/16 variants no `_ =>`; lattice + SW-1 census verified row-for-row; no recurring-class re-instantiation; suite 2× identical at the 3 carries, no flaps — 0694 verification 2/≥3). **BLOCKER (disposition-shaped) → 0706: MS-P7 family NOT closed — the W7 fix protects the immediate projection link only; chained may-alias shapes (nested projection; let-chained) still abort --link 134** (not a regression — too-many-decs pre-dates; the flat face is genuinely fixed). Required before close: chained-face pins (+2 REDs), scope-narrowed FIXED language, /qa family adjudication (S115 carry with the family-grain invariant), 0693 explicit re-anchored deferral (its before/with-W7 clause was overtaken; mask verified held). Close-out trio dispatched in parallel.
- 2026-07-20: **W7 typecheck lead DONE, committed `68cd7a96` — suite at 2 stable REDs + known flaps.** MS-P7 FIXED (falsifiability confirmed absent; scoped ProjectionOf fix — the burned first attempt proved the discriminator is the CONSUMER, projection-drop vs transfer-to-recur; l_c3 guard pinned; no second bump — the open 22 window + BUILD_ID cover the value change, CS-1 verified clear). Trait-shadow: ONE discriminator ×5 sites; the REAL seams were POST-INFERENCE (deferred-trait-calls + auto-curry) — fixed via carrier verdicts; the auto-curry face re-attributed to a backend AutoCurry-over-local GOT-slot gap (**0705**, non-trait repro — MC-E1 non-flip-is-evidence). F-D2-11 ×3 fixed (Result-widening propagation). **0694 flap GREEN both runs + isolation — stabilized.** 0693 mask NOT lifted (fix narrow). typecheck 789/789. /testing close-rider dispatched (MS-P6 re-plant + the 2 §11 pin-NOW cells).
- 2026-07-20: **Pre-W7 /qa disposition batch DONE (evidence-only; plan §11 = durable record).** 0698 folded into 0604 → deleted; **0604 re-targeted /dev(src), scheduled S115 early** (3-sprint-carry escalation flag; next step = MODULE_TRACE + declared-export-closure diagnostic at `commit_staging_to_live` + ONE deterministic-recipe run — the trace names the writer; get_mut deadlock hazard scribed); 0694 re-scoped to the nullary flap only (W7 may stabilize; ≥3-run Phase-7 verification; **standing counting convention: stable-REDs-exact + NAMED flap set, never one scalar**); 0702 matrix drawn as standing M3, re-targeted /spec, cells = S115 (polarity gated on the ruling); entry-Pure leak + GOT-slot carrier-loss both attributed + **pin-NOW /testing rider** (+2 intended REDs; fixes = S115 backend/typecheck); m1 retirement RULED compliant (3-prong capability-fence policy canonical in memory-safety-coverage.md §4.1).
- 2026-07-20: **W6 REVIEW: APPROVE-W/-REQUIRED-FIXES (no Blocker).** M1 grep honest (3 sanctioned raw sites); RA probes all clean (division/`/=`/`//bar` safe; RA-N5 no over-fire on compounds/stacked bounds); audit Done criteria met (one 0678 residual → 0703); W-D2 twin discipline proves seams see written source; 0701 fixture confirmed invalid per §5.2 (fix right, fixture wrong — land in-wave); suite 5135/5120/15/1 reproduced. NEW: **0702 (/qa, Important, pre-existing): the DOTTED-binder axis is unenforced vs spec §5 [S113] MUST — `(deftype A.B …)` silently mints ctor `user/B`** (the `{/, .}` × binder-position matrix owed; spec-prose/table/design three-way disagreement rides it); 0703 (/dev frontend, residual head-vocab mirrors); 0704 (/spec, §5.2.7 wording contradiction). /testing cleanup batch dispatched (0701 in-wave + 0690/m1 re-plants + 5 assertion updates).
- 2026-07-20: **W6 dev DONE, committed `8b2c3e20` — suite 5135/5120/15/1 (31 − 17 flips + 1 corpus casualty, exact).** All 17 Track-D flips green: BD-A ×6 (one `build_body_to_end` seam), deftype-ctor, RA-N1/N2/N5 + RA-N6 ×2 (ONE `consume_dotted_module_path`; `/bar` guard; bare-`:` non-type reject), M2-TP1 (converged on deftrait's reject), IQ-N ×4 (value-binder reject re-landed; NOTEs + mirror sentence retired). Audit batch 0677–0681 all landed + FIXMEs deleted (one splitter, classify_head, synth kit, dead parsers gone, peg claim + narrowing rustdoc fixed). Corpus casualty → **0701 (/testing)**: repl_persist seeds an INVALID two-bracket ctor HEAD silently accepted — the fixture is wrong, not the fix. Frontend 404/404, clippy −2. /review dispatched.
- 2026-07-20: **W5 REVIEW: APPROVE-W/-REQUIRED-FIXES (no Blocker).** All five mechanisms verified (C1 one-home + RED→GREEN confirmed by inspection; C2 exactly-+1 verified across all three allocators, no double-protect; C3 mechanics clean, `insert_detecting_ambiguity` untouched; 0671 canonical resolution P24-clean — the 5 failing assertions assert the PRE-FIX bug; 0674 gate defensible). Required follow-through filed: **0698 (/qa)** — the census MISSED `commit_staging_to_live` (worker.rs:439/:513 — the suspected writer itself; "bypasses every seam" claim unverifiable) + the W5 evidence bundle lives only in a commit message + the gate predicate provably passes the live phantom; **0699 (/design int)** — qualify-walk quote-shield gap (+3 minors); **0700 (/repl)** — §15.2.2 TTY ruling needs a durable trigger. Suite 5109/5078/31/1 reproduced exactly; 31 residuals close against the accounting. W6 dispatched.
- 2026-07-20: **W5 dev DONE, committed `58ac8e46` — suite 5109/5078/31/1.** C1: 0670 live defect DEMONSTRATED unit-RED→GREEN (escalation clause not needed); helpers one-home; W-D2 gate OPEN. C2: 0638 ×5 flipped (interior SCons spine cell pinned freed-while-reachable; protect-on-build). C3: chokepoint landed BUT **0604 RE-ATTRIBUTION: premise false (bit-and IS a bundled primitive); the phantom PUBLIC `bit-and→primitives/bit-and` entry in prelude's live table bypasses EVERY src/ install seam — writer cross-crate (typecheck staging→live) or session-side; correct check = declared-export closure not provider-existence; THIS VM reproduces 25/25 deterministically** — 0604 open, /qa re-attributes. C4 reanchor + C5 riders landed (0671 PS-D1 ×2 green). FIXMEs 0670/0671/0674/0675 deleted. **Owed queue: /qa** {0694, 0604 re-attribution, entry-return Pure leak, fn-as-value GOT-slot}; **/testing** {0690 + m1_quarantine re-plants, 5× canonical-home assertion updates}; **/repl** {0674 TTY §15.2.2 intent}. /review dispatched.
- 2026-07-20: **W4 fix change-set DONE, committed `d9de24c9` — W4 CLOSED.** 0691: exemption now scans ALL tail-call args; 0692: borrowed-mark scope-stratified (borrowed = property of a BINDER not a name, P20); 0695: exemption never applies toggle-off. 6 e2e pins (defect faces verified RED pre-fix) + 3 units; baseline diff = exactly the 3 flips, zero new failures; suite 5089 / ~5058 / 30-31 (the flap IS 0694) / 1. NEW /qa item noted: pre-existing scale-invariant toggle-off entry-return `Pure` leak (F-R1 family §6.1, reproduces at 0 iterations) — joins the /qa disposition queue (0694 + fn-as-value GOT-slot + this). W5 dispatched.
- 2026-07-20: **W4 REVIEW: APPROVE-WITH-REQUIRED-FIXES (1 Blocker, 4 Important, 2 Suggestion; FIXMEs 0691–0697 filed by /review).** Core verified sound (classifier toggle-invariant; R3 = dec-side of the escape gate, not MS-P7 compensation; F-R1 boundary probed clean; goldens certified; W-B5 deferral ENDORSED). Boundary probes found: **0691 BLOCKER — MS-P8 flush is positional-only: cross-position in-place COW → UAF regression** (toggle-off oracle exit 1; ON nondeterministic garbage); **0692 — R1 mark_borrowed is fn-lifetime + name-keyed → shadow/sibling name-reuse leaks** (regression); 0693 — R3 gate re-derives the COW-builtin from SYNTACTIC names (resolver-mirror class; latent UAF unmasks with W7's escape-fact fix; needs disagreement fence before/with W7); 0694 (/qa) — **suite count NOT reproducible: 5049/31 both runs, failing set SHIFTS** (the nullary face fails only under load — interleaving-dependent, breaks exact-ledger arithmetic); 0695 — in-place exemption × toggle-off = 1 leak/iter uncovered cell; 0696/0697 ride W-B5//design. In-wave fix dispatched: 0691+0692+0695, pins first.
- 2026-07-20: **USER: continue through the waves autonomously; check in at Phase 5 close.** (W4 review → W5 → W6 → W7 proceed without per-wave user check-ins; Phase 5 conclusion presented for the /sprint+user judgment per METHOD.)
- 2026-07-20: **W4 dev DONE, committed `f9435b37` — suite 5080 run / 5050 passed / 30 failed / 1 skipped (47 − 18 flips + 1 expected flip-hazard, exact).** The contract landed as designed: ONE liveness-only classifier (toggle-invariant by construction); R1 alias-bindings non-owning (BI-G, BI-I1 ×2 — the capture/store incs already existed, R1 completed them); R3 forwarding-suppression gated by the dec-side of the SAME COW escape gate (BI-F, BI-B-cow ×2, BI-C-off, BI-H-heap ×2, CS-1); F-R1 = protect suppressed only for entry-main tail-returned fresh construction (discriminator-confirmed rc=2; G2 preserved); MS-P8 = superseded heap loop-param flush at the TCO tail-jump (allocs==deallocs at N=20). **W-B5 patch collapse DEFERRED** (hygiene, no flips, real golden/RC regression risk — own reviewed change-set; /review to endorse or overrule). 7 golden frames re-baselined, every drift certified (flag to /testing). MS-P7 fences held (pin RED unchanged; fence-d report CONFIRMS the MayAliasOf fact absent at the consume seam — typecheck W7). FIXME 0690 filed (/testing MS-P6 re-plant — the parity cell's planted leak WAS the F-R1 fixture). Backend 427/427. /review dispatched.
- 2026-07-20: **Rider batch DONE, committed `a52d61b8` — suite 5070/5023/47/1 (38 + 9 new REDs, exact, zero regressions).** Escalations: **P1 is WRONG-ACCEPT, not a diagnostic gap** (shadowed value-position no-impl silently falls to the primitive-name fallback and returns false — the seam's own comment is false; F-D2-11 ×3 RED); **the auto-curry trait-shadow sibling ALSO misbehaves** (distinct resolution codeblock — W7 fixes both or names a P7 mirror); M2-TP2 = deftrait ALREADY rejects (mirror pressure for the W-D1 deftype fix); P2 conformant (F-D2-12 fences) BUT the true late-pinning shape is masked by a **NEW carrier-loss defect** (fn-as-value wrapper reached codegen with no GOT-slot carrier, fails even impl-present) — unpinned, needs /qa disposition (next /qa touchpoint). W4 dispatched with BI-H-heap ×2 in its flip set.
- 2026-07-20: **W3 REVIEW: APPROVE (0 Blockers, 3 Important, 3 Minor).** P26 acid test PASSES on the re-harvest (concrete types stable under the monotone subst — a pre-drain mint cannot disagree with its re-derivation; idempotence verified in code) BUT the harvest-window count grew 1→2→3 across two sprints — Important-2 routes to W7 /design (scribe-or-converge; 4th window forces an /arch class ruling). **Important-1 = probe-found LIVE defect: let-shadowed trait OPERATOR mis-dispatches (`(let [+ (fn [a b] 0)] (+ 1 2))` → 3, spec §4.6 violation)** — the post-unify resolution block keys on the raw AST name, ignores the available carrier verdict (violates §11.8.8 "name is TRIGGER, carrier is IDENTITY" + P24); pre-existing, same class the wave drains; auto-curry filler is the untested sibling. → /testing pin (rider batch) + W7 fix. Important-3: the Ruling-5 discriminator is 3× copy-pasted — extract (the I-1 fix makes 4). PS-SH1 fixture correction verified clean; suite 5053/5015/38/1 verified exact; comment sweep verified complete.
- 2026-07-20: **MS-P7 ADJUDICATED (/qa)**: owner /dev(typecheck) ownership analysis; class `uaf` (mode-divergence RETIRED — the mode axis was only the detector); the projection-out reaching context the S111 §3.7 MayAliasOf fix didn't cover (vec-set result released as ARG-TEMP). **W7 rider, priority OVER 0590.** Falsifiability clause: /dev first confirms the fact chain is absent at the publication seam; present-and-correct → re-attribute to W4 (non-flip is evidence). Flip hazard caught: the capability-green lane test uses the LIVE defect as its planted fault — /testing re-plants on synthetic in the same change-set. Important-3: BOTH swallow sites = propagation candidates (site 2 is wrong-accept-shaped — Err falls through to the primitive-name fallback); probe-first, F-D2-11/12 authored only on demonstrated gaps; P26 sweep rows SW-1/SW-2 added. W4 fences: no MS-P7 compensation in the consume contract (no pointer-equality guard, no unconditional single-dec); pin color-changes report to /qa as evidence.
- 2026-07-20: **W3 dev DONE, committed `078d324b` — suite 5053 run / 5015 passed / 38 failed / 1 skipped (44 − 6 flips, exact).** MC-X4/X4b = P26 post-settlement single-sig re-harvest (carrier-loss: pre-drain concreteness gate skipped consumers of unsettled multi-sig returns); MC-X5 = normalize-self-qualified before both overload-gate blocks; PS-SH1 ×3 = value-position gates honor local-shadow first (Ruling 5 mirror; control fences unshadowed reject); MC-X3 confirmed already closed by W2 totality (distinct root). One self-caught fixture regression fixed. Comment sweep complete (~13 sites + 2 test comments). **MS-P7 BRIEF: CLIF byte-identical across modes — NOT mode-divergence; shared-IR double-dec (COW in-place vec-set result aliases param, both dec'd) = the S111 §3.7 MayAliasOf COW-truth gap; --link abort is just glibc noticing.** /review + /qa (attribution + Important-3) dispatched in parallel.
