    ...
    let methods: Vec<String> = decl
        ...
        .map(|m| {
            if m.default_body.is_some() {
                // Show default methods with named params and "(default)" marker
                let params = m.default_param_names.join(" ");
                format!(
                    "({} [{}] {} (default))",
                    m.name,
                    params,
...
/// Recursively find type vars not already in `var_names` and assign letter names.
fn collect_extra_type_vars(
    ty: &Type,
    var_names: &mut HashMap<TypeId, String>,
    next_letter: &mut usize,
) {
    match ty {
        Type::Var(id) if !var_names.contains_key(id) => {
            var_names.insert(*id, String::from((b'a' + *next_letter as u8) as char));
            *next_letter += 1;
        }
        Type::ADT(_, args) => {
            for a in args {
                collect_extra_type_vars(a, var_names, next_letter);
            }
        }
        Type::Fn(params, ret) => {
            for p in params {
                collect_extra_type_vars(p, var_names, next_letter);
            }
            collect_extra_type_vars(ret, var_names, next_letter);
        }
        _ => {}
    }
}
...
pub fn format_type_def(td: &TypeDefInfo) -> String {
    ...
    let mut var_names: HashMap<TypeId, String> = td
        .type_params
        .iter()
        .enumerate()
    ...
    // Assign letter names to any extra type vars in constructors (e.g. existential 'b' in Bind)
    let mut next_letter = td.type_params.len();
    for ctor in &td.constructors {
        for (_, fty) in &ctor.fields {
            collect_extra_type_vars(fty, &mut var_names, &mut next_letter);
        }
    }
    ...
    let ctor_strs: Vec<String> = td
        ...
        .iter()
        .map(|ctor| {
            let base = if ctor.fields.is_empty() {
                // Nullary
                ctor.name.clone()
