...
filed_at: 2026-07-01
sprint_filed: 98
refers_to: crates/cranelisp-backend/src/compiler/apply.rs (compile_poll_effect §1049-1055 leaf-arg bake + build_poll_state_drop_glue), crates/cranelisp-backend/src/compiler/apply.rs (compile_consuming_arg_list), crates/cranelisp-backend/src/heap.rs (emit_rc_dec_guarded + the CRANELISP_RC_DEC_CHECK oracle), crates/cranelisp-intrinsics/src/{rc.rs (rc_dec_check), alloc.rs (header-integrity + freed_info), vec_runtime.rs (databuf_guard), drop.rs}, tests/launch_vec_send_corrupt.rs, tests/launch_grid_corrupt.rs, tests/exemplar_web.rs
status: open
---

> ## ✅ CONFIRMED (2026-07-01, /dev on `cranelisp-intrinsics`, layout-NEUTRAL instrumentation) — bug #2 is a **/backend poll-effect state-closure leaf-arg RC double-dec**; ALL prior hypotheses REFUTED
> Per the S98 mandate (confirm-first via layout-neutral instrumentation, iterate techniques until the exact site is pinned). Built a ladder of layout-neutral, debug/env-gated guards (side-table reads, NO redzone/padding/quarantine — so the race window stays open, unlike ASAN/`MALLOC_CHECK_`) and drove the deterministic fixture (`web_launch_vec_send_corrupt`, shrunk to vec-size 3 which STILL trips the *logical* double-dec via the oracle).
>
> **The exact defect (rc_dec_check oracle fired deterministically):** on the launched-strand teardown (`reactor::supervised` → `consume_io_tree(sub_tree)` → `consume_closure(cont1)` → cont1's JIT drop glue), a **tag-0, size-32 heap value** (`conn` — a 1-field Connection ADT) is RC-**dec'd after it was already freed**. BOTH the first free AND the stale dec are **JIT-emitted** (first free = `runtime/dealloc` called from `<unknown>` JIT code; second = a JIT closure drop-glue inline `atomic_rmw Sub`) — confirmed via a codegen-time-gated `runtime/rc_dec_check(ptr)` emitted before every inline dec (`heap.rs::emit_rc_dec_guarded`, gated on `CRANELISP_RC_DEC_CHECK`, off ⇒ byte-identical). Freed-value report: `(size=32, payload@16=0)`.
>
> **Root cause LOCATED — `cranelisp-backend/src/compiler/apply.rs::compile_poll_effect` §1049-1055:** the poll-effect (`send-conn`) state-closure bakes its leaf args into the closure env **"ownership transfer, no inc — consuming conv"**, and `build_poll_state_drop_glue` **unconditionally dec's each heap leaf arg** (that dec is run by the invariant-15 keep-alive `consume_closure` at resolve/teardown). This is correct for a **temporary** arg (`(make-resp req)` → a fresh `Response`, rc=1, genuinely transferred) but a **DOUBLE-FREE for a heap VARIABLE still owned elsewhere** — `conn`, which is simultaneously captured by the enclosing continuation `cont1 = (fn [req] …)`. The bake relies on the caller's `compile_consuming_arg_list` having inc'd non-last-use variable args; for `conn` in the doubly-nested continuation that inc is effectively absent by the time the state-closure drop glue runs, so decs exceed incs by one → `conn` freed early → cont1's teardown drop-glue dec is the UAF-write that corrupts a reused smallbin chunk → `free(): chunks in smallbin corrupted`.
>
> **Why `sleep` is load-bearing (the FIXME's own isolation):** `sleep` inserts the extra nested `(fn [_] (send-conn conn (make-resp req)))` closure level, so `conn` is captured across TWO continuation frames AND baked into the `send-conn` poll state-closure. Remove the suspension (no `sleep`) and that extra frame + poll bake collapse — clean. This is **also exactly "the invariant-15 keep-alive shifted a pre-fix UAF into a double-free"**: pre-keep-alive the state-closure was freed early (its drop glue never ran on the un-inc'd `conn`); the (correct, KEEP-IT) keep-alive now keeps the state-closure live so its drop glue *does* run and dec the un-inc'd `conn`.
>
> **REFUTED (each with hard layout-neutral evidence, so do NOT revisit):** (1) untracked Vec **data-buffer** double-free — `databuf_guard` (per-address live/freed side table in `vec_runtime.rs`, immune to address-reuse) stayed CLEAN; (2) **stale/UAF vec op** (corrupted `cap`/`data_ptr`) — `assert_live` on all four Rust vec ops CLEAN; (3) **tracked-alloc double-free** — `LIVE_ALLOCS` (now a size-map) CLEAN; (4) **overrun into a live chunk's header** — header-integrity check on dealloc + full-heap scan (`CRANELISP_HEAP_SCAN`) CLEAN; (5) **Rust-path stale dec** (`consume_io_tree`/`consume_closure`/`consume_shallow`/`atomic_dec_rc`) — `is_live` guards on all Rust dec paths CLEAN; (6) marshaling / Response-body String / `CLString` base-pointer — the victim is `conn` on the render→send path, freed+dec'd entirely by JIT RC, no marshaling involvement. The residual is NOT a data-buffer bug, NOT a reactor-consume-ordering bug, NOT a marshaling bug, NOT the (separately-refuted) borrowed-Var vec RC.
>
> **Owner = `/backend`** (`compile_poll_effect` + its `compile_consuming_arg_list` coupling), NOT intrinsics/platform. The keep-alive (invariant 15) is correct — KEEP IT.
>
> **The fix (a /backend deploy):** make the `send-conn` poll-effect state-closure balanced for a heap **variable** leaf arg — either inc each heap-typed leaf arg whose corresponding drop-glue dec is unconditional (mirroring `compile_lambda`'s capture inc, apply.rs would need the arg *expressions* for last-use discrimination so it inc's live-variable args but transfers temporaries), OR route poll-effect args through the same consuming-arg discipline that ordinary calls use and ensure it inc's `conn`. **Verify with the oracle:** `CRANELISP_RC_DEC_CHECK=1` must go from firing to silent, AND both guards flip GREEN, WITHOUT leaking the `Response` temporary (watch for a new leak in the RC-balance harvest). **Next localization technique for the exact missing-inc:** `/clif` (or `CRANELISP_CODEGEN_TRACE=1`) on `serve-loop`/`send-conn` in the shrunk fixture — inspect whether the `send-conn` arg-0 (`conn`) gets an inc at the consuming-arg site vs. the state-closure drop-glue dec.
>
> **Exact repro (deterministic):** build `-p cranelisp -p cranelisp-web`; spawn `target/debug/cranelisp --run tests/fixtures/web_launch_vec_send_corrupt/main.cl` with `CRANELISP_PORT/CRANELISP_PLATFORM_PATH=target/debug/CRANELISP_LIB=stdlib`; drive one HTTP/1.0 GET → SIGABRT (exit 134) `free(): chunks in smallbin corrupted`. Add `CRANELISP_RC_DEC_CHECK=1` → deterministic panic `STALE RC DEC (JIT inline) … Freed-value (size, payload@16) = Some((32, 0))` at the exact stale dec. (Shrinking `400`→`3` in `main.cl` still fires the oracle — small CLIF.)
>
> **Instrumentation LANDED (debug/env-gated, release byte-identical, ZERO new suite failures — 1792 pass / 2 fail = the two bug-#2 guards / 2 skip):** the guard ladder above is a permanent reproducing oracle for this defect class. `launch_grid_corrupt` + `launch_vec_send_corrupt` stay RED (unfixed); `exemplar_web::serves_form` stays `#[ignore]`'d; `0492` stays blocked. `0494` stays OPEN, **owner re-pointed to `/backend` with the exact site + oracle + next technique.**
