...
**Status:** DESIGN PROPOSAL (for user review). Concurrency-sensitive. No code changes yet.
...
> A sequence diagram (`.mmd`) is warranted; per `/arch` owner discipline it is `/arch`-authored. The existing `design/int/concurrency/dependency-protocol-target.mmd` already sketches a target dependency protocol — it should be reconciled to this in-call-stack shape as part of the implementing wave (flagged, not drawn in this proposal pass).
...
**Step 0 — sexps onto the work packet (enabling, low risk).** Change `PriorityWork::Typecheck(module)` → carry the cluster sexps (e.g. `Arc<[Sexp]>`) or have `register_module` parse-and-attach. Update `register_module_with_source`, `register_module`, the dispatch sites, and `take_priority_work*`. Remove the `module_sexps.insert(M, …)` publish at register time and the `module_sexps.get(M)` read in `handle_typecheck_work_shared`. **Risk: LOW** (mechanical data-flow change; the maps still exist for the dep half until step 2). *Concurrency hotspot: none — sexps become owned-by-packet.*

**Step 1 — lift Pass-0/1/2 into `cluster::process_cluster` (structural, build-red expected).** Move `process_cluster_with_staging`'s core + `commit_staging_to_live` into `cluster.rs`. Move the Pass-0 structural peel (import/export/mod/platform handling, `install_imports`/`install_exports`/mod-alias) from `process_module_forms` into `process_cluster`. Make `process_cluster` the live body. **Risk: HIGH** (the largest single move; ~1200 LOC of `process_module_forms` redistributes). *Hotspot: the Pass-0 `BlockAction::Block` returns become in-call-stack gap-drives (step 2 wires them).*

**Step 2 — in-call-stack gap-drive; delete the maps (the core, build-red).** Replace `ProcessResult::Blocked` + `handle_typecheck_work_shared`'s map-juggling + `register_dep_for_eval`'s republish with the single `drive_gap_to_readiness(shared, gap)` (= `ensure_registered` + `block_for_typecheck` + `wait_for_typecheck`, then `continue` the cluster retry loop). Delete `module_sexps`, `suspend_states`, `ModuleSuspendState`, `ProcessResult::Blocked`, `pass2_resume_index`, `republish_module_sexps_from_symbol_table`, `register_dep_for_eval`'s republish body. **Risk: VERY HIGH** (this is the concurrency-critical change). *Hotspot: the entire block→resume cycle; the H5 surface; OQ-1's option-(b) discipline.*
...
**Sequencing note (Plan cascade discipline):** steps 1–2 are one indivisible red→green span (the build is broken between them — expected, per the facade-walk-leaves-build-broken memory). `/dev` should land 0 → (1+2 together) → 3 → 4 → 5 → 6, regenerating the baseline only at the end. Steps 1+2 are the wave's center of gravity and its entire risk.
...
**This is S78-centerpiece-sized, NOT one more S77 wave. Recommend: land Step 0 in S77 (low-risk enabling), carry Steps 1–6 as the S78 centerpiece.**
...
**Step 0 (sexps-onto-packet) is genuinely a safe S77 wave** if S77 has room: it is mechanical, build-green, and de-risks the centerpiece by removing one of the two map payloads ahead of time. But it is optional — landing it inside S78 step 0 is equally fine.

**Counter-position considered:** could 1+2 land as one S77 wave if the wave is the *whole* sprint's remaining capacity? Only if S77 is otherwise empty and the user accepts a single-deliverable sprint. Given the risk, a dedicated S78 with explicit stress-validation gates is the safer call. Flagging both for the user / `/sprint`.

---

## 8. Cascade to the canonical set (when actioned — not in this proposal pass)

This restructure is int-interior (FIXME 0298: `SharedState`/`process_cluster`/scheduler/worker are internal, not a boundary). Expected canonical-set touches are therefore **small**:

- **`facades/int.md`** — the §"SharedState" field table loses `module_sexps` + `suspend_states` rows; the §"SharedState facade alignment plan" PIF rows for those two move from "deferred/PIF — delete via redesign" to "done"; the `process_cluster` pseudocode re-syncs to the three-pass + in-call-stack-gap shape (it is currently flagged "retained verbatim as gap-orchestration reference"). *Note:* per FIXME 0298, `facades/int.md` is itself slated to retire into `design/int/` + source rustdoc as W-Retire — so this cascade may fold into that doc-reorg rather than editing the facade in place. `/arch` + `/sprint` sequence W-Retire vs this restructure.
- **`bounded-contexts.md §6`** — §6.2 "Inter-cadence handoffs" already describes the target ("REPL submits work and waits; compilation signals when ready") in handoff-pattern terms that the in-call-stack model *realizes* — no contradiction, possibly a sharpening sentence. No invariant changes.
- **`design/int/concurrency*`** — this doc + the existing `concurrency-architecture.md §3.5`/§6.2 (which already recommend collapsing the split protocol + reducing SharedState) reconcile; `concurrency/dependency-protocol-target.mmd` re-draws to the in-call-stack shape (`/arch`-owned).
...
- **No `cranelisp-types` change, no cross-crate type change, no new boundary type.** `SymbolTableAccess`, `View`, `check_forms`, `ProcessedCluster` are all unchanged. This is pure int-interior re-plumbing.
