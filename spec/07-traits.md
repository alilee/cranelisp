...
**Kind is determined by usage.** The two head shapes are not interchangeable; a trait's kind is fixed by how the head variable is used, not by a written annotation:
...
> **Note — a name that resolves both as a type and as a value takes the type reading. [S115]** The rule above is an **iff on resolution as a type**, so a name that resolves *both* ways is **decided, not ambiguous**: the type reading wins and the method is **required**. The live case is the **product type**, whose type name doubles as its sole constructor ([§5.2.1](05-definitions.md#521-product-type-single-constructor)) — given `(deftype Point [:Int x :Int y])`, `Point` names a type *and* a value of type `(Fn [Int Int] Point)`. In `(mk [x] Point)` the trailing `Point` is therefore a **return type** and `mk` is required; the alternative reading — a default method whose body is that constructor function — is **unreachable in that spelling**. (An enum does not raise this: given `(deftype Color Red Green Blue)`, `Red` is a value and not a type, so a trailing `Red` is unambiguously a body expression.) The **unit type** `(deftype Unit [])` has the same doubling — it is the zero-field case of the very same product form ([§5.2.1](05-definitions.md#521-product-type-single-constructor)) — so a trailing `Unit` is likewise a return type; it differs only in that its constructor is nullary and so the value is `Unit :: Unit` rather than a function, and the annotated body spelling is correspondingly `(mk [x] :Unit Unit)`. (A nullary constructor can no longer produce this collision *without* the product form: a nullary constructor may not repeat its type's name, [§5.2.2](05-definitions.md#522-sum-type-multiple-constructors) [S115].) The collision is **invisible in the source** — nothing in `(mk [x] Point)` signals that a value named `Point` also exists — so a reader who intended a default body gets a required method with no diagnostic; that is the whole reason this note exists. **The remedy is to annotate.** A default body that genuinely needs to *be* the constructor value is written with a leading `:` — `(mk [x] :(Fn [Int Int] Point) Point)` — and this follows from rules already stated, with no new machinery: §7.1.1 requires a return position to be a **bare** `type_expr` with **no leading `:`**, so a trailing element carrying a `:` cannot be a return type; [§3.3.3](03-types.md#333-a-value-position-annotation-is-a-check-or-a-resolution-not-an-abstraction) governs `:Type expr` as an ordinary **value-position** annotation. The `:` therefore does double duty — it pins the type *and* selects the body reading — consistent with *types are inferred; annotations add constraints*. "Type wins" stands for the bare spelling; the annotated spelling is how you say you meant the value. This depends on the read-time annotation fold (§[1.4.5](01-lexical.md#145-colon-prefixed-symbols)), under which `:(Fn [Int Int] Point)` binds the following form so the two written elements fold into the **one** trailing element the `method_sig` production allows; before the fold lands the same source reads as *two* trailing elements and is rejected. (User ruling 2026-07-21: "type wins" stands; the precedence is unchanged.)
...
What a reader should do today is therefore stated decisively: there is **no** way to declare a marker trait, and the answer is **not** to write a trait whose methods never mention the implementing type. That spelling — `(deftrait Marker (mark [:Int x] Int))` — is **rejected** by the occurrence rule above, and it is a different thing in any case: an **undispatchable method**, not a marker. (User ruling 2026-07-21.)
...
**Application is what makes the head higher-kinded.** A parenthesized head is the higher-kinded form **only if** its head variable is applied at least once. A parenthesized head whose variable is never applied is malformed (§7.1) — there is no "kind-`*` trait with a head type variable"; conventional traits use the bare head and `self`.
...
**Governing principle.** An impl target's kind MUST equal the kind the trait abstracts over: **`*`** for a conventional (bare-head, `self`-based) trait, and the con_var's **usage-derived kind** (e.g. `* -> *`, §7.2.1) for a higher-kinded (parenthesized-head) trait. The three cases below are unfoldings of this one rule.

#### Case 1 — Conventional (kind-`*`) trait target
...
**Consequence.** The two forms **never collide for the same trait**. `Functor` is higher-kinded, so only `(impl (Functor f) (Functor C) …)` is legal for it; `Display` is conventional, so only `(impl Display <type> …)` is legal for it. Every malformed slot 2 is caught by interpreting it in the single mode the trait's kind dictates: a higher-kinded pairing whose head does **not resolve to the trait** (*Pairing-head identity*, below) fails as a bad pairing; a conventional target that isn't a well-formed type fails as "not a type." **No separate "is the slot-2 head a trait vs. a type-constructor" classifier is needed or wanted** — it would be pure redundancy, only ever agreeing with the kind-driven interpretation. The surface parallelism between `(Functor Option)` (a trait-constructor pairing) and `(Option a)` (a type application) is resolved **before** slot 2 is inspected, by the trait's declared kind.

**Pairing-head identity (settled 2026-07-18, user ruling — TB-25).** The slot-2 pairing `(Trait Constructor)` names the trait it is *about*; its **head** is an ordinary **trait-name reference**, resolved under the normal reference rules (§8.5). It is well-formed **iff it resolves to the same trait that slot 1 resolves to** — the match is by **resolved identity, not written spelling**. A **qualified** head (`(impl (Functor f) (fmt/Functor Option) …)`), an **imported bare** name, and **two different import paths** that all name the one `Functor` trait are equally valid: valid references to the same thing are the same thing, whatever reference syntax they use. A pairing head that resolves to a **different** trait, or that **fails to resolve** at all, is the "bad pairing" rejection. This contrasts with **slot 1's con_var** (`f` in `(Functor f)`), whose echo is matched by **spelling** (§7.3, "Slot 1 is fixed, not inferable"): the con_var is a **binder**, not a reference, so its verbatim spelling is what must be reproduced — whereas the pairing head is a **reference**, so only its **referent** matters. (The parallel to §5's declaration heads: a binder fixes a spelling where it is written; a reference is satisfied by anything that resolves to the intended target.)

### 7.3.6 Inline Constraints on Type Arguments
...
- **No marker traits.** A zero-method trait is not specified; every trait declares at least one method signature, and each method must mention the implementing type (see the parked-boundary note in [§7.1.1](#711-the-self-type)).
