...
use crate::intrinsics::{alloc_with_rc, free};
...
/// `vec-set-rc :: (Vec a, Int, a, inc_fn) -> (Vec a)`
/// Copies elements with RC inc via function pointer. The inc_fn is called for
/// each copied element (except at the replaced index). Codegen handles the new
/// value's RC — this function does NOT inc the new value.
#[unsafe(export_name = "vec-set-rc")]
pub extern "C" fn vec_set_rc(vec_ptr: i64, index: i64, val: i64, inc_fn: i64) -> i64 {
    unsafe {
        let ptr = vec_ptr as *const i64;
        let len = *ptr;
        let cap = *ptr.add(1);
        ...
        // Copy all elements then overwrite at index
        std::ptr::copy_nonoverlapping(old_data, new_data, len as usize);
        *new_data.add(index as usize) = val;

        // Inc each copied element (except the replaced one) via fn ptr
        if inc_fn != 0 {
            let inc: extern "C" fn(i64) -> i64 = std::mem::transmute(inc_fn as usize);
...
/// `vec-push-rc :: (Vec a, a, inc_fn) -> (Vec a)`
/// Copies elements with RC inc via function pointer. Codegen handles the new
/// value's RC — this function does NOT inc the new value.
#[unsafe(export_name = "vec-push-rc")]
pub extern "C" fn vec_push_rc(vec_ptr: i64, val: i64, inc_fn: i64) -> i64 {
    unsafe {
        let ptr = vec_ptr as *const i64;
        let len = *ptr;
        let old_data = *ptr.add(2) as *const i64;
        ...
        if len > 0 && !old_data.is_null() {
            ...
            // Inc each copied element via fn ptr
            if inc_fn != 0 {
                let inc: extern "C" fn(i64) -> i64 = std::mem::transmute(inc_fn as usize);
                for i in 0..len as usize {
                    inc(*new_data.add(i));
        ...
        // Append new element (codegen handles its RC)
        *new_data.add(len as usize) = val;
...
/// `vec-push-cow-grow :: (Vec a, a) -> (Vec a)`
/// COW realloc when capacity is exhausted and rc==1 (caller owns the Vec).
/// Allocates a new data buffer with doubled capacity, copies elements (raw memcpy,
/// no inc — ownership transfers), frees old data buffer, updates header in place.
#[unsafe(export_name = "vec-push-cow-grow")]
pub extern "C" fn vec_push_cow_grow(vec_ptr: i64, val: i64) -> i64 {
    unsafe {
        let header = vec_ptr as *mut i64;
        let len = *header;
        let old_cap = *header.add(1);
        let old_data = *header.add(2) as *const i64;

        // Double capacity (minimum 8 elements)
        let new_cap = if old_cap == 0 { 8 } else { old_cap * 2 };

        // Allocate new data buffer
        let new_data = alloc_with_rc((new_cap as usize) * 8) as *mut i64;

        // Copy existing elements (raw memcpy — ownership transfer, no inc)
        if len > 0 && !old_data.is_null() {
            std::ptr::copy_nonoverlapping(old_data, new_data, len as usize);
        }

        // Free old data buffer (just the allocation, not elements)
        if !old_data.is_null() {
            free(old_data as i64);
        }
