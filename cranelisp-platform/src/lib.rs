...
pub enum SchedulingClass {
    ...
    /// Two `ResourceSerial` effects in the same Par group are serialized at runtime
    /// if they carry the same non-zero resource token.
    ResourceSerial = 2,
}
...
impl<CL: CLType> CLIO<CL> {
    ...
    pub fn pure(val: CL) -> Self {
        ...
        let ptr = alloc(16); // 2 x i64: tag + value
        unsafe {
            *(ptr as *mut i64) = IO_TAG_PURE;
            *((ptr + 8) as *mut i64) = val.to_raw();
        }
    ...
    /// Resource token is set to 0 (unrestricted — may run in parallel with any other effect).
    ...
    ///
    /// **Token uniqueness**: tokens are compared globally across all platforms.
    /// Use heap pointer addresses for uniqueness:
    /// - Singleton resource: `&MY_STATIC as *const _ as i64`
    /// - Per-instance resource: `Box::into_raw(Box::new(state)) as i64`
    /// Do NOT use raw OS fd integers — they may collide across platforms.
    pub fn effect_on_resource(token: i64, f: impl FnOnce() -> CL + 'static) -> Self {
        ...
        let alloc = get_global_alloc();
        let effect = alloc(24); // 3 x i64: tag + thunk_ptr + resource_token
        unsafe {
            *(effect as *mut i64) = IO_TAG_EFFECT;
            *((effect + 8) as *mut i64) = thunk_ptr;
            *((effect + 16) as *mut i64) = token;
        }
        CLIO(effect, std::marker::PhantomData)
    }
}
...
/// Layout: `[total_size: i64][rc: i64][payload...]`, pointer points to payload.
pub trait CLHeap: CLType + Copy {
    ...
    /// Atomically decrement the reference count. Returns the old RC.
    /// If old RC was 1 (now 0), frees the allocation.
    fn dec_rc(&self) {
        let ptr = self.raw_ptr();
        let rc_addr = (ptr - 8) as *mut i64;
        let old_rc = unsafe {
        ...
        if old_rc == 1 {
            // RC reached 0 — free the allocation (base = ptr - 16)
            let base = (ptr - 16) as *mut u8;
            let total_size = unsafe { *((ptr - 16) as *const i64) } as usize;
            unsafe {
...
///     CLIO::effect(move || { println!("{}", s.as_str()); CLInt::from(0i64) })
/// }
///
/// declare_platform! {
///     name: "stdio",
