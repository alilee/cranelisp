...
fn build_list_expr(children: &[Sexp], span: Span) -> Result<Expr, CranelispError> {
    ...
    if let Sexp::Symbol(head, _) = &children[0] {
        match head.as_str() {
            ...
            "par-let" => return build_par_let(children, span),
...
fn build_par_let(children: &[Sexp], span: Span) -> Result<Expr, CranelispError> {
    // (par-let [name val name val ...] body) — minimum 2 bindings
    if children.len() != 3 {
        return Err(err("par-let requires bindings and body", span));
    }
    let (bracket_items, _) = expect_bracket(&children[1])?;
    let bindings = build_let_bindings(bracket_items)?;
    if bindings.len() < 2 {
        return Err(err(
            "par-let requires at least 2 bindings (use let for single binding)",
            span,
        ));
    }
    let body = build_expr_inner(&children[2])?;
...
fn build_trace(children: &[Sexp], span: Span) -> Result<Expr, CranelispError> {
    // (trace expr)                  — trace current module
    // (trace [mod1 mod2] expr)      — trace nominated modules
    match children.len() {
        2 => {
            // (trace expr)
        ...
        _ => Err(err(
            "trace requires: (trace expr) or (trace [modules...] expr)",
            span,
        )),
    }
}

fn build_run_tests(children: &[Sexp], span: Span) -> Result<Expr, CranelispError> {
    // (run-tests init pass-fn fail-fn)
    // (run-tests [mod1 mod2] init pass-fn fail-fn)
    match children.len() {
        4 => {
            // (run-tests init pass-fn fail-fn)
            let init = build_expr_inner(&children[1])?;
            let pass_fn = build_expr_inner(&children[2])?;
            let fail_fn = build_expr_inner(&children[3])?;
            Ok(Expr::RunTests {
                modules: vec![],
                init: Box::new(init),
        ...
        5 => {
            ...
            let modules = bracket_items
                ...
                .collect::<Result<Vec<_>, CranelispError>>()?;
            let init = build_expr_inner(&children[2])?;
            let pass_fn = build_expr_inner(&children[3])?;
            let fail_fn = build_expr_inner(&children[4])?;
            Ok(Expr::RunTests {
                modules,
                init: Box::new(init),
                pass_fn: Box::new(pass_fn),
...
fn build_deftrait(
    ...
    // Default methods are not supported on HKT traits
    if !type_params.is_empty() {
        if let Some(m) = methods.iter().find(|m| m.default_body.is_some()) {
            return Err(err(
                &format!(
                    "default method implementations are not supported on higher-kinded traits (method '{}')",
                    m.name
                ),
                m.span,
            ));
...
fn build_trait_method_sig(sexp: &Sexp) -> Result<TraitMethodSig, CranelispError> {
    // (method_name "doc"? [param_types] ret_type default_body?)
    let (children, span) = expect_list(sexp)?;
    if children.is_empty() {
    ...
    let remaining = children.len() - next;

    if remaining == 2 {
        // No default body: (name "doc"? [types] ret_type)
        ...
        // Default body: (name "doc"? [named_params] ret_type body)
        let (param_names, annotations) = build_annotated_params(&children[next])?;
        // Map annotations to TypeExprs: bare names → SelfType, annotated → the annotation
        ...
        let ret_type = build_type_expr(&children[next + 1])?;
        let default_body = children[next + 2].clone();
        ...
        Err(err(
            "trait method signature has too many elements",
            span,
        ))
    }
}
