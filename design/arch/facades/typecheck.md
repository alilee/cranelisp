...
5. **Trait method dispatch via `ResolvedCall::TraitMethod`.** Per Decision 14 — typecheck always emits `TraitMethod` for trait-dispatched operators; backend recognises known primitive impls. Typecheck stays clean of backend-specific lowering.
