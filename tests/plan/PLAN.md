...
| spec §12.3.1 + §7.x, generic trait-method instance releases its closure/heap params transitively (0903 family 2 — leaks today, shallow dec) | S119 W1 marginal-balance guard: subject/control pair differing in exactly one HKT/generic trait-method invocation (`Functor.fmap$primitives/Option` shape); exact marginal balance | `[S119]` QA-first, with family 1 above; cell #21 carries the class at application scale meanwhile (§11.3) |
