...
| `sprint23.rs` | 2744 | 61 | 80 | 5 | 15 | 25 | `regression.rs`, `repl_lifecycle.rs`, `spec_08_modules.rs`, `link.rs` (S64 W6 b2 Part A, NEW), `repl_shell.rs` (S64 W6 b2 Part A, NEW), `cache.rs` (S64 W6 b2 Part A, +3 cache_repl_*) | /int (heisenbug-race race-related internals) | Includes `heisenbug_race_reduced_concurrent_import_pairs` ledger entry — preserves signature. Defect risk: H6 residue ratecard may shift under e2e cadence. 5% quarantine isolates internal-API observation; 15% delete is duplication with sibling sprint-NN files. **W6 batch 2 Part A landed (2026-05-04)**: 25 carry-forwards across 3 target files — 11 in NEW `tests/link.rs` (--link cluster, all green; multi-module test was a Sprint 58 Wave 2c FIXME — now passes, preserved as REGRESSION-GUARD), 11 in NEW `tests/repl_shell.rs` (/sh cluster, all green — first §13 coverage in the new suite), 3 in `tests/cache.rs` (cache_repl_* REPL-mode integration, including the sole /reset+cache test). #56 `batch_main_int_exit_code` DROPPED per audit DUPLICATE-IN-LEGACY (1:1 with `build_confidence.rs::smoke_run_zero_arg_main_exits_zero`); #57 `batch_main_nonzero_exit_code` deferred to Part B. Part B (~32 tests: file-watching + persistence + heisenbug + H5 race) pending; sprint23.rs remains in place until Part B lands and quarantines it. |
