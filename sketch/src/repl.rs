...
use crate::macro_expand::{
    build_clause_entries, build_clause_infos, compile_macro, expand_sexp, flatten_begin,
    is_defmacro, is_deftype, parse_defmacro, MacroEnv,
};
use crate::cache;
...
pub struct ReplSession {
    ...
    /// Background cache writer. Processes .meta.json and .o writes on a background thread
    /// so the REPL returns immediately after JIT compilation. Initialized lazily on first use.
    cache_writer: Option<crate::cache_writer::CacheWriter>,
    /// Wall-clock compile duration (ms) from previous REPL input.
    pub last_compile_ms: Option<u64>,
    /// Wall-clock eval duration (ms) from previous REPL input.
    pub last_eval_ms: Option<u64>,
}
...
impl ReplSession {
    ...
    fn compile_module_graph(
        ...
        let timing = cache::timing_enabled();
        let t_cache_setup = std::time::Instant::now();
        ...
        if timing {
            eprintln!("; timing: cache_setup {}ms", t_cache_setup.elapsed().as_millis());
        }
        ...
        for module_id in &graph.compile_order {
            ...
            for sexp in &module.sexps {
                ...
                if is_defmacro(sexp) {
                    ...
                    let clause_infos = build_clause_infos(&info);
                ...
                for form in &forms {
                    if is_defmacro(form) {
                        ...
                        let clause_infos = build_clause_infos(&info);
                        let clause_entries = build_clause_entries(&info, ptrs);
                        let macro_name = info.name;
                        let macro_doc = info.docstring;
                        let macro_vis = info.visibility;
                        self.tc.register_macro_in_module(
                            &macro_name,
                            clause_infos,
                            macro_doc.clone(),
                ...
                for item in &items {
                    match item {
                        ...
                        TopLevel::TraitImpl(ti) => {
                            ...
                            for method in &ti.methods {
                                let mangled = crate::ast::Defn {
                                    ...
                                    name: crate::ast::mangle_impl_method(
                                        &ti.trait_name,
                                        &method.name,
                                        &target,
                                    ),
                        ...
                        TopLevel::Defn(defn) => {
                            ...
                            let mono_batch = match self.tc.monomorphise_all() {
                                Ok((monos, dispatches)) => {
                            ...
                            // Compile mono specializations BEFORE the calling function
                            self.compile_mono_specializations(&mono_batch, &mr, &et)?;

                            if scheme.is_constrained() {
                                // Ensure Def entry exists so constrained fn is visible
                                {
                                    let mod_path = ModuleFullPath::from(mod_name.as_str());
                                    let cm = self.tc.modules
                                        .entry(mod_path.clone())
                        ...
                        TopLevel::DefnMulti {
                            ...
                            // Register base name with overloaded variant info.
                            // Use the scheme saved from the loop — do NOT call
                            // finalize_defn_type again as it would remove+reinsert the
                            // first variant's module entry, wiping its codegen fields.
                            if let Some(scheme) = first_scheme {
                                // Build overload variants for cache round-trip
            ...
            // ── Submit cache write to background writer (non-blocking) ──
            {
                let fn_slots = self.jit.build_fn_slots_from_modules(&self.tc.modules);
                let cache_inputs = cache::extract_cache_inputs(&self.tc.modules);
                if let Some(packet) = cache::build_cache_packet(
                    &target_cache,
                    &mod_path,
                    &self.tc.modules,
                    &fn_slots,
                    ...
                    let writer = self.cache_writer.get_or_insert_with(crate::cache_writer::CacheWriter::new);
        ...
        // Cache manifests are written by the background CacheWriter on shutdown.
    ...
    pub fn load_prelude(&mut self) {
        ...
        if cache::timing_enabled() {
            eprintln!("; timing: load_prelude {}ms", t_prelude.elapsed().as_millis());
        }
    }
    ...
    /// Handle an inline `(mod name forms...)` form at the REPL.
    /// Extracts the body to a backing file and loads the child module.
    fn handle_repl_inline_mod(&mut self, name: &str, body: &[crate::sexp::Sexp]) {
        // 1. Determine child file path: parent_dir/parent_stem/name.cl
        let child_file = {
            let cm = self.tc.modules.get(&self.current_module);
            match cm.and_then(|cm| cm.file_path.as_ref()) {
                Some(parent_path) => {
                    ...
                    let stem = parent_path
                        ...
                        .and_then(|s| s.to_str())
                        .unwrap_or("");
                    dir.join(stem).join(format!("{}.cl", name))
                }
                None => self.project_root.join(format!("{}.cl", name)),
            }
        };

        // 2. Format body sexps as source and write to child file
        let child_source = body
            .iter()
            .map(|s| s.format_indented(0))
            .collect::<Vec<_>>()
            .join("\n\n");
        ...
        if let Err(e) = std::fs::write(&child_file, format!("{}\n", child_source)) {
            eprintln!(
                "Cannot write inline module '{}': {}",
                child_file.display(),
                e
            );
            return;
        ...
        // 3. Load the child module via standard module loading
        let target_id = ModuleFullPath(name.to_string());
        if self.loaded_modules.contains_key(&target_id) {
            // Already loaded — reload it with new content
            // Remove so load_module_by_name will re-discover
            self.loaded_modules.remove(&target_id);
            self.tc.modules.remove(&target_id);
        }
        ...
        // 4. Add to current module's mod_decls if not already present
        let mod_name = crate::names::ModuleName::from(name);
        if let Some(cm) = self.tc.modules.get_mut(&self.current_module) {
            if !cm.mod_decls.contains(&mod_name) {
                cm.mod_decls.push(mod_name);
        ...
        // 5. Save current module so (mod name) appears in the file
        self.save_current_module();

        // 6. Update watched paths and confirmation
        self.update_watched_paths();
        println!("mod {} (extracted to {})", name, child_file.display());
    }
    ...
    /// Submit the current module's cache files for background writing.
    ...
    fn write_current_module_cache(&mut self, source_hash: &str) {
        ...
        // Build packet and submit to background writer
        let fn_slots = self.jit.build_fn_slots_from_modules(&self.tc.modules);
        let cache_inputs = cache::extract_cache_inputs(&self.tc.modules);
        if let Some(packet) = cache::build_cache_packet(
            &project_cache_dir,
    ...
    /// Find modules that directly depend on the given module.
    /// Scans all loaded CompiledModules' import_specs and mod_decls.
    fn find_direct_dependents(&self, module: &ModuleFullPath) -> Vec<ModuleFullPath> {
        let target = module.0.as_str();
        let target_short = module.short_name();
        let mut dependents = Vec::new();

        for (mod_path, cm) in &self.tc.modules {
            if mod_path == module {
                continue;
            }
            // Check import_specs: does this module import from the target?
            let imports_target = cm.import_specs.iter().any(|spec| {
                spec.module_path == target || spec.module_path == target_short
            });
            // Check mod_decls: does this module declare (mod target_short)?
            let declares_target = cm.mod_decls.iter().any(|decl| {
                &**decl == target_short || &**decl == target
            });
            if imports_target || declares_target {
    ...
    /// Find all transitive dependents of the given module, topologically sorted
    /// (direct dependents first, then their dependents, etc.).
    fn find_transitive_dependents(&self, module: &ModuleFullPath) -> Vec<ModuleFullPath> {
        let mut result = Vec::new();
        let mut visited = HashSet::new();
        let mut queue = std::collections::VecDeque::new();

        // Seed with direct dependents
        for dep in self.find_direct_dependents(module) {
        ...
        while let Some(current) = queue.pop_front() {
            for dep in self.find_direct_dependents(&current) {
                if visited.insert(dep.clone()) {
                    queue.push_back(dep.clone());
                    result.push(dep);
                }
    ...
    fn handle_file_changes(&mut self, changed_paths: &[PathBuf]) {
        ...
        // Cascade: reload transitive dependents of successfully reloaded modules
        let mut cascade_targets: Vec<ModuleFullPath> = Vec::new();
        let mut cascade_seen = HashSet::new();
        for module_id in &reloaded {
            for dep in self.find_transitive_dependents(module_id) {
                if cascade_seen.insert(dep.clone()) && !reloaded.contains(&dep) {
                    cascade_targets.push(dep);
                }
            }
        }

        for dep_id in &cascade_targets {
            self.locked_modules.remove(dep_id);
            match self.reload_module(dep_id) {
                Ok(true) => {
                    eprintln!("; Cascade-reloaded {}", dep_id.short_name());
                }
                Ok(false) => {}
                Err(msg) => {
                    eprintln!(
                        "; Failed to cascade-reload {}: {}",
                        dep_id.short_name(),
                        msg
                    );
                    self.locked_modules.insert(dep_id.clone());
                }
            }
        }
    }
    ...
    pub fn run(&mut self) {
        ...
        if user_file.exists() {
            match ModuleGraph::build(&user_file) {
                Ok(graph) => {
                    // Entry module is "user" (from user.cl stem)
                    let entry_id = ModuleFullPath::from("user");
        ...
        if cache::timing_enabled() {
            eprintln!("; timing: startup_total {}ms", t_startup.elapsed().as_millis());
        }
    ...
    fn run_repl_loop(&mut self) {
        ...
        loop {
            ...
            let prompt = if input_buf.is_empty() {
                match (self.last_compile_ms, self.last_eval_ms) {
                    (Some(c), Some(e)) => format!("{}+{}ms; {}> ", c, e, self.current_module),
                    _ => format!("{}> ", self.current_module),
                }
            } else {
                let primary_len = match (self.last_compile_ms, self.last_eval_ms) {
                    (Some(c), Some(e)) => {
                        format!("{}+{}ms; {}> ", c, e, self.current_module).len()
                    }
                    _ => format!("{}> ", self.current_module).len(),
                };
                format!("{}... ", " ".repeat(primary_len.saturating_sub(4)))
            };
            ...
            if sexps.len() == 1 && is_defmacro(&sexps[0]) {
                ...
                match compile_macro(&mut self.tc, &mut self.jit, &info, Some(&self.macro_env)) {
                    Ok(ptrs) => {
                        self.last_compile_ms = Some(t_macro.elapsed().as_millis() as u64);
            ...
            // Intercept (mod ...): declare child module (file-based or inline)
            if sexps.len() == 1 {
                if let crate::sexp::Sexp::List(ref children, _) = sexps[0] {
                    if children.len() >= 2 {
                        if let crate::sexp::Sexp::Symbol(ref head, _) = children[0] {
                            if head == "mod" {
                                if let crate::sexp::Sexp::Symbol(ref mod_name, _) = children[1] {
                                    if children.len() == 2 {
                                        ...
                                        // Inline: (mod name forms...)
                                        let body = children[2..].to_vec();
                                        self.handle_repl_inline_mod(mod_name, &body);
                                    }
                                    continue;
                                }
                            }
        ...
        // Shutdown background cache writer, flushing pending writes and manifests
        let t_shutdown = std::time::Instant::now();
        if let Some(ref mut writer) = self.cache_writer {
            writer.shutdown();
        }
        if cache::timing_enabled() {
            eprintln!("; timing: shutdown {}ms", t_shutdown.elapsed().as_millis());
        }
    }
}
...
pub fn run_with_file(path: &Path) {
    ...
    // Entry module ID is the file stem (not compile_order.last(), which may
    // be a test submodule that depends on the entry module).
    let entry_stem = path
        .file_stem()
        .and_then(|s| s.to_str())
        .unwrap_or("user");
    let entry_id = ModuleFullPath::from(entry_stem);
