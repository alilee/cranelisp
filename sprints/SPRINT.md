...
| 1c | **Saturation gate** (0459) | /dev (backend) measurement spike | `CRANELISP_SATURATION_GATE` | **in-progress** — spark-iff-spare-capacity, inline-when-saturated. Hypothesis (post-1b re-diagnosis): running saturated subtrees inline **confines their vec-COW RC traffic thread-locally** → fewer cores bouncing shared cells → cut the +18s user-time. Measurement-first; /design formalises + /review gates default-on only if it moves the number. Re-benchmark F2/F4 N-worker vs current create-gate; record delta. |
