...
| **LOW-1** stale `cranelisp-runtime` / `facades/backend.md` refs in rustdoc | **STILL OPEN** | `int.rs:7-14`, `float.rs:5-6`, `bool.rs:5-6` still narrate "lifted … from `cranelisp-runtime` … keeps thin re-export shim until that crate retires per FIXME 0150 Phase 5"; `ring0.rs:4-5` still cites `design/arch/facades/backend.md`. Both artefacts are retired. See LOW-1 below. |
...
**Severity**: Low (doc staleness — Principle 8 residue, doc-only) · **routes**: `target: /dev`
