...
**Owner:** `/design` (int). **Author:** Phase 3 design pass (2026-06-10); as-built reconciliation (2026-06-11).
...
> **AS-BUILT RECONCILIATION (FIXME 0311).** The restructure landed three source-forced mechanism divergences from this doc's original Phase-3 prose (none change intent / scope / soundness; all S78 gate tests green). They are now folded into §1.1 / §1.2 / §3 / §4 / §6 inline. Summary:
> 1. **Packet = (1b), not the recommended (1a):** queues stay `VecDeque<ModuleFullPath>`; `ModuleState.sexps: Option<Arc<[Sexp]>>` carries the packet; `PriorityWork` has a **manual** `PartialEq`/`Eq` (module identity only). `SharedState.module_sexps` still deletes (the goal is met). §1.1.
> 2. **Per-symbol staging commit STAYS in `check_program_compat`/`commit_staging_to_live`**, NOT moved to `insert_cluster` (moving it breaks worker codegen ordering — codegen reads committed live entries before `insert_cluster`). `ProcessedCluster::empty()` on the worker path; the program rides `ClusterOnce::Done { program }`. §1.2 / §4 / §6.
> 3. **Orchestration core stays in `worker.rs`**, NOT moved to `cluster.rs` (depends on ~30 worker-private helpers). `worker::process_cluster_once` (core) + `worker::drive_module_dep` (the register-edge half this doc's §1.3 pins as `drive_gap_to_readiness`) live in `worker.rs`; `cluster::process_cluster` + `cluster::insert_cluster` are the `cluster.rs` facade entries that delegate. §1.2 / §3 / §4. **Naming note:** the §1.3 signature pin `drive_gap_to_readiness` is realized as `worker::drive_module_dep`; its register-edge-and-return contract (does NOT block internally) is preserved verbatim.
...
- `PriorityWork::Typecheck(ModuleFullPath)` → `PriorityWork::Typecheck { module: ModuleFullPath, sexps: Arc<[Sexp]> }`. **AS BUILT:** `PriorityWork` carries a **manual** `PartialEq`/`Eq` comparing module identity only (the `sexps` payload is ignored), rather than a derive — see §1.1 packet note above. (The Phase-3 plan to keep a `#[derive(… PartialEq, Eq)]` was dropped: deriving would impose `Sexp: Eq` payload equality on every queue comparison, which is neither the queue's identity semantics nor wanted; module identity is the sole comparison the scheduler needs.)
- **Where the sexps enter the packet — AS BUILT: (1b), sexps on `ModuleState`.** The scheduler's `register_module(module, delays_other)` (`scheduler.rs:304`) currently enqueues a bare `ModuleFullPath` into `typecheck_first`/`typecheck_next` (`VecDeque<ModuleFullPath>`). At Phase 3 this doc weighed two source-faithful options and recommended (1a) "queue carries the packet." **The restructure implemented (1b)** — recorded here per the doc's own "source-verified detail wins" rule (FIXME 0311). The reason (1a) proved disproportionate: the requeue path (`try_unblock_locked`) reconstructs the packet from `ModuleState`, so the sexps must live on `ModuleState` **regardless** of 1a vs 1b — (1a) would store them BOTH on the queue and on `ModuleState` (redundant), while churning ~10 queue-mutation sites (`push`/`pop`/`retain`) and forcing a manual `PartialEq`/`Eq` on `PriorityWork` anyway.
  - **(1b) as built.** The queues stay `VecDeque<ModuleFullPath>`. `ModuleState.sexps: Option<Arc<[Sexp]>>` holds the packet, stored at register time. `dispatch_typecheck_locked` reads it on pop to build `PriorityWork::Typecheck { module, sexps }`. The requeue path (`try_unblock_locked`) re-pushes the bare `ModuleFullPath`; the sexps are found on `ModuleState`. `PriorityWork` carries a **manual** `PartialEq`/`Eq` (module identity only — sexps ignored) rather than a derive, because `Arc<[Sexp]>` payload equality is neither wanted nor needed for the queue's identity semantics.
  - **The goal is met.** `SharedState.module_sexps` still deletes — there is no shared in-progress parking map. (1b) keeps a module→sexps association inside the scheduler's *own* `ModuleState`, but that is the scheduler owning its own work-item payload, not a cross-thread parking map; the soundness property (in-progress sexps never land in a shared map other threads read mid-flight) holds.
...
- `ProcessedCluster` is **unchanged** (`cluster.rs:67-154`) — it already carries `entries`, `warnings`, `resolved_imports`, `introspection_records`, and `from_parts`/`empty`/`is_empty`/`into_iter` accessors.
- **AS BUILT — the per-symbol staging commit stays in `check_program_compat`, NOT moved to `insert_cluster`.** Phase 3 proposed moving the commit-site out of `process_cluster_with_staging` into `insert_cluster` (routing drained entries through `ProcessedCluster.entries`). That was reverted in the restructure (FIXME 0311) because it would break worker codegen ordering: `derive_codegen_batch` + `inline_jit_codegen_for_module` read the committed entries from the **LIVE** `symbol_tables` to compile them, and the worker handler runs codegen **before** `insert_cluster` (per the "claim → process_cluster → codegen → insert_cluster → notify" order). If the commit lived in `insert_cluster`, codegen would run against an empty live table. As built: `process_cluster_once` keeps the staging commit in `commit_staging_to_live` (on Ok, as before); `ProcessedCluster::empty()` is returned for the worker path; `insert_cluster` drains only the cluster-level introspection metadata. The expanded `program` for codegen rides the `ClusterOnce::Done { program }` / `ClusterOutcome::Done { program }` outcome, **not** `ProcessedCluster`. `ProcessedCluster`'s shape is unchanged either way.
...
```
- The proposal's pseudocode (§2.2) is the intent; the source-faithful realization folds today's `process_module_forms` Pass-0 loop (`worker.rs:1272-1330`), `separate_macros` (`:1332`), `pass1_register`/`register_macro_in_module`/`register_default_methods` (`:1341-1349`), `pass2_check_bodies_with_expansion` (`:1359`), and `finalize_module`'s `check_program_compat` (`:1454`) into the single `staged_check` step.
- **AS BUILT — the orchestration core stays in `worker.rs`, NOT moved into `cluster.rs`.** Phase 3 proposed moving the staging core (`process_cluster_with_staging` + `commit_staging_to_live`) from `worker.rs` into `cluster.rs` and framing `process_cluster_once` as living in `cluster.rs`. That was not done (FIXME 0311): the Pass-0/1/2 core depends on ~30 `worker.rs`-private helpers (`classify_form`, `handle_import`/`handle_export`/`handle_mod`, `separate_macros`, `pass1_register`, `process_regular_form`, `top_level_to_parsed_entries`, `check_error_to_cranelisp_error`, …). Moving the core to `cluster.rs` would force ~30 `pub(crate)` promotions and a ~1200-LOC cross-file relocation for a doc-placement preference (Principle 6 — complexity has a budget; Principle 8 — no interim churn). As built: `worker::process_cluster_once` (the shared core) + `worker::drive_module_dep` (the register-edge half) stay in `worker.rs` where the helpers live; `cluster::process_cluster` (the worker wrapper) + `cluster::insert_cluster` are the `cluster.rs` facade entries that **delegate** into `worker.rs`. `process_cluster_with_staging` / `commit_staging_to_live` stay in `worker.rs`. The functional target (single live orchestration, packet-carries-sexps, no parking maps, in-call-stack retry-from-top, kernel unchanged) is fully met regardless of file placement.
...
**Design recommendation: (3a) — and as built.** Keep the loop owner explicit per caller. The shared core is `process_cluster_once` (expand + Pass-0 peel + stage + check → `Ok` via `ClusterOnce::Done { program }` | `Gap(ResolutionGap)` | `Err`); the worker wraps it in return-to-pool-requeue, the eval thread wraps it in wait-then-retry. This matches today's *de facto* split (worker loop vs `process_single_form` loop) and avoids a mode parameter. The register-edge half (named `drive_module_dep` as built) is shared by both wrappers; only the *wait* half differs (return-to-pool vs `wait_module_inmem_complete_blocking`). The proposal's single-`process_cluster` framing (§2.2) is the conceptual target; (3a) realizes it as one shared core + two thin wrappers, which is the honest source shape. **AS BUILT — `worker::process_cluster_once` (shared core) + `worker::drive_module_dep` (register-edge half) live in `worker.rs`** (near the ~30 private helpers they depend on, §1.2); `cluster::process_cluster` (the worker wrapper) + `cluster::insert_cluster` are the `cluster.rs` facade entries that delegate. `insert_cluster` stays the cluster-level introspection-drain step both paths call; the per-symbol staging commit stays in `commit_staging_to_live` inside the core (§1.2).
...
|---|---|---|---|
| **1+2 (indivisible)** | `cluster.rs` (`cluster::process_cluster`/`insert_cluster` as the facade entries delegating into worker.rs — **the orchestration core stays in `worker.rs` as built**, §1.2), `worker.rs` (new `process_cluster_once` core + `drive_module_dep` register-edge; delete `process_module_forms` + `ProcessResult` + `BlockAction` + `ModuleSuspendState` + `handle_typecheck_work_shared` map-juggle + `publish_dep_sexps` + `register_dep` publish; thin the worker loop; `process_cluster_with_staging`/`commit_staging_to_live` stay here), `scheduler.rs` (`PriorityWork::Typecheck { module, sexps }` with manual `PartialEq`/`Eq`; `ModuleState.sexps: Option<Arc<[Sexp]>>` carries the packet (1b); `register_module`/`dispatch_typecheck_locked`/`try_unblock_locked` requeue read it off `ModuleState`; `register_module` + `re_register_module` gain `Arc<[Sexp]>`), `session_v4.rs` (`SharedState` loses both fields + ctor; `register_module_with_source`/`reload_module` parse→packet; `process_single_form` calls the new core; `register_dep_for_eval` republish+guard deleted) | **RED throughout** — expected. The packet-shape change, the orchestration lift, and the map deletion cannot be cleanly separated (FIXME 0310 / proposal §2.3 publisher-vs-reader-side). | Span closes when: `cargo check` green; the dep-load e2e (`spec_08`, FQ-autoload suite), the cluster-atomicity tests, and the H5-replay test all green under one run. **Do NOT regenerate `public-api.txt` baselines until the span closes** (it is the binary crate — no `public-api.txt`; but `facade_pif_rows` field-count regrounds here, see §5). |
...
- **Behaviour tests that stay green by construction:** `spec_08` Defect-B resume; the FQ-autoload suite; cluster-atomicity (staging commit/discard) — the staging core stays in `worker.rs` (as built, §1.2) with `cluster.rs` facade entries delegating; its contract is identical regardless of file placement.
...
4. **Pass-0 structural peel moves into `process_cluster_once` before `staged_check`** (`install_imports`/`install_exports`/mod-alias run in-frame; a structural dep surfaces as a gap driven in-frame). §1.4. `ProcessedCluster` already carries the fields the new body needs (`entries`/`warnings`/`resolved_imports`/`introspection_records`). **AS BUILT (FIXME 0311):** the per-symbol staging commit STAYS in `commit_staging_to_live` inside the core (NOT moved to `insert_cluster` — that would break worker codegen ordering, §1.2); `ProcessedCluster::empty()` is returned on the worker path and `insert_cluster` drains only cluster-level introspection; the expanded program for codegen rides `ClusterOnce::Done { program }`. Int-internal.
...
> *"Cluster-mode staging is the live typecheck path (`process_cluster_with_staging`, `a2dcebd`). The remaining cluster-atomic work (S78) retired the `process_module_forms` outer per-form loop and folded Pass-0/1/2 + in-call-stack dep-drive into `worker::process_cluster_once` (the shared core) + `worker::drive_module_dep` (register-edge), with `cluster::process_cluster` + `cluster::insert_cluster` as the `cluster.rs` facade entries that delegate; the cross-thread `module_sexps`/`suspend_states` parking maps and the `eval_in_flight` guard are deleted, and the cluster sexps ride the work packet on `ModuleState`. `cluster::process_cluster` is now the single live orchestration entry; the `process_module_forms` worker loop is gone."*
...
- 2026-06-11 (`/design` (int), Wave 5 — as-built reconciliation, FIXME 0311): reconciled §1.1 / §1.2 / §3 / §4 / §6 / §7 prose to the three source-forced mechanism divergences the restructure landed (none change intent / scope / soundness; all S78 gate tests green). (1) Packet = (1b) on `ModuleState.sexps: Option<Arc<[Sexp]>>` with manual `PartialEq`/`Eq` on `PriorityWork`, NOT the recommended (1a); `SharedState.module_sexps` still deletes. (2) Per-symbol staging commit STAYS in `commit_staging_to_live`/`check_program_compat`, NOT moved to `insert_cluster` (preserves worker codegen ordering); program rides `ClusterOnce::Done { program }`; `ProcessedCluster` unchanged. (3) Orchestration core stays in `worker.rs` (`process_cluster_once` + `drive_module_dep`), with `cluster::process_cluster` + `cluster::insert_cluster` as the delegating `cluster.rs` facade entries; the §1.3 `drive_gap_to_readiness` pin is realized as `worker::drive_module_dep` (contract preserved). Added the AS-BUILT RECONCILIATION banner at the top. FIXME 0311 (`design/arch/fixmes/0311-*.md`) deleted (resolved). Doc-only; no code/test changes.
