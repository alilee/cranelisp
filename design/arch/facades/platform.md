...
`manifest_to_descriptors` is the public C-ABI → typed-Rust bridge: given a raw `PlatformManifest` (already located in a loaded DLL by the caller), it copies the descriptor list into safe Rust shapes and returns. Per BC §5, DLL lifecycle orchestration (`dlopen` + `libloading::Library` retention) is `int`'s job — the platform crate does not own DLL lifecycle. Per spec §8.9.3 (Submission 21), the loaded DLL handle lands on the synthetic platform module's own `SymbolTable.dll: Option<D>` field (via the `D: DllStore` generic — see `crates/cranelisp-types/src/module.rs` `SymbolTable` rustdoc), keyed by `symbol_tables["platform.<name>"]`. The JIT registers each fn pointer via `JITBuilder::symbol` keyed by `jit_name`. (The interim `SharedState.kept_dlls` field documented in `facades/int.md` is the transitional carrier; once the /dev wave-3 source migration lands, the handle moves to the SymbolTable.)
...
`PlatformError` is hosted in `cranelisp-types` per Decision 42 — coordinates as data via `ErrorLocation` carriers per variant. Re-exported here for caller convenience. See `crates/cranelisp-types/src/error.rs` `PlatformError` rustdoc for the canonical definition.
...
1. **Platform fn pointers live in `SymbolTable.got()`, indexed by `ModuleEntry::Def.got_slot`** (Decision 26, S66 amendment + rollback `1dc57ae` — GOT is the single source of truth for callable addresses). Per spec §8.9.3, `(platform <name>)` registers a synthetic module at `symbol_tables["platform.<name>"]`; the per-fn `ModuleEntry::Def` entries (with `kind: Primitive { primitive_kind: PlatformEffect { … } }` distinguishing the platform origin) live in **that synthetic module's** `symbols`, and each entry's `got_slot` indexes into the same module's GOT. At registration time the platform-load path allocates a GOT slot for each platform-fn, records it as `got_slot: Some(slot)` on the entry, and writes the descriptor's pointer via `symbol_table.got().store_slot(slot, desc.ptr)`. The runtime lookup target is `platform_module.got().load_slot(entry.got_slot.unwrap())`. `code` is `None` for platform entries; **the DLL handle is the lifecycle owner, retained on the platform module's own `SymbolTable.dll: Option<D>` field** (per spec §8.9.3 + `crates/cranelisp-types/src/module.rs` `SymbolTable` rustdoc — `D: DllStore` generic). Drop semantics: dropping the platform module's SymbolTable drops the DLL, so every `platform_fn_ptr` is valid for exactly the SymbolTable's lifetime. The retired `ModuleEntry::PlatformDecl` variant (Submission 21) previously stored a per-platform DLL record as an entry within the declaring module; that contradicted spec §8.9.3 (platforms are modules of their own, not entries within other modules). `scheduling_class` lives inside the variant `PrimitiveKind::PlatformEffect { scheduling_class }` — making ill-formed states (a class on a non-platform entry) unrepresentable.
