...
| `/review` | **codex** (external CLI, `scripts/codex-review.sh`); adjudicated on fable | high | Cross-model independence — the reviewer must not share the implementer's blind spots; verdicts other roles must act on; misclassified severity silently ships defects. The invoking fable agent composes the brief, supplies test evidence, verifies findings, and files FIXMEs (Codex is sandboxed read-only). User-ratified 2026-07-25. | Codex CLI unavailable or errors → the adjudicator performs the review itself on fable and records the fallback in the dispatch log |
