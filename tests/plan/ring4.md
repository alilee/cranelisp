...
**Decision 31 Scenario 1 (Sprint 57 carry — REPL-eval JIT reclaim)** — landed Sprint 58 Wave 3d in new `tests/v4_jit_reclaim.rs`:
- `decision31_scenario1_per_eval_jit_pages_reclaimed` [Tested+Neg tests/v4_jit_reclaim.rs::decision31_scenario1_per_eval_jit_pages_reclaimed] — positive: capture `MemSnapshot` (the same `cranelisp_runtime::bytes_current()` / `alloc_count()` / `dealloc_count()` counters that back the `/mem` slash command per spec §3.7) pre-eval (call it `A`); REPL eval `(add-i64 1 2)`; capture snapshot post-eval (call it `B`); assert `B.bytes_live - A.bytes_live <= 256` (eval allocations released; small-constant overhead bound `REPL_EVAL_OVERHEAD_BOUND`). Spec: Decision 31 Scenario 1 + `repl/spec.md` §3.7. Note: test renamed slightly from plan (`_repl_eval_mem_drops_after_eval` → `_per_eval_jit_pages_reclaimed`) to align with plan-file Decision 31 vocabulary; the assertion is identical. Test asserts directly on the runtime counters (not on `/mem` rendered output) because §3.7 is rendering and the reclaim contract is numerical — `/mem` rendering is covered separately in `src/session_v4.rs::mem_command_tests`.
- `decision31_scenario1_repeated_eval_no_unbounded_growth` [Tested+Neg tests/v4_jit_reclaim.rs::decision31_scenario1_repeated_eval_no_unbounded_growth] — negative (regression guard): baseline snapshot; REPL eval `(add-i64 1 2)` 100 times in succession; final snapshot; assert `final.bytes_live - baseline.bytes_live <= 256` (does NOT grow with repetitions; bound = small constant, NOT scaled by 100). Pre-Wave-3b the gap would grow ~100x because `kept_jits` accumulated; post-fix stays flat. Companion live-allocations assertion. Spec: Decision 31 Scenario 1 footnote.

**Decision 31 Scenario 2 (Step 5c headline payoff — defn redefinition JIT reclaim)** — per `/arch` Condition 7. Landed Sprint 58 Wave 3d in new `tests/v4_jit_reclaim.rs`:
- `decision31_scenario2_per_redefinition_jit_pages_reclaimed` [Tested+Neg tests/v4_jit_reclaim.rs::decision31_scenario2_per_redefinition_jit_pages_reclaimed] — positive (HEADLINE assertion): `(defn f [x] x)`; capture `Arc<Jit>` clone from f's `ModuleEntry::Def.code` field (read via `ReplSession::symbol_tables()`); record `jit_free_memory_call_count()`; `(defn f [x] (add-i64 x 1))`; assert (1) the redefined entry's Arc<Jit> is `!Arc::ptr_eq` to the captured first-batch Arc (fresh batch, not reuse), (2) the captured first-batch `Arc::strong_count == 1` (only the test's clone remains; session has dropped its references), (3) on dropping the test's clone, `jit_free_memory_call_count` increments by exactly 1 (Cranelift JIT pages reclaim via `Jit::drop`). Stronger than a `bytes_live` delta because it observes the reclaim primitive at the `Arc<Jit>::drop` level Decision 31 defines, not the muddied OS-page-aggregate level. Plan name `_repl_redefinition_mem_drops_on_redefinition` upgraded for precision. Spec: Decision 31 Scenario 2 + `symbol-table-generics.md` §2.3.
- `decision31_scenario2_repeated_redefinition_no_unbounded_growth` [Tested+Neg tests/v4_jit_reclaim.rs::decision31_scenario2_repeated_redefinition_no_unbounded_growth] — negative (regression guard, the strongest signal that Scenario 2 fires under load): `(defn f [x] x)` + warm-up call; baseline snapshot + reclaim-count snapshot; redefine `(defn f [x] (add-i64 x N))` 50 times with varying `N`; final snapshot + reclaim-count snapshot; assert (1) `final.bytes_live - baseline.bytes_live <= 512` (does NOT grow linearly in N; pre-Wave-3b would have grown ~50x because `kept_jits` accumulated each batch), (2) `reclaim_count_after - reclaim_count_before >= 50` (at least one `Jit::free_memory` call per redefinition; pre-fix would have shown 0 reclaims until session drop). Spec: Decision 31 Scenario 2 + `kept_jits` dissolution.

**Cache-fresh Code-enum coexistence test** — landed Sprint 58 Wave 3d:
- `decision31_code_linker_session_scope_only` [Tested tests/v4_jit_reclaim.rs::decision31_code_linker_session_scope_only] — positive (structural reclaim discipline check at the `Code::Linker` enum layer). Plan-file `code_enum_jit_and_linker_coexist_in_one_session` was downscoped after `/qa` review: the unit test `code_enum_jit_and_linker_coexist_serde_skip` in `src/code.rs::tests` already proves enum coexistence in one symbol table (with both Code::Jit AND Code::Linker entries inserted). What was missing — and what this test adds — is the integration-level companion to the `Code::Jit` reclaim primitive (`code_enum_jit_variant_carries_arc_jit`) for the `Code::Linker` variant. The test wraps a `cranelisp_backend::cache::Linker` in `Arc`, builds two `Code::Linker` clones (simulating two cache-loaded `Def` entries sharing one batch), drops them in sequence, asserts `Arc::strong_count` decrements at each step, and asserts `Linker::Drop` runs cleanly when the last Arc clone drops. Reclaim is structural via `MmapMut::Drop` (no global counter analogous to `jit_free_memory_call_count` exists or needs to exist — Linker reclaim is RAII through `Arc<Linker>` + `MmapMut`). Documents the design decision: Linker reclaim is session-scope-bounded by design, since cache-hit code lives until either session teardown or wholesale redefinition of every cache-loaded symbol (at which point each `Arc<Linker>` clone drops via the standard `Code` enum replacement path). Spec: `symbol-table-cache.md` §3.3 + `symbol-table-generics.md` §2.1.

Approximate counts: 3 unit (`/typecheck`) + 5 unit (`/int`) + 4 integration (`/qa` reclaim) + 1 integration (`/qa` Linker-reclaim companion). Sprint 58 Wave 3d delivered: 5 integration tests passing, 0 failing, 0 ignored.
