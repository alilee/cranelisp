...
- **★ CANDIDATE PRINCIPLE 25 (framed for USER RATIFICATION at Phase 7):** *"Narrowing carries its check: a safety elision is defined against, and checkable against, its conservative fallback."* (1) conservative-at-⊤ = reference semantics, elision w/o reachable fallback inadmissible; (2) widening free, narrowing names its justification + check-tier, green suites + adversarial review are DISCOVERY not checks; (3) a foundational safety invariant is ASSERTED at its seam, not merely tested. The enforcement arm of monotone soundness (P18 for dynamic judgments, P20 extended to analysis claims). Ratifiable as one or split checkability(1-2)+assert-at-seam(3).
