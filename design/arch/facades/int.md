...
pub enum SlashCommand {
    ...
    Expand(String), Quit,
    PlatformSchema(ModuleName),   // TARGET (platform-interface.md §5.5.1) — derive + transitively-close + emit a loaded platform's layout schema; introspection-family sibling
    /* … */
}
```
...
### Platform interface — the load path + `/platform-schema` (TARGET)

**TARGET STATE — user-ratified 2026-06-07; canonical: `design/arch/platform-interface.md` (read in full; normative). Implementation pending (tracked by an int FIXME — see below). Where as-built differs is marked.** A platform DLL exports exactly three things — its **GOT** (`__cranelisp_got_platform_<name>`, function pointers in manifest order), its **manifest** (declarative fn data), and its **schema + layout hash** (`__cranelisp_layout_hash_<name>`, the generated-schema binding). `--link` consumes only the GOT (+ hash); REPL/`--run` consume all three. The governing split: **the DLL builds the GOT (its facts); the host builds the SymbolTable (its invariants).**

**The load path (`load_platform_dll` + `register_platform_in_tc`, `src/platform.rs:148/247` rework — platform-interface.md §6.4 / §7.2):**

1. **dlopen** the dylib; read the manifest (unchanged `manifest_to_descriptors` path).
2. **Resolve + compile the associated `.cl` type module(s)** through ordinary module resolution (`resolve_module_file` — project tree + `CRANELISP_LIB`; NOT `CRANELISP_PLATFORM_PATH`, which stays dylib-only), into the symbol tables *before* the platform's sigs are parsed. They are **ordinary modules** (not `platform.<name>.*`-mounted); the FQ sig refs auto-load them per FIXME 0268.
3. **Layout-hash check (platform-interface.md §5.5.4):** regenerate the schema from the live tables (the backend generator, BC §3) and canonical-hash it; compare to `dlsym("__cranelisp_layout_hash_<name>")`. Mismatch → in the **REPL, warn-and-load** (the regeneration bootstrap — the only place `/platform-schema` can run); in **`--run` (non-REPL), hard `PlatformError` refusal** (both hashes + "run /platform-schema and rebuild" guidance), abort the load.
4. **dlsym the GOT** (`__cranelisp_got_platform_<name>`) and build the platform module's `GotTable::with_static_backing` **WRAPPING it in place — no copy** (the DLL's exported GOT is the single source of truth for callable addresses; BC §3 invariant 3). The dlopen handle the session retains keeps it alive.
5. **Build the SymbolTable** in host memory from the manifest: per fn, `got_slot = manifest index` (manifest array order IS GOT slot order — no host slot allocation), scheme from the **FQ sig** (`parse_type_expr` → `check_type_expr` resolving `primitives/Int`, `shapes/Rectangle` directly), `DefKind::PlatformEffect { scheduling_class }`, + docstring/param-names metadata. Retain the DLL handle on `SymbolTable.dll` (spec §8.9.3).

**`inject_primitives_import_for_platform` (`src/platform.rs:325`) RETIRES** — sigs are fully-qualified, so the platform module carries ZERO injected imports (it was a this-sprint 0233-step-1 bare-name convenience, never load-bearing under FQ sigs; the `parse_and_check_platform_type_sig` rustdoc that cites the injection is corrected to FQ-driven resolution). **The `jit_name`/`JITBuilder::symbol` platform-registration path RETIRES** — the old `(jit_name, ptr)` return vec from `register_platform_in_tc` goes away; fn pointers live in the GOT, dispatched GOT-indirect (backend BC §3), not registered by name. GOT-indirect dispatch replaces the direct `Linkage::Import`-against-mangled-`jit_name` path that worked in `--link` only because the platform rlib is `-force_load`ed (platform-interface.md §6.3 — a correctness/consistency fix, not a bug repair).

**As-built (transitional):** today `register_platform_in_tc` injects `(import [primitives [*]])`, parses each sig FQ-via-`parse_and_check_platform_type_sig`, inserts `Def { PlatformEffect{sched}, got_slot: None }`, and returns `(jit_name, ptr)` pairs registered with `JITBuilder::symbol`; dispatch is direct-extern against the mangled `jit_name`. None of the schema/hash machinery exists yet.

**The `/platform-schema <name>` command (platform-interface.md §5.5.1 / §6.0):** a REPL slash command (`SlashCommand::PlatformSchema(ModuleName)` above), an introspection-family sibling. Its handler — in REPL command dispatch in `src/` (alongside `/imports`/`/exports`) — looks up the loaded platform's `SymbolTable`, calls the **backend** schema generator (the closure-walk + substitution shared with the trace `DisplayDescriptor` baker; BC §3), and prints the emitted schema text (`;; layout-hash:` header + `Map<FQTypeName, Vec<(CtorName, tag, Vec<(Symbol, FieldType)>)>>` body). The author redirects it to `<name>.platform-schema`, which the platform embeds via `include_str!` and rebuilds. The command is a thin caller of the backend generator; the load-time hash check and the `--link` check are the two other callers — **one generator, multiple callers**.

**The `--link` hash check (platform-interface.md §6.0 / §7.3):** because `--link` statically links the platform rlib (no live session), the compiler regenerates the schema from the `.cl` modules it compiled, hashes it, and **bakes the hash into the startup object**; the startup stub compares it against the statically-linked `__cranelisp_layout_hash_<name>` at process start, aborting with rebuild guidance on mismatch (a stale platform *builds* but refuses at *run* — the accepted trade vs reading symbols out of rlib archives at build time). This bake-and-stub-check is exe-bundle / backend territory (BC §3); int's `--link` orchestration drives it.

**The `PlatformError` hash-refusal variant (Decision 0042):** the load-time `--run` refusal and the `--link` refusal both surface as a new `PlatformError` variant (a `cranelisp-types`-hosted enum with `ErrorLocation` carriers per Decision 0042). Authoring the variant is a `cranelisp-types` change flagged for the cascade when the design is actioned.
...
| `src/expander.rs` | `clause_matches`, `find_matching_clause`, `invoke_clause`, `rewrite_spans`, plus the `MacroExpander` impl + its `MacroClauseEntry`/`MacroEntry` lookup shapes | All `pub(crate)`. **The macro-execution callback host (S76 W-Macro, FIXME 0175 resolution).** int implements `cranelisp_types::MacroExpander` here over the per-invocation core: `invoke_clause` (marshal args → signal-protected JIT call via `invoke_jit_protected` → unmarshal) + `rewrite_spans` (fresh synthetic spans on the result). The free-standing **walk** — `MacroResolver` trait, `expand_sexp_recursive`, `expand_macro_call_with_entry`, and the `SymbolTableMacroResolver` + `resolve_macro_definition` chain-walk (`src/worker.rs`) — **DELETES**: the expansion fixpoint is now int's Pass-1 `process_cluster` loop, and macro recognition is the `cranelisp-types` primitive `cranelisp_types::resolve_macro_head` int calls over the committed view (resolution-primitive fold-in; BC §2 invariant 11 + §6; `design/arch/macro-availability-model.md` §0.9; `macro-expansion-ownership.md`). `EXPANSION_DEPTH_LIMIT` moves to typecheck's interior (the depth bound is now typecheck's loop invariant). Clause-body dispatch reads through the unified `Def { kind: DefKind::Macro { clauses_meta }, … }` parent and GOT-dispatches to each `{macro-name}$clause-{N}` mangled-variant Def (Submission 13 macro-unification). The `MacroExpander` impl + invocation core are the durable shape; `clause_matches`/`find_matching_clause`/`invoke_clause` clause-walk-and-match logic is unchanged in substance. |
| `src/marshal.rs` | `sexp_to_runtime`, `runtime_to_sexp`, `build_runtime_slist`, `rc_inc`, `debug_dump_sexp` | Sexp ADT marshalling, called only from the `MacroExpander` impl (`src/expander.rs`) — the int-side macro-execution callback host (S76 W-Macro). `pub(crate)`. Internal cooperation between the marshal substrate and macro execution; not a stable boundary. Relocates to `cranelisp-intrinsics` if/when the macro-Sexp ABI is formalised across the boundary (no near-term plan). |
...
| `src/platform.rs` | `LoadedPlatform` (struct + fields), `resolve_platform_path`, `load_platform_dll`, `register_platform_in_tc`, `is_platform_form`, `extract_platform_name`, `load_and_register_platform` | Platform-loading lifecycle from int's side. `LoadedPlatform` is the type stored in `SharedState.kept_dlls` per the alignment plan; the load helpers compose with `cranelisp_platform::{load_manifest}`. **TARGET (platform-interface.md §6.4 / §7.2) — load path reworked; see §"Platform interface — the load path (TARGET)" below.** `inject_primitives_import_for_platform` (`src/platform.rs:325`) **RETIRES** (sigs are FQ — platform-interface.md §2 q-sig-ref-style); `parse_type_sig`/`derive_jit_name` drop from the composed set (GOT-indirect dispatch + FQ sigs). Internal cooperation, not facade. |
...
| `src/session.rs` | `CacheState::{new, cache_dir, record_recompiled, source_hashes, source_hashes_mut, record_module, flush, flush_manifest, is_cache_valid, record_cache_hit}`, `load_project_config_lib_dirs`, `assemble_lib_dirs`, `assemble_platform_dirs`, `resolve_prelude`, `determine_exit_code`, `inject_prelude_if_needed (pub(crate))`, `apply_bind_chain_analysis (pub(crate))` | `CacheState` is the per-session manifest + hash records held under `SharedState.cache_state`. The free functions are session-init helpers consumed by `CompilerSession::new`. `inject_prelude_if_needed` discovers + loads the prelude module and **sets the per-module `prelude_fallback` bit** (S78 prelude-as-outer-scope, Principle 19) — it no longer flattens prelude bindings into the module table. Internal; absorbed into the SharedState plan's `ObjectCache` cohesion target (PIF-author). |
...
`int::process_cluster` is the sole orchestrator of the cluster-processing chain. It composes `frontend::build_form` (returning `Vec<ParsedEntry>` per S66 FIXME 0156) and the single-call typecheck surface (`cranelisp_typecheck::check_forms` per Decision 44's 2026-05-13 third amendment); catches `ResolutionGap` returns; dispatches to the scheduler; and retries until the cluster fully processes or a non-gap error fires.

**S76 W-Macro — macro expansion is the Pass-1 expand loop, no longer a `frontend::expand` step (FIXME 0175 resolution; DECISION LOCKED 2026-06-03).** The pseudocode below still shows a `frontend::expand` call at step 1; that depiction is **superseded** by `design/arch/macro-availability-model.md` §0 + `macro-expansion-ownership.md`. Under the locked **three-pass** design:
- **Pass 1 (expand)** is `process_cluster`'s expand loop, owned by int. Macro **recognition** is the `cranelisp-types` resolution primitive `cranelisp_types::resolve_macro_head` (resolution-primitive fold-in, 2026-06-03), which int calls directly over the **committed** symbol tables (`View::single(live)` first-hop — no staging exists during Pass 1): descend each form, resolve the head, `Some(fq)` iff the canonical entry is `DefKind::Macro`. This is a `cranelisp-types` query — recognition carries **zero int→typecheck dependency** and replaces the former int-side `SymbolTableMacroResolver` chain-walk (`src/worker.rs`). Macro **execution** is int's, behind the injected `&dyn cranelisp_types::MacroExpander` callback (impl over `src/expander.rs`'s invocation core + `src/marshal.rs`). int `build_form`s each raw-`Sexp` result and re-enters the loop to fixpoint (nested macros + `def`-style structural re-classification). **Dependency-module forms a macro clause needs are typechecked-and-compiled just-in-time** during this loop (pause-and-fetch — the same gap mechanism used for cross-module value/type refs, Decision 0030). **Same-module non-macro definitions are NOT available at expansion** (they are Pass-2/3 entities; round-trip safety — `macro-availability-model.md` §0.3).
- **Passes 2+3 = `check_forms`**, run once over the fully-expanded `Vec<ParsedEntry>`. `check_forms` never triggers macro execution; Decision 44 atomic-commit is intact over the expanded set.

**The dead `block_for_macro_codegen` path is deleted, not wired** — the locked decision forbids same-module non-macro clause callees, so there is no empty-GOT-slot case to populate (the §4.4-flagged "boost the clause's `defn`-callee closure" addition is **withdrawn**; see the "Gap design rationale" note below, which stands unqualified). The `priority_boost_jit` + `wait_for_inmem` gap remains for the **cross-module FQ** half (lazy-load + wait on a dependency module's macro). **The pseudocode below will be re-synced to the three-pass shape when /dev lands; it is retained verbatim for now as the gap-orchestration reference.**
...
- The orchestrator owns the **macro-vs-fn discrimination**. After `wait_for_typecheck_symbol` completes, it peeks at the entry: only forces a JIT (`priority_boost_jit` + `wait_for_inmem`) if the entry actually IS a macro with missing code. Functions are NOT speculatively JIT-pushed — the function will be JIT'd when its caller is processed. This avoids yanking a function ahead of pending priority work for code that expand never actually needs. *(S76 W-Macro DECISION LOCKED — this statement stands **unqualified**. The §4.4-flagged exception — "a macro clause's transitive `defn` callees ARE boosted when the clause is forced in-mem for expansion" — is **WITHDRAWN**: the locked decision forbids same-module non-macro expansion-time references (`macro-availability-model.md` §0.1–0.3), so a macro clause has no same-module `defn` callees to boost. The functions a clause legitimately needs are in **dependency modules**, pulled in by the Pass-1 just-in-time dependency typecheck-and-compile, not by a speculative function-caller boost.)*
...
- Composes `cranelisp_frontend::parse(src)` + the typecheck-driven expansion fixpoint (recognition in typecheck, execution via int's `MacroExpander` impl, per S76 W-Macro / `design/arch/macro-expansion-ownership.md`) for each top-level form, then displays the post-expansion Sexp. (Pre-S76 this composed the retired `cranelisp_frontend::expand`; the slash command surface is unchanged — it shows the fully-expanded form.)
...
- **`cranelisp-frontend`** — `parse`, `build_form` (returns `Vec<ParsedEntry>` per S66 FIXME 0156; replaces the prior `build_ast` shape at the per-form boundary), `build_expr`, `extract_module_declarations`, `synthesize_macro_clause_defn`, `next_synthetic_span`, `is_defmacro`, `is_begin`, `flatten_begin`, `expand_quasiquotes`, `parse_preserving_comments`, `ParseProduct`, `Ast`. (Per FIXME 0156 resolution: `parse_defmacro` becomes `pub(crate)` inside `build_form`'s dispatcher; `DefmacroInfo` and `ParsedEntry` move to `cranelisp-types` — int imports both from there.) **`expand` + `ExpansionError` retired (S76 W-Macro, FIXME 0175):** macro recognition moved to typecheck, execution to int's `MacroExpander` impl — int no longer consumes a frontend `expand`. See `design/arch/macro-expansion-ownership.md`. **S81 (FIXME 0329):** int will additionally consume the frontend's new **form-sequence builder** (the top-level `:Type` annotation-pairing entry — BC §1 "S81 — top-level `:Type` annotation-pairing builder"), replacing `build_program_compat`'s per-sexp `build_expr` driving for the bare-expression branch so top-level `:Type value` pairs into `Expr::Annotate` in the frontend, not int. Annotation-pairing is NOT re-implemented int-side (BC §6 "Annotation-pairing is frontend-driven").
...
- **`cranelisp-platform`** — `HostContext`, `HostCallbacks`, `OwnedPlatformFnDescriptor`, `PlatformFn`, `load_manifest`. `int` constructs `HostCallbacks` at session init pointing at runtime fns. **TARGET (platform-interface.md):** `parse_type_sig`/`derive_jit_name` consumption RETIRES (FQ sigs route through `parse_type_expr`/`check_type_expr`; dispatch is GOT-indirect through the DLL's exported GOT, not by mangled `jit_name`); `HostCallbacks` shrinks (`validate_schema` drops — the layout-hash gate replaces validation, platform-interface.md §5.5.4); `ABI_VERSION` 2→3.
