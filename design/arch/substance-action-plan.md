...
**Progress.**
- Step 0 (register migration) — **DONE**, commit `1eeae53`. Decisions, Principles, FIXMEs relocated to `design/arch/{decisions,principles,fixmes}/`.
- Step 1a (structural separation) — **DONE**, commit `3316599`. Non-canonical `design/arch/*.md` working/subsystem docs moved to `design/arch/legacy/` (sorting bucket; per-crate `/design` skills pull back content as needed).
- Step 1b (substance commitments) — in progress.
- Step 1c (procedural reconciliation), Step 1d (acceptance gate) — pending.
- Steps 2–4 — pending Step 1 close.
