...
fn platform_fn_non_io_return_is_rejected() {
    ...
    match err {
        CranelispError::ModuleError { message, .. } => {
            assert!(
                message.contains("shapes/area"),
                "names platform/fn: {message}"
            );
            assert!(
                message.contains("IO"),
...
fn test_register_platform_in_tc() {
    ...
    assert!(
        read_entry.is_some(),
        "read-line should be in platform.stdio"
    );
...
fn test_assemble_platform_dirs_env_var() {
    ...
    let platform_dirs = crate::session_setup::assemble_platform_dirs(dir.path());
...
fn split_slashed_type_ref_rejects_non_type_leaves() {
    assert!(
        split_slashed_type_ref("Rectangle").is_none(),
        "no slash → no split"
    );
    assert!(
        split_slashed_type_ref("a").is_none(),
        "bare type var → no split"
    );
    assert!(
