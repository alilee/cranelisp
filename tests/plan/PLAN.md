...
| §12.3.1 composed application residual | `exemplar_ownership_residue_s116::sudoku_warm_serial_solve_residue_at_most_1400` | downstream observer; passing at the existing ≤1400 threshold after the S120 0917 correction; exact re-derivation remains separately allocated and is not 0917 acceptance authority |
...
| spec §5.2.6 + §8.5.2, field accessors mint uniformly across definition variants — the CONSTRUCTOR-ARM axis (0867, re-reduced W1: polymorphism is NOT causal; a named ctor arm whose name differs from the type mints no accessors — every sum type, every distinct-name product; §5.2.6's own `Option.unwrap` example non-conforming) | `tests/spec_field_accessor.rs::polymorphic_product_mints_canonical_and_unique_bare_accessors` + the W1 variant × polarity matrix (distinct-name concrete arm RED, poly deftype-level control GREEN) with `bare_alias_resolves_when_field_unique` as the concrete control | `[S118]` RED landed W1, failing-not-ignored. Attribution FINALIZED (`s118-test-plan.md` §6.2): `/dev`(typecheck), `adt.rs::synthesise_field_accessors` gated on `is_product`/`ctor_infos[0]` only; `class=enumeration-miss` ratified. Fix capacity-dependent; unfixed at close needs an explicit user-approved carry |
...
| spec §12.3.1 + §6.2.4, var-pattern arm consuming an owned temporary releases exactly once (0782) | `var_pattern_arm_consuming_owned_temporary_releases_it_once_linked` | `[S118]` GREEN at HEAD `e15ff20f` **without a fix — NOT closure** (`s118-test-plan.md` §2.6): the double release is live in the emitted CLIF (two `atomic_rmw.i64 sub` on the same scrutinee, verified 2026-07-25); the `--link` abort is layout-latent. Cell stays as the 0782 regression guard; 0782 closes only with its fix change-set + one-release CLIF evidence |
...
| memory-safety-coverage §2.1, eliminator axis in the generative harness (FIXME 0830, closed W1) | `gen_ownership_flows` positions `matched_in_place`, `let_bound_then_matched`, **`matched_in_tail_loop`** (the operative row — the two proposed rows measured GREEN with 0810 live; only match-as-tail-loop-body reaches the tail-jump seam; `s118-test-plan.md` §4.2) | `[S118]` landed W1; `matched_in_tail_loop` RED for `adt_with_heap_field`, flips at Track B W3; exact-balance, `--link`-face caveat binding; 0810 Face B named as the v2 residual |
| spec §12.3.1, SList/Sexp construct/consume ownership (0835 runtime pair) — leak face exact-balance + abort face on the runner path, process-abort-guarded | `tests/slist_sconcat_ownership_0835.rs` repros A + B + GREEN controls | `[S118]` RED landed W1; flips at W2b runtime-pair fix, NOT Track B; abort-face survival after the fix is a NEW attribution |
| spec §12.3.1, prelude load itself is allocation-balanced (ambient macro-expansion residue, `s118-test-plan.md` §2.5) | W2b rider: one prelude-face cell (trivial program + macro-invoking mini-prelude fixture, exact balance) | `[S118]` `/qa`-directed, authored by `/testing` IN the W2b change-set; the P4 probe re-run is the W2b acceptance check; today's cells #10/#19/#20/#23 carry the face |
...
| spec §10.2/§10.3, releasing a concrete `IO T` value compiles and balances (FIXME 0907 — Bind's existential ctor defeats canonical glue derivation) | committed carriers: `spec_10_io::{pure_pattern_accepted, match_arms_all_io_pure, io_internal_ctors_stay_excluded_from_exhaustiveness_neg}`, `ctor_as_value` ×2, `examples` (21-hello-io/23-io-sequence), `stdlib_conformance` (core.io) — 7 RED cells, W3-surfaced; minimal repro in 0907 | `[S119]` flips with the 0903/0907 co-ruling's implementing wave; attribution record `s118-test-plan.md` §11.1 |
| spec §12.3.1 + §5.2.6, synthetic accessor of a generic/undeclared-field product releases its `self` transitively (0903 family 1 — leaks today, shallow dec) | S119 W1 marginal-balance guard: subject/control pair differing in exactly one accessor call over a `(deftype (Box a) [:a v])`-shaped product; exact marginal balance | `[S119]` QA-first, authored BEFORE the 0903 implementing wave (0765 precondition); decision + rationale `s118-test-plan.md` §11.2 |
| spec §12.3.1 + §7.x, generic trait-method instance releases its closure/heap params transitively (0903 family 2 — leaks today, shallow dec; **and RC-ops residual-`Var` slots as pointers — the 0916 wild-write face, memory-UNSAFE**) | S119 W1 marginal-balance guard: subject/control pair differing in exactly one HKT/generic trait-method invocation (`Functor.fmap$primitives/Option` shape); exact marginal balance; **MUST include a wild-write subject (scalar payload ≥ `NULLARY_TAG_THRESHOLD`, exit-status assert) per `s118-test-plan.md` §11.8.5** | `[S119]` QA-first, with family 1 above; cell #21 is NOT this class's carrier (re-attributed to 0917 at P6 close, §11.8.1); the P6-batch 0916 pair carries the wild-write face meanwhile |
| spec §12.3.1, a match arm returning a nullary ctor beside a boxed arm must not strand the caller's tree (FIXME 0917 — unbalanced guarded protect inc at `protect_return_value`, backend) | `nullary_arm_beside_boxed_arm_0917::{nullary_arm_beside_boxed_arm_frees_its_loop_under_run, nullary_arm_beside_boxed_arm_frees_its_loop_under_link}`; subject/control differs only in the arms' returned ctors | `[Tested]` exact marginal zero under `--run --no-cache` and `--link`; S120 correction; attribution `s118-test-plan.md` §11.8.1 |
| spec §05-functions TCO §, self tail call beside a trait-dispatched scrutinee survives — and no scalar in a residual-`Var` slot is RC'd as a pointer (FIXME 0916) | P6-batch triple: trait-scrutinee subject n=2000 (exit-status), boundary control n=1023, plain-`defn` control n=400000 | `[S118]` subject RED in the P6 close batch (SIGSEGV = wild write at payload+8, threshold exactly 1024); flips with the 0903/0916 co-ruling; `s118-test-plan.md` §11.8.5 |
| spec §12.3.1 + result-owner design, a REPL result with a residual type parameter is released like a concrete one (FIXME 0913 — lenient-view placeholder, typecheck) | P6-batch marginal REPL-children pair: N `(Err "boom")` turns vs the same turns annotation-pinned; exit-counter instrument (`CRANELISP_ALLOC_PARITY_DUMP`, not `/mem` per 0914); exact marginal 0 | `[S118]` authored RED in the P6 close batch; fix = `/design`(typecheck) S119 lenient-view row; `s118-test-plan.md` §11.8.3 |
| mode equivalence, exemplar standalone `--link`-then-run parity (the 0875 recurrence guard — symptom not reproducible at HEAD, FIXME closed §11.8.2) | standing cell: fresh scratch exemplar tree, no cache, `--link` + run produced binary, stdout byte-identical to `--run` | `[S118]` authored GREEN in the P6 close batch |
| repl spec §5.5, compiler-stage diagnostic frame (located span, single prefix, user-spelled subject — FIXME 0915) | frame guard over a codegen refusal with a post-0907 trigger | `[S119]` deferred — every current e2e trigger is 0907's refusal; authored in the 0907/0903 fix window; attribution split `s118-test-plan.md` §11.8.4 |
