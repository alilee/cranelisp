...
| **Type-signature match predicate (Pillar 3 importable-symbol search) — alpha-equivalence / exact-structural-shape** | **`signature-match.md`** | **Current** (Sprint 90, agentic-REPL fluency phase; **design-only** — Pillar 3 implementation gated on the FIXME 0432 root fix). Pins: MVP = name-fragment + **exact-structural-shape** match (alpha-equivalence up to consistent bijective var renaming, **NO unifier**); the pure free-function interface `signature_matches_exact(&Type, &Type) -> bool` the `int` indexer calls; canonicalise-then-`==` via reused `collect_var_ids_ordered` (Principle 7); FQ-ADT discipline; Hoogle subsumption + query-hole syntax recorded as explicit deferred follow-ups (`/typecheck` algorithm / `/spec` consult, NOT this sprint). Cites `repl-embedded-agent.md §11.2/§11.4` (R3/R6). |
