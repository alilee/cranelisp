...
| `rc_inline_adt_arg_cross_module_equivalence` | RC + Modules + Consuming convention | [added S61 S5 J, 2026-04-22] Sprint 61 Slice 2 Layer 3 shape: `(f (Ctor [heap-val]))` where `f` is defined in module B and `Ctor` constructed in module A, MUST be observationally equivalent to `(let [x (Ctor [heap-val])] (f x))` — final value, RC balance, bytes before/after, no SIGSEGV. Parameter domain: `T` ∈ `{Int, String, (Vec Int), (Option Int)}`. Pins the `borrowed_vars` rule (`design/backend/ring2-rc.md §5.5`) at cross-module boundaries. Reference: `tests/plan/sprint-61-plan-gap-retro.md`. NOT authored Sprint 61; reserved for S62+. |
