...
/// Check if `main` returns `IO` by inspecting its scheme across all modules.
pub fn main_returns_io(modules: &HashMap<ModuleFullPath, CompiledModule>) -> bool {
    for cm in modules.values() {
        if let Some(crate::module::ModuleEntry::Def { scheme, .. }) = cm.get("main") {
            if let crate::types::Type::Fn(_, ret) = &scheme.ty {
                return matches!(ret.as_ref(), crate::types::Type::ADT(name, _) if name == "IO");
            }
            return false;
        }
    }
...
/// 3. If `main_returns_io`, forces IO task tree via `cranelisp_run_io`
/// 4. Truncates i64 to i32 via `ireduce`
/// 5. Calls libc `exit(i32)`
pub fn generate_startup_object(
    ...
    // Declare `cranelisp_run_io` as imported (trampoline, takes i64 IO ptr, returns i64)
    ...
    let mut func_ctx = FunctionBuilderContext::new();
    {
        ...
        // If main returns IO, force the task tree via trampoline
