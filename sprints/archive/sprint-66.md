...
**Status**: PHASE 7 CLOSED (2026-05-14)
...
**Sprint closed 2026-05-14**. All 8 crate facades hammered in at their edges; D43 runtime split complete; cluster-atomic typecheck protocol active end-to-end. Workspace at 58 unique test failures vs S64 baseline of 33 — net +25, of which ~22 are documented baseline carries (FIXME 0121/0142/0145/0148) and the remainder are 6 named follow-up FIXMEs (0099/0172/0177-residual/0179-followup/0180/0181).

### Delivered

**Architectural** (canonical state changes):
- **D43 runtime split complete**: `cranelisp-runtime` physically retired (commit `c04b5bf`). Source split into `cranelisp-intrinsics` (backend-emitted-call targets: alloc, drop, io, ivar, panic, rc, string-internals, trace, vec-internals, ops) and `cranelisp-primitives` (user-callable: string user APIs, marshal, int/float/bool, vec-len, Ring 0 shims). 8 canonical crates → 7.
- **Cluster-atomic typecheck protocol active** (Decision 44 third amendment, commit `a2dcebd`). Single `check_forms(parsed, &mut ctx, symbol_tables) -> Result<(), CheckError>` entry. Staging write redirection + View-based read union both active. Activation flip recovered 2 atomicity tests (`failed_redefn_*`).
- **Pattern B impl resolution** (Decision 45): TraitImpl entries live in trait's defining module; resolution via per-symbol chain-follow.
- **Principle 17 module locality**: short-name lookups current-module-only; chain-follow for cross-module; synthetic modules empty `imports`/`exports`.
- **FQSymbol Option B convention**: trait-method bindings use composite `Symbol::from("Trait.method")` within the trait's defining module; `FQTypeName` unchanged (commit `7518f01`).
- **Ring 0 primitives uniform dispatch** (commit `94dfd2a`): 23 Rust shim fns published from cranelisp-primitives; GOT slots allocated; `operators.rs` reshaped to `primitives_inline.rs` (optional substitution, not the only emission path); GOT-indirect fallback enables mappable-path (e.g., `(let [f not] (f true))`).
- **Intrinsic unification** (commit `9efb54b`): `run-test`, `discover-tests`, `cranelisp_trace_format` registered unconditionally at JIT setup. Conditional `program_uses_test_forms` / `program_needs_trace` scanning machinery deleted. `TestRunnerState` lifted to `SharedState` (single init, session-stable). `facades/backend.md` non-goals section forbids operator special-casing.

**Per-crate facade compliance** (commits across the sprint):
- `cranelisp-types`: View enum + ClusterContext + PlatformError + FQSymbol convention (commit `7518f01`, baseline `98f5554`)
- `cranelisp-frontend`: `build_form -> Vec<ParsedEntry>` + `expand` migration + `SymbolTables` alias (commit `93deca0`)
- `cranelisp-typecheck`: `check_forms` single-call + ClusterContext + write-redirect + read-union; ADT match exhaustiveness FQ fix (commits `55066e8`, `2dc12a9`, `3f166f1`, `a2dcebd`, `7680bc9`)
- `cranelisp-backend`: display.rs deleted; GotObserver authored; operators.rs reshaped to primitives_inline.rs (commits `3b9e254`, `94dfd2a`); migrated off cranelisp_runtime imports (`2d1a82b`)
- `cranelisp-intrinsics`: 9 source files absorbed from runtime; IoObserver hosted; ops module added (commits `9e4d9b1`, `e4cc3c9`)
- `cranelisp-primitives`: user-callable bodies relocated; Ring 0 shims; view via re-export from intrinsics (commits `6654a6e`, `ac98de7`, `94dfd2a`)
- `cranelisp-platform`: PlatformError pre-landed; unit test coverage added (commit `127d054`)
- `cranelisp-exe-bundle`: staticlib re-exports migrated from runtime (commit `a05c231`)
- `cranelisp` (int binary): consumer-side migration of display, observers, PlatformError; Cargo dep swap; runtime references zero (commits `0fd858c`, `1bb3b0e`, `bbd60e3`)

**Test infrastructure**:
- `tests/trait_imports.rs` authored (commit `d841943`): 10 tests, FQSymbol-shape-agnostic, decouples trait-import regression from stdlib drift; 1 test recovered when Ring 0 GOT slots landed
- `public_api_check_runs_against_all_seven_crates` (renamed from `_eight_` post-runtime-retirement, passing)
- `process_form_dispatch_*` cluster-atomic gates: 3/4 passing (remaining `macro_after_import_succeeds_in_one_eval` is FIXME 0181 territory)
- The Wave 3a-tail cross-call poly bug (TypeId collision) became a permanent regression guard via typecheck unit test `check_forms_cross_call_constrained_poly_mono_terminates`

**FIXMEs closed**: 0099 (GotObserver authoring; consumer-half persists), 0103 (IoObserver host + trace migration), 0104 (PlatformError adoption), 0107 (non_exhaustive — verified zero edits needed), 0108 (display.rs relocation), 0150 (runtime split — Phase 5 done, file deleted), 0167/0168/0169 (Decision 44 amendments), 0173 (mode_equiv_macro_user_defined), 0174 (operators.rs uniform dispatch), 0175 → consolidated under primitives-inline reshape, 0177 (state-threading hole — closed by collapse), 0178 (intrinsic inventory in facade — landed inline in facade text), 0179 (read-union via View — landed in 3b-2c.3)

### Deferred (with rationale)

- **FIXME 0121** (spec_08_modules `(mod ...)` discovery + glob/transitive re-exports, ~11 tests): pre-S66 baseline carry; owns `/int`. Not in S66 facade-adoption scope.
- **FIXME 0142** (REPL parse_error_unclosed_paren_neg): pre-S66 baseline carry; owns `/int`.
- **FIXME 0145** (regression d6_exemplar SEGVs, ~4-5 tests): pre-S66 baseline carry; owns `/backend`.
- **FIXME 0148** (regression wave6_exemplar_solver stack overflow): pre-S66 baseline carry.
- **FIXME 0172** (trait-method short-name resolution under Principle 17, ~4 stdlib_trait_impls + ~2 trait_imports mappable paths): named in `design/typecheck/wave-3a-check-form.md §8`; out of S66 cluster-atomic scope per design. Target /int — per-method `ModuleEntry::Import` injection through prelude loading.
- **FIXME 0176** (int Wave A+B+C structural items: SharedState refactor, display.rs into-int, broader file relocations beyond what 4a.pre.exe-bundle did): scope-deferred multi-day work; facade compliance achieved without these by routing through compat shims that the facade text now treats as the durable surface.
- **FIXME 0180** (primitives string/vec physical relocation from intrinsics, unblocked post-runtime-retirement): Cargo cycle dissolved; primitives can now physically host string/vec bodies if desired. Cosmetic facade-purity follow-up.
- **FIXME 0181** (cross-module 3-module macro chain stack-overflow, 4 spec_09_macros tests): filed today during autonomous /loop isolation. Minimal repro included. Same isolation technique as the Wave 3a-tail poly fix recommended. Target /typecheck.
- **macro_after_import_succeeds_in_one_eval** (1 process_form_dispatch test): synthetic-DLL fixture infrastructure + frontend macro-invocation gap (FIXME 0175 territory consolidated into primitives-inline).
- **public-api drift micro-cleanup**: post-Wave-3a-α residual short-name fallback chains in typecheck bootstrap (FIXME 0172 site 1 + site 2 in checker.rs); cosmetic now that the architecture is correct.

### Findings

- **Compliance as forcing function worked.** Adopting the new facade at each crate's edge surfaced design holes the paper specs didn't reveal:
  - Wave 3a-α-then-β's typecheck collapse exposed the cross-call state-threading hole (FIXME 0177) — closed by collapsing two-pass split into single `check_forms` (Decision 44 third amendment, /arch round 2026-05-13).
  - The Cluster mode activation in 3b-2c.3 exposed write-then-read-back-on-staging patterns in `register_type_def` and trait-impl default-method registration. Resolution: read-union via View (FIXME 0179) + write-redirect via TypeCheckEnv staging override (3b-2c.1).
  - The Wave 3b-1 publish/consume split exposed that producer-side surfaces (IoObserver, GotObserver, PlatformError) need explicit consumer wiring; led to a clear Wave 3b-2a/b/c sequence.
  - Operators.rs special-casing exposed that Ring 0 primitive Def entries needed GOT slots + code pointers to support mappable path — the gap behind `stdlib_not_mappable_path` failure.
- **Decision 44 third amendment was load-bearing.** The two-pass `check_form_signatures` + `check_form_body` split (FIXME 0167 era) was an over-decomposition that exposed implementation phasing across the facade. User-arbitrated collapse to `check_forms` simplified the facade AND closed the state-threading hole by construction. The original /design phase produced docs that looked consistent but didn't specify where Pass-1-to-Pass-2 working state lived.
- **Agent narrow-deployment requires pre-flight workspace surveys.** Wave 4a's first fire halted correctly when it discovered backend's 92 refs to `cranelisp_runtime::*` + exe-bundle's 11 refs — β-3's scoping had focused on int alone. Pre-flight grep across the workspace before scoping `/dev` fires would have caught this earlier. Recorded as a methodology improvement for future sprints.
- **Stack-overflow regressions resolve to specific TypeId collisions or recursion sites.** The Wave 3a-tail poly fix (`7680bc9`) — TypeId collision in `instantiate_scheme` between fresh per-call AtomicU32 vars and stored scheme TypeIds — was the canonical example. The cross-module macro overflow (FIXME 0181) likely follows the same shape: a per-call counter or chain-follow that loses termination guarantee when crossing a module boundary it didn't expect.
- **Architectural principles 15 + 17 are correctly load-bearing.** Principle 17 (module locality) drove the chain-follow primitive, the universe-FQ KnownTypes tier, and the FQSymbol Option B convention — these architectural artifacts emerged from facade compliance not from abstract specification. Principle 15 (single-consumer types) was clarified mid-sprint when ModuleCheckAccumulator's relocation to int was retracted — the clarifying paragraph added to `principles/15-*.md` documents the distinction between "consumer of structure" and "holder of opaque carrier" so the misapplication doesn't recur.
- **Test count delta**: workspace tests grew from ~990 (S64 close) to ~1030 (S66 close) — adds include trait_imports (10), platform unit tests (6), io_observer unit tests (5), got_observer unit tests (5), Ring 0 primitive unit tests (13), typecheck cluster-mode tests (3), and assorted others. Coverage broadened in the facade-touched surfaces.
- **Sprint sizing observation**: Wave 3 expanded from the original Sprint 65 estimate by ~6-9 days due to Wave 3a's three architectural rounds (FIXME 0167 → 0168 → 0169 → user-arbitrated Decision 44 third amendment) and Wave 3a-α's locality refactor (Decision 46). The cause was the original /design phase undersizing the cross-cutting structural work. The Phase-7 sequencing into smaller per-wave fires (3b-2a through 3b-2d.3) emerged organically when the previous int /dev agent declined a too-broad scope and filed FIXME 0176; this sequencing pattern is now durable methodology.
