...
The three string **query** primitives (`Str → Bool`-ish, returning 0 / 1 in i64) carry trailing-`?` Clojure-style predicate names at the symbol-table layer per `#[export_name = "…"]`: `contains?` (`str_contains`), `starts-with?` (`str_starts_with`), `ends-with?` (`str_ends_with`). The `?` suffix is the user-visible binding — the substring-tests `str-contains` / `str-starts-with` / `str-ends-with` on the underlying Rust identifier are NOT the names a user sees. (The remaining string ops likewise drop the `str-` prefix at the symbol-table layer: `char-at`, `substring`, `to-upper`, `to-lower`, `trim`, `split`, `join`, `replace` — only `str-concat`, `str-len`, `str-eq` retain the prefix where the bare form would collide with a list/vec op of the same name.)
