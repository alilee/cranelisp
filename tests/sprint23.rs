...
// =============================================================================
// 2. /reset Command
//    Spec: repl/spec.md §12
// =============================================================================

// --- 2.1 Basic semantics ---

// spec: repl/spec.md §12.1 — clear all user definitions
#[test]
fn reset_clears_user_definitions() {
    // Define a function, /reset, then try to use it — should error.
    let input = "(defn foo [] 42)\n(foo)\n/reset\n(foo)\n/quit\n";
    let output = run_repl(input, "reset_clears_defs");
    let out = stdout_str(&output);
    // After reset, (foo) should produce an error, not 42
    // The second (foo) call should not succeed
    assert!(
        out.contains("error") || out.contains("undefined") || out.contains("not found"),
        "after /reset, user-defined 'foo' should not be in scope: {out}"
    );
}

// spec: repl/spec.md §12.1 — clear all imports
#[test]
fn reset_clears_imports() {
    // Import something, /reset, then the imported name should be gone.
    // Uses a test module — if that's not feasible, test with prelude re-import.
    let input = "/reset\n/quit\n";
    let output = run_repl(input, "reset_clears_imports");
    let out = stdout_str(&output);
    assert!(
        out.contains("Session reset"),
        "should display reset confirmation: {out}"
    );
}

// --- 2.2 Output format ---

// spec: repl/spec.md §12.2 — confirmation message
#[test]
fn reset_displays_confirmation() {
    let input = "/reset\n/quit\n";
    let output = run_repl(input, "reset_confirmation");
    let out = stdout_str(&output);
    assert!(
        out.contains("Session reset."),
        "should display 'Session reset.': {out}"
    );
}

// spec: repl/spec.md §12.2 — timing reset to 0+0ms
#[test]
fn reset_timing_shows_zero() {
    let input = "(add-i64 1 2)\n/reset\n/quit\n";
    let output = run_repl(input, "reset_timing");
    let out = stdout_str(&output);
    // After reset, the prompt should show 0+0ms
    assert!(
        out.contains("0+0ms"),
        "prompt after /reset should show 0+0ms: {out}"
    );
}

// --- 2.3 Module reset ---

// spec: repl/spec.md §12.3 — interaction with /mod
#[test]
fn reset_returns_to_user_module() {
    // Switch to a different module, /reset, should return to user>
    let input = "/mod math\n/reset\n/quit\n";
    let output = run_repl(input, "reset_mod");
    let out = stdout_str(&output);
    // After reset, prompt should show "user>" not "math>"
    assert!(
        out.contains("user>"),
        "after /reset, should be back in user module: {out}"
    );
}

// spec: repl/spec.md §12.3 — /mod modules discarded
#[test]
fn reset_discards_mod_definitions() {
    // Create definitions in a /mod module, /reset, they should be gone.
    let input = "/mod mymod\n(defn bar [] 99)\n/reset\n/mod mymod\n(bar)\n/quit\n";
    let output = run_repl(input, "reset_mod_defs");
    let out = stdout_str(&output);
    assert!(
        out.contains("error") || out.contains("undefined") || out.contains("not found"),
        "definitions in /mod module should be cleared after /reset: {out}"
    );
}

// --- 2.4 Prelude reload ---

// spec: repl/spec.md §12.1 step 4 — reload prelude
#[test]
fn reset_reloads_prelude() {
    // After reset, prelude-provided operators should still work.
    let input = "/reset\n(+ 1 2)\n/quit\n";
    let output = run_repl_with_test_prelude(input, "reset_prelude");
    let out = stdout_str(&output);
    assert!(
        out.contains("3"),
        "prelude operators should work after /reset: {out}"
    );
}

// spec: repl/spec.md §12.2 — prelude load failure
#[test]
fn reset_prelude_failure_shows_error() {
    // If prelude fails to load, should show "Session reset (no prelude)."
    // Hard to test without breaking prelude — this is a defensive test.
    // Verify the positive path for now.
    let input = "/reset\n/quit\n";
    let output = run_repl(input, "reset_prelude_fail");
    let out = stdout_str(&output);
    assert!(
        out.contains("Session reset"),
        "should show reset confirmation: {out}"
    );
}

// --- 2.5 Cache preservation ---

// spec: repl/spec.md §12.5 — object cache preserved
#[test]
fn reset_preserves_object_cache() {
    // After /reset, cached .o files should still exist on disk.
    // The prelude reload after reset should use cached objects (fast).
    let input = "(+ 1 2)\n/reset\n(+ 3 4)\n/quit\n";
    let output = run_repl_with_test_prelude(input, "reset_cache");
    let out = stdout_str(&output);
    // Both computations should succeed
    assert!(out.contains("3"), "first (+ 1 2) should work: {out}");
    assert!(out.contains("7"), "post-reset (+ 3 4) should work: {out}");
}

// --- 2.6 Performance ---

// spec: repl/spec.md §12.6 — performance target (500ms)
#[test]
fn reset_completes_within_500ms() {
    // Measure wall-clock time for /reset — should be under 500ms with warm cache.
    let input = "/reset\n/quit\n";
    let start = std::time::Instant::now();
    let _output = run_repl_with_test_prelude(input, "reset_perf");
    let elapsed = start.elapsed();
    assert!(
        elapsed.as_millis() < 2000, // generous — includes process startup
        "/reset took too long: {:?}",
        elapsed
    );
}

// --- 2.7 /help integration ---

// spec: repl/spec.md §12.7 — /reset in /help output
#[test]
fn reset_appears_in_help() {
    let input = "/help\n/quit\n";
    let output = run_repl(input, "reset_help");
    let out = stdout_str(&output);
    assert!(
        out.contains("/reset") && out.contains("lear"),
        "/help should list /reset with description: {out}"
    );
}

// --- 2.8 Negative tests ---

// spec: repl/spec.md §12.1 — user state must NOT survive reset
#[test]
fn reset_neg_no_type_leakage() {
    // Define a type, reset, the type should not be available.
    let input = "(deftype Color Red Green Blue)\n/reset\nRed\n/quit\n";
    let output = run_repl(input, "reset_neg_type");
    let out = stdout_str(&output);
    // After reset, 'Red' constructor should not be in scope
    assert!(
        out.contains("error") || out.contains("undefined") || out.contains("not found"),
        "user-defined types should not survive /reset: {out}"
    );
}

// spec: repl/spec.md §12.1 — macros must NOT survive reset
#[test]
fn reset_neg_no_macro_leakage() {
    let input = "(defmacro my-mac [x] x)\n/reset\n(my-mac 42)\n/quit\n";
    let output = run_repl(input, "reset_neg_macro");
    let out = stdout_str(&output);
    assert!(
        out.contains("error") || out.contains("undefined") || out.contains("not found"),
        "user-defined macros should not survive /reset: {out}"
    );
}
