...
impl TypeChecker {
            ...
            TypeExpr::Applied(name, args) => {
                if let Some(&con_id) = con_var_map.get(name) {
                    ...
                    let resolved_args: Vec<Type> = args
                        ...
                        .map(|a| self.resolve_type_expr_hkt(a, con_var_map, type_var_map))
                        .collect();
                    let bare = crate::names::bare_from_qualified(name);
                    Type::ADT(bare.to_string(), resolved_args)
                }
            }
    ...
    pub(crate) fn validate_impl(&self, ti: &TraitImpl) -> Result<(), CranelispError> {
        ...
        // Check that all required methods (those without defaults) are implemented
    ...
    /// Check whether an impl covers a given method — either explicitly or via a trait default.
    fn impl_covers_method(&self, ui: &TraitImpl, method_name: &str) -> bool {
        if ui.methods.iter().any(|m| m.name == method_name) {
            return true;
        }
        if let Some(decl) = self.find_trait_decl(&ui.trait_name) {
            return decl
                .methods
                .iter()
                .any(|m| m.name == method_name && m.default_body.is_some());
        }
        false
    }
    ...
    fn find_impl_for_type(
        ...
        if !concrete_args.is_empty() && concrete_args.iter().all(|a| !has_type_var(a)) {
            for ui in self.all_impls() {
                ...
                if !self.impl_covers_method(ui, method_name) {
                    continue;
                }
        ...
        for ui in self.all_impls() {
            if ui.trait_name == trait_name
                ...
                && self.impl_covers_method(ui, method_name)
            {
                return Some(ui.impl_target_mangled());
            }
        }
    ...
    fn resolve_one_method(
        ...
        match &concrete {
            Type::Var(_) => {
                ...
                let all_impls_vec: Vec<_> = self.all_impls().cloned().collect();
                let matching: Vec<String> = all_impls_vec
                    .iter()
                    .filter(|ui| {
                        ui.trait_name == trait_name
                            && self.impl_covers_method(ui, method_name)
                ...
                if matching.len() == 1 {
                    ...
                    let mangled_name =
                        crate::ast::mangle_impl_method(trait_name, method_name, target_mangled);
                    // Try to unify with the target type
                    let target_type_name = &all_impls_vec
                        .iter()
                        .find(|ui| {
                            ui.trait_name == trait_name
                                && self.impl_covers_method(ui, method_name)
                    ...
                    self.find_impl_for_type(trait_name, method_name, &concrete)
                {
                    ...
                    if let Type::ADT(base_name, args) = &concrete {
                        if args.iter().any(has_type_var) {
                            ...
                            let matching_impl = all_impls_vec2
                                .into_iter()
                                .find(|ui| {
                                    ui.trait_name == trait_name
                                        && ui.target_type == *base_name
    ...
    pub fn resolve_methods(&mut self) -> Result<MethodResolutions, CranelispError> {
        ...
        for (span, trait_name, method_name, arg_type) in pending {
            if let Some(resolved) =
                self.resolve_one_method(span, &trait_name, &method_name, &arg_type)?
            {
...
/// Counter for unique synthetic spans in default method bodies.
static DEFAULT_SPAN_COUNTER: AtomicUsize = AtomicUsize::new(2_000_000);

/// Rewrite all spans in a Sexp tree to unique synthetic values.
fn rewrite_default_sexp_spans(sexp: &Sexp) -> Sexp {
    let next_span = || -> Span {
        let n = DEFAULT_SPAN_COUNTER.fetch_add(1, Ordering::Relaxed);
        (n, n)
    };
    match sexp {
        Sexp::Symbol(s, _) => Sexp::Symbol(s.clone(), next_span()),
        Sexp::Int(v, _) => Sexp::Int(*v, next_span()),
        Sexp::Float(v, _) => Sexp::Float(*v, next_span()),
        Sexp::Bool(v, _) => Sexp::Bool(*v, next_span()),
        Sexp::Str(s, _) => Sexp::Str(s.clone(), next_span()),
        Sexp::List(children, _) => Sexp::List(
            children.iter().map(rewrite_default_sexp_spans).collect(),
            next_span(),
        ),
        Sexp::Bracket(children, _) => Sexp::Bracket(
            children.iter().map(rewrite_default_sexp_spans).collect(),
            next_span(),
        ),
    }
...
/// Synthesize `Defn` nodes for trait methods with defaults that are not
/// explicitly overridden in the impl.  Each `Defn` gets a mangled name
/// (`method$target`) and unique synthetic spans so the typechecker can
/// process them without span collisions.
pub fn synthesize_default_defns(
    decl: &TraitDecl,
    impl_methods: &[Defn],
    target: &str,
    ...
    for method_sig in &decl.methods {
        // Skip methods that are explicitly implemented
        if impl_methods.iter().any(|m| m.name == method_sig.name) {
            continue;
        }
        // Skip methods without defaults
        let body_sexp = match &method_sig.default_body {
            Some(sexp) => sexp,
            None => continue,
        };

        // Rewrite spans to unique synthetic values
        let rewritten = rewrite_default_sexp_spans(body_sexp);
        let body = build_expr(&rewritten)?;

        // Build param annotations from the TypeExpr params.
        // SelfType is resolved via pre-unification, not annotation, so map it to None.
        let annotations: Vec<Option<TypeExpr>> = method_sig
            .params
            .iter()
            .map(|p| match p {
                TypeExpr::SelfType => None,
                other => Some(other.clone()),
            })
            .collect();

        // Get a unique span for this synthesized defn
        let defn_span = {
            let n = DEFAULT_SPAN_COUNTER.fetch_add(1, Ordering::Relaxed);
            (n, n)
        };
        ...
        defaults.push(Defn {
            ...
            name: crate::ast::mangle_impl_method(&decl.name, &method_sig.name, target),
            docstring: None,
