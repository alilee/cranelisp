...
fn parse_and_build_repl(input: &str) -> Result<TopLevel, CranelispError> {
    ...
    if is_top_level_form(&sexps[0]) {
        ...
        assert!(
            !entries.is_empty(),
            "expected at least one TopLevel-shaped entry"
        );
...
fn build_form_multi_arity_clause_body_annotation() {
    let sexps = crate::reader::parse("(defn g ([:a x] :a x) ([:a x :Int n] x))").unwrap();
        ...
        TopLevel::Defn(defn) => {
            ...
            match &defn.params()[0].1 {
                Some(TypeExpr::Bounds(bounds)) => {
                    let names: Vec<&str> = bounds.iter().map(|t| t.name.as_ref()).collect();
                    assert_eq!(
                        names,
                        vec!["Eq", "Display"],
                    ...
                    assert!(
                        bounds.iter().all(|t| t.module.is_none()),
                        "unqualified bounds carry no module"
                    );
                }
                other => panic!("expected Some(Bounds([Eq, Display])), got {other:?}"),
            }
        }
        other => panic!("expected Defn, got {other:?}"),
    }
...
fn stacked_trait_bounds_two_params_no_duplicate_error() {
    let prog = parse_and_build_program("(defn f [:Eq :Display a :Eq :Display b] a)")
    ...
    match &prog[0] {
        TopLevel::Defn(defn) => {
            ...
            for (i, name) in [(0usize, "a"), (1usize, "b")] {
                match &params[i].1 {
                    Some(TypeExpr::Bounds(bounds)) => {
                        let ns: Vec<&str> = bounds.iter().map(|t| t.name.as_ref()).collect();
                        assert_eq!(
                            ns,
                            vec!["Eq", "Display"],
                    ...
                    other => panic!("param {name} expected Bounds([Eq, Display]), got {other:?}"),
                }
            }
        }
        other => panic!("expected Defn, got {other:?}"),
...
fn test_build_deftype_bracketed_ctor_field_still_ok() {
    let prog = parse_and_build_program("(deftype Rotation (L [:Int n]) (R [:Int m]))").unwrap();
    match &prog[0] {
        TopLevel::TypeDef {
            name, constructors, ..
        } => {
...
fn deftype_qualified_ctor_name_rejected_both_arms_bare_twins_accept() {
    ...
    assert!(
        ...
        "bare arm: qualified-binder message; got: {}",
        bare_err.message()
    );
    ...
    assert!(
        ...
        "list arm: qualified-binder message; got: {}",
        list_err.message()
    );
...
fn deftype_qualified_field_name_rejected_bare_twin_accepts() {
    ...
    assert!(
        ...
        "annotated field: qualified-binder message; got: {}",
        ann_err.message()
    );
    ...
    assert!(
        ...
        "bare field: qualified-binder message; got: {}",
        bare_err.message()
    );
...
fn defmacro_param_rejects_qualified_value_binders_still_accept() {
    ...
    let err =
        build_one("(defmacro m [a/b] a/b)").expect_err("a qualified defmacro param is rejected");
    assert!(
        err.message().contains("qualified name") && err.message().contains("binder"),
        "defmacro param reject names the rule; got: {}",
        err.message()
    );
    assert!(
        build_one("(defmacro m [ab] ab)").is_ok(),
        "defmacro param twin"
    );
    ...
    // names bind normally.
    assert!(
        parse_and_build_program("(defn f [ab] ab)").is_ok(),
        "defn param"
    );
    assert!(parse_and_build_expr("(fn [ab] ab)").is_ok(), "fn param");
    assert!(parse_and_build_expr("(let [ab 1] ab)").is_ok(), "let name");
    assert!(
        parse_and_build_expr("(match x [ab ab])").is_ok(),
        "match var"
    );
    assert!(
        parse_and_build_expr("(match x [(Some ab) ab])").is_ok(),
        "match ctor binding"
    );
}
...
fn test_build_deftrait_simple() {
    let prog = parse_and_build_program("(deftrait Display (show [self] String))").unwrap();
    match &prog[0] {
...
fn test_build_deftrait_with_docstring() {
    let prog =
        parse_and_build_program("(deftrait Display \"Convert to string\" (show [self] String))")
            .unwrap();
    match &prog[0] {
        TopLevel::TraitDecl(decl) => {
...
fn test_build_deftrait_multiple_methods() {
    ...
    let prog = parse_and_build_program("(deftrait Num (+ [a b] self) (- [a b] self))").unwrap();
    match &prog[0] {
        TopLevel::TraitDecl(decl) => {
...
fn test_build_deftrait_hkt() {
    ...
    let prog =
        parse_and_build_program("(deftrait (Functor f) (fmap [:(Fn [a] b) g :(f a) x] (f b)))")
            .unwrap();
    match &prog[0] {
        TopLevel::TraitDecl(decl) => {
...
fn test_build_deftrait_private() {
    let prog = parse_and_build_program("(deftrait- Internal (method [self] Int))").unwrap();
    match &prog[0] {
        TopLevel::TraitDecl(decl) => {
...
fn test_build_impl_concrete() {
    let prog =
        parse_and_build_program("(impl Display Int (defn show [x] (int-to-string x)))").unwrap();
    match &prog[0] {
...
fn test_build_impl_polymorphic_with_constraint() {
    let prog =
        parse_and_build_program("(impl Display (Option :Display a) (defn show [x] x))").unwrap();
    match &prog[0] {
        TopLevel::TraitImpl(imp) => {
            assert_eq!(imp.trait_name.name.as_ref(), "Display");
...
fn test_build_impl_hkt() {
    let prog = parse_and_build_program("(impl Functor Option (defn fmap [f opt] opt))").unwrap();
    match &prog[0] {
        TopLevel::TraitImpl(imp) => {
...
fn test_build_impl_repl() {
    match parse_and_build_repl("(impl Eq Int (defn = [x y] (eq-i64 x y)))").unwrap() {
        TopLevel::TraitImpl(imp) => {
...
fn test_build_deftrait_repl() {
    match parse_and_build_repl("(deftrait Showable (show [self] String))").unwrap() {
        TopLevel::TraitDecl(decl) => {
...
fn build_impl_qualified_primitive_target_splits_typeref() {
    let prog = parse_and_build_program("(impl Num primitives/Int (defn + [x y] x))").unwrap();
    match &prog[0] {
...
fn build_impl_qualified_user_target_splits_typeref() {
    let prog = parse_and_build_program("(impl Tagger user/Widget (defn tagit [w] 99))").unwrap();
    match &prog[0] {
        TopLevel::TraitImpl(imp) => match &imp.target {
            TypeExpr::Named(n) => {
...
fn build_impl_bare_target_keeps_no_module() {
    let prog = parse_and_build_program("(impl Num Int (defn + [x y] x))").unwrap();
    match &prog[0] {
        TopLevel::TraitImpl(imp) => match &imp.target {
            TypeExpr::Named(n) => {
...
fn build_impl_qualified_type_arg_splits_typeref() {
    let prog = parse_and_build_program("(impl Display (Option primitives/Int) (defn show [x] x))")
        .unwrap();
    match &prog[0] {
        TopLevel::TraitImpl(imp) => match &imp.target {
            TypeExpr::Applied(head, args) => {
...
fn build_impl_qualified_constraint_splits_traitref() {
    let prog =
        parse_and_build_program("(impl Display (Option :fmt/Eq a) (defn show [x] x))").unwrap();
    match &prog[0] {
        TopLevel::TraitImpl(imp) => {
            assert_eq!(imp.type_constraints.len(), 1);
            assert_eq!(imp.type_constraints[0].0, "a");
...
fn build_impl_bare_constraint_keeps_no_module() {
    let prog = parse_and_build_program("(impl Display (Option :Eq a) (defn show [x] x))").unwrap();
    match &prog[0] {
        TopLevel::TraitImpl(imp) => {
            assert_eq!(imp.type_constraints.len(), 1);
            let tr = &imp.type_constraints[0].1;
...
fn build_impl_qualified_trait_name_splits_traitref() {
    let prog = parse_and_build_program("(impl primitives/Num Int (defn + [x y] x))").unwrap();
    match &prog[0] {
        TopLevel::TraitImpl(imp) => {
...
fn build_impl_bare_head_records_no_con_var() {
    let prog = parse_and_build_program("(impl Display Int (defn show [x] x))").unwrap();
    match &prog[0] {
        TopLevel::TraitImpl(imp) => {
...
fn build_impl_hk_head_records_con_var_and_keeps_slot2_applied() {
    let prog =
        parse_and_build_program("(impl (Functor f) (Functor Option) (defn fmap [g x] x))").unwrap();
    match &prog[0] {
        TopLevel::TraitImpl(imp) => {
            let cv = imp
                .head_con_var
                .as_ref()
                .expect("expected head_con_var Some");
...
fn build_impl_hk_head_records_con_var_verbatim() {
    let prog =
        parse_and_build_program("(impl (Functor g) (Functor Option) (defn fmap [h x] x))").unwrap();
    match &prog[0] {
        TopLevel::TraitImpl(imp) => {
            let cv = imp
                .head_con_var
...
fn build_impl_hk_qualified_head_splits_traitref_and_records_con_var() {
    let prog =
        parse_and_build_program("(impl (fmt/Functor f) (Functor Option) (defn fmap [g x] x))")
            .unwrap();
    match &prog[0] {
        TopLevel::TraitImpl(imp) => {
...
fn build_impl_malformed_head_shapes_rejected_with_fix_naming_errors() {
    ...
    let cases: &[(&str, &str, &str, usize)] = &[
        ...
        (
            "(impl (Functor f g) Int (defn m [x] x))",
            "too many elements",
            "(Functor f g)",
            0,
        ),
    ...
    for (src, needle, span_snippet, nth) in cases {
        let err =
            parse_and_build_program(src).expect_err(&format!("expected `{src}` to be rejected"));
        let msg = err.message();
        assert!(
            msg.contains(needle),
...
fn build_deftrait_method_qualified_type_refs_split() {
    let decl =
        match parse_and_build_repl("(deftrait Scaler (scale [:primitives/Int x] primitives/Int))")
            .unwrap()
        {
...
fn reject_qualified_binder_head_rejects_slash_and_names_bare_fix() {
    ...
    assert!(
        reject_qualified_binder_head("/", span).is_ok(),
        "bare `/` operator must pass"
    );
    assert!(reject_qualified_binder_head("foo/", span).is_ok());
...
fn reject_qualified_binder_head_rejects_dotted_and_names_member_fix() {
    let span = Span::new(3, 10);
    let err = reject_qualified_binder_head("a.b", span).expect_err("a dotted binder is rejected");
    let msg = err.message();
    assert!(
    ...
    assert!(
        msg.contains("write 'b'"),
        "message names the bare fix `b`; got: {msg}"
    );
    assert_eq!(
        err.location().span,
        span,
        "reject is located at the binder span"
    );
    ...
    assert!(
        both.message().contains("qualified name"),
        "got: {}",
        both.message()
    );
    ...
    assert_eq!(
        split_qualified_name("core.io/pure"),
        Some(("core.io", "pure"))
    );
}
...
fn native_binder_heads_reject_dotted_and_accept_bare_twin() {
    let cases: &[(&str, &str, &str)] = &[
        ...
        ("(deftype- A.B [:Int n])", "A.B", "(deftype- AB [:Int n])"),
        (
            "(deftype (A.Pair a b) [:a x :b y])",
            "A.Pair",
            "(deftype (APair a b) [:a x :b y])",
        ),
        ...
        (
            "(deftype Shape (A.b [:Int r]))",
            "A.b",
            "(deftype Shape (Ab [:Int r]))",
        ),
        // Field name (mints a `Type.field` accessor — §5.2.6).
        ("(deftype P [:Int a.b])", "a.b", "(deftype P [:Int ab])"),
        (
            "(deftrait A.B (m [self] self))",
            "A.B",
            "(deftrait AB (m [self] self))",
        ),
        (
            "(deftrait- A.B (m [self] self))",
            "A.B",
        ...
        (
            "(deftrait (Cat.X f) (fmap [g self] self))",
            "Cat.X",
            "(deftrait (CatX f) (fmap [g self] self))",
        ),
        // con_var (§7.2 bare lowercase type-constructor variable).
        (
            "(deftrait (Functor a.b) (fmap [g self] self))",
            "a.b",
            "(deftrait (Functor ab) (fmap [g self] self))",
        ),
        // Method-signature name (§5.3.3).
        (
            "(deftrait Foo (a.b [self] self))",
            "a.b",
            "(deftrait Foo (ab [self] self))",
        ),
    ...
    for (dotted, head, bare) in cases {
        let err = build_one(dotted).expect_err(&format!("dotted head `{dotted}` must be rejected"));
        assert!(
...
fn deftype_type_param_rejects_qualified_and_dotted_located_at_param() {
    ...
    // The `.` twin, same seam.
    let dotted =
        build_one("(deftype (Duo a.b c) [:c x])").expect_err("a dotted type param is rejected");
    assert!(
        dotted.message().contains("dotted name"),
        "got: {}",
        dotted.message()
    );
...
fn native_binder_heads_reject_qualified_and_accept_bare_twin() {
    ...
    let cases: &[(&str, &str, &str)] = &[
        ...
        (
            "(deftype (fmt/Pair a b) [:a x :b y])",
            "fmt/Pair",
            "(deftype (Pair a b) [:a x :b y])",
        ),
        // S3: deftrait / deftrait- — bare head arm.
        (
            "(deftrait fmt/Foo (m [self] self))",
            "fmt/Foo",
            "(deftrait Foo (m [self] self))",
        ),
        (
            "(deftrait- fmt/Foo (m [self] self))",
            "fmt/Foo",
        ...
        (
            "(deftrait (fmt/Functor f) (fmap [g self] self))",
            "fmt/Functor",
            "(deftrait (Functor f) (fmap [g self] self))",
        ),
        // S5: deftrait method-signature name.
        (
            "(deftrait Foo (fmt/show [self] self))",
            "fmt/show",
            "(deftrait Foo (show [self] self))",
        ),
...
fn qualified_defn_head_rejects_identically_at_toplevel_and_impl_method() {
    ...
    let impl_method = parse_and_build_program("(impl Foo Int (defn fmt/foo [self] self))")
    ...
    assert!(
        top.message().contains("write 'foo'"),
        "got: {}",
        top.message()
    );
}
...
fn qualified_trait_head_binder_reject_diverges_deftrait_from_impl() {
    ...
    let deftrait_err = parse_and_build_program("(deftrait fmt/Foo (m [self] self))")
    ...
    let prog = parse_and_build_program("(impl fmt/Foo Int (defn m [self] self))")
...
fn type_expr_to_trait_ref_trusts_upstream_split_no_resplit() {
    ...
    let prog = parse_and_build_program("(defn f [:fmt/Eq :Display a x] x)").unwrap();
    match &prog[0] {
...
fn impl_target_qualified_lowercase_arg_routes_to_named_not_typevar() {
    let prog = parse_and_build_program("(impl Show (Pair mod/x) (defn show [s] s))").unwrap();
    match &prog[0] {
        TopLevel::TraitImpl(imp) => match &imp.target {
            TypeExpr::Applied(head, args) => {
    ...
    let bare = parse_and_build_program("(impl Show (Pair a) (defn show [s] s))").unwrap();
    match &bare[0] {
        TopLevel::TraitImpl(imp) => match &imp.target {
...
fn impl_target_qualified_constrained_var_rejected_bare_twin_accepts() {
    ...
    let prog = parse_and_build_program("(impl Show (Pair :Eq a) (defn show [s] s))").unwrap();
    match &prog[0] {
        TopLevel::TraitImpl(imp) => {
            assert_eq!(imp.type_constraints.len(), 1);
...
fn test_docstring_captured_in_defn() {
    let prog = parse_and_build_program("(defn greet \"docstring\" [x] x)").unwrap();
    match &prog[0] {
        TopLevel::Defn(defn) => {
...
fn test_docstring_in_deftype() {
    let prog =
        parse_and_build_program("(deftype Color \"Primary colors\" Red Green Blue)").unwrap();
    match &prog[0] {
...
fn test_defn_params_still_work() {
    ...
    match parse_and_build_program("(defn foo [x] x)")
        .unwrap()
        .as_slice()
    {
        [TopLevel::Defn(defn)] => {
...
fn build_form_defmacro_yields_single_macro_with_all_clauses() {
    let entries = build_form(&parse_one("(defmacro when ([cond body] (if cond body 0)))")).unwrap();
    assert_eq!(entries.len(), 1);
    match &entries[0] {
...
fn build_form_multi_clause_defmacro_yields_single_macro() {
    let entries = build_form(&parse_one("(defmacro pick ([x] x) ([x y] x) ([x y z] x))")).unwrap();
    assert_eq!(entries.len(), 1);
    match &entries[0] {
        ParsedEntry::Macro { info } => {
...
fn build_form_rejects_unknown_head() {
    ...
    let msg = format!("{err}");
    assert!(msg.contains("unknown top-level form"), "got: {msg}");
}
...
mod rendered_diagnostics {
    ...
    fn assert_no_debug_artifacts(e: &cranelisp_types::CranelispError) {
        ...
        assert!(
            !m.contains("Span {"),
            "message leaks a Debug span struct: {m}"
        );
        assert!(
            !m.contains("Sexp::"),
            "message leaks a Debug Sexp variant: {m}"
        );
        assert!(
            !m.contains("ErrorLocation"),
            "message leaks ErrorLocation: {m}"
        );
    }
...
mod track_d_wd1 {
    ...
    fn impl_method_body_ascription_and_trailing() {
        assert!(parse_and_build_program("(impl T Int (defn m [x] :Int x))").is_ok());
...
mod head_classifier {
    ...
    fn routing_predicate_agrees_with_classifier() {
        ...
        for head in ["begin", "import", "mod", "platform", "add-i64", "let"] {
            assert!(
                !head_is_top_level_form(head),
                "{head} is NOT a build_form head"
            );
        }
    }
}
