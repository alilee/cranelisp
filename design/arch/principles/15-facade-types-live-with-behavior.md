...
Moves out (single-implementation-crate origin): `CheckResult`, `CheckError`, `ResolutionGap`, `FormCheckResult`, `CheckPass`, `CheckState`, `TypeCheckEnv`, `ModuleCheckAccumulator`, `ReplSnapshot` → `cranelisp-typecheck`. `CompilationError`, `GotEvent`, `GotEventTag`, `GotProvenance`, `GotObserver` → `cranelisp-backend`. `IoEvent`, `IoObserver`, `register_io_observer` → `cranelisp-intrinsics` (the IO observation extension-point API per Decision 40 + Decision 43; `facades/intrinsics.md` §"IO observation"). `IoTraceFlushGuard`, `SchedulerTraceFlushGuard` → `src/` (integration-layer guard types — the consumer-side ring buffer machinery lives in `src/io_trace/` and `src/trace/` per FIXME 0103). `ExtractedDeclarations`, `StructuralDecls` → `cranelisp-frontend` (already there per facade). The relocation is tracked by `/dev` FIXMEs spanning the affected crates; `int` import sites are rewritten as part of the same FIXMEs.

*(Sprint origin: Sprint 64 — FIXME 0002 resolution. Updated Sprint 65 W2.5 to reflect the Decision 43 runtime split — `IoEvent`/`IoObserver` host moves with the BC-§4b half; the trace flush guards move with the consumer ring-buffers into `src/`.)*
