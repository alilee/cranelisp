...
| 52 | Failure triage + FIXME clearance | parse-int→stdlib migration (2 regressions), remaining 23 pre-existing failures (sketch-port, IO, checked_div, ring2, cache, E2E). impl_index on SharedState. Prelude loading fix (seq.lazy→None). All FIXMEs. Sprint 23 test triage (3x deferred — user approval required). Post-restructure architecture doc. Prior-ring spec traceability. | all skills |
