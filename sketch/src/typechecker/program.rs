...
impl TypeChecker {
    ...
    pub fn check_program(&mut self, program: &Program) -> Result<CheckResult, CranelispError> {
        ...
        let mut impl_method_defns = crate::ast::impl_method_defns(program);

        // Synthesize default method defns for any trait methods not explicitly overridden
        let mut default_method_defns: Vec<Defn> = Vec::new();
        for ti in crate::ast::trait_impls(program) {
            if let Some(decl) = self.find_trait_decl(&ti.trait_name).cloned() {
                let target = ti.impl_target_mangled();
                let defaults = super::traits::synthesize_default_defns(
                    &decl, &ti.methods, &target,
                )?;
                default_method_defns.extend(defaults);
            }
        }
        impl_method_defns.extend(default_method_defns.clone());
        ...
        for ti in crate::ast::trait_impls(program) {
            ...
            for method in &ti.methods {
                let mangled =
                    crate::ast::mangle_impl_method(&ti.trait_name, &method.name, &target);
                impl_self_types.insert(mangled, self_type.clone());
            }
            // Also register self types for default methods
            if let Some(decl) = self.find_trait_decl(&ti.trait_name) {
                for method_sig in &decl.methods {
                    if method_sig.default_body.is_some()
                        && !ti.methods.iter().any(|m| m.name == method_sig.name)
                    {
                        let mangled = crate::ast::mangle_impl_method(
                            &ti.trait_name,
                            &method_sig.name,
                            &target,
                        );
