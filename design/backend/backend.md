...
| Transitive drop glue + displacement (S116) | `transitive-drop-glue.md` | **Live (DESIGN, S116 Phase 3)**. One named/per-concrete type-directed drop function, declaration-first finite construction for recursive types, compilation-local identity registry, identical JIT/object/cache behavior, and no depth cutoff or shallow fallback. Owns the common release mechanism used by match-owned scrutinees, explicit and compiler-synthesised capture environments, Vec/ADT fields, and the one TCO replacement/transfer predicate. Includes the binding no-interim list and submodule × complexity/edge/negative unit matrix. |
