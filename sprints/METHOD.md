...
**Phase 1 — Scope.** `/sprint` scans open FIXMEs (`design/arch/fixmes/`) + prior-sprint archive for carries. Proposes the next increment. Deferral is governed by §2.4 — including that size alone is not a reason and that the FIXME backlog is drained rather than parked.

**Scope a drawdown sprint from a test run, not from prose (S77).** For a get-to-green or defect-drawdown increment, the Phase-1 scope is built from an actual `cargo nextest run --no-fail-fast`, collapsed to root causes and classified (code defect / fixture defect / gated) — never from the prior sprint's close notes or ROADMAP prose. Close notes summarise *intent*; named carries drift from the live failing set. S77's prose-built scope covered 13 of the 38 real failures, and the user's "seems small" is what caught it. Two calibrations from the same episode: the 38 collapsed to about 10 roots, so N failing tests never means N fixes; and several "defects" were test-design defects, so check the test against the spec before assuming the code is wrong.

**The user pre-approves architectural substance (established mid-S66).** The user is the architect at the language-shape level; `/arch` is a deputy that drafts, ratifies, and applies substance the user has approved — it does not invent it. So before `/sprint` files a FIXME that *proposes* an architectural decision, or dispatches an `/arch` agent that will author or amend one, the substance plus rationale plus rejected alternatives go to the user first, and `/sprint` waits for an explicit OK. This applies to formal register decisions and to informal choices that meaningfully affect language shape, compiler structure, or boundary contracts; it does **not** apply to implementation details falling out of an approved decision, propagation of approved substance, per-crate slice authorship, or FIXMEs proposing no architectural change. Once the user has approved substance in-conversation, proceed without re-asking — but the downstream FIXME text or agent prompt must match what was reviewed. The rule exists because several S66 decisions landed via `/arch` agents answering `/sprint`-filed FIXMEs without the substance being endorsed first, and had to be corrected retroactively (Decision 44 amended twice mid-sprint; Decision 45 reversed after a `/dev` attempt exposed a lookup-cost mismatch with the user's own principle), each correction costing an `/arch` round and sometimes a `/dev` re-run.
...
**A `/review`-caught correctness defect is a testing miss, not a review win (user, S108).** Review is the last line of defence; correctness is the test suite's job. When `/review` surfaces a **correctness** defect — wrong output, mis-count, contract violation, as opposed to a design-intent, mirror, or altitude finding — QA-first and the `/dev` unit tests had a blind spot, and the response includes a "why wasn't this tested?" retrospective plus a guard **verified to fail if the fix is reverted** ("existing tests still pass" does not cover it). The S108 exhibit: `/review` caught three Important correctness defects in the `/search` lifecycle change-set, and all three were knowable beforehand — `/arch` had pre-flagged the collision/dedup case in its verdict, §17.19.3 was scribed that same increment stating the non-conflation as a MUST, and the non-TTY byte-identical piped contract is a standing invariant. Two consequences bind:

- **A deferral from `/testing` to `/dev` unit tests must enumerate the cases** — each boundary, each negative, each spec MUST — and `/dev` plus `/review` confirm each has a guard. The S108 hole was a bare "the latch logic will be unit-pinned" that named no cases, so `/dev` pinned the happy path and the boundaries fell through.
- **Spec MUSTs and `/arch`-pre-flagged boundaries are the highest-signal QA-first targets**, ahead of the happy path.

The class-level counterpart is `/qa`'s standing coverage-by-definition-variants lens: a whole recurring *class* surfacing only at review or use is this same miss at the matrix level.
...
4. **Size is not a deferral reason — decompose instead (user, S103).** When a `/dev` or `/sprint` scope call is "this is too big, defer to next sprint," the first move is to break the work into more focused waves, not to push it out: *"no point in deferring to another sprint just due to size — best to break into more waves."* This is load-bearing rather than stylistic — splitting the S103 Wave 3 into single-mechanism sub-waves (3a R5 / 3b reuse) is what let `/review` catch two UAF Blockers that a bundled wave would have buried under a green suite. Reach for deferral only once decomposition is exhausted.
5. **A carry needs evidence of unreachability, not a judgment call (user, S103).** *"We aren't reducing the scope of the sprint unless we have evidence that we can't reach the goal."* The default is to keep pushing toward the target; a carry requires a proven-unreachable finding and is then carried **with** that proof as an honest re-scope. Never present "carry it" as the recommendation without the evidence — and where the finding asserts a *mechanism* (contention, RC, cache) behind a large ratio, require a profile before believing it: an asserted "§3.1 contention" for a 100× parallel slowdown was refuted by the demanded profile (it was rayon scheduler overhead) and had already corrupted the roadmap's forward premise.

**FIXME backlog is drained, not parked (user, S91).** Phase 1 pulls in **every** open FIXME unless there is a genuine reason to defer; reflexively confirm-deferring carries to established tracks is what lets the backlog accumulate silently. Genuine defer reasons: explicitly Phase H / release-tier work; a hard dependency on a not-yet-started track; or a trigger-FIXME whose trigger is unmet. "Scheduled elsewhere in the roadmap" is **not** by itself one. Present the deferral list with a reason per item so the user can challenge it.
