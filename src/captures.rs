...
pub fn free_vars(expr: &Expr, globals: &HashSet<String>) -> HashSet<String> {
    match expr {
        ...
        Expr::Var { name, .. } => {
            // Dotted names (Type.Constructor, Trait.method) are always global references
        ...
        Expr::ParLet { bindings, body, .. } => {
            // Bindings are independent — none can reference another par-let binding
            let mut fv = HashSet::new();
            for (_, val_expr) in bindings {
                fv.extend(free_vars(val_expr, globals));
            }
            let bound: HashSet<String> = bindings.iter().map(|(n, _)| n.clone()).collect();
            let body_fv = free_vars(body, globals);
            for v in body_fv {
                if !bound.contains(&v) {
                    fv.insert(v);
        ...
        Expr::ParBind { bindings, body, .. } => {
            // Same as ParLet: bindings independent, body scoped
            let mut fv = HashSet::new();
            for (_, val_expr) in bindings {
                fv.extend(free_vars(val_expr, globals));
            }
        ...
        Expr::Trace { body, .. } => free_vars(body, globals),
        Expr::RunTests { init, pass_fn, fail_fn, .. } => {
            let mut fv = free_vars(init, globals);
            fv.extend(free_vars(pass_fn, globals));
            fv.extend(free_vars(fail_fn, globals));
            fv
        }
    }
}
