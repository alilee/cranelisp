...
use cranelisp_types::{
    CranelispError, ModuleEntry, ModuleFullPath, ModuleStructure, Span, SymbolTable, Type,
};
...
/// Find platform rlib paths from loaded module structures.
///
...
pub fn find_platform_rlibs(
    module_structures: &[(ModuleFullPath, ModuleStructure)],
) -> Vec<PathBuf> {
    let mut rlibs = Vec::new();
    for (mod_path, structure) in module_structures {
        if !mod_path.as_ref().starts_with("platform.") {
            continue;
        }
        if let Some(dll_path) = &structure.dll_path {
...
/// Collect platform manifest symbol names from loaded module structures.
///
...
pub fn collect_platform_manifest_names(
    ...
    let has_platform = module_structures
        .iter()
        .any(|(mod_path, _)| mod_path.as_ref().starts_with("platform."));
    if has_platform {
...
mod tests {
    ...
    fn find_platform_rlibs_empty() {
        let structures: Vec<(ModuleFullPath, ModuleStructure)> = vec![];
        let rlibs = find_platform_rlibs(&structures);
    ...
    fn collect_platform_manifest_names_empty() {
        let structures: Vec<(ModuleFullPath, ModuleStructure)> = vec![];
        let names = collect_platform_manifest_names(&structures);
