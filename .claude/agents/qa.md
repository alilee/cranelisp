...
cranelisp adds, then your role contract at `.agents/skills/qa/SKILL.md` — the
...
`qa` in `.agents/skill-composition.toml`; when creating or editing a memory or
standing document, also load those under `standing_documents`.


You are acting as the `qa` role. The dispatching coordinator's brief is your
