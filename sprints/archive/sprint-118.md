...
**DESCOPED 2026-07-26 (user-approved):** the sprint's definition of success is now the **ownership-mechanism collapse only** — one drop-glue mechanism, zero coexisting alternatives, the ownership RED family flipped. Tracks A+B stand; Track C (load characterization + certification), Track D (0863/0867/0868 implementation; 0869 implementation — its carrier ruling landed and carries), and Track E's dev items (0870/0874) move to S119. The user simultaneously commissioned an `/arch` structural option paper (below, §Commissioned S119 inputs) after the shared assessment that the manual-RC stratum's cost structure — not the process — is the systemic reliability problem.
...
**W2b+ truthful instrument — COMPLETE 2026-07-26** (commits `a9be8472`…HEAD). The marginal-balance harness landed as a reusable primitive (`tests/helpers/marginal.rs`) with a three-cell capability fence — including the load-bearing precondition that two independent full-stdlib children report an *identical* ambient residual, making the subtraction exact. **Four cells flipped GREEN on real measurements** (#10 int-loop marginal 0/0; #19/#20 conj 21 allocs/21 frees marginal 0; #23 M3 clean child 1/1 marginal 0) — none by construction; a nonzero marginal would have stayed red as a finding. 0889 pins committed at exact documented values, one per closed-form term (arg +2 / result +1), so any half-fix or drift flips a pin. Honesty carried: 0688 not retired (its absence is now *proven* rather than invisible; cure-vs-unreached remains open per plan §2.2.1); the armed cells auto-tighten to their original contracts when 0889 lands. 0887 resolved (fmt clean workspace-wide); **0890 filed** (/qa: #21's ≤1400 bound is ~87% ambient — re-derive marginally at W3 acceptance). **Baseline: 28 → 23** (#22 W2a; #10/#19/#20/#23 here).

**W2b gate — PASS (re-review 2026-07-26, zero findings).** 0884/0885 resolutions verified at source (adjudicator independently re-derived the move-variant arithmetic; nullary-guard-before-tally confirmed in `rc.rs`); subprocess cost ruled warranted (the cheaper alternative required arming the suite bans); new CLAUDE.md conventions verified accurate; production byte-identity across the finding-fix confirmed. 0886 rides to `/design`; 0887 filed (`/testing`, ambient fmt drift, pre-existing). **W2b closes: RE-1 landed and fenced; abort face attributed to the backend 0810-B/0782 seam ahead of W3; Branch-F probe next.**

**W2b `/review` (first pass) — BLOCKED 2026-07-26 (delegated; production fix clean, findings test-scoped).** S1–S4 exact; nullary safety verified at the `NULLARY_TAG_THRESHOLD` seam; fmt commit mechanically confirmed formatting-only; measurement appendix spot-checked against the committed pins; **RE-3 audit: no additional deep-inc site in `marshal.rs`** (quote pair remains the only Branch-F producer question). Findings: **0884 (Blocker, /dev)** — test helpers are safe fns whose SAFETY comments infer provenance from the nullary guard (unsound inference; make them `unsafe fn` with honest contracts); **0885 (Important, /dev)** — verified by arithmetic that the §3-rejected move-variant passes every committed row, so the inc-count fence doesn't pin the unconditional-consume ruling; close by asserting the inc tally rises exactly |xs|+1; **0886 (Suggestion, /design)** — §5's "under M1 quarantine" wording conflicts with the arming-discipline guard; the committed substitute is correct, the doc should say so.

**W2b `/dev` — COMPLETE 2026-07-26** (fix `959833ea`; fmt/0882 `9ad5faca`; 0835 measurement appendix `4b551c35`). **D0–D3 pre-fix measurements settled the abort face: candidate (i)** — the deep walk was NOT the wild write (precheck armed rejects nothing; (ii) falsified); M1 quarantine makes the abort *disappear* pre-fix (write into a reused freed chunk); M2 exposes a use-after-free read from 2 chained `sconcat` up (`match failed` on poison — FIXME 0815's symptom reproduced under detection, present in `--run` too). RE-1 fix landed: head-only inc, `deep_rc_inc_slist` deleted, rustdoc rewritten; eight RED-first unit rows observed failing then green; RE-2 consumer-invariance pin green before and after; 417/417 crate gate; 0810 colors unchanged name-for-name (no S98 flag); FIXME 0882 resolved. **Post-fix, the armed lane LOCATED the surviving abort face**: `STALE RC DEC (JIT inline)` double-releasing a one-byte `HeapString` extracted under a constructor pattern — the 0810-B/0782 backend family, exactly the design's candidate-(i) prediction (shapes clean pre-fix now abort under M1 post-fix because the masking surplus refs are gone). W3 owns the fix; the attribution is in hand *before* the wave. **Branch F on the prelude residue**: P-ladder byte-identical (P4 = 1143 unchanged); the binding prediction failed; lead recorded — `quote_sexp`/`quote_slist` is the other macro-expansion-path producer in `marshal.rs`, untouched by RE-1, +2-per-invocation signature. Flips: 0835 B1/B2 green; B3 honestly red on a constant +2 remainder (M3: one `HeapString` + one `SexpBool`); repro-A red on the located backend face; #10/#19/#20/#23 stay red per Branch F — **user scope decision owed per plan §2.5**.
...
**W3 GATE — PASS 2026-07-26 (change-set `2df95c41..966d298e`).** Three converging delegated-review rounds: round 1 cleared the architecture (all focus questions favorable) and blocked on the unpinned latent-defect fixes (0892/0893, METHOD §2.2); round 2 verified those resolved with double-sided falsification and blocked only on test SAFETY comments (0895); round 3 confirmed the nine per-site discharges genuine and the change-set comment-only. Crate 523/523; fence green; all consumer-family guards green. **The P8 bridge is closed: one drop-glue mechanism, zero coexisting alternatives** — arch ruling 10's condition met.

### W3 — Track B backend consumers — **/dev COMPLETE 2026-07-26 (six commits `2df95c41`..`b47a9997`)**

Focused e2e **21 RED → 1** (survivor = W4's cell #15, unchanged as expected). S0 invariance byte-identical name-for-name; S1 +3 flips (early vs. design prediction — traced: canonical glue un-truncated the depth-4 field walk, which also explains 0835's B3 remainder as a backend stranded-interior face); S3 +14 (all nine 0810, both 0726, **both repro-A abort cells** — the located `STALE RC DEC` face removed, armed sessions balanced 22/22); S4 +2 (0760 #11/#12); S5+S6 fence green, twelve-symbol deletion grep-zero live. Crate 519/519; armed legs clean both toggles. **Two latent S116-foundation defects found while wiring the first consumers**: the registry's Vec arm called total-teardown `vec_drop` unconditionally (would have freed shared Vecs; the arm had never run), and `tail_transfer_skip` never checked borrowed-ness (now `BorrowedInvalid`, loud). **D2 STOP rule fired** → FIXME 0891 (`/design`: monomorphic undeclared-field ctor template leaves a residual `Type::Var`; one named branch, behavior identical, ruling owed). Riders: `/testing` — 0796 exclusion removal verified green (the *entire* exclusion list removable), stale `// defect:` loci re-locus; `/qa` — cell #21 measures **12431** vs ≤1400 post-W3 (new attribution per plan §4.4; joins 0890's marginal re-derivation). **Baseline: 23 → 10** (nine 0810 + three 0760 + transitive-termination flipped).
...
**W4 GATE — PASS 2026-07-26 (change-set `fc3375f9..16a26408`).** Two converging rounds: round 1 cleared the architecture (all seven focus questions favorable incl. §11 non-interference and the release-key deviation's faithfulness) and blocked on the zero-address transmute gap (0897) + visibility (0899); the closing round verified the guard genuine (unit row discriminates the guard, not the debug detector; SAFETY grounds match the as-built constructor structure) with zero findings. 768/768 lib; 14/14 focused; **FIXME 0745 CLOSED**. Standing: 0898 open (`/arch`), 0901 open (`/design` int, doc re-count), 0896-renumber + ratification owed by `/design`(int).

### W4 — Track B result owner (int/exe-bundle) — **/dev COMPLETE 2026-07-26 (`fc3375f9`..`3ffab566`)**

All four flip targets GREEN with per-mode commits (I3 run: #15 2/2 + #16 5/5 both toggles; I4 REPL: #18 4/4; I5 link: #17 exit 0, 5/5); scalar controls unchanged. `main.rs` ordering fixed (exit-code → release → shutdown; `trampoline` returns `OwnedProgramResult`, the `Type::Int` fallback unreachable). **Second standing defect closed at I5**: the linked stub `ireduce`d every result unconditionally — a `--link`/`--run` divergence (heap results exited with pointer low bits); both modes now share `result_is_exit_code`. Armed legs all balanced both toggles; unit gate 768/768 (+34); `repl/format.rs`'s second-IO-owner branch deleted and made unconstructable (P20). STOP rule fired once → **FIXME filed as 0892 (COLLISION — number retired earlier today by the vec-pin resolution; renumber to 0896 rides the /design(int) dispatch)**: release key = the entry's `codegen_view` ConcreteType (observed-type narrowing falsified by spec-required `(Vec t1)`/`(Option t2)` displays); the unpinned-`[]` lenient-view gap recorded as pre-existing, not a regression. I2 cache-hit adapter built-as-designed but production-unreached (recorded; /design call). Cell #15 re-locused per convention with the §9.1 falsification named. **NEW UNATTRIBUTED REDs surfaced**: `spec_10_io` ×3 (`constructor 'Bind' disagrees on declared parameter identity` codegen error) — proven not W4's by stash/pop; outside all focused sets to date; `/qa` attribution owed before the W8 gate. **Baseline: 10 → 6** (all explicit carries or pending attribution).
...
### Commissioned S119 inputs (user-approved 2026-07-26) — **PAPER DELIVERED (`a17dfcdd`)**

`design/arch/ownership-stratum-options.md` (indexed; register row R16 added disposing 0883; 0872 window confirmed). Four options laid out with mechanism/prevention/staging/cost/composition: (1) typed handles — ~83 shims/~131 signatures/~31 consume sites in four independently-shippable tranches, RE-1 class unrepresentable, trusted base narrows to the shim layer; (2) uniform-but-redundant emission — promotes the ratified all-Owned oracle to dev-tier default, would have prevented 0810/0782/0688 and the masking phenomenon, gated on an unmeasured atomic-traffic number (first act: a marginal-harness measurement gate); (3) harness generalization — rule-shaped unit fences (the anti-blind-spot form), /qa co-owned; (4) emission audit — honest verdict: sound-and-complete equals the ownership analysis itself; realistic target is a conservative linter for locally-decidable shapes; report-only spike. **Recommended S119 shape**: core = option-1 tranches A+B + option-3 generalization; option-4 one bounded spike after W3; option-2 decided on paper now but committed only on measurement; **0889 recovery routed through tranche B** (typed handles make marshal counts truthful; plain consume is then alias-correct — avoids the 0638 burn; arena/epoch is the recorded fallback). Five user decisions listed in the paper §7 for S119 Phase 1.

### Commissioned S119 inputs (user-approved 2026-07-26)

**`/arch` structural option paper** — an S118 deliverable feeding S119 Phase 1, dispatched after W2b lands (doc-only, serialized behind the source wave). Scope: the ownership-stratum cost structure. Options to be laid out with evidence, costs, and staged paths: (1) **typed handle discipline in the runtime pair** — newtype the raw `i64` heap handles into owned/borrowed forms with transfer encoded in signatures (`#[must_use]`, consume-exactly-once), generalizing the proven S117 Vec-of-String boundary; (2) **uniform-but-redundant RC emission** — collapse the generated-code special cases (move/COW exemptions, borrowed aliases, TCO carry-forward) into always-inc/dec correct-by-construction emission, with elision deferred to the `--release` optimizer tier under a verification guard; (3) **marginal-balance test harness** — operation-delta-vs-control measurement primitive on the proven ledger seams, ambient-immune, retrofitting the leak lanes; (4) **emission-side balance audit** — design spike: a debug pass over emitted CLIF checking inc/dec pairing per value path. The paper folds in W2b's D0–D4/P4 evidence and the W1/W2 blind-spot findings; the uniform-emission question is a user decision made on paper, not in a wave.

### W5 — Track C load characterization + certification (`/qa`-led) — **DESCOPED TO S119 (2026-07-26)**

The load-dependent guards (`launch_grid`, 0694 family, 0604/0818) remain failing-not-ignored and attributed; the certification obligations carry explicitly. The cache-guard interleaving observation and the W1 flap evidence carry as Track C's S119 starting state.
...
### W6 — Track D (`src/`) — **DESCOPED TO S119 (2026-07-26)**

0863 (design + §6.5 delta note READY), 0867 (repro committed, retargeted `/dev`(typecheck)), 0868 (schema-free), and 0869 implementation (carrier ruling authored and in force) all carry with their S118 groundwork intact. The DF1/DF2 and cache-guard REDs remain attributed carries.

### W7 — Track E platform slice — **DESCOPED TO S119 (2026-07-26)**

0870/0874 dev items carry; 0873's design and `/arch` selection gate completed in S118 — only implementation carries.
...
| W2b | /dev | runtime pair: D0–D2 measurements, RE-1 fix S1–S4, P4 probe, flip verification | opus[1m] (shim) | high | — |
...
| W3 | /review | cranelisp-backend: six-commit consumer migration (delegated) — BLOCKED: 0892/0893 (latent-defect fixes unpinned, METHOD §2.2), 0894 (stale rustdoc) | codex (delegated) + fable adjudicator | high | — |
| W3 | /dev | cranelisp-backend: 0892/0893/0894 resolution (`80bde4f7`; double-sided falsification) | opus[1m] (shim) | high | Blocker resolution |
| W3 | /review | cranelisp-backend: re-review after 0892/0893/0894 (delegated) — prior findings resolved; new 0895 (test SAFETY comments) | codex (delegated) + fable adjudicator | high | gate re-review |
...
- 2026-07-26: Carry-note for the future 0748 `/arch` dispatch (from the W3 re-review): 0748's body (lines ~77/93) and `design/arch/safety-invariants.md:81` cite the deleted `escape_symbol` present-tense as the proven injectivity model — re-point both to `cranelisp_types::drop_glue_symbol_name` (length-prefixed hex, module.rs:2654). `/qa`'s s115-instrumentation-matrix R4 "in place today" row is likewise stale; rides the 0890/#21 pass.
- 2026-07-26: Branch-F CLOSED by user decision: **truthful instrument now + a FIXME for recovering the leak.** The probe attributed the 1143 residue to the int-side macro-turn marshal boundary — a documented by-design compile-time leak (marshalled args never dec'd, expansion results never consumed; closed-form model exact on every probe point; quote path and 0835 class both excluded empirically). S118 lands marginal/twin-control accounting so the five affected cells measure runtime behavior; **FIXME 0889 filed** (user-required): the leak itself is recovered in S119 under the option paper's disposition (aliasing-aware release or arena/epoch — the 0638 history warns naive releasing burns). 0888 resolved (decision delivered). `/testing` dispatched for the instrument cells + exact-value leak pins; the marginal harness is built as a reusable helper — it is the first instance of the commissioned paper's option 3.
- 2026-07-26: Branch-F user decision: **probe only, then decide.** `/qa` runs the quote-path falsification probe (same method as the sconcat attribution); the fix-vs-carry decision returns to the user with the numbers. Sequenced after the W2b review (test-run serialization).
- 2026-07-26: USER raised the systemic-reliability question; `/sprint` assessment: the failure concentration is the manual-RC ownership stratum (contracts in prose, raw handles, count-based outside-process tests), not the codebase or process broadly. USER APPROVED both proposals: (a) **descope S118 to the mechanism collapse** — Tracks C/D/E carry to S119 with groundwork intact; (b) **commission the `/arch` structural option paper** (typed handles, uniform-but-redundant emission, marginal-balance harness, emission audit spike) as an S118 deliverable feeding S119 Phase 1. Execution order: W2b lands → `/arch` paper → W3 → W4 → Phase-5 gate → bounded Phases 6–7.
...
- **The ownership-mechanism collapse — the descoped sprint's definition of success — is complete.** One canonical drop-glue mechanism; the legacy inline emitter, `MAX_DROP_GLUE_DEPTH`, and the second glue-identity home deleted atomically (twelve symbols grep-zero, fence-guarded); all consumer families migrated (match per-arm, capture/curry, TCO, program-result) with armed-detector re-demonstration per slice. Arch ruling 10's P8 bridge is closed.
- **Track A in full**: eight detector rows proven with per-row fail-on-revert evidence (incl. the E6/E7 ordering proof); the precheck hoist; the plant protocol; 0850 closed after an S87-era origin; the ruling-7 subtractive API removal.
- **FIXME 0745 closed** (open since S115): exact-once result release across REPL/`--run`/link, plus a standing `--link`/`--run` exit-code divergence closed as a bonus. **FIXME 0835 closed in substance**: RE-1 ruled and landed; the abort face located and fixed in W3; the prelude face carved to 0889.
- **28 attributed REDs at open → 16 at the W8 gate on 139 more tests, with the first deterministic two-run certification since S115** (byte-identical failure sets, zero flappers). Phase 6 added 4 attributed intended REDs (0917 ×2, 0913, 0916) → **final certified state 20 REDs, all traced**; the standing exemplar link-parity gate landed green (0875 closed not-reproducible-at-HEAD).
- **Instrumentation as deliverable**: the marginal-balance harness with its capability fence (the option paper's option 3, working instance); exact-value pins on 0889's closed form; structural CLIF guard-polarity assertions; the arming-discipline gate.
- **The user-commissioned structural option paper** with five S119 decisions queued.
- **Method change, first production cycle**: `/review` delegated to Codex — ten rounds, every substantive round found something real; the adjudication layer both confirmed findings and disproved two false claims against the corpus.
- **Phase 6 bounded passes** across all five proxies + the `cranelisp-types` audit (design Strong — the acid-test exemplar; R1–R5 queued): 37/38 demos, 36/38 stdlib modules, 35/37 examples, docs reconciled with verified transcripts, exemplar 40/40 with byte-identical run/link parity.
- **Track E partial**: 0873 designed and arch-approved (S119 implementation); BC §5 converged (0872); the falsified must-succeed premise reconciled (0902).

### Deferred (with rationale)

- **Tracks C/D/E** (user-approved descope 2026-07-26): load-dependent certification (0694/0604/0818, launch_grid); 0863 (design READY); 0867 (repro landed, retargeted `/dev`(typecheck)); 0868/0869 (carrier ruling in force); 0870/0874. Groundwork banked.
- **The S119 backend design window**: the **0903+0916+0907 co-ruling** — the non-concrete-release class, three-faced: loud refusal (0907, workaround falsified, trait instances leak), silent leak (accessors), and **memory-unsafe wild write** (0916, threshold-proven n=1024); plus 0917 (nullary-arm strand, one-instruction attribution, per-solve-linear at application scale) and 0915's backend half. 0891 deferred blocked-on-0903, falsification preserved paste-ready.
- **0913** (`/design` typecheck lenient-view row; scope corrected), **0914** (`/mem` truthfulness), **0889** (user-required recovery via option-paper tranche B), **0898** (ruled, one-line types delta), **0912** (spec question framed for user), 0900/0906 nits.
- **The option paper's five decisions + both audits' recommendations** — the S119 Phase-1 agenda.

### Findings (recorded in FIXMEs/records where actionable)

- **Instrumentation-first paid repeatedly**: the precheck-hoist discovery; the D0–D4 abort-face discrimination (the masked double-free located to a named seam before the wave that fixed it); the P-ladder attribution; the 0916 threshold measurement that reclassified a "TCO loss" as a wild write.
- **Falsification-driven course corrections worked four times** (0835 attribution, Branch-H prediction, 0891 frame key, 0916 framing) — each stopped before landing damage, each preserved its evidence.
- **Review convergence held**: three rounds max per wave (architecture → pins → mechanical); cross-model review caught classes same-model review historically missed (unsafe-audit discipline above all) at ~100–220k Codex tokens per round.
- **Process debts**: `git add -A` in a shared tree (near-miss; path-scoped staging is now the recorded rule); the S116 foundation's latent defects prove "landed with zero consumers under static-only review" is not landed; harness infrastructure cost real wall-clock (two watchdog stalls, four 529 terminations — all recovered, zero work lost).
- **Frontmatter audit (mechanical)**: 14 rows consistent with §II.3 as amended (the `/review` fable frontmatter denotes the adjudicator tier per the delegation amendment). Dispatch log: 30+ dispatches, shim-pinned defaults throughout except the recorded delegated-review rows and documented stall re-dispatches.
