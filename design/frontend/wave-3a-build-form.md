...
1. **No `MacroResolver` trait.** Per Decision 8 (retracted as part of D43/Principle 15 reframing): lookup goes directly against `&symbol_tables`. `ModuleEntry::Macro` entries are reachable through `SymbolTables[module].get(&name)`; the JIT'd code pointer is on `ModuleEntry::Macro.code`'s `Some(_)`. Frontend never names `Jit` / `Linker` — only `Some(code)`.
