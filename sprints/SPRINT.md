...
| 0494 | → **/backend** (`vec_codegen.rs`) | open | **A** — TWO gate-stops narrowed it (no wasted fix). (1) `e46b44d`: vec-RC double-dec **REFUTED** (RC balanced; `MALLOC_CHECK_=3` → untracked-heap overrun, not RC). (2) `f2e227a` ASAN (`-Zsanitizer=address`, host+cdylib): both Rust candidates **RULED OUT** (Response-body lifetime + `CLString` marshaling base-ptr both instrumented & clean) — workload runs **clean under ASAN** because the faulting store is **JIT-emitted** (inline `vec-get/set/push` COW path, `cranelisp-backend/src/compiler/vec_codegen.rs`); ASAN shadow can't cover JIT. **Residual = a backend JIT-codegen OOB/stale-ptr store on the borrowed-Var two-live-vec COW path**, surfacing only under {launched}×{`sleep` suspend}×{`send-conn` DLL crossing} free-timing. Localization tool = **CLIF inspection** (`CRANELISP_CODEGEN_TRACE=1` / `/clif make-resp`·`churn`) or **valgrind** (DBT instruments JIT — not installed). |
