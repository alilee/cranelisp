...
### Phase 7: REPL Polish + New Commands (spec: repl/spec.md §3.3-3.5, §4.1, §11)
...
#### /imports command (spec: repl/spec.md §3.4) [R3 S15]
- `repl_imports_unfiltered_by_category` — `/imports` shows all imports organized by category (Special forms, Macros, Traits, Types, Fns); names only, no type sigs [R3 S15]
- `repl_imports_special_forms_always_present` — `/imports` always shows Special forms category even with no explicit imports [Tested tests/e2e::e2e_s3_4_imports_special_forms, tests/e2e::e2e_s3_4_imports_special_forms_always]
- `repl_imports_alphabetical_names` — Names within each category are sorted alphabetically [R3 S15]
- `repl_imports_shows_individual_names_from_glob` — After `(import [mod [*]])`, `/imports` shows the individual names that were imported, not just `*` [R3 S15]
- `repl_imports_filter_by_module` — `/imports mod` filters to show only names from that module, grouped under `From mod:` [Tested tests/e2e::e2e_s3_4_imports_filter_by_module, tests/e2e::e2e_s3_4_imports_filter_shows_from]
- `repl_imports_immediate_source` — For re-exported names, `/imports` shows the immediate source module, not the ultimate origin [R3 S15]
- `repl_imports_implicit_prelude_visible` — The implicit `(import [prelude [*]])` IS visible in `/imports` output [R3 S15]
- `repl_imports_no_imports_shows_special_forms` — In a fresh session with no imports and no prelude, `/imports` shows only Special forms [Tested tests/e2e::e2e_s3_4_imports_empty]
- `repl_imports_nonexistent_module` — `/imports nonexistent` shows empty output (silent re-prompt, not error) [Tested+Neg tests/e2e::e2e_s3_4_neg_imports_nonexistent_not_error, tests/e2e::e2e_s3_4_neg_imports_nonexistent_silent]
- `repl_imports_includes_reexports` — Both Import and Reexport module entries are included [Tested tests/e2e::e2e_s3_4_imports_includes_imports]

#### /exports command (spec: repl/spec.md §3.5) [R3 S15]
- `repl_exports_shows_public_symbols` — `/exports mod` lists public symbols by category (Modules, Macros, Traits, Types, Fns) [Tested tests/e2e::e2e_s3_5_exports_lists_symbols]
- `repl_exports_no_arg_shows_usage` — `/exports` with no argument prints usage hint [Tested tests/e2e::e2e_s3_5_exports_no_arg_usage]
- `repl_exports_not_found` — `/exports nonexistent` prints module not found error [Tested tests/e2e::e2e_s3_5_exports_not_found]
- `repl_exports_empty_module` — `/exports mod` on module with no public symbols prints appropriate message [R3 S15]
- `repl_exports_excludes_imports` — `/exports mod` does NOT show the module's own imports/reexports [R3 S15]

#### /list — module definitions (spec: repl/spec.md §3.3) [R3 S15]
...
- `repl_list_fns_category_name` — `/list` uses "Fns" as the category name for user-defined functions [Tested tests/e2e::e2e_s3_3_list_fns_category_name]
- `repl_list_no_special_forms` — `/list` MUST NOT show special forms (they belong on `/imports`) [Tested+Neg tests/e2e::e2e_s3_3_list_neg_no_special_forms]
- `repl_list_no_imports` — `/list` MUST NOT show imported names [Tested+Neg tests/e2e::e2e_s3_3_list_neg_no_imports]
- `repl_list_empty_shows_no_definitions` — `/list` on empty module prints `(no definitions)` [Tested tests/e2e::e2e_s3_3_list_empty_module]
- `repl_list_constructors_in_types` — Constructors appear in Types category alongside their type [Tested tests/e2e::e2e_s3_3_list_constructors_in_types]
- `repl_list_constructors_not_in_fns` — Constructors MUST NOT appear in Fns category [Tested+Neg tests/e2e::e2e_s3_3_list_neg_ctors_not_in_fns]
- `repl_list_prefix_filter` — `/list <prefix>` performs case-insensitive prefix match [Tested tests/e2e::e2e_s3_3_list_prefix_filter]
- `repl_list_empty_categories_omitted` — Empty categories MUST NOT appear in `/list` output [Tested+Neg tests/e2e::e2e_s3_3_list_neg_empty_categories_omitted]

#### Macro introspection — universal format (spec: repl/spec.md §11.2.2-11.2.4, §11.4, §4.1.6) [R3 S15]
- `repl_info_macro_universal_format` — `/info name` for a macro shows `:module/name ; defmacro` with clause signatures [R3 S15]
- `repl_info_macro_docstring` — `/info name` for a macro with a docstring shows docstring in classification comment [R3 S15]
- `repl_sig_macro_universal_format` — `/sig name` for a macro shows `:module/name ; defmacro` with clause signatures [R3 S15]
- `repl_sig_macro_variadic` — `/sig name` for a variadic macro shows `& rest` in clause signature [R3 S15]
- `repl_sig_macro_bracket` — `/sig name` for a bracket-destructuring macro shows bracket notation [R3 S15]
- `repl_doc_macro_present` — `/doc name` for a macro with docstring shows `:module/name ; defmacro - docstring` [R3 S15]
- `repl_doc_macro_absent` — `/doc name` for a macro without docstring shows `no docstring` message [R3 S15]
- `repl_bare_macro_lookup` — Entering a macro name bare shows `:module/name ; defmacro` + clause signatures [Tested tests/ring3_repl::r3_bare_macro_shows_universal_format, tests/ring3_repl::r3_bare_macro_multi_clause_all_sigs]
- `repl_bare_macro_no_old_format` — Bare macro MUST NOT show old `name :: macro` format [Tested+Neg tests/ring3_repl::r3_neg_macro_display_no_old_format]
- `repl_bare_macro_docstring` — Bare macro with docstring includes docstring in classification comment [Tested tests/ring3_repl::r3_macro_docstring_in_classification]

#### Universal output format — bare symbol lookup (spec: repl/spec.md §1.1, §4.1) [R3 S15]
- `repl_bare_fn_classification` — Bare function name shows `; defn` classification suffix [Tested tests/e2e::e2e_s4_1_bare_fn_classification]
- `repl_bare_fn_docstring` — Bare function with docstring shows `; defn - docstring` [R3 S15]
- `repl_bare_ctor_classification` — Bare constructor shows `; deftype` classification suffix [Tested tests/e2e::e2e_s4_1_bare_constructor_classification]
- `repl_bare_type_related_ctors` — Bare type name shows `; match:` section with constructors [Tested tests/e2e::e2e_s4_1_bare_type_match_section]
- `repl_bare_type_related_impls` — Bare type name shows `; impl:` section with trait names [R3 S15]
- `repl_bare_trait_related_methods` — Bare trait name shows `; defn:` section with method names [Tested tests/e2e::e2e_s4_1_bare_trait_defn_section]
- `repl_bare_trait_related_impls` — Bare trait name shows `; impl:` section with implementing types [R3 S15]
- `repl_bare_trait_method_dot_notation` — Bare operator shows `Trait.method` notation (e.g. `core.num/Num.+`) [R3 S15]
- `repl_bare_primitive_lookup` — Bare primitive name (e.g. `add-i64`) shows `:Type primitives/name ; defn` [R3 S15]
- `repl_bare_builtin_type_impl` — Bare builtin type (e.g. `Int`) shows `:primitives/Int ; type` (classification only; `; impl:` section requires prelude) [Tested tests/e2e::e2e_s4_1_bare_builtin_type]
- `repl_bare_special_form_classification` — Bare special form shows `; special form - description` [Tested tests/e2e::e2e_s4_1_bare_special_form_classification, tests/ring3_repl::r3_bare_special_form_classification, tests/ring3_repl::r3_bare_special_form_let]
- `repl_defn_response_classification` — `(defn ...)` response includes `; defn` classification [Tested tests/ring3_repl::r3_defn_response_classification, tests/ring3_repl::r3_neg_defn_display_has_classification]
- `repl_deftype_response_related` — `(deftype ...)` response includes `; match:` section with constructors [Tested tests/ring3_repl::r3_deftype_response_related, tests/ring3_repl::r3_deftype_result_match_section]
- `repl_deftrait_response_related` — `(deftrait ...)` response includes `; defn:` section with method names [Tested tests/ring3_repl::r3_deftrait_response_related, tests/ring3_repl::r3_deftrait_result_defn_section, tests/ring3_repl::r3_neg_deftrait_display_not_bare]
