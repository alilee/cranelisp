...
use crate::macro_expand::{
    ...
    is_defmacro, is_deftype, parse_defmacro,
};
use crate::module::{
...
fn run(source: &str) -> Result<(), CranelispError> {
    ...
    let mut user = build_program(&user_sexps)?;

    let mut tc = TypeChecker::new();
    for item in &mut user {
        if let TopLevel::Defn(defn) = item {
            crate::schedule::auto_schedule_defn(defn, &tc);
        }
    }
...
fn compile_project_pipeline(
    ...
    for module_id in &graph.compile_order {
        ...
        for sexp in &module.sexps {
            ...
            let mut items = build_program(&non_macro_forms.iter().map(|s| (*s).clone()).collect::<Vec<_>>())?;
            for item in &mut items {
                if let TopLevel::Defn(defn) = item {
                    crate::schedule::auto_schedule_defn(defn, &session.tc);
                }
            }
            ...
            for item in &items {
                match item {
                    ...
                    TopLevel::TraitImpl(ti) => {
                        ...
                        if let Some(decl) = session.tc.find_trait_decl(&ti.trait_name).cloned() {
                    ...
                    TopLevel::Defn(defn) => {
                        ...
                        // so they are available in fn_slots during codegen.
                        session.compile_mono_specializations(&mono_batch, &mr, &et)?;
                        for (_, defining_mod) in &mono_batch {
                            dirty_modules.insert(defining_mod.clone());
                        }
                        ...
                        if scheme.is_constrained() {
                            ...
                            // to other modules via qualified names and imports.
                            {
                                let mod_path = ModuleFullPath::from(mod_name.as_str());
                                let cm = session.tc.modules
                                    .entry(mod_path.clone())
    ...
    // ── Deferred cache writes: build packets then process in parallel ──
    {
        let fn_slots = session.jit.build_fn_slots_from_modules(&session.tc.modules);
        let cache_inputs = cache::extract_cache_inputs(&session.tc.modules);

        // Build packets sequentially (reads from session.tc.modules)
        let packets: Vec<cache::CacheWritePacket> = dirty_modules
            .iter()
            .filter_map(|mod_path| {
                let (_, target_cache, is_lib) = module_cache_info.get(mod_path)?;
                let source_hash = session.tc.modules.get(mod_path)
                    .and_then(|cm| cm.content_hash.clone())
                    .unwrap_or_default();
                cache::build_cache_packet(
                    target_cache,
                    mod_path,
                    &session.tc.modules,
                    &fn_slots,
        ...
        // Clear cache transients after snapshotting
        ...
        // Process packets in parallel (compile .o + write files)
        use rayon::prelude::*;
        ...
        // Write manifests sequentially from parallel results
        for (mod_path, source_hash, is_lib) in &results {
            if *is_lib {
                lib_cache_manifest.upsert_module(mod_path.clone(), source_hash.clone());
                lib_cache_dirty = true;
            } else {
...
pub fn build_executable(entry_path: &Path, output_path: &Path) -> Result<(), CranelispError> {
    ...
    let entry_stem = entry_path
        ...
        .and_then(|s| s.to_str())
        .unwrap_or("user");
    let entry_mod_path = ModuleFullPath::from(entry_stem);
    ...
    let main_returns_io = crate::exe::main_returns_io(&_session.tc.modules);
    let startup_bytes =
        crate::exe::generate_startup_object(&platform_manifests, main_returns_io)?;
...
pub(crate) fn try_load_cached_module(
    ...
    if timing {
        eprintln!("; timing:   read_meta {} {}ms", mod_name, t0.elapsed().as_millis());
    }
    ...
    // Reconstruct tc.overloads and tc.resolved_overloads from cached DefKind::Overloaded entries
    if let Some(cm) = session.tc.modules.get(mod_path) {
        let overloaded: Vec<_> = cm
            .symbols
            .iter()
            .filter_map(|(sym, entry)| {
                ...
                {
                    Some((sym.to_string(), variants.clone()))
                } else {
                    None
                }
            })
            .collect();
        for (base_name, variants) in overloaded {
            session
                .tc
                .register_overloads_from_cache(&base_name, &variants);
        }
    }
    ...
    // macros call JIT functions via GOT slots.
    {
        ...
        if timing {
            eprintln!("; timing:   link_object {} {}ms", mod_name, t0.elapsed().as_millis());
        }
    ...
    if let Some(cm) = session.tc.modules.get(mod_path) {
        let macro_entries: Vec<_> = cm
            ...
            .filter_map(|(name, entry)| {
                ...
                {
                    Some((name.to_string(), sexp.clone(), docstring.clone()))
                } else {
                    None
                }
            })
        ...
        macro_count = macro_entries.len();
        for (name, sexp, docstring) in macro_entries {
            let info = parse_defmacro(&sexp)?;
            let ptrs = compile_macro(
                &mut session.tc,
                &mut session.jit,
    ...
    if timing && macro_count > 0 {
        eprintln!(
            "; timing:   macro_reconstruct {} {}ms ({} macros)",
            mod_name, t0.elapsed().as_millis(), macro_count
        );
    }
    ...
    if timing {
        eprintln!("; timing:   cache_load_total {} {}ms", mod_name, t_total.elapsed().as_millis());
    }
