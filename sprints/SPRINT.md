...
**Status**: PHASE 5 LANGUAGE (ACTIVE) — Wave 2 (Wave 1 committed `5833bd1`)
...
|---|---|---|
| **U1** | Dispatch (§5.3) | **ADOPT + REFINED (user, 2026-06-22, Wave-2 reality-gap).** The original "parses-as-form-or-slash → REPL; else agent" is INSUFFICIENT — the reader parses multi-word prose as `Ok(N bare symbols)`, so prose routed to REPL not agent. **Refined rule (symbol-resolution-aware):** `/`→slash; blank/comment→nothing; `/ask`→agent (explicit door); else parse → unclosed=continuation, **parse-error=agent**, `Ok`: a **compound form** (`(…)`/`[…]`)→REPL; **bare atoms** → resolve: **literals + all-known-symbols → REPL** (§4 describe); **any unbound symbol → agent** (bare `yes`/typo/prose reach the agent; known `map` still described). Feature-off byte-identical (lookup→agent routing is entirely `#[cfg(feature="agent")]`). |
...
| Step | Skill | Crate | Task |
|---|---|---|---|
| 2a | /dev | src/ | ✅ **DONE** — `agent` feature + `src/agent/` skeleton (`agent_turn` placeholder, no rig); §5.3 classifier wired in `main.rs` (feature-OFF byte-identical); `/ask` + `/refs`·`/tests-for` (default build, on-demand scan); agent-prose style role; `--agent`/`--no-agent` no-op flags. **+ REFINED** (U1 reality-gap): `classify_for_agent` now symbol-resolution-aware (reuses `lookup_with_prelude_fallback`+intrinsic-name; known→Repl/§4, unbound→Agent). Default 1515/1515; `--features agent` 18/18; both `cargo check` clean. Docs (§17.1, agent.md) aligned by /repl+/design. ⚠ pre-existing `display.rs:714 approx_constant` clippy (not this change-set). |
| 2R | /review | all | ✅ **DONE — GATE-READY, 0 Blocker/0 Important.** All 7 lenses pass: feature-OFF byte-identical (entire agent surface cfg-gated); §4 self-doc preserved (known symbols still described); no second resolver (`symbol_is_known` reuses `lookup_with_prelude_fallback`, P7); classifier correct vs §17.1/agent.md §2.2; `/refs`·`/tests-for` correct default-build. 1515/1515 + agent 18/18. `display.rs:714 approx_constant` clippy confirmed pre-existing (not Wave 2). |

**Wave 2 CLOSED.** Suggestions (non-blocking): (S1) `main.rs:489` `_agent_enabled` is the Wave-3 thread-in seam (dead binding now — note for Wave 3, don't remove as dead code); (S2) `scan_referers` silent no-introspection branch (batch-mode only; harmless in REPL); (S3) `agent.md §9.2` AST-walk precision refinement remains a documented future option (MVP uses sexp+token scan).
...
- **Phase 5 Wave 2 COMPLETE** (2026-06-22): 2a `/dev`(src/) LLM-free dispatch foundations (`agent` feature + `src/agent/` skeleton, no rig; classifier; `/ask`; `/refs`·`/tests-for` default-build; style role; `--agent` flags) → **U1 reality-gap surfaced + refined** (symbol-resolution-aware classifier, user-directed) → docs aligned (`/repl` §17.1, `/design` agent.md §2.2, stale "two-bare-symbols=parse-error" premise removed w/ guard) → 2R `/review` GATE-READY (0 Blocker). Default suite **1515/1515**, `--features agent` 18/18; feature-OFF byte-identical verified. Serial-source held. → Wave 3 (the LLM Advisor MVP — adds rig).
