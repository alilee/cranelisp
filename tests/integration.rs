...
use cranelisp::macro_expand::{build_clause_entries, compile_macro, expand_sexp, expand_sexps, flatten_begin, is_defmacro, parse_defmacro};
...
// Global shared macro session. Loading the prelude (~4.3s) is done once
// and reused across all macro-using batch tests.
// ReplSession contains raw pointers (GOT tables, JIT code) that are valid
// cross-thread but prevent auto-Send. We wrap in a Mutex for exclusive access.
struct SendableSession(std::sync::Mutex<cranelisp::repl::ReplSession>);
// SAFETY: ReplSession's raw pointers are JIT code/GOT pointers valid for
// program lifetime. Mutex ensures exclusive access.
unsafe impl Send for SendableSession {}
unsafe impl Sync for SendableSession {}

static MACRO_SESSION: std::sync::LazyLock<SendableSession> =
    std::sync::LazyLock::new(|| {
        let mut session = cranelisp::repl::ReplSession::new().unwrap();
        session.load_prelude();
        // Install macros bare names so compile_macro can resolve Sexp constructors
        // (SexpInt, SexpList, etc.) in user-written macro bodies.
        session.tc.install_macros_bare_names();
        SendableSession(std::sync::Mutex::new(session))
    });

fn with_macro_session<F, R>(f: F) -> R
where
    F: FnOnce(&mut cranelisp::repl::ReplSession) -> R,
{
    let mut session = MACRO_SESSION.0.lock().unwrap();
    f(&mut session)
}
...
/// Force an IO task tree to completion via the trampoline, returning the result value.
fn unwrap_io_val(ptr: i64) -> i64 {
    unsafe { cranelisp::intrinsics::IoTask::from_raw(ptr) }.run()
}

/// Check if main's return type is IO, and if so trampoline it; otherwise return raw.
fn maybe_trampoline_main(
    raw: i64,
    modules: &std::collections::HashMap<
        cranelisp::names::ModuleFullPath,
        cranelisp::module::CompiledModule,
    >,
) -> i64 {
    for cm in modules.values() {
        if let Some(cranelisp::module::ModuleEntry::Def { scheme, .. }) = cm.get("main") {
            if let cranelisp::types::Type::Fn(_, ret) = &scheme.ty {
                if matches!(ret.as_ref(), cranelisp::types::Type::ADT(name, _) if name == "IO") {
                    return unwrap_io_val(raw);
                }
            }
            return raw;
        }
...
/// Fast batch compile path for tests that don't use macros.
/// Skips the expensive macro session + prelude loading (~4.3s → ~0.1s).
fn compile_and_run_simple(src: &str) -> Result<i64, CranelispError> {
    ...
    compile_program(prelude, user)
}

/// Batch compile path for tests that use macros (list, cond, do, bind!, vec, defmacro, etc.).
/// Uses a shared macro session to avoid reloading the prelude per test.
fn compile_and_run(src: &str) -> Result<i64, CranelispError> {
    ...
    // Expand using shared prelude macro session + any user defmacros
    let expanded = with_macro_session(|macro_session| {
        // Compile user defmacros into the shared session
        for sexp in &user_sexps {
            if is_defmacro(sexp) {
                let info = parse_defmacro(sexp)?;
                let ptrs = compile_macro(
                    &mut macro_session.tc,
                    &mut macro_session.jit,
                    &info,
                    Some(&macro_session.macro_env),
                )?;
                let clause_entries = build_clause_entries(&info, ptrs);
        ...
        expand_sexps(user_sexps, &macro_session.macro_env)
    })?;
...
/// Shared batch compile: typecheck and JIT-compile prelude + user program.
fn compile_program(prelude: Vec<cranelisp::ast::TopLevel>, user: Vec<cranelisp::ast::TopLevel>) -> Result<i64, CranelispError> {
    ...
    Ok(maybe_trampoline_main(raw, &tc.modules))
}

#[test]
fn hello() {
    let result = compile_and_run_simple(r#"(defn main [] (print "hello"))"#).unwrap();
...
fn factorial() {
    ...
    "#;
    let result = compile_and_run_simple(src).unwrap();
    ...
    fn trait_impl(&mut self, src: &str) -> Result<(), CranelispError> {
        ...
        match input {
            ReplInput::TraitImpl(ti) => {
                ...
                // Synthesize and compile default methods
                if let Some(decl) = self.tc.find_trait_decl(&ti.trait_name).cloned() {
                    let default_defns = cranelisp::typechecker::synthesize_default_defns(
...
fn string_type_error_int_to_print() {
    ...
    assert!(compile_and_run_simple(src).is_err());
}
...
fn default_method_used_when_not_overridden() {
    let src = r#"
        (deftrait Greetable
          (greet [self] Int)
          (wave [x] Int (+ (greet x) 10)))

        (impl Greetable Int
          (defn greet [x] x))

        (defn main [] (pure (wave 5)))
    "#;
    let result = compile_and_run_simple(src).unwrap();
    assert_eq!(result, 15);
}
...
#[test]
fn default_method_calls_other_trait_method() {
    // Default calls both < and = (from Eq and Ord traits)
    let src = r#"
        (defn main [] (pure (if (<= 3 5) 1 0)))
    "#;
    let result = compile_and_run_simple(src).unwrap();
    assert_eq!(result, 1);
}
...
#[test]
fn default_method_less_equal_boundary() {
    let src = r#"
        (defn main [] (pure (if (<= 5 5) 1 0)))
    "#;
    let result = compile_and_run_simple(src).unwrap();
    assert_eq!(result, 1);
}
...
#[test]
fn default_method_greater_equal() {
    let src = r#"
        (defn main [] (pure (if (>= 5 3) 1 0)))
    "#;
    let result = compile_and_run_simple(src).unwrap();
    assert_eq!(result, 1);
}
...
#[test]
fn repl_default_method_basic() {
    let mut r = ReplSession::new();
    r.trait_decl(
        "(deftrait Greetable (greet [self] Int) (wave [x] Int (+ (greet x) 10)))",
    )
    .unwrap();
    r.trait_impl("(impl Greetable Int (defn greet [x] x))")
        .unwrap();
    assert_eq!(r.eval("(wave 5)").unwrap(), 15);
}

#[test]
fn repl_default_method_override() {
    let mut r = ReplSession::new();
    r.trait_decl(
        "(deftrait Greetable (greet [self] Int) (wave [x] Int (+ (greet x) 10)))",
    )
    .unwrap();
    r.trait_impl("(impl Greetable Int (defn greet [x] x) (defn wave [x] (* x 100)))")
        .unwrap();
    assert_eq!(r.eval("(wave 5)").unwrap(), 500);
}

#[test]
fn default_method_on_adt() {
    let src = r#"
        (deftrait Countable
          (count [self] Int)
          (count-plus-one [x] Int (+ (count x) 1)))

        (deftype Color Red Green Blue)

        (impl Countable Color
          (defn count [c]
            (match c
              [Red 1
               Green 2
        ...
        (defn main [] (pure (count-plus-one Green)))
    "#;
    let result = compile_and_run_simple(src).unwrap();
    assert_eq!(result, 3);
}
...
#[test]
fn default_method_validate_impl_missing_required() {
    // Missing the required method 'greet' should error
    let mut r = ReplSession::new();
    r.trait_decl(
        "(deftrait Greetable (greet [self] Int) (wave [x] Int (+ (greet x) 10)))",
    )
    .unwrap();
    assert!(r
        .trait_impl("(impl Greetable Int (defn wave [x] 42))")
        .is_err());
}
...
fn annotation_expr_int() {
    ...
    assert_eq!(compile_and_run_simple(src).unwrap(), 42);
}
          ...
          (let [x :(Option Int) None]
            (match x
              [None (pure 1)
               (Some v) (pure v)])))
    "#;
    assert_eq!(compile_and_run_simple(src).unwrap(), 1);
}
...
fn annotation_param_trait_constraint() {
    ...
    let src = r#"
        ...
        (defn main [] (pure (add 1 2)))
    "#;
    assert_eq!(compile_and_run_simple(src).unwrap(), 3);
}
...
fn seq_fmap_take() {
    ...
    "#;
    assert_eq!(compile_and_run_simple(src).unwrap(), 11);
}
...
fn seq_lazy_reduce() {
    ...
    "#;
    assert_eq!(compile_and_run_simple(src).unwrap(), 15);
}
...
fn seq_iterate() {
    ...
    "#;
    assert_eq!(compile_and_run_simple(src).unwrap(), 32); // 2^5
}
...
fn seq_repeat() {
    ...
    "#;
    assert_eq!(compile_and_run_simple(src).unwrap(), 126); // 42*3
}
...
fn multisig_reduce_vec() {
    ...
    "#;
    assert_eq!(compile_and_run_simple(src).unwrap(), 6);
}
...
fn multisig_map_vec_to_seq() {
    ...
    "#;
    assert_eq!(compile_and_run_simple(src).unwrap(), 2);
}
...
fn multisig_take_vec() {
    ...
    "#;
    assert_eq!(compile_and_run_simple(src).unwrap(), 30);
}
...
fn multisig_drop_vec() {
    ...
    "#;
    assert_eq!(compile_and_run_simple(src).unwrap(), 40);
}
...
fn seq_pipeline_fmap_filter_take_reduce() {
    ...
    assert_eq!(compile_and_run_simple(src).unwrap(), 24);
}
...
fn functor_fmap_seq_with_range() {
    ...
    "#;
    assert_eq!(compile_and_run_simple(src).unwrap(), 36);
}
...
fn real_repl_session() -> cranelisp::repl::ReplSession {
    ...
    s.load_prelude();
    // Install macros bare names so compile_macro can resolve Sexp constructors
    s.tc.install_macros_bare_names();
    s
}
...
fn repl_sexp_prelude_trait_impl() {
    let r = real_repl_session();
    let view = r.tc.get_def_codegen("Display.show$Int").expect("Display.show$Int should exist");
...
/// Uses the shared macro session for prelude macros.
fn compile_and_run_with_macros(src: &str) -> Result<i64, CranelispError> {
    ...
    let expanded = with_macro_session(|macro_session| {
        ...
        expand_sexps(final_sexps, &macro_session.macro_env)
    })?;
    let user = build_program(&expanded)?;
...
fn macro_repl_compile_and_register() {
    ...
    let ptrs = compile_macro(&mut r.tc, &mut r.jit, &info, Some(&r.macro_env)).unwrap();
    r.macro_env.register(
        info.name.clone(),
        info.docstring.clone(),
        build_clause_entries(&info, ptrs),
    );
...
fn macro_repl_expand_sexp() {
    use cranelisp::macro_expand::{build_clause_entries, compile_macro, expand_sexp, parse_defmacro, MacroEnv};
    use cranelisp::sexp::parse_sexp;
...
fn example_hello() {
    // Output: 42
    assert_eq!(
        compile_and_run_simple(include_str!("../examples/hello.cl")).unwrap(),
        0
    );
}
...
fn example_factorial() {
    // Output: 3628800
    assert_eq!(
        compile_and_run_simple(include_str!("../examples/factorial.cl")).unwrap(),
        0
    );
}
...
fn example_traits() {
    // Output: 42
    assert_eq!(
        compile_and_run_simple(include_str!("../examples/traits.cl")).unwrap(),
        0
    );
}
...
fn repl_full_session_list_no_inline_primitives() {
    // /l listing should not include internal inline primitives like add-i64
    // User-facing inline primitives (bind) are allowed
    let allowed_inline = ["bind"];
    let mut r = cranelisp::repl::ReplSession::new().unwrap();
    r.load_prelude();
...
#[test]
#[ignore] // Checked arithmetic: calls process::exit(1) — run with `cargo test -- --ignored --test-threads=1`
fn checked_division_by_zero_panics() {
    let src = r#"(defn main [] (pure (/ 1 0)))"#;
    let _ = compile_and_run_simple(src);
    // Should not reach here — cranelisp_panic calls process::exit(1)
    unreachable!("division by zero should have panicked");
}

#[test]
#[ignore] // Checked arithmetic: calls process::exit(1)
fn checked_add_overflow_panics() {
    let src = r#"(defn main [] (pure (+ 9223372036854775807 1)))"#;
    let _ = compile_and_run_simple(src);
    unreachable!("integer overflow in + should have panicked");
}

#[test]
#[ignore] // Checked arithmetic: calls process::exit(1)
fn checked_sub_overflow_panics() {
    let src = r#"(defn main [] (pure (- -9223372036854775808 1)))"#;
    let _ = compile_and_run_simple(src);
    unreachable!("integer overflow in - should have panicked");
}
...
fn checked_mul_overflow_panics() {
    let src = r#"(defn main [] (pure (* 9223372036854775807 2)))"#;
    let _ = compile_and_run_simple(src);
    unreachable!("integer overflow in * should have panicked");
}
...
fn non_exhaustive_match_is_compile_error() {
    ...
    "#;
    let err = compile_and_run_simple(src).unwrap_err();
    let msg = format!("{}", err);
    assert!(msg.contains("non-exhaustive"), "expected non-exhaustive error, got: {}", msg);
    assert!(msg.contains("Red"), "should list missing constructor Red, got: {}", msg);
}
...
fn exhaustive_match_all_constructors() {
    let src = r#"
        (deftype Color Red Green Blue)
        (defn main []
          (pure (match Red [Red 1 Green 2 Blue 3])))
    "#;
    let result = compile_and_run_simple(src).unwrap();
    assert_eq!(result, 1);
}
...
fn non_exhaustive_option_missing_none() {
    let src = r#"
        (deftype (Option a) None (Some [:a val]))
        (defn main []
          (pure (match (Some 42) [(Some x) x])))
    "#;
    let err = compile_and_run_simple(src).unwrap_err();
    let msg = format!("{}", err);
    assert!(msg.contains("non-exhaustive"), "expected non-exhaustive error, got: {}", msg);
    assert!(msg.contains("None"), "should list missing constructor None, got: {}", msg);
}

#[test]
fn exhaustive_match_product_type() {
    let src = r#"
        (deftype Point [:Int x :Int y])
        (defn main []
          (pure (match (Point 3 4) [(Point a b) (+ a b)])))
    "#;
    let result = compile_and_run_simple(src).unwrap();
    assert_eq!(result, 7);
}
...
#[test]
fn exhaustive_match_non_adt_scrutinee() {
    // Non-ADT scrutinee (Int) — no exhaustiveness check applies
    let src = r#"
        (defn main []
          (pure (match 42 [x (+ x 1)])))
    "#;
...
// Removed: known_issue_integer_overflow_wraps — overflow is now checked (panics)
// See checked_add_overflow_panics in the ignored section.
...
fn known_issue_qualified_name_resolution_error() {
    ...
    let result = compile_and_run_simple(r#"(defn main [] (pure core.option/Some))"#);
...
#[test]
#[ignore = "flaky: accessor name 'val' is shared by Wrapper and Box; the second deftype's accessor \
             shadows the first, causing Wrapper.val to hold type Box->a instead of Wrapper->a. \
             When Wrapper.val is called on a Wrap value the runtime tag check fails, calling \
             cranelisp_panic which invokes process::exit(1) and kills the test harness in parallel \
             mode. Root cause: accessor functions use global names without type-namespace scoping. \
             See known_issue_adt_accessor_shadowing."]
fn dotted_field_accessor_resolution() {
    // Type.field syntax resolves field accessors.
    // Two types both define a field named 'val'; accessor shadowing (KNOWN_ISSUES.md)
    // causes Wrapper.val to be mistyped at runtime when a second type reuses the name.
    let src = r#"
...
// ── Cache round-trip regression tests ────────────────────────────────

#[test]
fn cache_macro_round_trip() {
    // Regression test: macro-using programs must produce identical output
    // from fresh compilation AND from cache. This catches linker bugs
    // like .Lfn symbol/GOT-slot collisions across cached .o files.
    let dir = setup_cache_test_dir();
    write_source(dir.path(), "test.cl", r#"
        (platform stdio)
        (import [platform.stdio [*]])
        (defmacro double [x] `(+ ~x ~x))
        (defn main [] (do (print (show (double 21)))))
    "#);

    let output1 = run_in_dir(dir.path(), "test.cl");
    assert!(output1.status.success(), "fresh run failed: {}", String::from_utf8_lossy(&output1.stderr));
    let stdout1 = String::from_utf8_lossy(&output1.stdout);

    let output2 = run_in_dir(dir.path(), "test.cl");
    assert!(output2.status.success(), "cached run failed: {}", String::from_utf8_lossy(&output2.stderr));
    let stdout2 = String::from_utf8_lossy(&output2.stdout);

    assert_eq!(stdout1.trim(), "42", "fresh run wrong output");
    assert_eq!(stdout2.trim(), "42", "cached run wrong output");
}

#[test]
fn cache_multisig_round_trip() {
    // Regression test: multi-signature functions (reduce, map, filter, etc.)
    // must work both from fresh compilation AND from cache. This catches the
    // bugs where (1) tc.overloads/resolved_overloads weren't reconstructed
    // from cache, and (2) finalize_defn_type wiped the first variant's
    // codegen fields.
    let dir = setup_cache_test_dir();
    // Use reduce with a Vec (variant 0 — the one that was broken)
    // Main returns the reduced value as Int, verifiable via exit code.
    write_source(dir.path(), "test.cl",
        "(defn main [] (pure (reduce + 0 [1 2 3 4 5])))");
...
// ── Reader shortcuts integration tests ──────────────────────────────

#[test]
fn quote_symbol_produces_sexp() {
    // 'foo → (SexpSym "foo"), verify it matches SexpSym constructor
    let src = r#"
        (defn main []
          (pure (match 'foo
            [(SexpSym _) 1
             _ 0])))
    "#;
    assert_eq!(compile_and_run_with_macros(src).unwrap(), 1);
}

#[test]
fn quote_int_produces_sexp() {
    let src = r#"
        (defn main []
          (pure (match '42
            [(SexpInt n) n
...
#[test]
fn quote_list_produces_sexp() {
    // '(+ 1 2) → SexpList, verify it matches
    let src = r#"
        (defn main []
          (pure (match '(+ 1 2)
            [(SexpList _) 1
...
#[test]
fn quote_nested_structure() {
    // '(if true 1 2) — verify it builds SexpList
    let src = r#"
        (defn main []
          (pure (match '(if true 1 2)
            [(SexpList _) 1
             _ 0])))
    "#;
    assert_eq!(compile_and_run_with_macros(src).unwrap(), 1);
...
#[test]
fn gensym_macro_binds_correctly() {
    let src = r#"
        (defmacro my-let1 [v b] `(let [x# ~v] (+ x# ~b)))
        (defn main [] (pure (my-let1 10 5)))
    "#;
    assert_eq!(compile_and_run_with_macros(src).unwrap(), 15);
}

#[test]
fn gensym_no_capture() {
    // Gensym should not capture outer variable
    let src = r#"
        (defmacro my-let1 [v b] `(let [x# ~v] (+ x# ~b)))
        (defn main [] (pure (let [x 100] (+ (my-let1 10 5) x))))
    "#;
    assert_eq!(compile_and_run_with_macros(src).unwrap(), 115);
}

#[test]
fn gensym_different_calls_different_names() {
    // Two macro calls should get different gensyms
    let src = r#"
        (defmacro my-let1 [v b] `(let [x# ~v] (+ x# ~b)))
        (defn main [] (pure (+ (my-let1 10 5) (my-let1 20 3))))
    "#;
    assert_eq!(compile_and_run_with_macros(src).unwrap(), 38);
}

#[test]
fn anon_fn_single_param() {
    let src = r#"
        (defn main [] (pure (#(+ % 1) 5)))
    "#;
    assert_eq!(compile_and_run_with_macros(src).unwrap(), 6);
}

#[test]
fn anon_fn_two_params() {
    let src = r#"
        (defn main [] (pure (#(* %1 %2) 3 4)))
    "#;
    assert_eq!(compile_and_run_with_macros(src).unwrap(), 12);
}

#[test]
fn anon_fn_zero_params() {
    let src = r#"
        (defn main [] (pure (#(+ 1 2))))
    "#;
    assert_eq!(compile_and_run_with_macros(src).unwrap(), 3);
}

#[test]
fn anon_fn_with_map() {
    // Use #(...) with map on a list
    let src = r#"
        (defn main []
          (pure (let [xs (list 1 2 3)]
            (let [ys (map #(* % %) xs)]
              (reduce + 0 ys)))))
    "#;
    // 1*1 + 2*2 + 3*3 = 14
    assert_eq!(compile_and_run_with_macros(src).unwrap(), 14);
}

// ── Multi-clause defmacro integration tests ─────────────────────────────

#[test]
fn multi_clause_arity_dispatch() {
    // Two-clause macro: base case and recursive case
    let src = r#"
        (defmacro my-add
          ([x] x)
          ([x y] `(+ ~x ~y)))
        (defn main [] (pure (+ (my-add 10) (my-add 3 4))))
    "#;
    // (my-add 10) → 10, (my-add 3 4) → (+ 3 4) = 7, total = 17
    assert_eq!(compile_and_run_with_macros(src).unwrap(), 17);
}

#[test]
fn multi_clause_three_clauses() {
    // Three-clause macro with different arities
    let src = r#"
        (defmacro my-str
          ([] (SexpStr ""))
          ([x] `(show ~x))
          ([x y] `(str-concat (show ~x) (show ~y))))
        (defn main []
          (do
            (print (my-str))
            (print (my-str 42))
...
#[test]
fn multi_clause_recursive() {
    // Recursive multi-clause macro (cond-like pattern)
    let src = r#"
        (defmacro my-cond
          ([x] x)
          ([x body & rest] `(if ~x ~body (my-cond ~@rest))))
        (defn main []
          (pure (my-cond false 1 false 2 99)))
    "#;
    assert_eq!(compile_and_run_with_macros(src).unwrap(), 99);
}

#[test]
fn multi_clause_with_docstring() {
    let src = r#"
        (defmacro my-max "Return the maximum of args"
          ([x] x)
          ([x y] `(if (> ~x ~y) ~x ~y)))
        (defn main [] (pure (my-max 10 20)))
    "#;
    assert_eq!(compile_and_run_with_macros(src).unwrap(), 20);
}

#[test]
fn multi_clause_no_match_error() {
    // Calling with wrong arity should give a clear error
    let src = r#"
        (defmacro pair
          ([x y] `(+ ~x ~y)))
        (defn main [] (pure (pair 1 2 3)))
    "#;
    let result = compile_and_run_with_macros(src);
    assert!(result.is_err(), "expected error for no matching clause");
    let err_msg = format!("{}", result.unwrap_err());
    assert!(err_msg.contains("no clause matching"), "error should mention no clause matching, got: {}", err_msg);
}

#[test]
fn multi_clause_zero_arg_clause() {
    // Zero-arg clause triggers bare-symbol expansion
    let src = r#"
        (defmacro my-nil
          ([] `Nil)
          ([x & rest] `(Cons ~x (my-nil ~@rest))))
        (defn main []
          (pure (let [xs (my-nil 1 2 3)]
            (+ (head xs) (head (tail xs))))))
    "#;
    assert_eq!(compile_and_run_with_macros(src).unwrap(), 3);
}

// ── Bracket destructuring integration tests ─────────────────────────────

#[test]
fn bracket_destructure_simple() {
    // Bracket pattern extracts elements from SexpBracket arg
    let src = r#"
        (defmacro my-let [[name val] body]
          `(let [~name ~val] ~body))
        (defn main [] (pure (my-let [x 42] (+ x 1))))
    "#;
    assert_eq!(compile_and_run_with_macros(src).unwrap(), 43);
}

#[test]
fn bracket_destructure_with_rest() {
    // Bracket pattern with & rest
    let src = r#"
        (defmacro first-of [[x & rest] body]
          `(let [~body ~x] ~body))
        (defn main [] (pure (first-of [99 2 3] y)))
    "#;
    // first-of [99 2 3] y → (let [y 99] y) → 99
    assert_eq!(compile_and_run_with_macros(src).unwrap(), 99);
}

#[test]
fn bracket_destructure_empty() {
    // Empty bracket pattern matches empty brackets
    let src = r#"
        (defmacro check-empty
          ([[] body] body)
          ([[x & rest] body] `(+ ~x ~body)))
        (defn main [] (pure (+ (check-empty [] 10) (check-empty [3] 7))))
    "#;
    // (check-empty [] 10) → 10, (check-empty [3] 7) → (+ 3 7) = 10, total = 20
    assert_eq!(compile_and_run_with_macros(src).unwrap(), 20);
}

#[test]
fn multi_clause_private() {
    // defmacro- with multi-clause
    let src = r#"
        (defmacro- my-add2
          ([x] x)
          ([x y] `(+ ~x ~y)))
        (defn main [] (pure (my-add2 3 4)))
    "#;
    assert_eq!(compile_and_run_with_macros(src).unwrap(), 7);
}

// ── Checked arithmetic: normal operations ─────────────────────────────

#[test]
fn checked_arithmetic_normal_add() {
    let src = r#"(defn main [] (pure (+ 10 32)))"#;
    assert_eq!(compile_and_run_simple(src).unwrap(), 42);
}

#[test]
fn checked_arithmetic_normal_sub() {
    let src = r#"(defn main [] (pure (- 50 8)))"#;
    assert_eq!(compile_and_run_simple(src).unwrap(), 42);
}

#[test]
fn checked_arithmetic_normal_mul() {
    let src = r#"(defn main [] (pure (* 6 7)))"#;
    assert_eq!(compile_and_run_simple(src).unwrap(), 42);
}

#[test]
fn checked_arithmetic_normal_div() {
    let src = r#"(defn main [] (pure (/ 84 2)))"#;
    assert_eq!(compile_and_run_simple(src).unwrap(), 42);
}

#[test]
fn checked_arithmetic_negative_values() {
    let src = r#"(defn main [] (pure (+ -10 -32)))"#;
    assert_eq!(compile_and_run_simple(src).unwrap(), -42);
}

#[test]
fn checked_arithmetic_max_no_overflow() {
    // MAX - 1 should be fine
    let src = r#"(defn main [] (pure (- 9223372036854775807 1)))"#;
    assert_eq!(compile_and_run_simple(src).unwrap(), 9223372036854775806);
}

#[test]
fn raw_primitive_add_wraps() {
    // add-i64 is unchecked: MAX + 1 wraps to MIN
    let src = r#"(defn main [] (pure (add-i64 9223372036854775807 1)))"#;
    let result = compile_and_run_simple(src).unwrap();
    assert!(result < 0, "expected negative wrap-around, got {}", result);
}

#[test]
fn float_div_zero_returns_infinity() {
    // IEEE 754: 1.0 / 0.0 = Infinity (positive bits)
    let src = r#"(defn main [] (pure (/ 1.0 0.0)))"#;
    let result = compile_and_run_simple(src).unwrap();
    let f = f64::from_bits(result as u64);
...
fn derive_eq_sum() {
    run_derive_test(r#"
        (derive [Eq] (deftype (MyOpt a) MyNone (MySome [:a val])))
        (defn main []
          (let [a (MySome 42)
                b (MySome 42)
                c (MySome 99)
...
fn derive_ord_enum() {
    run_derive_test(r#"
        (derive [Eq Ord] (deftype Color Red Green Blue))
        (defn main []
          (if (< Red Green)
            (if (< Green Blue)
              (if (< Blue Red) (pure 1) (pure 0))
              (pure 2))
            (pure 3)))
    "#);
}
...
#[test]
fn derive_display_enum() {
    run_derive_test(r#"
        (derive [Eq Display] (deftype Color Red Green Blue))
        (defn main []
          (if (= (show Red) "Red")
            (if (= (show Blue) "Blue") (pure 0) (pure 1))
            (pure 2)))
    "#);
}
...
fn derive_multiple_traits() {
    run_derive_test(r#"
        ...
        (defn main []
          (if (= Red Red)
            (if (< Red Blue)
              (if (= (show Green) "Green") (pure 0) (pure 1))
              (pure 2))
            (pure 3)))
    "#);
}
...
#[test]
fn derive_eq_direct() {
    // Using derive-Eq directly (not through derive dispatch)
    run_derive_test(r#"
        (deftype Color Red Green Blue)
        (derive-Eq (deftype Color Red Green Blue))
        (defn main []
...
#[test]
fn derive_eq_enum_all_variants() {
    // Test every combination of enum equality
    run_derive_test(r#"
        (derive [Eq] (deftype AB A B))
        (defn main []
          (if (= A A)
            (if (= B B)
              (if (= A B) (pure 1)
                (if (= B A) (pure 2) (pure 0)))
              (pure 3))
            (pure 4)))
    "#);
}
...
#[test]
fn derive_ord_enum_leq_geq() {
    // Default methods <= and >= should work from derived Ord
    run_derive_test(r#"
        (derive [Eq Ord] (deftype Color Red Green Blue))
        (defn main []
          (if (<= Red Red)
            (if (<= Red Green)
              (if (>= Blue Green) (pure 0) (pure 1))
              (pure 2))
            (pure 3)))
    "#);
}
...
#[test]
fn derive_example_file() {
    let entry = std::path::Path::new("examples/derived.cl");
    cranelisp::batch::run_project(entry).unwrap();
}
...
fn par_let_basic() {
    let src = r#"
        (defn main [] (pure (par-let [a 1 b 2] (+ a b))))
    "#;
    let result = compile_and_run_simple(src).unwrap();
    assert_eq!(result, 3);
}
...
#[test]
fn par_let_with_computation() {
    let src = r#"
        (defn fib [n]
          (if (<= n 1) n (+ (fib (- n 1)) (fib (- n 2)))))
        (defn main []
          (pure (par-let [a (fib 10)
                          b (fib 12)]
            (+ a b))))
    "#;
    let result = compile_and_run_simple(src).unwrap();
    assert_eq!(result, 55 + 144);
}

#[test]
fn par_let_with_captures() {
    let src = r#"
        (defn main []
          (let [x 10]
            (pure (par-let [a (+ x 1)
...
#[test]
fn par_let_independence_error() {
    // Bindings cannot reference each other
    let src = r#"
        (defn main [] (pure (par-let [a 1 b a] b)))
    "#;
    assert!(compile_and_run_simple(src).is_err());
}

#[test]
fn par_let_single_binding_error() {
    // par-let requires at least 2 bindings
    let src = r#"
        (defn main [] (pure (par-let [a 1] a)))
    "#;
    assert!(compile_and_run_simple(src).is_err());
}

#[test]
fn par_let_three_bindings() {
    let src = r#"
        (defn main []
          (pure (par-let [a 10 b 20 c 30]
            (+ a (+ b c)))))
    "#;
    let result = compile_and_run_simple(src).unwrap();
    assert_eq!(result, 60);
...
// ===== bind! IO chaining tests (previously par-bind! — now auto-scheduled) =====

#[test]
fn bind_two_io() {
    // Two sequential IO bindings via bind!
    let src = r#"
        (defn main []
          (bind! [r1 (pure 10)
                  r2 (pure 20)]
...
#[test]
fn bind_type_error_non_io() {
    // Binding expressions must be IO type
    let src = r#"
        (defn main []
          (bind! [r1 10
                  r2 20]
...
#[test]
fn bind_body_must_be_io() {
    // Body must be IO type
    let src = r#"
        (defn main []
          (bind! [r1 (pure 10)
                  r2 (pure 20)]
...
#[test]
fn bind_three_io() {
    // Three sequential IO bindings via bind!
    let src = r#"
        (defn main []
          (bind! [r1 (pure 10)
                  r2 (pure 20)
...
#[test]
fn par_let_example_file() {
    let entry = std::path::Path::new("examples/parallel.cl");
    cranelisp::batch::run_project(entry).unwrap();
}

// ── Lenient evaluation (Step 13) ──────────────────────────────────────

#[test]
fn lenient_two_independent_calls() {
    // Two independent function calls should be sparked and produce correct results
    let src = r#"
        (defn double [:Int x] (* x 2))
        (defn triple [:Int x] (* x 3))
        (defn main []
            (pure (let [a (double 10)
                        b (triple 10)]
                (+ a b))))
    "#;
    let result = compile_and_run_simple(src).unwrap();
    assert_eq!(result, 50); // 20 + 30
}

#[test]
fn lenient_dependent_bindings_not_sparked() {
    // b depends on a, so neither should be sparked (only 1 independent)
    let src = r#"
        (defn double [:Int x] (* x 2))
        (defn main []
            (pure (let [a (double 5)
...
#[test]
fn lenient_mixed_independent_dependent() {
    // a and b are independent (sparkable), c depends on both (sequential)
    let src = r#"
        (defn double [:Int x] (* x 2))
        (defn triple [:Int x] (* x 3))
        (defn main []
    ...
    "#;
    let result = compile_and_run_simple(src).unwrap();
    assert_eq!(result, 20); // 8 + 12
}

#[test]
fn lenient_trivial_bindings_not_sparked() {
    // Literals and simple ops are too cheap to spark
    let src = r#"
        (defn main []
            (pure (let [a 10
                        b 20
...
#[test]
fn lenient_single_sparkable_not_sparked() {
    // Only one sparkable binding — needs ≥2 to spark
    let src = r#"
        (defn double [:Int x] (* x 2))
        (defn main []
            (pure (let [a (double 5)
...
#[test]
fn lenient_heap_typed_results() {
    // String values (heap-typed) should work correctly after parallel evaluation
    let src = r#"
        (defn greet [:String name] (str-concat "Hello " name))
        (defn farewell [:String name] (str-concat "Bye " name))
        (defn main []
            (pure (let [a (greet "Alice")
                        b (farewell "Bob")]
                (str-concat a (str-concat " and " b)))))
    "#;
    let result = compile_and_run_simple(src).unwrap();
    let s = unsafe {
        let len = *(result as *const i64) as usize;
        let bytes = std::slice::from_raw_parts((result as *const u8).offset(8), len);
        std::str::from_utf8(bytes).unwrap().to_string()
    };
    assert_eq!(s, "Hello Alice and Bye Bob");
}

#[test]
fn lenient_closures_with_captures() {
    // Sparked thunks correctly capture enclosing scope variables
    let src = r#"
        (defn add-to [:Int x :Int y] (+ x y))
        (defn main []
            (pure (let [base 100]
                (let [a (add-to base 1)
                      b (add-to base 2)]
    ...
    "#;
    let result = compile_and_run_simple(src).unwrap();
    assert_eq!(result, 203); // 101 + 102
}

#[test]
fn lenient_nested_let() {
    // Inner let may have its own spark group, independent of outer
    let src = r#"
        (defn double [:Int x] (* x 2))
        (defn triple [:Int x] (* x 3))
        (defn main []
    ...
    "#;
    let result = compile_and_run_simple(src).unwrap();
    assert_eq!(result, 65); // double(10) + triple(15) = 20 + 45
}

#[test]
fn lenient_cheap_builtins_excluded() {
    // + and * are cheap builtins — should not be sparked even if independent
    let src = r#"
        (defn main []
            (pure (let [a (+ 1 2)
                        b (* 3 4)]
    ...
    "#;
    let result = compile_and_run_simple(src).unwrap();
    assert_eq!(result, 15); // 3 + 12
}

#[test]
fn lenient_three_independent_calls() {
    // Three independent sparkable bindings
    let src = r#"
        (defn f [:Int x] (+ x 1))
        (defn g [:Int x] (+ x 2))
        (defn h [:Int x] (+ x 3))
    ...
    "#;
    let result = compile_and_run_simple(src).unwrap();
    assert_eq!(result, 36); // 11 + 12 + 13
}

// ── Inline module REPL helper ────────────────────────────────────────────────
/// Start the REPL with `file` as the initial load target, pipe `input` as
/// stdin, and return stdout. Sets up lib/ and target/debug/ symlinks so the
/// prelude and platform DLLs are available.
fn run_repl_with_file(file: &Path, input: &str) -> String {
    use std::io::Write;
    let dir = tempfile::tempdir().unwrap();
    let lib_src = Path::new(env!("CARGO_MANIFEST_DIR")).join("lib");
    std::os::unix::fs::symlink(&lib_src, dir.path().join("lib")).unwrap();
...
#[test]
fn inline_module_extraction() {
    // (mod helper (defn ...)) should extract to helper.cl and work
    use std::io::Write;
    let dir = tempfile::tempdir().unwrap();
    let main_path = dir.path().join("main.cl");
    let mut f = std::fs::File::create(&main_path).unwrap();
    writeln!(
        f,
        r#"(mod helper
  (defn inc-value [:Int x] :Int (+ x 1)))

(defn main [] (helper/inc-value 41))"#
    )
    .unwrap();
    drop(f);
    cranelisp::batch::run_project(&main_path).unwrap();
    ...
    let child_file = dir.path().join("main").join("helper.cl");
    assert!(
        child_file.exists(),
        "inline module should be extracted to {}",
        child_file.display()
    );

    // Verify parent file was rewritten
    let parent_content = std::fs::read_to_string(&main_path).unwrap();
    assert!(
        parent_content.contains("(mod helper)"),
        "parent should be rewritten with (mod helper), got: {}",
        parent_content
    );
    assert!(
        !parent_content.contains("(defn inc-value"),
        "parent should not contain inline defn after extraction, got: {}",
        parent_content
    );
}

#[test]
fn inline_module_with_super_import() {
    // Inline module uses (import [super [*]]) to access parent definitions.
    // Parent does NOT reference child (test module pattern).
    use std::io::Write;
    let dir = tempfile::tempdir().unwrap();
    let main_path = dir.path().join("main.cl");
    let mut f = std::fs::File::create(&main_path).unwrap();
    writeln!(
        f,
        r#"(defn double [:Int x] :Int (* x 2))

(mod test
  (import [super [*]])
  (defn test-double [] :Int (double 21)))
...
(defn main [] (double 21))"#
    ...
    assert!(
        result.is_ok(),
        "inline module with super import should compile: {:?}",
        result.err()
    );

    // Verify child file was extracted
    let child_file = dir.path().join("main").join("test.cl");
    assert!(
        child_file.exists(),
        "inline test module should be extracted to {}",
        child_file.display()
    );

    // Verify child file contains the import and defn
    let child_content = std::fs::read_to_string(&child_file).unwrap();
    assert!(
        child_content.contains("import") && child_content.contains("test-double"),
        "child should contain import and defn, got: {}",
        child_content
    );
}

#[test]
fn super_import_in_top_level_module_errors() {
    // (import [super [*]]) at top level should error
    use std::io::Write;
    let dir = tempfile::tempdir().unwrap();
    let main_path = dir.path().join("main.cl");
    let mut f = std::fs::File::create(&main_path).unwrap();
    writeln!(f, "(import [super [*]])").unwrap();
    writeln!(f, "(defn main [] 0)").unwrap();
    drop(f);
    let result = cranelisp::batch::run_project(&main_path);
    assert!(result.is_err());
...
#[test]
fn repl_mod_switches_to_inline_submodule() {
    // Regression: /mod <short_name> was broken for submodules loaded under
    // their full dotted path (e.g. "main.test" stored in loaded_modules but
    // /mod test looked for exact key "test").
    use std::io::Write;
    let dir = tempfile::tempdir().unwrap();
    let main_path = dir.path().join("main.cl");
    let mut f = std::fs::File::create(&main_path).unwrap();
...
(defn main [] 0)"#
    ...
    // First run batch so the inline module is extracted to main/test.cl
    cranelisp::batch::run_project(&main_path).unwrap();

    // Now start the REPL with the file and switch to the test submodule
    let stdout = run_repl_with_file(&main_path, "/mod test\n");
    assert!(
        stdout.contains("test>"),
        "/mod test should switch prompt to 'test>', got: {}",
        stdout
    );
}

#[test]
fn repl_run_tests_finds_test_functions() {
    // /run-tests should discover test-* functions in .test submodules and
    ...
    writeln!(
        f,
        r#"(defn add [:Int x :Int y] :Int (+ x y))

(mod test
  (import [super [*]])
  (defn test-add [] (if (= (add 1 2) 3) None (Some "add failed"))))
...
(defn main [] 0)"#
    ...
    let stdout = run_repl_with_file(&main_path, "/run-tests\n");
    assert!(
        stdout.contains("test-add"),
        "/run-tests should find test-add, got: {}",
        stdout
    );
    assert!(
        stdout.contains("ok"),
        "/run-tests should report ok, got: {}",
        stdout
    );
}

#[test]
fn test_assertions_library() {
    // Test that lib/test.cl assertions work: assert-eq, assert-true, assert-false, check
    use std::io::Write;
    let dir = tempfile::tempdir().unwrap();
    let main_path = dir.path().join("main.cl");
    // Symlink lib dir so test module is discoverable
    let lib_src = Path::new(env!("CARGO_MANIFEST_DIR")).join("lib");
    std::os::unix::fs::symlink(&lib_src, dir.path().join("lib")).unwrap();
    let mut f = std::fs::File::create(&main_path).unwrap();
    writeln!(
...
(defn test-pass [] (assert-eq 1 1))
(defn test-fail [] (assert-eq 1 2))
(defn test-true [] (assert-true true))
(defn test-false [] (assert-false false))

(defn test-check-pass []
  (check (assert-eq 1 1) (assert-true true) (assert-false false)))

(defn test-check-fail []
  (check (assert-eq 1 1) (assert-eq 1 2) (assert-true true)))

(defn main []
  (let [r1 (match (test-pass) [None 0 (Some _) 1])
        r2 (match (test-fail) [None 0 (Some _) 1])
        r3 (match (test-true) [None 0 (Some _) 1])
        r4 (match (test-false) [None 0 (Some _) 1])
        r5 (match (test-check-pass) [None 0 (Some _) 1])
        r6 (match (test-check-fail) [None 0 (Some _) 1])]
    (+ r1 (* 10 (+ r2 (* 10 (+ r3 (* 10 (+ r4 (* 10 (+ r5 (* 10 r6))))))))))))
"#
    )
    .unwrap();
    drop(f);
    ...
    assert!(
        result.is_ok(),
        "test assertions should compile and run: {:?}",
        result.err()
    );
}

// ── Gap 3: Vec out-of-bounds behavior ────────────────────────────────────────
//
// vec-get with an out-of-range index calls cranelisp_panic which invokes
// process::exit(1). This kills the test harness in parallel mode, so the
// test is marked #[ignore]. The existing known_issue_vec_out_of_bounds
// test documents this; the test below documents the same for vec-set.
//
// To observe the behavior manually:
//   cargo test vec_oob -- --ignored --test-threads=1

#[test]
#[ignore = "vec-get out-of-bounds calls cranelisp_panic → process::exit(1); \
             kills test harness in parallel mode. Run with --ignored --test-threads=1."]
fn vec_get_out_of_bounds_panics() {
    // Accessing index 10 in a 3-element vec triggers the bounds check
    // and calls cranelisp_panic which exits the process.
    let src = r#"(defn main [] (pure (vec-get [1 2 3] 10)))"#;
    let _ = compile_and_run(src);
    unreachable!("vec-get out-of-bounds should have panicked");
}

#[test]
#[ignore = "vec-get negative index calls cranelisp_panic → process::exit(1); \
             kills test harness in parallel mode. Run with --ignored --test-threads=1."]
fn vec_get_negative_index_panics() {
    // Negative index is also rejected by the bounds check.
    let src = r#"(defn main [] (pure (vec-get [1 2 3] -1)))"#;
    let _ = compile_and_run(src);
    unreachable!("vec-get negative index should have panicked");
}
...
fn error_type_error_int_plus_bool() {
    // Adding Int to Bool is a type error.
    let result = compile_and_run_simple("(defn main [] (pure (+ 1 true)))");
...
fn error_type_error_bool_as_int() {
    ...
    let result = compile_and_run_simple("(defn main [] (pure (+ true false)))");
    assert!(result.is_err(), "expected type error");
    let msg = format!("{}", result.unwrap_err());
    assert!(msg.contains("Type error"), "expected 'Type error' in: {}", msg);
}
...
fn error_parse_error_unclosed_paren() {
    // Missing closing paren is a parse error.
    let result = compile_and_run_simple("(defn main [] (pure (+ 1 2)");
...
fn error_parse_error_extra_closing_paren() {
    // Extra closing paren.
    let result = compile_and_run_simple("(defn main [] (pure 42)))");
    assert!(result.is_err(), "expected parse error");
    let msg = format!("{}", result.unwrap_err());
    assert!(
        msg.contains("Parse error") || msg.contains("parse"),
...
fn error_unbound_symbol() {
    // Referencing a symbol that has never been defined.
    let result = compile_and_run_simple("(defn main [] (pure no-such-symbol))");
    assert!(result.is_err(), "expected error for unbound symbol");
    let msg = format!("{}", result.unwrap_err());
    assert!(
        msg.contains("no-such-symbol") || msg.contains("unbound") || msg.contains("undefined"),
        "expected mention of the unbound symbol in: {}",
        msg
    );
}
...
fn error_wrong_arity_too_many_args() {
    // Calling a one-argument function with two arguments.
    let result = compile_and_run_simple(
        r#"(defn id [x] x) (defn main [] (pure (id 1 2)))"#,
    );
    assert!(result.is_err(), "expected arity/type error");
    // The error message should indicate something went wrong with the call.
    let msg = format!("{}", result.unwrap_err());
...
fn error_wrong_arity_too_few_args() {
    // Calling a two-argument function with one argument (should auto-curry, but type mismatch
    // will surface when the partial application is returned where Int is expected).
    let result = compile_and_run_simple(
        r#"(defn add [x y] (+ x y)) (defn main [:Int] (pure (add 1)))"#,
    );
    // This may auto-curry rather than error immediately; what matters is the return type
    // is a function, not an Int, so wrapping in main return type (IO Int) will fail.
    // Accept either Ok (auto-curry) or Err (type error).
    let _ = result; // Acceptable either way — we just verify it doesn't panic.
}

#[test]
fn error_non_exhaustive_match_caught_at_compile_time() {
    // Non-exhaustive match patterns are caught by the type checker.
    let src = r#"
        (deftype Shape Circle Square Triangle)
        (defn main []
          (pure (match Circle [Circle 1 Square 2])))
    "#;
    let result = compile_and_run_simple(src);
    assert!(result.is_err(), "expected non-exhaustive match error");
    let msg = format!("{}", result.unwrap_err());
    assert!(
        msg.contains("non-exhaustive") || msg.contains("Triangle"),
        "expected mention of non-exhaustive or missing constructor: {}",
        msg
    );
}
...
fn error_type_mismatch_if_branches() {
    ...
    let result =
        compile_and_run_simple(r#"(defn main [] (pure (if true 1 "hello")))"#);
    assert!(result.is_err(), "expected type error for mismatched if branches");
    let msg = format!("{}", result.unwrap_err());
    assert!(msg.contains("Type error"), "expected 'Type error' in: {}", msg);
}
...
fn error_defn_body_type_mismatch() {
    // Declared return type annotation contradicts actual body type.
    let result = compile_and_run_simple(
        r#"(defn f [:Int x] :String (+ x 1)) (defn main [] (pure 0))"#,
    );
    assert!(
        result.is_err(),
        "expected type error for body/return-type mismatch"
    );
    let msg = format!("{}", result.unwrap_err());
    assert!(msg.contains("Type error"), "expected 'Type error' in: {}", msg);
}

// ── Gap 5: REPL slash command tests ──────────────────────────────────────────
//
// The REPL command handlers (cmd_sig, cmd_type, cmd_info, cmd_list) all drive
// through TypeChecker::describe_symbol.  We test the underlying data paths
// directly so tests don't depend on stdout capture.

#[test]
fn repl_slash_sig_user_fn() {
    // /sig on a user-defined function returns its type scheme.
    let mut r = ReplSession::new();
    r.defn("(defn add [x y] (+ x y))").unwrap();
    let info = r.tc.describe_symbol("add");
    assert!(info.is_some(), "/sig add: symbol should be found");
    let formatted = cranelisp::repl::format::format_symbol_info("add", &info.unwrap());
    // The formatted output should contain the function name and "Fn" type.
    assert!(
        formatted.contains("add"),
        "/sig add: output should contain 'add': {}",
        formatted
    );
    assert!(
        formatted.contains("Fn") || formatted.contains("->") || formatted.contains("Int"),
        "/sig add: output should describe a function type: {}",
        formatted
    );
}

#[test]
fn repl_slash_sig_builtin() {
    // /sig on a builtin like `show` returns trait method info.
    let r = ReplSession::new();
    let info = r.tc.describe_symbol("show");
    assert!(info.is_some(), "/sig show: symbol should be found in prelude");
    let formatted = cranelisp::repl::format::format_symbol_info("show", &info.unwrap());
    assert!(
        formatted.contains("show"),
        "/sig show: output should contain 'show': {}",
        formatted
    );
}

#[test]
fn repl_slash_type_expression() {
    // /type on an expression: we check the type via TypeChecker directly.
    // /type calls parse_repl_input + check_expr internally.
    let mut r = ReplSession::new();
    // check_expr on a literal returns its type
    let input = cranelisp::ast_builder::parse_repl_input("42").unwrap();
    if let cranelisp::ast::ReplInput::Expr(expr) = input {
        let ty = r.tc.check_expr(&expr).unwrap();
        let resolved = r.tc.resolve(&ty);
        let formatted = cranelisp::repl::format::format_type_for_display(&resolved);
        assert!(
            formatted.contains("Int"),
            "/type 42: expected 'Int', got: {}",
            formatted
        );
    } else {
        panic!("expected Expr ReplInput for '42'");
    }
}

#[test]
fn repl_slash_type_bool_expression() {
    // /type on a boolean literal returns Bool.
    let mut r = ReplSession::new();
    let input = cranelisp::ast_builder::parse_repl_input("true").unwrap();
    if let cranelisp::ast::ReplInput::Expr(expr) = input {
        let ty = r.tc.check_expr(&expr).unwrap();
        let resolved = r.tc.resolve(&ty);
        let formatted = cranelisp::repl::format::format_type_for_display(&resolved);
        assert!(
            formatted.contains("Bool"),
            "/type true: expected 'Bool', got: {}",
            formatted
        );
    } else {
        panic!("expected Expr ReplInput for 'true'");
    }
}

#[test]
fn repl_slash_info_user_fn() {
    // /info on a user function returns SymbolInfo::UserFn with its scheme.
    let mut r = ReplSession::new();
    r.defn("(defn double [x] (* x 2))").unwrap();
    let info = r.tc.describe_symbol("double");
    assert!(
        matches!(info, Some(cranelisp::typechecker::SymbolInfo::UserFn { .. })),
        "/info double: expected UserFn, got: {:?}",
        info.map(|i| std::mem::discriminant(&i))
    );
    let formatted = cranelisp::repl::format::format_symbol_info("double", &info.unwrap());
    assert!(
        formatted.contains("double"),
        "/info double: output should contain 'double': {}",
        formatted
    );
}

#[test]
fn repl_slash_info_constructor() {
    // /info on a constructor (Some) returns SymbolInfo::Constructor.
    let r = ReplSession::new();
    let info = r.tc.describe_symbol("Some");
    assert!(
        matches!(info, Some(cranelisp::typechecker::SymbolInfo::Constructor { .. })),
        "/info Some: expected Constructor, got: {:?}",
        info.map(|i| std::mem::discriminant(&i))
    );
    let formatted = cranelisp::repl::format::format_symbol_info("Some", &info.unwrap());
    assert!(
        formatted.contains("Some"),
        "/info Some: output should contain 'Some': {}",
        formatted
    );
}

#[test]
fn repl_slash_list_finds_user_fn() {
    // After defining a function, describe_symbol finds it.
    // /list internally iterates module entries and calls describe_symbol on each.
    let mut r = ReplSession::new();
    r.defn("(defn my-special-fn [x] (+ x 99))").unwrap();
    let info = r.tc.describe_symbol("my-special-fn");
    assert!(
        info.is_some(),
        "/list should be able to find 'my-special-fn' via describe_symbol"
    );
}

#[test]
fn repl_slash_list_prelude_symbols() {
    // Prelude symbols (show, None, Some) are discoverable via describe_symbol.
    let r = ReplSession::new();
    assert!(
        r.tc.describe_symbol("None").is_some(),
        "/list: 'None' should be visible in prelude"
    );
    assert!(
        r.tc.describe_symbol("Some").is_some(),
        "/list: 'Some' should be visible in prelude"
    );
    assert!(
        r.tc.describe_symbol("show").is_some(),
        "/list: 'show' trait method should be visible in prelude"
    );
}
