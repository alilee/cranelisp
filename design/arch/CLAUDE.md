...
- [0044](decisions/0044-cluster-atomic-typecheck-orchestrator-staging.md) — Cluster-atomic typecheck via orchestrator-owned staging + two pure passes (`check_form_signatures` + `check_form_body`); `View<'_, C, L>` newtype; reframes 0038's per-form check_form shape; pre-implementation; filed by FIXME 0166)
