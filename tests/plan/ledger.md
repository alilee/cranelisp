...
| 21 | `ownership_fences::h2_rc_stats_reports_per_mechanism_counters` | Hook H2 owed (gate-blocking I-G3/I-G7) | **RESOLVED (S102, /dev for /backend)** — per-mechanism `[RC_STATS]` family landed (`stack_slot` / `reuse_hit` / `reuse_miss` / `rc_nonatomic` / `rc_atomic`); grammar pinned `design/backend/ownership-codegen.md §13.2.1`; counters owned in `cranelisp-intrinsics::rc`, backend the sole writer; byte-identical-off (golden diff empty). GREEN. |
| 22 | `ownership_fences::h3_rc_stats_reports_per_extern_adaptation_pairs` | Hook H3 owed (L-D5 report lane) | **RE-DISPOSITIONED → increment II** (S102, /dev for /backend). H3 is report-grade, NOT gate-blocking (no I-G gate reads per-extern attribution). Per-extern adaptation-pair attribution needs a runtime, name-keyed tally map + per-extern emitted hooks — machinery that rides the **L-D5 sibling-expansion** decision (the `str-len$borrowed` dual-symbol convention, §9.2), which is itself increment-II work. It does NOT fall out of H2's codegen-time counters cheaply, so it is deferred rather than forced. Test stays RED (owed-signal) targeting increment II / the L-D5 landing. |
