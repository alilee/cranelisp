...
| Skill | Crate | Task | Status |
|---|---|---|---|
| /spec | spec/ | Editorial align §8.6.4/§8.8: implicit prelude = outer scope consulted on a miss, not materialised; "injection" = activating the fallback. | **DONE** (§8.6.1 layer 2 + §8.6.4 subsection + §8.8.1; conflict detection scoped to inner tier; linter clean; §8.13.1/§8.1.1 entry-module wording flagged for W5) |
| /qa | tests/ | explicit/local silently shadows prelude; explicit-vs-explicit still ambiguous (§8.6.5); `(import [prelude []])` refusal → no fallback; primitives-via-prelude resolve; `/imports` shows "Prelude (implicit)" group. | **DONE** — `spec_08_prelude_outer_scope.rs` (15) + `repl_introspection.rs` (3). **6 RED / 12 GREEN.** |
| /design | cranelisp-typecheck, src/ | Refine the outer-scope fallback per crate. | **DONE** — §2.7 REALIZATION authored. |

**Wave 4 /qa NARROWING (key):** the outer-scope reshape's correctness surface is SMALL. 12 GREEN guards already work under the current flattened model (local-def shadow, ambiguity ×2, refusal+qualified, selective, **primitives-via-prelude bare+qualified+coexist**). Only **6 RED**: (a) **3 explicit-import-shadows-prelude** (`undefined variable: gulp` — the flattened prelude entry + explicit import collide → poison the bare name; the fix makes the explicit import the sole inner-table entry so it wins, NO ambiguity in stderr, exit 1005); (b) **3 `/imports`** presentation (no "Prelude (implicit)" group exists yet). **Contract for /dev:** delete `is_seeded` + stop flattening → fix the 3 import-shadow REDs WITHOUT regressing the 12 greens — esp. `bare_primitive_resolves_via_prelude_reexport` (the `"primitives"` arm of `is_seeded`) must survive via prelude's `(export [primitives [*]])` re-export chain-follow through the fallback, not a name-key.
**Wave 4 /design CONTRACT (§2.7):** fallback bit = `prelude_fallback: DashMap<ModuleFullPath, bool>` on `SharedState` (session-side, unserialized). `check_forms` gains a 5th read-only param `prelude_fallback: &PreludeFallback`, on `TypeCheckEnv` beside `module_aliases`. **Resolution:** `probe_current_or_prelude` wrapper (chokepoint 1) + two-hop View (chokepoint 2) — bare-name current-module miss + bit ON → re-probe `prelude`'s table (chain-follows its `(export [primitives [*]])` re-exports → primitives). Explicit `fq.module` probes do NOT fall back. **Installer:** `inject_prelude_if_needed` replaces the two `install_imports([prelude])` flatten calls with `prelude_fallback.insert(M, true)` (keeps discovery/load); delete `is_seeded`. **/sprint decision: `PreludeFallback` alias = option (b)** — `pub type PreludeFallback = DashMap<ModuleFullPath, bool>` in **`cranelisp-typecheck`**, NO `cranelisp-types` change. **Landing order:** /dev typecheck (param + chokepoints, green standalone via empty-map test callers) → /dev src/ (field + thread bit + installer + is_seeded delete + introspection; int green + /qa 6 RED→green, 12 hold). Lockstep `check_forms` signature = the int↔typecheck boundary.
