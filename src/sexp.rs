...
/// Build an anonymous function from `#(...)` reader syntax.
///
/// Walks the body to find `%`, `%1`-`%9` params, normalizes `%` to `%1`,
/// and wraps as `(fn [%1 %2 ... %N] body)`.
fn build_anon_fn(children: Vec<Sexp>, span: Span) -> Sexp {
    // Collect all percent params from the body tree
    fn collect_percent_params(sexp: &Sexp, params: &mut std::collections::BTreeSet<u8>) {
        match sexp {
            Sexp::Symbol(s, _) => {
                if s == "%" {
                    params.insert(1);
                } else if s.len() == 2 && s.starts_with('%') {
                    if let Some(d) = s.as_bytes().get(1) {
            ...
            Sexp::List(children, _) => {
                for child in children {
                    collect_percent_params(child, params);
                }
            }
            Sexp::Bracket(children, _) => {
                for child in children {
    ...
    // Replace bare `%` with `%1` in the body tree
    fn normalize_percent(sexp: Sexp) -> Sexp {
        match sexp {
            Sexp::Symbol(ref s, span) if s == "%" => Sexp::Symbol("%1".to_string(), span),
            Sexp::List(children, span) => {
                Sexp::List(children.into_iter().map(normalize_percent).collect(), span)
            }
            Sexp::Bracket(children, span) => {
                Sexp::Bracket(children.into_iter().map(normalize_percent).collect(), span)
            }
            other => other,
        }
    }
    ...
    for child in &children {
        collect_percent_params(child, &mut params);
    }

    // Find max param index (0 if no params)
    let max_param = params.iter().copied().max().unwrap_or(0);

    // Build params bracket: [%1 %2 ... %N]
    let param_syms: Vec<Sexp> = (1..=max_param)
        .map(|i| Sexp::Symbol(format!("%{}", i), span))
        .collect();
    let params_bracket = Sexp::Bracket(param_syms, span);

    // Normalize bare % to %1 in body and wrap as the body expression
    let body = Sexp::List(children.into_iter().map(normalize_percent).collect(), span);

    // (fn [%1 %2 ... %N] body)
    Sexp::List(
        vec![Sexp::Symbol("fn".to_string(), span), params_bracket, body],
        span,
    )
}
...
peg::parser! {
    grammar sexp_parser() for str {
        ...
        rule gensym_symbol() -> (String, Span)
            = s:position!() name:$(['a'..='z' | 'A'..='Z' | '_'] symbol_char()* "#") e:position!()
              { (name.to_string(), (s, e)) }

        rule percent_param() -> (String, Span)
            = s:position!() name:$("%" ['1'..='9']?) e:position!()
              { (name.to_string(), (s, e)) }
        ...
        rule atom() -> Sexp
            ...
            / v:gensym_symbol() { Sexp::Symbol(v.0, v.1) }
            / v:percent_param() { Sexp::Symbol(v.0, v.1) }
        ...
        rule anon_fn() -> Sexp
            = s:position!() "#(" ws() children:form()* ws() ")" e:position!()
              { build_anon_fn(children, (s, e)) }

        rule quote_reader() -> Sexp
            = s:position!() "'" f:form() e:position!()
              {
                  Sexp::List(vec![
                      Sexp::Symbol("quote".to_string(), (s, s + 1)),
                      f,
                  ], (s, e))
              }
        ...
        rule dollar_sym() -> Sexp
            = s:position!() "$" "(" ws() inner:form() ws() ")" e:position!()
              {
                  Sexp::List(vec![
                      Sexp::Symbol("SexpSym".to_string(), (s, s + 1)),
                      inner,
                  ], (s, e))
              }
            / s:position!() "$" name:symbol() e:position!()
              {
                  Sexp::List(vec![
                      Sexp::Symbol("SexpSym".to_string(), (s, s + 1)),
                      Sexp::Symbol(name.0, name.1),
                  ], (s, e))
              }

        rule form() -> Sexp
            = ws() f:(quote_reader() / quasiquote() / unquote_splicing() / unquote()
                      / dollar_sym() / anon_fn() / list() / bracket() / atom()) ws()
              { f }
...
mod tests {
    ...
    #[test]
    fn parse_dollar_sym_name() {
        // $foo → (SexpSym foo)
        let sexp = parse_sexp("$foo").unwrap();
        match sexp {
            Sexp::List(children, span) => {
                assert_eq!(children.len(), 2);
                assert_eq!(children[0], Sexp::Symbol("SexpSym".to_string(), (0, 1)));
                assert_eq!(children[1], Sexp::Symbol("foo".to_string(), (1, 4)));
                assert_eq!(span, (0, 4));
            }
            other => panic!("expected List, got {:?}", other),
    ...
    #[test]
    fn parse_dollar_sym_expr() {
        // $(some-expr) → (SexpSym some-expr)
        let sexp = parse_sexp("$(name)").unwrap();
        match sexp {
            Sexp::List(children, span) => {
                assert_eq!(children.len(), 2);
                assert_eq!(children[0], Sexp::Symbol("SexpSym".to_string(), (0, 1)));
                assert_eq!(children[1], Sexp::Symbol("name".to_string(), (2, 6)));
                assert_eq!(span, (0, 7));
            }
            other => panic!("expected List, got {:?}", other),
        }
    ...
    #[test]
    fn parse_dollar_sym_nested_expr() {
        // $((str-concat "a" "b")) → (SexpSym (str-concat "a" "b"))
        let sexp = parse_sexp("$((str-concat \"a\" \"b\"))").unwrap();
        match sexp {
            Sexp::List(children, span) => {
                assert_eq!(children.len(), 2);
                assert_eq!(children[0], Sexp::Symbol("SexpSym".to_string(), (0, 1)));
                match &children[1] {
                    Sexp::List(inner, _) => {
                        ...
                        assert_eq!(
                            inner[0],
                            Sexp::Symbol("str-concat".to_string(), (3, 13))
                        );
                    }
                    other => panic!("expected inner List, got {:?}", other),
                }
    ...
    fn parse_quote_symbol() {
        ...
        match sexp {
            Sexp::List(children, (0, 4)) => {
                assert_eq!(children.len(), 2);
                assert_eq!(children[0], Sexp::Symbol("quote".to_string(), (0, 1)));
                assert_eq!(children[1], Sexp::Symbol("foo".to_string(), (1, 4)));
            }
            other => panic!("expected List, got {:?}", other),
        }
    ...
    #[test]
    fn parse_quote_integer() {
        let sexp = parse_sexp("'42").unwrap();
        match sexp {
            Sexp::List(children, _) => {
                assert_eq!(children[0], Sexp::Symbol("quote".to_string(), (0, 1)));
                assert_eq!(children[1], Sexp::Int(42, (1, 3)));
            }
            other => panic!("expected List, got {:?}", other),
        }
    }
    ...
    fn parse_quote_list() {
        ...
        match sexp {
            Sexp::List(children, _) => {
                assert_eq!(children[0], Sexp::Symbol("quote".to_string(), (0, 1)));
                match &children[1] {
                    Sexp::List(inner, _) => assert_eq!(inner.len(), 3),
                    other => panic!("expected inner List, got {:?}", other),
                }
            }
            other => panic!("expected List, got {:?}", other),
    ...
    #[test]
    fn parse_gensym_symbol() {
        let sexp = parse_sexp("x#").unwrap();
        assert_eq!(sexp, Sexp::Symbol("x#".to_string(), (0, 2)));
    }

    #[test]
    fn parse_gensym_multi_char() {
        let sexp = parse_sexp("my-var#").unwrap();
        assert_eq!(sexp, Sexp::Symbol("my-var#".to_string(), (0, 7)));
    }

    #[test]
    fn parse_percent_bare() {
        let sexp = parse_sexp("%").unwrap();
        assert_eq!(sexp, Sexp::Symbol("%".to_string(), (0, 1)));
    }

    #[test]
    fn parse_percent_numbered() {
        let sexp = parse_sexp("%1").unwrap();
        assert_eq!(sexp, Sexp::Symbol("%1".to_string(), (0, 2)));

        let sexp = parse_sexp("%9").unwrap();
        assert_eq!(sexp, Sexp::Symbol("%9".to_string(), (0, 2)));
    }

    #[test]
    fn parse_anon_fn_simple() {
        // #(+ % 1) → (fn [%1] (+ %1 1))
        let sexp = parse_sexp("#(+ % 1)").unwrap();
        match sexp {
            Sexp::List(children, _) => {
                assert_eq!(children[0], Sexp::Symbol("fn".to_string(), (0, 8)));
                match &children[1] {
                    Sexp::Bracket(params, _) => {
                        assert_eq!(params.len(), 1);
                        assert_eq!(params[0], Sexp::Symbol("%1".to_string(), (0, 8)));
                    }
                    other => panic!("expected Bracket, got {:?}", other),
                }
            }
    ...
    #[test]
    fn parse_anon_fn_two_params() {
        // #(* %1 %2) → (fn [%1 %2] (* %1 %2))
        let sexp = parse_sexp("#(* %1 %2)").unwrap();
        match sexp {
            Sexp::List(children, _) => {
                match &children[1] {
                    Sexp::Bracket(params, _) => assert_eq!(params.len(), 2),
                    other => panic!("expected Bracket, got {:?}", other),
                }
            }
            other => panic!("expected List, got {:?}", other),
    ...
    #[test]
    fn parse_anon_fn_zero_params() {
        // #(+ 1 2) → (fn [] (+ 1 2))
        let sexp = parse_sexp("#(+ 1 2)").unwrap();
        match sexp {
            Sexp::List(children, _) => {
                match &children[1] {
                    Sexp::Bracket(params, _) => assert_eq!(params.len(), 0),
                    other => panic!("expected Bracket, got {:?}", other),
                }
            }
            other => panic!("expected List, got {:?}", other),
