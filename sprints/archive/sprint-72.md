...
S72 drove `cranelisp-typecheck` to its facade target shape and RETIRED the facade
(4th data point: types S69 §7 → frontend S70 §1 → platform S71 §5 → typecheck S72 §2).

**Acceptance met**: `cargo check -p cranelisp-typecheck` + `--tests` GREEN; 346 pass / 0 fail / 0 skip.
Workspace-wide green explicitly NOT in scope (int realignment deferred to FIXMEs 0098/0187).

**Waves delivered**:
- W1 — kludge purge; FIXMEs 0222/0177/0179/0033 closed; lib+tests green.
- W2 — /review PASS-WITH-FINDINGS; ClusterRead/Write → SymbolTableRead/Mut; FIXME 0239 filed.
- W3 — Phase B: IntrinsicType activation (Int/Bool/Float/String), fqtn fallback + universe-walk
  delete, resolve_trait/resolve_type/resolve_constructor naming unification (ResolveError 5-variant);
  all 26 IntrinsicType cluster failures resolved.
- W6b — internal kludge fixes: TypeCheckEnv single constructor (OnceLock global deleted),
  pending_gap Mutex → typed in-band return (CheckState regains Sync/Freeze), ResolveError demoted
  from pub use (internal-only, projects into CheckError).
- Baseline convention — `--omit blanket-impls,auto-derived-impls` standardized across crates
  (strips Clone/Debug/From noise; KEEPS Send/Sync/Freeze semver signal).
- W5 — facade RETIRED: contract folded into lib.rs `//!` + per-item `///` (canonical surface);
  cross-surface narrative + invariants 1–10 + module-locality rationale → bounded-contexts.md §2;
  cross-refs swept; arch/CLAUDE.md exception list extended; facades/typecheck.md git-rm'd.

**FIXMEs closed/deleted**: 0173, 0222, 0177, 0179, 0033, 0240 (int remainder → 0098/0187).
**Carried forward**: backend/primitives/exe-bundle baselines need re-regen under new convention once
backend builds; FIXME 0218 (compliance-test re-anchor, /qa) left for re-anchor given workspace breakage.
