...
- **`cache.rs`**: Compiled module cache system. Manages the `.cranelisp-cache/` directory, source hashing (SHA-256), manifest read/write, `CompiledModule` serialization/deserialization, and `compile_module_to_object()` which re-compiles function definitions into a relocatable `.o` file using `cranelift-object` `ObjectModule` with PIC mode. Provides `CacheWritePacket` (owned, Send-safe snapshot) and `build_cache_packet()` for background/parallel cache writes. `extract_cache_inputs()` pre-extracts module data (fn_to_module, primitive entries, global names) so `compile_module_to_object()` doesn't need the full modules map.
- **`cache_writer.rs`**: Background cache writer for REPL responsiveness. Receives `CacheWritePacket`s via an mpsc channel, processes them on a background thread (compile .o + write files), and writes accumulated manifests on shutdown.
...
3. **Cascade reload**: When a module is successfully reloaded, all transitive dependents (modules that import from or declare the changed module) are automatically cascade-reloaded in BFS order. If a dependent fails to recompile, it is locked and cascade stops for its sub-dependents.
...
Cache writes in batch mode are deferred until all modules finish compiling (so mono specializations from later modules can land in earlier modules), then processed in parallel:

1. **Accumulate**: As each function is compiled to JIT code, the `Defn`, `Scheme`, `MethodResolutions`, and `expr_types` are accumulated into per-module vectors. Dirty modules are tracked.
2. **Build packets**: After the compile loop, `CacheWritePacket`s are built sequentially from each dirty module — this snapshots all needed data into owned, Send-safe structs.
3. **Parallel processing**: Packets are processed via `rayon::par_iter()`. Each packet independently:
   - Serializes the `CompiledModule` to `<module>.meta.json` via serde
   - Calls `compile_module_to_object()` to create a `cranelift-object` `ObjectModule` (PIC mode) and re-compiles function definitions using the same `compile_function_indirect<M: Module>()` codegen path. GOT references become `DataSymbol` entries (imported data symbols resolved at link time)
   - Writes the `.o` relocatable object file
4. **Update manifests**: After parallel processing completes, `manifest.json` is updated sequentially for each cache directory.

All cache writes are best-effort — failures produce warnings but do not abort compilation.

### Write Path (REPL mode)

REPL cache writes use a background thread for responsiveness:

1. After JIT compilation of a module, a `CacheWritePacket` is built (snapshots module data).
2. The packet is submitted to a `CacheWriter` background thread via an mpsc channel. The REPL prompt returns immediately.
3. The background thread processes packets sequentially (compile .o + write files), accumulating manifest entries.
4. On REPL exit, `CacheWriter::shutdown()` writes final manifests and joins the background thread.
...
| Variable | Purpose |
|----------|---------|
| `CRANELISP_LIB` | Override standard library search path |
| `CRANELISP_RC_TRACE=1` | Enable per-operation RC trace logging (debug builds) |
| `CRANELISP_TIMING=1` | Print startup/shutdown phase timings to stderr |
| `CRANELISP_NO_LENIENT=1` | Disable automatic sparking for parallel evaluation |
