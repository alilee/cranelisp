...
## Trait-Qualified Name Mangling

Trait impl methods are mangled as `Trait.method$Type` (e.g., `Num.+$Int`, `Display.show$Int`). This avoids collisions when different traits define the same method name (e.g., `Num.+` vs `Unchecked.+`).

The helper `ast::mangle_impl_method(trait_name, method_name, target)` generates mangled names at all creation sites. Consumption sites (codegen pattern matching, mono resolution) use the full `Trait.method$Type` format.
...
**Checked inline primitives** (Num trait, Int) — emit overflow/div-by-zero checks with panic on failure:

| Resolved Name | Cranelift IR |
|---|---|
| `Num.+$Int` | `iadd` + overflow detect (`bxor`/`band`/`ushr_imm`) + `brif` → panic or continue |
| `Num.-$Int` | `isub` + overflow detect + `brif` → panic or continue |
| `Num.*$Int` | `imul` + `smulhi`/`sshr_imm` comparison + `brif` → panic or continue |
| `Num./$Int` | `icmp` zero check + `icmp` MIN/-1 check + `sdiv` |

**Unchecked inline primitives** (Unchecked trait + raw primitives, Int):

| Resolved Name | Cranelift IR |
|---|---|
| `Unchecked.+$Int` / `add-i64` | `iadd(lhs, rhs)` |
| `Unchecked.-$Int` / `sub-i64` | `isub(lhs, rhs)` |
| `Unchecked.*$Int` / `mul-i64` | `imul(lhs, rhs)` |
| `Unchecked./$Int` / `div-i64` | `sdiv(lhs, rhs)` |

**Comparison inline primitives** (Eq/Ord traits, Int):

| Resolved Name | Cranelift IR |
|---|---|
| `Eq.=$Int` / `eq-i64` | `icmp(Equal, lhs, rhs)` + `uextend.i64` |
| `Ord.<$Int` / `lt-i64` | `icmp(SignedLessThan, lhs, rhs)` + `uextend.i64` |
| `Ord.>$Int` / `gt-i64` | `icmp(SignedGreaterThan, lhs, rhs)` + `uextend.i64` |
| `Ord.<=$Int` / `le-i64` | `icmp(SignedLessThanOrEqual, lhs, rhs)` + `uextend.i64` |
| `Ord.>=$Int` / `ge-i64` | `icmp(SignedGreaterThanOrEqual, lhs, rhs)` + `uextend.i64` |

**Inline primitives** (Float) — bitcast i64→f64, operate, bitcast f64→i64 (IEEE 754 handles edge cases):

| Resolved Name | Cranelift IR |
|---|---|
| `Num.+$Float` / `add-f64` | `bitcast` + `fadd` + `bitcast` |
| `Num.-$Float` / `sub-f64` | `bitcast` + `fsub` + `bitcast` |
| `Num.*$Float` / `mul-f64` | `bitcast` + `fmul` + `bitcast` |
| `Num./$Float` / `div-f64` | `bitcast` + `fdiv` + `bitcast` |
| `Eq.=$Float` / `eq-f64` | `bitcast` + `fcmp(Equal)` + `uextend.i64` |
| `Ord.<$Float` / `lt-f64` | `bitcast` + `fcmp(LessThan)` + `uextend.i64` |
| `Ord.>$Float` / `gt-f64` | `bitcast` + `fcmp(GreaterThan)` + `uextend.i64` |
| `Ord.<=$Float` / `le-f64` | `bitcast` + `fcmp(LessThanOrEqual)` + `uextend.i64` |
| `Ord.>=$Float` / `ge-f64` | `bitcast` + `fcmp(GreaterThanOrEqual)` + `uextend.i64` |

**User-defined trait methods** — compiled as `call` to the mangled function name (e.g., `Display.show$Int` calls `int-to-string`).

## Checked Arithmetic Detail

For `Num` trait integer operations, the codegen emits inline overflow/div-by-zero checks that call `cranelisp_panic` on failure. Each check creates a panic block and a continue block:

- **Add/Sub**: Compute result, detect sign-bit overflow via `bxor`/`band`/`ushr_imm`, branch to panic or continue.
- **Mul**: Compute result, compare `smulhi` (upper 64 bits) against `sshr_imm(result, 63)` (sign extension). Mismatch → panic.
- **Div**: Two checks — `r == 0` (div-by-zero) then `l == MIN && r == -1` (overflow). Both → panic.

The `emit_panic_with_message` helper allocates a cranelisp string `[i64 len][u8 bytes...]` and calls `cranelisp_panic`. Panic blocks terminate with `trap` (unreachable after panic exits the process).

Float operations use IEEE 754 semantics (Infinity, NaN) and need no checked variants.
