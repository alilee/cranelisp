...
pub enum DefKind {
    ...
    /// Stores resolved variants: (param_types, ret_type, mangled_name).
    Overloaded {
        variants: Vec<(Vec<crate::types::Type>, crate::types::Type, Symbol)>,
    },
}
...
pub enum ModuleEntry {
    ...
    Macro {
        name: String,
        clauses: Vec<crate::macro_expand::MacroClauseInfo>,
        docstring: Option<String>,
        visibility: Visibility,
        sexp: Option<crate::sexp::Sexp>,
...
impl CompiledModule {
    ...
    /// Insert an overloaded (multi-sig) function base name entry.
    pub fn insert_overloaded_def(
        &mut self,
        name: Symbol,
        scheme: Scheme,
        visibility: Visibility,
        ...
        self.symbols.insert(
            ...
            ModuleEntry::Def {
                ...
                docstring,
                param_names: vec![],
                kind: DefKind::Overloaded { variants },
                meta: None,
            },
        );
    }
    ...
    pub fn compiled_defns(&self) -> Vec<(&Defn, &Scheme)> {
        let mut defns: Vec<(&Defn, &Scheme)> = self.symbols
            .values()
            .filter_map(|entry| {
                if let ModuleEntry::Def {
                    kind: DefKind::UserFn { codegen, .. },
            ...
            .collect();
        // Sort by function name for deterministic compilation order (important for cache .o files)
        defns.sort_by(|(a, _), (b, _)| a.name.cmp(&b.name));
        defns
    }
...
/// An inline module declaration: `(mod name form1 form2 ...)`.
#[derive(Debug)]
                    ...
                    match head.as_str() {
                        "mod" => {
                            if children.len() >= 2 {
                                if let Sexp::Symbol(ref name, _) = children[1] {
                                    if children.len() == 2 {
                                        ...
                                        inline_mods.push(InlineModuleDecl {
                                            name: name.clone(),
                            ...
                            return Err(CranelispError::ParseError {
                                message: "(mod ...) requires a module name".to_string(),
                                offset: span.0,
                            });
                        }
                        "import" => {
...
impl ModuleGraph {
    ...
    fn discover(
        ...
        // Extract inline modules to files.
        // Inline modules are extracted to separate files and rewritten as (mod name)
        // in the parent source. They are discovered with a child→parent dependency
        // (NOT parent→child) so that super imports work: parent compiles first.
        if !decls.inline_mods.is_empty() {
            let parent_dir = file_path.parent().unwrap_or_else(|| Path::new("."));
            let parent_stem = file_path
                .file_stem()
                .and_then(|s| s.to_str())
                .unwrap_or("");

            // Rewrite parent file: replace inline (mod name ...) with (mod name)
            let mut new_source = source.clone();
            // Process in reverse span order to preserve byte offsets
            let mut sorted_inlines: Vec<&InlineModuleDecl> = decls.inline_mods.iter().collect();
            sorted_inlines.sort_by(|a, b| b.span.0.cmp(&a.span.0));
            for inline in &sorted_inlines {
                let replacement = format!("(mod {})", inline.name);
                new_source.replace_range(inline.span.0..inline.span.1, &replacement);
            }

            for inline in &decls.inline_mods {
                // Write child file: parent_dir/parent_stem/name.cl
                let child_dir = parent_dir.join(parent_stem);
                let child_file = child_dir.join(format!("{}.cl", inline.name));

                // Format body sexps as source
                let child_source = inline
                    .body
                    .iter()
                ...
                if let Err(e) = fs::create_dir_all(&child_dir) {
                    return Err(CranelispError::ModuleError {
                        message: format!(
                ...
                if let Err(e) = fs::write(&child_file, format!("{}\n", child_source)) {
                    return Err(CranelispError::ModuleError {
                        message: format!(
                ...
                // Record as mod_decl for source regeneration (not as dependency)
                decls.mod_names.push((inline.name.clone(), inline.span));
            }

            // Rewrite the parent file (replace inline forms with bare mod references)
            if let Err(e) = fs::write(file_path, &new_source) {
                return Err(CranelispError::ModuleError {
                    message: format!(
                        "cannot rewrite '{}' after inline module extraction: {}",
                        file_path.display(),
                        e
                    ),
                    file: Some(file_path.to_path_buf()),
        ...
        // Resolve `super` in imports: replace with parent module path.
        // e.g., inside "math.test", (import [super [*]]) becomes (import [math [*]])
        for import in &mut decls.imports {
            if import.module_path == "super" {
                if let Some((parent, _)) = module_id.0.rsplit_once('.') {
                    import.module_path = parent.to_string();
                } else {
                    return Err(CranelispError::ModuleError {
                        message: format!(
        ...
        for (child_name, span) in &decls.mod_names {
            ...
            // Check if child imports the parent (via super import,
            // now resolved to parent's name). If so, parent must compile
            // first — don't add parent→child dep, add child→parent instead.
            let parent_short = module_id.short_name();
            let child_imports_parent = modules
                .get(&child_id)
                .map(|info| {
                    info.imports
                        .iter()
                        .any(|imp| imp.module_path == parent_short)
                })
                .unwrap_or(false);
            if child_imports_parent {
                // Child uses super → needs parent compiled first.
                // The import discovery skipped this edge (visit_stack cycle
                // avoidance), so add it explicitly now.
                if let Some(child_info) = modules.get_mut(&child_id) {
                    if !child_info.dependencies.contains(&module_id) {
                        child_info.dependencies.push(module_id.clone());
                    }
                }
            } else {
                // No super import → normal parent→child (child first)
...
mod tests {
    ...
    fn module_graph_single_file() {
        ...
        // +2 for auto-discovered prelude + core, +10 for core submodules (dev fallback)
        assert_eq!(graph.modules.len(), 13);
        assert_eq!(graph.compile_order.len(), 13);
    ...
    fn module_graph_two_files() {
        ...
        let graph = ModuleGraph::build(&main_path).unwrap();
        // +2 for auto-discovered prelude + core, +10 for core submodules
        assert_eq!(graph.modules.len(), 14);
        assert_eq!(graph.compile_order.len(), 14);
    ...
    fn module_graph_diamond() {
        ...
        // +2 for auto-discovered prelude + core, +10 for core submodules
        assert_eq!(graph.modules.len(), 16);
