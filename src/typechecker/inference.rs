...
/// Resolve a pattern constructor name to its bare name.
/// Handles "module/Ctor", "module/Type.Ctor", and "Type.Ctor" forms.
fn resolve_pattern_ctor_name(name: &str) -> String {
    if let Some((_module, local)) = split_qualified(name) {
        if let Some((_parent, member)) = split_dotted(local) {
            return member.to_string();
        }
        return local.to_string();
    }
    if let Some((_parent, member)) = split_dotted(name) {
        return member.to_string();
    }
    name.to_string()
}
...
impl TypeChecker {
    ...
    pub(crate) fn infer_expr(&mut self, expr: &Expr) -> Result<Type, CranelispError> {
        match expr {
            ...
            Expr::Apply {
                ...
                if let Expr::Var { name, .. } = callee.as_ref() {
                    let effective = resolve_bare_name(name);
                    // For dotted calls (Trait.method), use the explicit trait name;
                    // for bare calls, use scope-aware lookup.
                    let local = match split_qualified(name) {
                        Some((_, local)) => local,
                        None => name.as_str(),
                    };
                    let trait_name_opt = if let Some((parent, _)) = split_dotted(local) {
                        self.find_trait_decl(parent).map(|d| d.name.as_str())
                    } else {
                        self.find_trait_for_method(effective)
                    };
                ...
                if let Expr::Var { name, .. } = callee.as_ref() {
                    ...
                    let is_trait_method = if split_dotted(local).is_some() {
                        true // Dotted calls are always trait methods
                    } else {
                        self.find_trait_for_method(effective).is_some()
                    };
                    if !is_trait_method {
                ...
                if let Expr::Var { name, .. } = callee.as_ref() {
                    // Use full name for qualified references (e.g. "macros/__sconcat__"),
                    // bare name for unqualified ones.
                    let lookup_name = if split_qualified(name).is_some() {
                        name.as_str()
                    } else {
                        resolve_bare_name(name)
                    };
                    if let Some(fq) = self.resolve_builtin_fq(lookup_name) {
                        self.builtin_resolutions.insert(
                            *span,
                            crate::typechecker::ResolvedCall::BuiltinFn(fq),
                        );
            ...
            Expr::ParLet {
                ...
                // Infer all binding types WITHOUT adding names to scope (enforces independence)
                let mut binding_types = Vec::new();
                for (_name, val_expr) in bindings {
                    let val_ty = self.infer_expr(val_expr)?;
                    binding_types.push(val_ty);
                }
                // Now add all names to scope
                for ((name, _), val_ty) in bindings.iter().zip(binding_types) {
                    self.local_env
                        .insert(Symbol::from(name.as_str()), Scheme::mono(val_ty));
                }
                let ty = self.infer_expr(body)?;
            ...
            Expr::ParBind {
                ...
                // Each binding must be IO T; unwrap T and add names to scope
                let mut binding_types = Vec::new();
                for (name, val_expr) in bindings {
                    let val_ty = self.infer_expr(val_expr)?;
                    // Unify with IO(fresh_var) to ensure it's an IO type
                    let inner_ty = self.fresh_var();
                    let io_ty =
                        Type::ADT("IO".to_string(), vec![inner_ty.clone()]);
                    self.unify(&val_ty, &io_ty, val_expr.span()).map_err(
                        |_| CranelispError::TypeError {
                            message: format!(
                                "par-bind! binding '{}' must be IO type",
                                name,
                            ),
                            span: val_expr.span(),
                        },
                    )?;
                    let resolved_inner = apply(&self.subst, &inner_ty);
                    binding_types.push(resolved_inner);
                }
                // Add all unwrapped types to scope
                for ((name, _), inner_ty) in bindings.iter().zip(binding_types) {
                    self.local_env
                        .insert(Symbol::from(name.as_str()), Scheme::mono(inner_ty));
                }
                // Body must also be IO
                let body_ty = self.infer_expr(body)?;
                let body_inner = self.fresh_var();
                let body_io =
                    Type::ADT("IO".to_string(), vec![body_inner.clone()]);
                self.unify(&body_ty, &body_io, *span).map_err(
                    |_| CranelispError::TypeError {
                        message: "par-bind! body must be IO type".to_string(),
                        span: *span,
                    },
                )?;
                let ty = apply(&self.subst, &body_ty);
                self.record_expr_type(expr, &ty);
                Ok(ty)
            }

            Expr::Trace { body, span, .. } => {
                // Typecheck the body (type is discarded — only used for validity)
                let _body_ty = self.infer_expr(body)?;
                // Result type is always Trace
                let ty = Type::ADT("Trace".to_string(), vec![]);
            ...
            Expr::RunTests { init, pass_fn, fail_fn, span, .. } => {
                ...
                let acc_ty = apply(&self.subst, &acc_ty);
                ...
                // Result type: acc_ty
                let ty = apply(&self.subst, &acc_ty);
                self.record_expr_type(expr, &ty);
                Ok(ty)
            }
        }
    ...
    fn resolve_dotted_var(
        ...
        if let Some(td) = self.resolve_type_def_via_modules(parent) {
            // Try constructor first, then field accessor (skip internal constructors)
            let scheme: Option<Scheme> = if td.constructors.iter().any(|c| c.name == member && !c.internal) {
                self.lookup_constructor_scheme(parent, member).cloned()
            } else if td
                .constructors
                .iter()
                .any(|c| !c.internal && c.fields.iter().any(|(f, _)| f == member))
            {
    ...
    /// Check whether a match expression covers all constructors of an ADT type.
    /// Returns an error if constructors are missing and no wildcard/var catch-all exists.
    fn check_match_exhaustiveness(
        ...
        // Wildcard or variable pattern covers all remaining cases
        for arm in arms {
            match &arm.pattern {
        ...
        // Resolve scrutinee type through substitutions
        let resolved = apply(&self.subst, scrut_ty);
        ...
        let type_def = match self.resolve_type_def_via_modules(type_name) {
            Some(td) => td,
            None => return Ok(()),
        };

        // Collect all user-visible constructor names from the type (skip internal)
        ...
        // Collect covered constructor names from match arms
        let mut covered = std::collections::HashSet::new();
        for arm in arms {
            if let Pattern::Constructor { name, .. } = &arm.pattern {
                // Resolve to bare constructor name (strip module/Type. prefix)
                let bare = resolve_pattern_ctor_name(name);
                covered.insert(bare);
            }
        }

        // Find missing constructors (sorted for deterministic error messages)
        let mut missing: Vec<&str> = all_ctors
            .iter()
        ...
        for item in &prelude_program {
            match item {
                ...
                TopLevel::TraitImpl(ti) => {
                    ...
                    // Synthesize and typecheck default methods
                    if let Some(decl) = tc.find_trait_decl(&ti.trait_name).cloned() {
                        let default_defns = crate::typechecker::synthesize_default_defns(
