...
Status: PHASE 5 CONCLUDED (2026-07-17) — I-C + 0628 RESOLVED + SCRIBED + COMMITTED (2026-07-18). Both implementation tracks CARRIED to S112 (user-directed): the 0628/I-C compiler wave (FIXMEs 0628/0639/0642) + the memory-safety-soundness infrastructure (/qa + /arch docs, 0641). Declared scope DELIVERED; 16 REDs attributed w/ owner+trigger, zero genuine regressions. Remaining spec-open: §7.3.6 sub-q1 (parked). NEXT: 0641 scope call, then Phase 6 (user-facing + cranelisp-frontend audit) → Phase 7 close.

## ★★ HEADLINE FINDING (user-directed, 2026-07-17) — memory-safety soundness is a systemic-coverage risk, not an instance backlog
The sprint's most important output. The S111 memory-safety layers (drop-glue under-key + injectivity, multi-arity 3 vectors, false-`Fresh` container/capture 0641, R2 ×4, 0637, 0638) were all found INCIDENTALLY (adversarial review / exercising the language anew), never by the suite. Root: (1) default test mode is BLIND to the failure mode (leak invisible w/o RC-trace; UAF returns plausible garbage in `--run`, deterministic only under `--link`/oracle; suite is "run→assert output"); (2) tests share the implementation's mental model (0623 matrix hand-enumerated by the same reasoning → blind to the axes it missed; the ADVERSARIAL refute-hunts are what worked); (3) the failure space is COMBINATORIAL (heap flow × container × capture × match × ADT field × IO) but coverage is a curated list. Instance-by-instance adversarial fixing is ASYMPTOTIC (each fix revealed the next composition; R2 got 4 attributions). **More layers almost certainly remain here AND elsewhere** (0637 = the same class already found on a sibling GOT slot; candidate surfaces: RC/borrow elision, drop-glue/dealloc, keyed-identity resolution, cache (de)serialization boundaries, spark-admission, macro marshalling).
- **User directive: /qa OWNS + actively MANAGES this risk** (standing strategy: enforced differential-oracle CI gate + generative/property harness + adversarial authorship + a standing audit category "safety-elision-by-static-analysis, tested-by-example") — DISPATCHED. **/arch + /design own FOUNDATIONAL INVARIANTS + assertion mechanisms** (soundness-by-construction narrowing = differential-checkability as an arch requirement; a foundational-invariant register — asserted-vs-tested-only; a candidate Principle; the /design cascade) — DISPATCHED.
- **NEW TRACK (roadmap): memory-safety soundness infrastructure.** The oracle-gate + invariant-assertion mechanism closes the CLASS; it is more important than the next instance fix.
- **0641 (+ the false-`Fresh` instance fixes) HELD behind this** — do NOT fix instance-by-instance ahead of the class-closing mechanism (user-directed).

### /qa strategy DELIVERED (`e2936895`, `tests/plan/memory-safety-coverage.md`, normative+standing)
Four legs: (1) **differential-oracle CI gate** — `assert_safety_matrix(program,prelude)` combinator over {REPL,--run,--link}×{ownership-on, `NO_OWNERSHIP`} asserting toggle-equivalence + RC-balance + `DEC_CHECK` + clean `--link`, ridden as a nextest-counted lane (`safety_oracle_lane.rs` over `fixtures/safety_corpus/*.cl`) — closes the false-`Fresh` CLASS mechanically; (2) **generative harness v1** — deterministic enumerator (~5 heap kinds × ~10 flow operators × ~3 consumptions, depth-2) that would have emitted 0641's B-1/I-1/I-2 UNPROMPTED; (3) adversarial authorship institutionalized (matrix axes from spec/design invariants NOT the diff); (4) standing audit category "safety-elision-by-static-analysis, tested-by-example" (7 candidate surfaces enumerated). **EXPOSURE QUANTIFIED (the concrete gap): oracle toggle = 0.6% of the ~4630 suite; `--link` faces = 3%; `DEC_CHECK` = ZERO standing positive assertions; the ~2670-test unit tier runs no JIT (structurally blind). ≈97% can't see a non-output-perturbing UAF; >98% can't see a leak.** First increment = 2 /testing dispatches (combinator+lane+seed corpus → then generator v1; acceptance: 0641 B-1 RED under the lane, §3.7 family GREEN, ≤60s). 0641-gate stated bindingly (§1.5 / PLAN §I.5). From S112 P3, ownership/RC plan rows carry an `[oracle]` mark.

### /arch assessment DELIVERED (`c8bd5686`, `design/arch/safety-invariants.md`, new canonical doc)
**Generalized finding:** monotone soundness protects only WIDENING; every optimization is a NARROWING (a static judgment elides a safety op) and nothing mechanically checks it. The whole S111 ledger = ONE class across 4 surfaces (ownership summaries, keyed identity, cache trust, table invariants); the fixes that CLOSED their class did so by an ASSERTION MECHANISM (CS-2 seam asserts, CS-1.2 decoder witness), the instance-patches (CS-1.1, CS-5) each needed an adversarial follow-up.
- **Soundness-by-construction (gates 0641):** conservative all-Owned lowering IS the reference semantics (elision correct iff equivalent; no reachable conservative twin ⇒ inadmissible); the ownership walk becomes lattice-monotone abstract interpretation — (a) explicit provenance ⊤ (info loss → May, never a stronger claim; B-1 is an anti-monotone rule), (b) producer P20 split `Origin::{Unconditional,Conditional}` so a hard `AliasOf`/`ProjectionOf` from a may-origin (B-2) is UNREPRESENTABLE, (c) the transfer rules enumerated in a normative classified table, `/review` rejects an unregistered rule. Discharge tiers: rule-table review + matrix pins + the standing differential oracle; non-oracle surfaces → injectivity witness / diagnosed-error-at-load.
- **★ PRINCIPLE 25 — RATIFIED (user-approved S111 Phase-7 close, 2026-07-18; single principle, split declined):** *"Narrowing carries its check: a safety elision is defined against, and checkable against, its conservative fallback."* Canonical: `design/arch/principles/25-narrowing-carries-its-check.md` (indexed in `principles.md`; imported into the four skill defs; FIXME 0643 actioned). (1) conservative-at-⊤ = reference semantics, elision w/o reachable fallback inadmissible; (2) widening free, narrowing names its justification + check-tier, green suites + adversarial review are DISCOVERY not checks; (3) a foundational safety invariant is ASSERTED at its seam, not merely tested. The enforcement arm of monotone soundness (P18 for dynamic judgments, P20 extended to analysis claims). Ratifiable as one or split checkability(1-2)+assert-at-seam(3).
- **Foundational-invariant register R1–R13** (invariant → asserted/tested-only → owner). Gaps: R1 ownership-summary-truth (THE gap), R4 keyed-injectivity (other mangle families UNAUDITED), R6 persisted-index trust (generalize 0637 to every persisted index incl. `MayAliasOf(k)`/`callees` via ONE `deserialise_meta` seam), R7 prelude-export closure (UNASSERTED, 0604), R9 differential-oracle equivalence (rotted, → standing gate), R13 fork-join ferry (unasserted, parked).
- **/design cascade (§6):** typecheck (monotone walk — gates 0641 — + 0623 axis ext + CS-5 rustdoc correction + paired backend half for B-2/I-2), backend (elision-consumer census + mangle-injectivity census), backend/cache (persisted-trust generalization), int (R7/0604 live-table asserts). 0641 FIXME updated w/ the binding gating note (land as rule-table corrections, NEVER a VecLit spot-fix — the CS-1.1→0640 lesson).
...
- **P5 design-reconciliation** (I-C + 0628, no-commit → tree dirty, /sprint batch commit pending): spec × pass-1 (I-C §5.1.2 + Form A + `param*` + §7.1.1 + echo-the-head + `:a`→`self`); spec × pass-2 (§7.3.5 kind-matching OPEN→settled) — both default (spec tier). Sequenced serially (single source-toucher). No escalations, no fallbacks.
...
### ★ I-C + 0628 RESOLVED + SCRIBED (user-settled 2026-07-18; /spec landed both passes, uncommitted)
Worked through empirically with the user; both were NORMATIVE, both settled by the user:
- **I-C — §5.1.2's "checked independently / no back-flow" is the DRIFT.** Empirical proof: un-annotated multi-sig `rp4` ERRORS ("ambiguous, not pinned §5.1.2"), but the IDENTICAL logic as two separate functions (`rp4a` calls `rp4b`) COMPILES clean — the only difference is the §5.1.2 rule artificially blocking the inference. **Ruling: a multi-sig is inference-equivalent to its clauses written as separate (mutually-recursive) functions** — a self-call to a sibling clause pins types via that clause's signature exactly like any call; annotations are DESCRIPTIVE (types from usage, §3.3 "a written type var does not add rigidity"). Drop the independence/no-back-flow clause + the "cannot be left polymorphic" restriction; keep only genuine ambiguity (§5.1.1) + dispatch-coherence rules. **CONSEQUENCE: the whole multi-arity "memory-safety" saga dissolves** (rp4 = `(Fn [Int Int] Int)`, safe; the UAF was an artifact of the drift + monomorphise-by-sibling). **CS-4.1 param-subtraction, the carried CS-4.2 reject, and the `multi_arity_clause_param_51_2` repros asserting rejection are SUPERSEDED — to be UNWOUND against the corrected §5.1.2, not landed.**
- **0628 — kind-by-usage + echo-the-head impl form + return-type dispatch ALLOWED.** `(deftrait (X a))` head ⇒ higher-kinded (con-var must be APPLIED `(f a)`); never-applied `(X a)` is malformed — use the conventional `self`-based form (**Form A**: `(deftrait Zeroable (zed [] self))`, NOT `(deftrait (Zeroable a) …)`). Impl form: **slot-1 AS DECLARED** — `(impl (Functor f) (Functor Option) …)` for HK (binder echoes the deftrait head), `(impl Display (Option a) …)` for conventional. **CORRECTED from the first framing: Zeroable is ADMITTED, not rejected** — `param+`→`param*` (zero-param methods legal), §7.1.1 rule = implementing type in **parameter OR return** position; the 0628 codegen leak on the resolved `(:Int (zed))→0` case is a BUG TO FIX (backend), not a language rejection (§3.3.3 MUST(d)/(e) stand). Inline constraints `(impl (Display (Option :Display a)) …)`. **Kind-matching table (§7.3.5, settled — 3 cases):** (1) conventional target must be kind `*`, bare/under-applied constructor `(impl Display Option)` is the sole reject (kind-mismatch, names the fix); (2) HK slot-2 constructor must be bare + matching arity — primitive→"not a type constructor", fully-applied `(Functor (Option Int))`→kind-mismatch, wrong-arity→arity-mismatch; (3) cross-check is ONE deterministic path — resolve trait by name, slot-1 echoes the declared head, slot-2 interpreted strictly per the trait's declared kind; the two forms never collide for one trait; NO redundant slot-2-head classifier. Worked: fmap for `(Functor Option)`, show for `(Display (Option a))` + `(Display Int)`, zed for `Zeroable`.
- **/spec LANDED both passes** (uncommitted, tree dirty): pass 1 (I-C §5.1.2 + Form A + `param*` + §7.1.1 any-position + echo-the-head syntax + `:a`→`self` fix), pass 2 (§7.3.5 kind-matching table OPEN→settled, 3 cases; §7.3.6 sub-q2 folded in). **STILL OPEN: §7.3.6 sub-question 1** — is "method-level type variables only" the complete answer for a non-`self` type parameter in a conventional trait? NOT ratified this pass; parked (candidate next-sprint or a quick user confirm).
- **CARRIED to S112 (user-directed 2026-07-18): the 0628/I-C compiler wave** — (a) §5.1.2 back-flow fix `cranelisp-typecheck` + UNWIND CS-4.1/carried-CS-4.2/`multi_arity_clause_param_51_2` (rp4 → accepting test) → **FIXME 0642 (NEW, target /design)**; (b) new trait/impl form `cranelisp-typecheck`+registry (kind-by-usage register, echo-the-head impl, kind-matching table §7.3.5, `register_trait_decl` guard fix, migrate ~7 e2e + ~24 unit fixtures off the `(X a)`-head mismodel to `self`-form) → **FIXMEs 0628 + 0639 (open, target /design; their "needs arbitration" question is now RESOLVED by the settled spec — /design actions against §7.1.1/§7.2/§7.3.4-6 and deletes)**; (c) 0628 codegen fix `cranelisp-backend` (return-type-dispatch leak). Coordinated cross-skill wave per FIXME 0639 landing path — NOT a /dev-only carry.
...
## Outcome (Phase 7) — authored 2026-07-18, awaiting user close approval

### Delivered
- **CENTREPIECE — vec-assoc COW ownership root (CS-5, `e99535e4`).** `ResultMode::MayAliasOf(usize)` + schema 19→20 + primitives truthful COW facts (vec-set/vec-push) + typecheck 5-sites/1-helper + consumer join + 0621 rider. **`vec_assoc_param_mutate_return_uaf` 17/17 GREEN; baseline 17→8.** No golden frame drifted; differential all-Owned oracle behaviourally equivalent (sound direction).
- **Backend audit-drain (s107 + s110, CS-1/CS-2) — Phase-1 must-ship subset shipped:** dup `build_isa`, dispatch funnels, drop-glue keying, GOT exhaustion. No 3rd deferral (past 2× threshold).
- **Drop-glue keying class CLOSED (CS-1.1 / CS-1.2)** — mangle under-key + injectivity, via an assertion mechanism (decoder witness), not an instance patch.
- **Quasiquote-legal-everywhere (0613, CS-3)** — ruled (A); frontend `expand_quasiquotes` fold + int-side quote shield; typecheck/backend see no quasiquote.
- **0614 stdlib fix; golden lane rescued + gated (CS-0.5).**
- **NORMATIVE (design deliverable): I-C + 0628 resolved and scribed (`c9f05b64`)** — §5.1.2 multi-sig back-flow correction, kind-by-usage traits, echo-the-head impl, §7.3.5 kind-matching table, return-type dispatch admitted. Two `/spec` passes, user-ratified across a slowed-down design conversation.
- **★ HEADLINE — memory-safety-soundness recognised as a systemic-coverage risk** (not an instance backlog): `/qa` strategy (`tests/plan/memory-safety-coverage.md`, `e2936895` — differential-oracle CI gate + generative harness + standing audit category; exposure quantified ≈97% blind to a non-output-perturbing UAF) + `/arch` invariant register (`design/arch/safety-invariants.md`, `c8bd5686` — soundness-by-construction narrowing, R1–R13, **Principle 25 — RATIFIED** at this close, canonical `design/arch/principles/25-narrowing-carries-its-check.md`).

### Deferred (with rationale)
- **The 0628/I-C compiler wave → S112** (FIXMEs 0628 + 0639 trait/impl form; **0642** new, §5.1.2 back-flow + unwind superseded repros). Spec settled this sprint; implementation is a clean, sizable typecheck+registry+backend wave (incl. ~7 e2e + ~24 unit-fixture migrations off the `(X a)`-head mismodel). User-directed carry (2026-07-18).
- **Memory-safety-soundness infrastructure → S112** (NEW TRACK). The class-closing mechanism (oracle gate + monotone-walk + invariant assertions) is more important than the next instance fix; **0641 held behind it** (do not fix instance-by-instance ahead of the mechanism — user-directed). Carried with `/qa` + `/arch` docs + 0641 as carriers.
- **Phase 6 SKIPPED (user-directed 2026-07-18)** — no user-facing assessment, no `cranelisp-frontend` audit rotation this sprint. **Audit-rotation debt persists** (frontend still longest-since-assessment, s87) — re-target S112.
- **Attribution carries (deterministic repros / FIXMEs guard them):** CS-6/0604 (seam not locatable — observability follow-up bundled), R2 entry-`main` IO-box teardown leak, 0638 macro-clause double-free, I-3 renamed-import (pre-existing since S9 — §8.3.5 never implemented), 0553 SET-capture.
- **Spec-open: §7.3.6 sub-q1** (non-`self` type parameter → "method-level type variables only"?) — parked; quick user confirm or S112.

### Findings (Phase-7 checks)
- **Frontmatter-vs-table audit: CLEAN.** All 14 `.claude/agents/*.md` + 14 `.claude/commands/*.md` `model:`/`effort:` cells match `artefacts.md` §II.3 exactly. No drift.
- **Dispatch calibration: zero model-tier escalations, zero named fallbacks all sprint** — despite genuinely hard spots (memory-safety soundness, adversarial centrepiece review). The hard judgment was absorbed by the already-top-tier fable roles (`/review`, `/arch`, `/qa`) and direct user design conversation; no per-dispatch upgrade was warranted. Allocation table served the sprint correctly.
- **No `/audit` dispatch** (Phase 6 skipped) — the audit-calibration check (recommendations dying at Phase-1 acceptance) does not apply this sprint.
- **Cross-cutting finding — "the corpus models the wrong thing" recurs in the trait grammar.** The `(X a)`-head parametric-trait mismodel (7 green e2e + ~24 unit fixtures) and the §5.1.2 no-back-flow drift both looked authoritative *because the test/FIXME corpus encoded them* — the same failure shape as the memory-safety headline (tests share the implementation's mental model), now seen in type-system syntax. Reinforces the standing `/qa` "tested-by-example is not asserted" category beyond memory safety.
- **Principle 25 "Narrowing carries its check" — RATIFIED** (user-approved S111 Phase-7 close, 2026-07-18, as a single principle; split declined). Canonical `design/arch/principles/25-narrowing-carries-its-check.md`, indexed in `principles.md`, imported into the four skill defs; binding frame for the S112 memory-safety-soundness mechanism. FIXME 0643 (import propagation) actioned + deleted.
