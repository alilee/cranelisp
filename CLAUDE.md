...
- **Failing-not-ignored defect repros (S101 close)**: the canonical `cargo nextest run --no-fail-fast` carries **22 intentional failing tests** — known-defect guards, not regressions, un-ignored per `memory/feedback_failing_not_ignored.md` so they flip green when the owning skill fixes each defect. Itemized: **3×** `tests/vec_cow_value_use_leak.rs` (0474 COW copy-branch RC leak; owner `/backend`, rides increment I's `fn_as_value` seam rework) + **19** S101 Phase-6b guards across 7 files for the 6a/6b defect set: 0483 vec-op-as-value SIGBUS at ≥2 instantiations (×3), 0484 import-shadow order dependence (×1), 0486 bare-lookup introspection corruption (×2), 0488 generic-fn missing monomorphisation (×3), 0489 restart lockout on broken backing file (×1), 0491 `__expr` cascade-report leak (×2), trap-message §18.5 format (×1), 0492 `/sig` ≠ bare-lookup (×1), 0493 nested parameterized-ADT display (×2), D1 def-poisons-directory persist (×1), D2 backing-file authorship fidelity (×1), D3 file-backed redefinition after cache restore (×1). Durable record + owners: `tests/plan/ledger.md` §"Sprint 101 Phase 6a/6b defect set". (The S100 `vec_query_value_use` guards — 7 after the cat-3 widening — flipped green when S101 fixed the NULL-slot defect.) A genuine regression is any RED **beyond** these named guards. Verified at S101 close: **3480 run — 3458 passed / 22 failed / 1 skipped** (two consecutive runs, identical fail sets).
