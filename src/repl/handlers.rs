...
impl ReplSession {
    ...
    pub fn handle_command(&mut self, cmd: ReplCommand<'_>, _full_input: &str) -> bool {
        match cmd {
            ...
            ReplCommand::Help => {
                ...
                println!("  /run-tests [prefix]  Discover and run test-* functions in .test modules");
            ...
            ReplCommand::RunTests(arg) => self.cmd_run_tests(arg),
        }
        false
    }
        ...
        match &info {
            ...
            SymbolInfo::Macro {
                ...
                if clauses.len() == 1 {
                    ...
                    for p in &clause.fixed_params {
                        parts.push(format!(":Sexp {}", p.display_name()));
                    }
            ...
            ReplInput::Expr(expr) => {
                ...
                match self.jit.compile_expr(&expr, &method_resolutions, &et, &fn_slots, &self.tc.modules) {
                    Ok(eval_fn) => {
                        ...
                        if matches!(&resolved, crate::types::Type::ADT(name, _) if name == "IO") {
                            let inner_val =
                                unsafe { crate::intrinsics::IoTask::from_raw(result) }.run();
                            let inner_ty = match &resolved {
                                crate::types::Type::ADT(_, args) if !args.is_empty() => {
                                    args[0].clone()
                                }
                                _ => resolved.clone(),
                            };
                            let value_str =
                                format_result_value(inner_val, &inner_ty, &self.tc);
                            println!("{} :: {}", value_str, formatted_type);
                        } else {
                            let value_str =
                                format_result_value(result, &resolved, &self.tc);
                            println!("{} :: {}", value_str, formatted_type);
                        }
                        println!(
                            "  parse: {:.2}ms  typecheck: {:.2}ms  compile: {:.2}ms  exec: {:.2}ms",
    ...
    fn cmd_mod(&mut self, arg: &str) {
        ...
        let target = if arg.is_empty() {
            ...
            // loaded_modules keys are full paths (e.g. "test-demo.test"), so
            // also check by short_name to handle submodules already compiled.
            let already_loaded = self.loaded_modules.contains_key(&target)
                || self.loaded_modules.keys().any(|id| id.short_name() == arg);
            if !already_loaded {
    ...
    fn cmd_exe(&mut self, arg: &str) {
        ...
        let main_returns_io = crate::exe::main_returns_io(&self.tc.modules);
        let startup_bytes =
            match crate::exe::generate_startup_object(&platform_manifests, main_returns_io) {
            Ok(b) => b,
            Err(e) => {
    ...
    fn cmd_run_tests(&mut self, prefix: &str) {
        use std::time::Instant;

        let mut test_modules: Vec<(String, Vec<(String, *const u8)>)> = Vec::new();

        // Discover .test modules matching prefix.
        // loaded_modules has full paths (e.g., "user.test"), tc.modules has
        // short names (e.g., "test"). We iterate loaded_modules for discovery
        // and look up in tc.modules using the short name.
        let test_mod_ids: Vec<_> = self
            .loaded_modules
            .keys()
            .filter(|id| {
                let path_str = id.0.as_str();
        ...
        for full_path in &test_mod_ids {
            let path_str = full_path.0.as_str();
            let short = full_path.short_name();
            let mod_path = crate::names::ModuleFullPath::from(short);
            let cm = match self.tc.modules.get(&mod_path) {
                Some(cm) => cm,
                None => continue,
            };

            // Find test-* functions that are zero-arg and return (Option String)
            let mut tests = Vec::new();
            for (sym_name, entry) in &cm.symbols {
                let name = sym_name.as_ref();
                if !name.starts_with("test-") {
                    continue;
                }
                if let crate::module::ModuleEntry::Def {
                    scheme,
                    kind: crate::module::DefKind::UserFn { codegen, .. },
                    ..
                } = entry
                {
                    // Check it's a zero-arg function returning (Option String)
                    let is_test_fn = match &scheme.ty {
                        Type::Fn(params, ret) => {
                            params.is_empty()
                                && matches!(ret.as_ref(),
                    ...
                    if let Some(ptr) = codegen.code_ptr {
                        if !ptr.is_null() {
            ...
            if !tests.is_empty() {
                ...
                test_modules.push((path_str.to_string(), tests));
            }
        }

        if test_modules.is_empty() {
            if prefix.is_empty() {
                println!("No .test modules found.");
            } else {
                println!("No .test modules found matching prefix '{}'.", prefix);
            }
            return;
        }

        test_modules.sort_by(|a, b| a.0.cmp(&b.0));

        let mut total_passed = 0usize;
        let mut total_failed = 0usize;
        let start = Instant::now();

        for (mod_path, tests) in &test_modules {
            println!("Running tests in {}...", mod_path);
            for (test_name, code_ptr) in tests {
                let test_fn =
                    unsafe { std::mem::transmute::<*const u8, extern "C" fn() -> i64>(*code_ptr) };
                let result = test_fn();

                // Interpret result: Option String
                // None = tag 0 (bare i64 value 0) = pass
                // Some = heap pointer with tag 1 at [0], string at [1] = fail
                if result == 0 {
                    // None → pass
                    println!("  {} {} ok", test_name, ".".repeat(40_usize.saturating_sub(test_name.len())));
                    total_passed += 1;
                } else {
                    // Some → fail; extract string from heap
                    let reason = unsafe {
                        let string_ptr = *((result as *const i64).add(1));
                        super::util::read_cranelisp_string(string_ptr)
                    };
                    println!(
                        "  {} {} FAILED: {}",
                        test_name,
                        ".".repeat(40_usize.saturating_sub(test_name.len())),
                        reason
                    );
                    total_failed += 1;
                }
