...
1. **Expand** the form. If the form is a macro call:
   ...
   - If not: walk the macro's call graph and call `block_for_macro_codegen` on the scheduler. The worker releases this module and returns to the priority ladder for new work. When dependencies are compiled, the module unblocks and the worker resumes from this form.

   The call graph is derived from the macro's typechecked body. When a defmacro is registered (step 5), the TypeChecker records the macro's clause ASTs. When the macro is first used (this step), the worker typechecks the macro body (producing method resolutions and resolved calls), then walks the resolved calls to collect the transitive closure of function dependencies. This walk uses the TypeChecker's module tables — each resolved call names a symbol in a module, and that symbol's own resolved calls are already available (it was typechecked before the macro, per spec §9.2.5). The result is the `needed: Vec<(ModuleFullPath, Symbol)>` passed to `block_for_macro_codegen`.
