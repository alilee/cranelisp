...
| 1d | **Allocator swap** — the (a) cure | /qa (benchmark; mimalloc already built 0.2) | `--features thread-caching-alloc` | **in-progress — mimalloc first** (user 2026-07-02): benchmark the already-built C-mimalloc on F1–F4 vs system alloc, lead with the **sys-time (a) signal** (immune to F4 search variance), + the **combined shippable stack** (mimalloc build + saturation-gate env). Native candidates (Ferroc/rimalloc) evaluated only if mimalloc's win justifies chasing a dependency-free version. Adoption posture (opt-in vs default-on) recommended from the number. |
