...
- **Definition over a name in scope (via import or export)** `[S102]`: **It is ALWAYS a compile-time error to redefine or shadow a name that is in scope via `import` (private) or `export` (public).** A definition (`defn`, `deftype`, etc.) in the current module that has the same name as a name already brought into the module's inner scope — whether brought in **private via `import`** or **public via `export`** (§8.4.0) — is unconditionally rejected. The error does not depend on textual order (def-before-import or import-before-def), on import shape (specific, renamed, member, glob, or glob re-export), or on visibility (private import or public export):
...
**Contrast — prelude-provided names remain shadowable.** The always-error rule above ranges over names brought into the inner scope by an explicit `import` or `export` in *this* module. An implicit-prelude name is **not** brought in by any such form — it lives in the outer scope (§8.8.1) and is never materialised into the module's inner table — so it falls entirely outside the rule's scope. A module-local definition over a name that reaches the module only through the implicit-prelude fallback (the outer scope, §8.8.1) is therefore NOT a conflict: the inner-scope definition shadows the outer scope structurally, per §"Explicit Imports Shadow the Implicit Prelude" above. This is not an exception to the always-error rule — an implicit-prelude name was never in the rule's domain. Redefining `count` is permitted when `count` arrives via the prelude; it is an error when the module *explicitly imported* (or exported) `count`. The distinction is intent: an explicit import is the author's statement "this module's `count` is `collections.vec/count`", which a same-name local definition contradicts rather than refines — whereas prelude names are ambient convenience the author never asked for by name. (Clojure draws the same line: defining over a `:refer`ed name is an error; defining over a `clojure.core`-provided name — the prelude analogue — is a permitted shadow.)
