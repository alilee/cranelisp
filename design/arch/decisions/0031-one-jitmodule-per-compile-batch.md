...
## Amendment (Sprint 66 — fn_ptr unification + rollback, 2026-05-09)

Scenario 2's lifecycle reclaim semantics are unchanged. The S66 work briefly relocated the per-entry ptr from inside `Code` variants to a sibling `fn_ptr: Option<*const u8>` field on `ModuleEntry::Def` (commit `b09ec76`); same day, that placement was rolled back (`1dc57ae`) once /arch identified that the per-module `GotTable` already carried the same address. Post-rollback the call address lives at `symbol_table.got().load_slot(slot)`, indexed by `ModuleEntry::Def.got_slot`. `Code::Jit(Arc<Jit>)` continues to be the reclaim primitive: `Arc<Jit>` refcount → 0 → custom `Drop` → `unsafe free_memory()`. The GOT slot's stored ptr becomes invalid the same instant the JIT pages are freed — same lifecycle semantics as either of the considered field placements. The "`Arc<Jit>` lives directly on `ModuleEntry::Def.code`" wording in the table above remains correct; only the call-address storage relocated, and it now lives in the GOT (the single source of truth) rather than on a sibling field. See Decision 41's "S66 amendment + rollback" for the canonical post-rollback statement.
