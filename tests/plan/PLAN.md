...
| §12.3.1 composed application residual | `exemplar_ownership_residue_s116::sudoku_warm_serial_solve_residue_at_most_1400` | live defect threshold; `[S116]` authored RED; re-attributed to FIXME 0917 at S118 P6 close (`s118-test-plan.md` §11.8.1) |
...
| spec §12.3.1, a match arm returning a nullary ctor beside a boxed arm must not strand the caller's tree (FIXME 0917 — unbalanced guarded protect inc at `protect_return_value`, backend) | P6-batch marginal pair (subject/control differing only in the arms' returned ctors) × {`--run --no-cache`, `--link`} | `[S118]` authored RED in the P6 close batch; flips at the S119 `/design`(backend) 0917 ruling; attribution `s118-test-plan.md` §11.8.1 |
