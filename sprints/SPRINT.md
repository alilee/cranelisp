...
**Status**: PHASE 5 LANGUAGE (ACTIVE) — Stage 2 Wave 3a (/dev backend II-B1 R5 flattening)
...
| 3a | /dev → /review | `cranelisp-backend` (+ co-land `cranelisp-typecheck` 1-line + `cranelisp-types` for 0528) | **II-B1 R5 flattening**: `HeapCategory::Value` arm (bare-word move, no RC, drop-glue skip, vec null-elem-fn path) consuming `value_layout`; **co-land** the typecheck tables-flip `None`→`Some(env.modules())` (`fixpoint.rs:219`) in the SAME commit; **0528** mandatory ctor-resolution unify (II-B1 makes backend a new `value_layout` consumer). Flips `r5_value_flatten…` (II-G1). | scan `target:/backend`; co-land verified (no flip-without-flatten); differential golden EMPTY; behavioral (no web-poll-class UAF) |
| 3b | /dev → /review | `cranelisp-backend` | **II-B2 reuse tokens** (drop-guided, off-ABI, per-call entry check, static-proof elision reading `unique_static` off the **fresh-producing node**; `reuse_hit/miss` runtime tallies) + **h3/L-D5** per-extern RC_STATS emission + COW primitive `result_unique=true` bit for the II-G2 fusion witness. Flips the 4 reuse/chaining REDs + h3. **★ CLOSE-SHORT SEAM after II-B2**; II-B3 deferred rider. | scan `target:/backend`; **route 0474 + 0483 stale-cured deletions to /backend here**; L-C3 fence green incl. proof-elided reuse (no dynamic backstop — behavioral verify); differential golden EMPTY |
| 3c | /dev → /review | `cranelisp-backend` | **0495** backend `tests.rs` split + thin-submodule drain (mechanical; applies §2.2 to the new mechanism scenario matrices). **0526** close (§3.3 re-frame already authored → /arch deletes; II-B3 mechanism stays deferred). | scan `target:/backend`; suite green |
