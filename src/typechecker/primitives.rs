...
use super::{ConstructorInfo, PrimitiveKind, SymbolMeta, TypeChecker};
...
impl TypeChecker {
    ...
    pub(crate) fn register_primitives(&mut self) {
        ...
        self.register_builtin_with_jit_name(
            ...
            SymbolMeta::Primitive {
                kind: PrimitiveKind::Extern,
                docstring: "Test string equality".into(),
                param_names: vec!["a".into(), "b".into()],
            },
            "str-eq",
        );
    ...
    /// bind :: forall a b. (IO a) -> (Fn [a] (IO b)) -> (IO b)
    /// Constructs Bind nodes (tag=2) — the trampoline interprets them.
    fn register_bind_primitive(&mut self) {
        let a_id = self.next_id;
        self.next_id += 1;
        let b_id = self.next_id;
        self.next_id += 1;

        let io_a = Type::ADT("IO".to_string(), vec![Type::Var(a_id)]);
        let io_b = Type::ADT("IO".to_string(), vec![Type::Var(b_id)]);
        ...
        self.register_builtin(
            ...
            SymbolMeta::Primitive {
                kind: PrimitiveKind::Inline,
                docstring: "Chain IO actions, passing result of first to second".to_string(),
                param_names: vec!["io".into(), "cont".into()],
            },
        );
    }
    ...
    fn register_macro_types(&mut self) {
        ...
        // Register __sconcat__: (SList Sexp, SList Sexp) -> SList Sexp
        // Internal primitive used by quasiquote expansion for ~@ splicing.
        // Named with underscores to avoid conflict with the user-facing sconcat in core/syntax.
        ...
        self.register_builtin_with_jit_name(
            ...
            SymbolMeta::Primitive {
                kind: PrimitiveKind::Extern,
                docstring: "Concatenate two SLists (internal)".into(),
                param_names: vec!["xs".into(), "ys".into()],
            },
            "sconcat",
        );
    ...
    /// IO is a deferred computation tree: Pure (completed value), Effect (thunk),
    /// and Bind (internal — chain of actions with existential intermediate type).
    fn register_io_type(&mut self) {
        ...
        let io_def = TopLevel::TypeDef {
            ...
            constructors: vec![
                ConstructorDef {
                    // tag=0: completed value
                    name: "Pure".to_string(),
                    docstring: Some("A completed IO value".to_string()),
                    fields: vec![FieldDef {
                ...
                ConstructorDef {
                    // tag=1: deferred effect (opaque thunk pointer)
                    name: "Effect".to_string(),
                    docstring: Some("A deferred effect (opaque thunk)".to_string()),
                    fields: vec![FieldDef {
                        name: "thunk".to_string(),
                        // Approximate type for display — real type is opaque Box<FnOnce()->i64>
                        type_expr: TypeExpr::TypeVar("a".to_string()),
                    }],
                    span: (0, 0),
                },
        ...
        // Add Bind as internal constructor (tag=2).
        // Bind has existential type: exists b. (IO b) * (b -> IO a)
        // which HM can't express. We add it to TypeDefInfo for REPL documentation
        // but mark it internal so users can't construct or match on it.
        self.add_internal_bind_constructor();

        // Add Par as internal constructor (tag=3).
        // Par holds N IO tasks to execute concurrently.
        self.add_internal_par_constructor();

        self.current_module_path = saved_module;
    }

    /// Register Trace type as a compiler-seeded ADT in the `primitives` module.
    /// Trace records execution trees produced by the `(trace ...)` special form.
    /// Layout: TraceCall [:String tname
    ///                    :(SList String) tparams
    ///                    :String tresult
    ///                    :(SList Trace) tchildren
    ///                    :Int tnanos]
    ...
    fn register_trace_type(&mut self) {
        ...
        let trace_def = TopLevel::TypeDef {
            name: "Trace".to_string(),
            docstring: Some("Execution trace tree produced by (trace ...) special form".to_string()),
            type_params: vec![],
            constructors: vec![ConstructorDef {
                // tag=0: a recorded function call
                name: "TraceCall".to_string(),
                docstring: Some("A recorded function call with its children and timing".to_string()),
                fields: vec![
                    FieldDef {
                    ...
                    FieldDef {
                        name: "tparams".to_string(),
                        // macros/SList String — formatted parameter strings
                        type_expr: TypeExpr::Applied(
                    ...
                    FieldDef {
                        name: "tresult".to_string(),
                        type_expr: TypeExpr::Named("String".to_string()),
                    },
                    FieldDef {
                        name: "tchildren".to_string(),
                        // macros/SList Trace — children are an SList of Trace values.
                        // Use qualified name so resolve_type_def_via_modules can find SList
                        // in the macros module (SList is not auto-imported into primitives).
                        type_expr: TypeExpr::Applied(
                            "macros/SList".to_string(),
                            vec![TypeExpr::Named("Trace".to_string())],
                        ),
                    },
                    FieldDef {
                        name: "tnanos".to_string(),
                        type_expr: TypeExpr::Named("Int".to_string()),
                    },
                ],
                span: (0, 0),
        ...
        self.register_type_def(&trace_def);

        self.current_module_path = saved_module;
    }

    /// Add the Bind constructor to the IO type's TypeDefInfo as an internal constructor.
    /// Bind chains IO actions: Bind(inner: IO b, cont: b -> IO a) where b is existential.
    fn add_internal_bind_constructor(&mut self) {
        ...
        if let crate::module::ModuleEntry::TypeDef { info, .. } = entry {
            // Get IO's 'a' type param
            let a_id = info.type_params[0];

            // Fresh type var for existential 'b'
            let b_id = self.next_id;
            self.next_id += 1;

            // Bind fields: inner :: (IO b), cont :: (Fn [b] (IO a))
            let io_b = Type::ADT("IO".to_string(), vec![Type::Var(b_id)]);
            let io_a = Type::ADT("IO".to_string(), vec![Type::Var(a_id)]);
            ...
            info.constructors.push(ConstructorInfo {
                ...
                tag: 2,
                fields: vec![
                    ("inner".to_string(), io_b),
                    ("cont".to_string(), cont_ty),
                ],
                docstring: Some(
    ...
    /// Add the Par constructor to the IO type's TypeDefInfo as an internal constructor.
    /// Par holds N IO tasks to execute concurrently, constructed by the auto-scheduler.
    fn add_internal_par_constructor(&mut self) {
        let mod_path = ModuleFullPath::from("primitives");
        let cm = self
            .modules
            .get_mut(&mod_path)
        ...
        if let crate::module::ModuleEntry::TypeDef { info, .. } = entry {
            ...
            // Par fields: count :: Int, ios :: opaque (variable-length array of IO tasks)
            let io_a = Type::ADT("IO".to_string(), vec![Type::Var(a_id)]);
            ...
            info.constructors.push(ConstructorInfo {
                ...
                docstring: Some(
                    "Concurrent IO tasks (internal — constructed by the auto-scheduler)".to_string(),
                ),
                internal: true,
            });
        }
    ...
    fn register_special_forms(&mut self) {
        let forms: &[(&str, &str, &str)] = &[
            ...
            ("quote", "quote :: special form: (quote expr) or 'expr — produce Sexp value", "Quote an expression to produce its Sexp representation at runtime. Shorthand: 'expr"),
            ("par-let", "par-let :: special form: (par-let [name val ...] body) — parallel let bindings", "Evaluate bindings in parallel using rayon thread pool, then evaluate body with all bindings in scope. Requires at least 2 bindings."),
            ("trace", "trace :: special form: (trace expr) or (trace [mod1 mod2] expr) — record execution trace", "Evaluate expr while recording function call boundaries. Returns a Trace ADT representing the call tree. Lenient evaluation is disabled inside the trace body to ensure complete traces."),
            ("run-tests", "run-tests :: special form: (run-tests init pass-fn fail-fn) — fold over test results with trace", "REPL-only. Discovers all test-* functions in loaded .test modules, runs each with full GOT-swap tracing, and folds results via pass-fn (acc tname nanos) and fail-fn (acc tname nanos reason trace). Returns the final accumulator value."),
        ];
    ...
    pub fn register_platform(
        ...
        for desc in descriptors {
            ...
            // Record scheduling class for the auto-scheduling pass.
    ...
    /// Return the scheduling class for a platform function by its bare name.
    /// Returns `Sequential` (the safe default) for unknown names.
    pub fn scheduling_of(&self, name: &str) -> crate::platform::SchedulingClass {
        self.platform_scheduling
            .get(name)
            .copied()
            .unwrap_or(crate::platform::SchedulingClass::Sequential)
    }
}
...
mod platform_tests {
    use super::*;
    use crate::platform::{OwnedPlatformFnDescriptor, SchedulingClass};
