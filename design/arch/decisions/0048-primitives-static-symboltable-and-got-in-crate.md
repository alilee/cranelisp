...
- `cranelisp-primitives`' published Rust API collapses to one item (`PRIMITIVES_TABLE`); the ~22 individual `pub extern "C" fn` items demote to `pub(crate)`. **DCE-prevention amendment (S68 Wave 4 as-built, FIXME 0212 — Option 2):** the as-built discipline is NOT `#[used]` on the per-fn items (Wave 4 added `#[unsafe(export_name = "…")]` but no `#[used]`). DCE is prevented by the in-crate `extern_shims()` harvest, which takes every extern fn's address into a static `HashMap` at `PRIMITIVES_TABLE` static-init — a static-data reference that keeps each fn live. Canonical mechanism: `crates/cranelisp-primitives/src/lib.rs` crate-root rustdoc §"force-link discipline" + BC §4a (the "Option-2 DCE-survival" wording). The earlier "`#[used]` discipline" prescription is superseded by the `extern_shims()` mechanism.
