...
fn parse_filter_from_env_unset_is_none() {
    ...
    unsafe {
        std::env::remove_var(scheduler_trace_env_var());
    }
    let parsed = parse_filter_from_env();
    if let Some(v) = prev {
        unsafe {
            std::env::set_var(scheduler_trace_env_var(), v);
        }
    }
...
fn parse_filter_from_env_one_is_all() {
    let prev = std::env::var_os(scheduler_trace_env_var());
    unsafe {
        std::env::set_var(scheduler_trace_env_var(), "1");
    }
    let parsed = parse_filter_from_env();
    match prev {
...
fn s61w3_new_tags_record_via_module_event() {
    ...
    assert!(matches!(
        dumped[0].tag,
        SchedulerTraceTag::RepublishFromSymbolTable
    ));
    assert!(matches!(
        dumped[1].tag,
        SchedulerTraceTag::RegisterImportsLookup
    ));
}
