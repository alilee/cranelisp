...
| 0494 | → **/dev(intrinsics)** teardown free-ownership | open | **A** — THREE gate-stops, no wasted fix. (1) `e46b44d`: vec-RC double-dec **REFUTED** (RC balanced). (2) `f2e227a` ASAN: workload runs CLEAN — but `0d209df` CLIF then **refuted** the "JIT vec-COW store" that ASAN-by-elimination implied (vec_codegen.rs reloads data-ptr after every realloc; provably no stale/OOB store) **and showed ASAN-clean was PERTURBATION not proof** (redzones/quarantine closed the race window, like `MALLOC_CHECK_=3` did) — so the 2 "ruled-out" Rust candidates are back in play. **Refined residual = a free-timing UAF/double-free of UNTRACKED heap** (a Vec data buffer or String byte buffer — not RC-tracked) on the **reactor launched-strand teardown after `send-conn`** (`cranelisp-intrinsics` reactor/drop/marshaling). Next tool = a **non-layout-perturbing DEF-6 heap-header-integrity `debug_assert!`** at `free_data_buffer`/`vec_drop`/`dealloc` (ASAN/MALLOC/valgrind all perturb this class away). |
