...
Under Phase 3 these `classify` calls take a `&ConcreteType` obtained through the conversion choke point — `Ok` yields the exact category; `Err` (a residual signature var, template path only) classifies `Mixed` per the amended rule above. `classify` is total over `ConcreteType` on every call, from either source.

> **Note for /dev(backend):** the `extract_constructor`/`CtorField` and the param-binding `variable_types` currently carry `Type` (with a `Var` variant). The minimal Phase-3 move is to convert *at the `classify` call site* (the `signature_heap_category` funnel — `Ok` → exact category, `Err` → `Mixed`), not to retype `CtorField`/`variable_types` to `ConcreteType` wholesale (that wider retype is optional cleanup, out of the FIXME-0391 minimum). *(S118 amendment: the original text here prescribed a `.expect(…§3.11.1…)`; superseded by the template-path `Mixed` classification above — the as-built `signature_heap_category` shape is the ruled one.)*
