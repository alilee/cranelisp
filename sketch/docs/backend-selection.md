...
### 5.1. Release Backend (LLVM or Native Toolchain)

An offline release compiler that recompiles cranelisp programs with full optimizations. This is **not** a REPL backend — it's a separate build tool that produces optimized standalone binaries.

**Motivation**: Cranelift produces ~70-80% of LLVM -O2 code quality, which is excellent for interactive development but leaves performance on the table for deployed programs. An offline release compiler closes this gap without compromising REPL responsiveness.

**Key optimizations enabled at -O1/-O2**:
- **Escape analysis**: heap allocations that don't escape a function are promoted to stack — eliminates RC overhead for temporaries
- **Full tail call optimization**: `musttail` attribute handles mutual recursion — no manual loop transformation needed
- **Scalar Replacement of Aggregates (SRA)**: automatic unboxing of small structs
- **Inlining**: cross-function optimization that Cranelift's single-pass design cannot do
- **LTO (Link-Time Optimization)**: whole-program optimization across module boundaries

**Implementation options** (in order of pragmatism):
1. **LLVM via `inkwell`**: same AST-to-IR logic, targeting LLVM IR instead of Cranelift IR. Mechanical translation. Heavy build dependency (~100MB, minutes to compile).
2. **C code emission**: emit C source from the typed AST, compile with `cc -O2`. Lightweight dependency, good optimization, easy debugging. No `inkwell`/LLVM version management.
3. **Cranelift AOT with system linker**: use Cranelift's existing `.o` emission + system linker. No new dependency, but no additional optimization beyond what Cranelift already provides.

**Pipeline**: the release compiler shares the entire frontend and analysis pipeline (reader → macro expand → AST → typecheck → RC analysis). Only the final IR emission stage differs. The same `Backend` trait surface applies, though an `AotCompiler` sub-trait may be more natural (see Open Questions).

**Build integration**: Cargo features gate the release backend:

```toml
[features]
default = ["cranelift"]
cranelift = ["cranelift-jit", "cranelift-module", "cranelift-native", "cranelift"]
llvm-release = ["inkwell"]
```

**CLI**: `cranelisp --release build foo.cl` or `cranelisp --release build` (uses `cranelisp.toml` entry point)
...
### 5.3. Two-Tier Compilation Strategy

The backends above compose into a two-tier compilation strategy:

| | Tier 1: Interactive | Tier 2: Release |
|---|---|---|
| **Backend** | Cranelift JIT | LLVM / C emission / native toolchain |
| **Use case** | REPL, development, batch testing | Deployed programs, benchmarks |
| **Compile speed** | ~1ms per function (single-pass) | Seconds to minutes (multi-pass optimization) |
| **Code quality** | ~70-80% of optimized | Full -O2 with escape analysis, TCO, inlining |
| **Output** | In-memory JIT code | Standalone binary (no JIT runtime) |
| **Invocation** | `cranelisp foo.cl`, `cranelisp --run foo.cl` | `cranelisp --release build foo.cl` |

**ABI compatibility**: both tiers use the same uniform i64 / `extern "C"` calling convention and the same heap layout (`[size | rc | payload]`). Release binaries link against the same platform DLLs. A release build is semantically identical to a JIT execution — same types, same RC behavior, same IO model — just faster.

**Precedent**: Julia (LLVM with tiered compilation), LuaJIT (interpreter + trace JIT), GraalVM (interpreter + Graal JIT). The common pattern is fast feedback during development, optimized code for deployment.

**Implementation order**: Tier 1 (Cranelift) is the primary backend throughout reimplementation. Tier 2 is a post-Ring-4 deliverable — it depends on the full pipeline being stable and is not needed for language development or testing.
...
| **JIT trait vs AOT trait** | Tier 1 (JIT) needs `eval_expr()`, `compile_defn()`, function redefinition. Tier 2 (AOT) needs `compile_program_to_object()`, linker integration, LTO. Should these share a single `Backend` trait with optional methods, or be separate traits (`JitBackend`, `AotCompiler`)? |
| **C emission as lightweight tier 2** | Emitting C source avoids the LLVM system dependency entirely. The C compiler handles optimization. Trade-off: less control over optimization choices, but dramatically simpler build and maintenance. Worth prototyping before committing to LLVM. |
