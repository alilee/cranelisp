...
pub use traits::synthesize_default_defns;
...
pub enum SymbolInfo<'a> {
    ...
    OverloadedFn { schemes: Vec<Scheme> },
    ...
    Macro {
        name: &'a str,
        clauses: &'a [crate::macro_expand::MacroClauseInfo],
...
pub struct ConstructorInfo {
    ...
    #[serde(default)]
    pub internal: bool, // if true, not user-constructable or matchable (e.g. IO Bind)
}
...
pub struct TypeChecker {
    ...
    /// Pending method resolution records: (span, trait_name, method_name, arg_type).
    pending_resolutions: Vec<(Span, String, String, Type)>,
    ...
    deferred_resolutions: Vec<(Span, String, String, Type)>,
    ...
    // ── Platform scheduling metadata ──────────────────────────────────────
    /// Scheduling class for each registered platform function.
    /// Key = bare function name (e.g. "print", "http-get").
    /// Used by the auto-scheduling pass to decide which IO calls can be parallelized.
...
pub struct ConstrainedFn {
    pub defn: Defn,
    pub scheme: Scheme,
    pub deferred_resolutions: Vec<(Span, String, String, Type)>,
...
pub struct CheckResult {
    ...
    pub expr_types: HashMap<Span, Type>,
    /// Synthesized default method defns (from trait defaults not overridden in impls).
...
impl TypeChecker {
    pub fn new() -> Self {
        TypeChecker {
            ...
            platform_scheduling: HashMap::new(),
        }
    }
    ...
    fn resolve_builtin_fq(&self, name: &str) -> Option<FQSymbol> {
        ...
        {
            // For qualified names like "macros/__sconcat__", build FQSymbol directly
            // from the module/symbol parts (qualified_name doesn't handle slash-qualified names).
            if let Some((module, local)) = crate::names::split_qualified(name) {
                Some(FQSymbol::new(ModuleFullPath::from(module), Symbol::from(local)))
            } else {
    ...
    pub fn find_trait_decl(&self, trait_name: &str) -> Option<&TraitDecl> {
        for cm in self.modules.values() {
    ...
    /// Find which trait a method belongs to, scoped to traits visible in the current module.
    /// Only checks TraitDecl entries reachable from the current module (direct or imported).
    fn find_trait_for_method(&self, method_name: &str) -> Option<&str> {
        ...
        // Collect symbol names to avoid borrow conflict with resolve_entry_in_module
    ...
    /// Reconstruct overloads and resolved_overloads from cached DefKind::Overloaded data.
    pub fn register_overloads_from_cache(
        &mut self,
        base_name: &str,
        variants: &[(Vec<Type>, Type, crate::names::Symbol)],
    ) {
        let overload_entries: Vec<(String, usize)> = variants
            .iter()
            .map(|(pts, _, mn)| (mn.to_string(), pts.len()))
            .collect();
        self.overloads
            .insert(base_name.to_string(), overload_entries);
        self.resolved_overloads.insert(
            base_name.to_string(),
            variants
                .iter()
                .map(|(pts, rt, mn)| (pts.clone(), rt.clone(), mn.to_string()))
                .collect(),
        );
    }
