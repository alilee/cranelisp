...
### ★ I-C + 0628 RESOLVED as SPEC CORRECTIONS (user-settled 2026-07-18) → /spec dispatched
...
- **0628 — kind-by-usage + echo-the-head impl form.** `(deftrait (X a))` head ⇒ higher-kinded (con-var must be APPLIED `(f a)`); never-applied `(X a)` is malformed (use the conventional `self`-based form). Impl form: **slot-1 AS DECLARED** — `(impl (Functor f) (Functor Option) …)` for HK (binder echoes the deftrait head), `(impl Display (Option a) …)` for conventional; kind-checking disambiguates (`(impl Display Option)` = kind error); inline constraints `(Option :Display a)`. §7.1.1 dispatch-param rule rejects Zeroable (no implementing-type param). Worked: fmap for `(Functor Option)`, show for `(Display (Option a))` + `(Display Int)`.
- **/spec DISPATCHED** to draft both amendments for USER ratification.
