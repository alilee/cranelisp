...
| `repl.rs` | Slash-command dispatch, prompt/banner/line-editor, and introspection-display helpers (`describe_symbol`, `format_eval_result`, `format_def_entry`, the `format_*` free fns). `QUIT_SENTINEL` lives here, re-exported from `session_v4`. |
...
**Single-implementation discipline.** `process_form::compile_macro_clause_core` is the sole macro-clause compiler (`compile_macro_clause_with_state` and `_inline` are thin adapters). Do not re-introduce a byte-identical mirror.
...
## Prelude as an OUTER SCOPE (not flattened)

The implicit prelude is an **outer scope resolved by a session-side fallback bit**, NOT materialised into each module's symbol table (`design/int/s78-entry-module.md` §2).
