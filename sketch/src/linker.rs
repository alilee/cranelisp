...
impl Linker {
    ...
    pub fn load_object(
        ...
        //
        // LOCAL symbols (anonymous functions like .Lfn0) are kept in a per-load
        // map to prevent name collisions across .o files. Only exported symbols
        // go into the global `defined_symbols` map.
        ...
        for sym in obj.symbols() {
            ...
            if let Ok(name) = sym.name() {
                if !name.is_empty() {
                    ...
                    // Local symbols (e.g. .Lfn0 anonymous functions) go into a
                    // per-object map to avoid cross-module name collisions.
                    if clean_name.starts_with(".L") {
                        local_symbols.insert(clean_name.to_string(), addr);
                    } else {
                        debug_assert!(
                            !self.defined_symbols.contains_key(clean_name),
                            "linker: duplicate symbol '{}' at {:#x} (existing: {:#x})",
                            clean_name,
                            addr,
                            self.defined_symbols[clean_name]
                        );
        ...
        for (offset, reloc) in text_section.relocations() {
            ...
            let target_addr = match reloc.flags() {
                ...
                {
                    // For local symbols (.Lfn*), use a unique GOT key per .o file
                    // to prevent cross-module name collisions in GOT slots.
                    let got_key = if target_name.starts_with(".L") {
                        format!("{:#x}:{}", base_addr, target_name)
                    } else {
                        target_name.clone()
                    };
                    let slot_addr = self.get_or_create_got_slot(&got_key, raw_target_addr);
                    slot_addr
                }
                _ => raw_target_addr,
            };
