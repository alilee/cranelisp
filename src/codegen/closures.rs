...
impl<'a, M: Module> FnCompiler<'a, M> {
    ...
    pub(crate) fn compile_lambda(
        ...
        // Use empty globals set so free_vars returns ALL unbound names, then filter
        // to only names that are actual local bindings (in self.variables). This avoids
        // false negatives when parameter names collide with global function/accessor names.
        let empty_globals = std::collections::HashSet::new();
        let free = captures::free_vars(expr, &empty_globals);
        let mut capture_names: Vec<String> = free
            .into_iter()
        ...
        // Generate closure drop function if any captures are heap-typed.
        // Must happen before inner FnCompiler creation (borrows self.module).
        let heap_capture_offsets: Vec<(i32, crate::types::Type)> = capture_names
            .iter()
            .enumerate()
            .filter_map(|(i, name)| {
                let ty = self.variable_types.get(name)?;
                if ty.is_heap_type() {
                    Some((((i + 2) * 8) as i32, ty.clone()))
                } else {
                    None
                }
            })
        ...
        let drop_func_id: Option<FuncId> = if heap_capture_offsets.is_empty() {
            None
        } else {
            // Pre-resolve ADT drop functions for capture types
            for (_, ty) in &heap_capture_offsets {
                if let crate::types::Type::ADT(..) = ty {
                    super::resolve_drop_fn(
                        ty,
                        self.module,
                        self.type_defs,
                        self.free_func_id,
            ...
            Some(super::compile_closure_drop_fn(
                self.module,
                &heap_capture_offsets,
                self.type_defs,
                self.free_func_id,
                &self.drop_fn_cache,
            ))
        ...
        // Compile lambda body in a fresh Function
        {
            ...
            // Extract lambda param types from the lambda expression's type in expr_types
            let lambda_param_types: Vec<crate::types::Type> = inner
                .expr_types
                .get(&span)
                .and_then(|ty| match ty {
                    crate::types::Type::Fn(params, _) => Some(params.clone()),
                    _ => None,
                })
                .unwrap_or_default();
            ...
            for (i, param_name) in params.iter().enumerate() {
                ...
                if let Some(ty) = lambda_param_types.get(i) {
                    inner.variable_types.insert(param_name.clone(), ty.clone());
                    // Track heap-typed params for consuming convention scope-exit dec
                    if ty.is_heap_type() {
                        inner.track_binding(param_name.clone(), var, ty.clone());
                        // Consuming convention: callee is sole owner → mark unique
                        inner.mark_unique(var);
                    }
                }
            }
            ...
            for (i, cap_name) in capture_names.iter().enumerate() {
                ...
                // Record capture type for inner codegen (but don't track in scope_stack —
                // captures are owned by the closure env, not by this invocation)
        ...
        // Allocate closure: [code_ptr, drop_ptr, cap0, cap1, ...]
        ...
        // Store drop_ptr at offset 8 (null if no heap captures)
        let drop_ptr_val = if let Some(dfid) = drop_func_id {
            let drop_ref = self.module.declare_func_in_func(dfid, self.builder.func);
        ...
        self.builder
            .ins()
            .store(MemFlags::trusted(), drop_ptr_val, closure_ptr, 8);
        ...
        for (i, cap_name) in capture_names.iter().enumerate() {
            ...
            if let Some(ty) = self.variable_types.get(cap_name).cloned() {
                ...
                // Another reference → captured var no longer unique
                if let Some(&var) = self.variables.get(cap_name) {
                    self.remove_unique(var);
                }
            }
        }
    ...
    pub(crate) fn compile_constructor_as_closure(
        ...
        // Wrap as closure: [code_ptr, drop_ptr(null)] (no captures)
        let closure_ptr = self.compile_alloc(16, span)?;
        let func_ref = self.module.declare_func_in_func(func_id, self.builder.func);
        let code_ptr = self.builder.ins().func_addr(types::I64, func_ref);
        self.builder
            .ins()
        ...
        self.builder
            .ins()
            .store(MemFlags::trusted(), null, closure_ptr, 8);
    ...
    pub(crate) fn compile_auto_curry(
        ...
        // Allocate closure: [code_ptr, drop_ptr(null), applied_arg0, applied_arg1, ...]
        let closure_size = ((2 + applied_count) * 8) as i64;
        let closure_ptr = self.compile_alloc(closure_size, span)?;
        ...
        // Store captured applied args at offsets 16, 24, ...
    ...
    pub(crate) fn compile_func_as_closure(
        ...
        // Allocate closure [wrapper_code_ptr, drop_ptr(null)]
        let closure_ptr = self.compile_alloc(16, span)?;
        let wrapper_ref = self
            .module
            .declare_func_in_func(wrapper_func_id, self.builder.func);
