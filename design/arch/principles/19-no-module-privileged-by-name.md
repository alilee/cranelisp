...
- `prelude` — the one **implied import** (spec §8.8.1), special by its *declared role*, realised as a per-module resolution fallback (the prelude-fallback bit), not by a name check. The fallback is a resolution-mechanism detail with zero semantic weight — there is no "outer scope" as a language concept, and prelude-provided names obey the §8.6.4 conflict rules identically to explicit imports (S108 Wave G refinement; `prelude-import-convergence.md`).
...
- **`prelude_fallback` per-module bit** — the prelude is resolved by a **fallback** consulted on a bare-name resolution miss, not flattened (materialised) into each module's table. The bit is ON for modules that receive the implicit prelude and OFF for modules that refuse or reference it (`(import [prelude []])` / selective import) — the same gate that governed implicit-prelude injection before. (S108 Wave G: the bit is consulted once, at resolution-scope construction — never per call site; `prelude-import-convergence.md` §3.) Primitives reach user code via prelude's `(export [primitives [*]])` re-export, chain-followed through the same fallback (Decision 0048 uniformity) — there is no per-module primitives seeding and no `"primitives"` name-key.
...
- Principle 17 — Module locality in typecheck (the resolution-side companion: short-name resolution never probes `user` or any other module by name; the prelude fallback is a declared-role mechanism, not a name-keyed sweep).
