...
**Disposition.** **No action (S69 Sub 41) — audit framing retracted.** The original "requires /qa work" disposition (substring-grep is a hole because dropping `pub use module::SymbolTable` while keeping `pub mod module;` reachable means "SymbolTable" still appears in pub-api.txt under the deep path) conflated compile-check (path reachability) with the compliance test's responsibility (coverage). User-arbitrated reframing: path reachability is enforced by the Rust compiler — consumers writing `use cranelisp_types::SymbolTable;` get a compile error if it's dropped. Baseline diff also surfaces it (the line disappears from pub-api.txt; reviewer sees per S67 baseline-diff discipline per `design/arch/CLAUDE.md` §"Baseline-diff discipline (Sprint 67 close)"). The compliance test's substring-grep is correctly a coverage mechanism, not enforcement. PIF-row enhancement against /qa would be redundant with what compile-check + baseline-diff already cover. The hole as framed does not exist. NOTE: C-HOLE-6 (Sub 41) makes the deep-path concern moot anyway — submodules become `pub(crate)`, so the deep paths the audit worried about are no longer reachable for any future drift.
...
**Disposition.** **No action (S69 Sub 41) — audit framing retracted (same reframing as C-HOLE-1).** Original "requires /qa work for per-critical-field PIF rows" disposition framed field-level type regressions (e.g., `Scheme.constraints: HashMap<TypeId, Vec<FQTraitName>>` → `Vec<TraitName>`) as a hole in the compliance test. User-arbitrated reframing: such regressions are caught by (a) baseline diff — the type-signature line changes verbatim and the reviewer sees per S67 baseline-diff discipline — and (b) compile-check — any consumer typed against `Vec<FQTraitName>` fails. PIF rows would extend the compliance test to per-field type-signature granularity, but that catches problems compile-check + baseline-diff already catch. Compliance test is coverage, not enforcement. PIF rows are redundant.
...
**Disposition.** **No action (S69 Sub 41 confirms).** Covered by Wave 2 /qa orphan-filter refinement per SPRINT.md — auto-trait projections (Send/Sync/Unpin/RefUnwindSafe/UnwindSafe) are filtered from the comparison set before the compliance test runs. No structural question; original audit call holds.
...
**Disposition.** **RESOLVED by source narrowing (S69 Sub 41).** `string_newtype!` macro's inner field changed from `pub struct $name(pub String);` to `pub struct $name(String);` at `crates/cranelisp-types/src/newtype.rs:11`. The facade's "wrapper is opaque; field access is not exposed" claim (now extended in §"`#[non_exhaustive]` policy") is now structurally enforced per Principle 18. Consumers reach the inner string via the existing `Deref<Target=str>` / `AsRef<str>` / `Borrow<str>` / `Display` / `PartialEq<str>` / `PartialEq<&str>` impls. Consumer cascade (~10 production sites; all `.0` reads, no writes) deferred wave-3:

- `crates/cranelisp-frontend/src/ast_builder.rs:349, 354, 361, 516, 523`
- `crates/cranelisp-backend/src/cache/manifest.rs:75, 85, 90, 151`
- `crates/cranelisp-backend/src/cache/mod.rs:113`
- `src/worker.rs:2977, 2995`

Migration pattern: `.0` → `.as_ref()` (or implicit deref); `.0.clone()` → `.to_string()`. Grounded by Principle 18 + facade-text opacity claim alignment.
...
**Disposition.** **RESOLVED by source narrowing (S69 Sub 41) — audit's deferral overridden.** Original audit "low priority, not S69 work, schedule deferral acceptable" was deferred on disruption grounds. User-directed trace surfaced effectively zero production deep-path consumers: workspace-wide grep for `cranelisp_types::(ast|check|error|heap|module|newtype|parsed|sexp|scheduling|got|pipeline|marshal|view|span|types)::` returned (a) one production `use` statement at `crates/cranelisp-typecheck/src/program.rs:34` (`use cranelisp_types::types::Subst;`), (b) one docstring at `crates/cranelisp-platform/src/lib.rs:34` naming the deep path in a comment, and (c) two example-output comments at `tests/facade_compliance.rs:96-97`. Cascade is **one source-code site** (one-line use-statement flip).

Baseline impact: `cranelisp-types/public-api.txt` shrinks from 3743 lines to ~730 lines (~80% reduction). The eliminated lines are auto-trait projections duplicated per path (`impl core::marker::Send for cranelisp_types::ast::Expr` AND `impl core::marker::Send for cranelisp_types::Expr` — only the crate-root form remains). One-time editorial baseline regen.

Implementation: `crates/cranelisp-types/src/lib.rs:4-28` — every submodule narrowed `pub mod X; → pub(crate) mod X;`. Crate-root re-exports (`pub use ast::*;` etc.) stay unchanged; re-exports work regardless of inner module visibility within the same crate. Intra-crate `use crate::module::*` statements continue to compile.

Wave-3 follow-up (cascade): flip `crates/cranelisp-typecheck/src/program.rs:34` from `use cranelisp_types::types::Subst;` to `use cranelisp_types::Subst;` (already re-exported at the crate root). Informational-only (non-blocking): update the stale docstring at `crates/cranelisp-platform/src/lib.rs:34` from `cranelisp_types::scheduling::SchedulingClass` to `cranelisp_types::SchedulingClass`; update example comments at `tests/facade_compliance.rs:96-97`. The two doc/comment sites have no compile impact.

Grounded by Principles 13 (interfaces.md auditable; cargo-public-api gateable) + 18 (`pub(crate)` defaulting). Reframing parallel to Sub 38's HeapCategory relocation — audit's mechanical-bucket deferral was on premises that trace contradicted.
...
| C-HOLE-1 | Crate-root `pub use` set mechanical gap | **No action (Submission 41) — audit framing retracted** | User-arbitrated reframing: compile-check + baseline-diff already enforce; compliance test is coverage not enforcement; PIF row would be redundant |
| C-HOLE-2 | Field-type drift mechanical gap | **No action (Submission 41) — audit framing retracted** | Same reframing as C-HOLE-1: baseline-diff + compile-check already catch field-type regressions; per-field PIF rows redundant |
| C-HOLE-3 | Auto-trait projections | No action (Submission 41 confirms) | Wave 2 /qa orphan-filter (filters Send/Sync/Unpin/RefUnwindSafe/UnwindSafe before comparison) |
| C-HOLE-4 | `pub String` in `string_newtype!` opacity | **RESOLVED by source narrowing (Submission 41)** | Principle 18 + facade opacity claim — inner field made private; ~10-site `.0` consumer cascade deferred wave-3 |
| C-HOLE-5 | `SymbolTable` `pub` fields | Source moves | Bundled with S-DRIFT-19 (Principle 18) |
| C-HOLE-6 | Submodule `pub mod` exposure | **RESOLVED by source narrowing (Submission 41) — audit deferral overridden** | Principles 13 + 18 — all submodules narrowed `pub(crate)`; trace surfaced 1-site production cascade (`program.rs:34`); baseline ~80% reduction; reframing parallel to Sub 38's HeapCategory relocation |

**Final counts by disposition class:**

| Class | Count |
|---|---|
| Source moves | 25 (H2, H3, H4, H5, H6, H7, H9, H10, H11, U11, S-DRIFT-4, S-DRIFT-10, S-DRIFT-19, S-DRIFT-20, S-DRIFT-21, C-HOLE-5 — plus partial source-side moves in H8, S-DRIFT-12, S-DRIFT-15, U9 = 20 hard + ~5 partial; C-HOLE-4 + C-HOLE-6 reclassified D→"RESOLVED by source narrowing" per Submission 41 — count decremented by 2; S-DRIFT-8 RESOLVED in-place Submission 31 — source moved as predicted; direction column unchanged; wave-3 cascade deferred. S-DRIFT-10 RESOLVED in-place Submission 34 — reclassified from bucket C "arbitration genuine" to "source moves" + landed; source rewritten from `pub enum View { Single, Union }` to `pub struct View { staging: Option<&'a SymbolTable>, live: &'a SymbolTable }` with private fields per Decision 44 opacity intent + Principle 18; typecheck ClusterContext pattern-match consumer cascade deferred to wave-3.) |
...
| No action | 6 (U3, U6, U18, C-HOLE-1, C-HOLE-2, C-HOLE-3) — U3 confirmed Submission 40 (variant uniformity holds; `is_public` uniform check at module.rs:827 grounds the informational-stub field). S-DRIFT-22 reclassified to "Facade moves" per Submission 29 (scope-extended editorial sharpening; opacity policy intact per Principle 15); U21 reclassified D→"RESOLVED by facade enumeration" per Submission 39 — audit's "No action" overridden under S67 baseline-diff discipline (every pub-api line named); S-DRIFT-16 reclassified D→"RESOLVED by source narrowing + facade enumeration + FIXME-cascade" per Submission 40 — original "No action — already covered by facade disposition table" framing superseded by Sub 30/36/39's fuller-surface-accuracy trajectory + Principle 7 single-source-of-truth grounding (ModDecl was sole bool outlier in decl/entry family); count decremented by 1. C-HOLE-1 + C-HOLE-2 reclassified D→"No action" per Submission 41 — user-arbitrated audit-framing retraction: original "requires /qa work" disposition conflated compile-check (path reachability + per-field type contracts — both enforced by the Rust compiler) with the compliance test's responsibility (coverage substring-grep). Baseline-diff per S67 discipline surfaces any line-level regression to the reviewer; compile-check breaks consumer crates on signature change; PIF-row enhancements would be redundant with what these two mechanisms already cover. The "holes as framed" do not exist. C-HOLE-3 stays — audit call held under Sub 41 confirmation. Count incremented by 2. |
| Requires /qa work | 0 — C-HOLE-1 + C-HOLE-2 reclassified D→"No action" per Submission 41 (audit framings retracted; PIF-row enhancements would be redundant with compile-check + S67 baseline-diff). |
