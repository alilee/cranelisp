...
fn parse_rest_param() {
    ...
    assert_eq!(
        info.clauses[0].rest_param.as_ref().unwrap().as_ref(),
        "args"
    );
}
...
fn parse_multi_clause_rest_with_space() {
    let sexp =
        parse_one("(defmacro my-cond ([x] x) ([x body & rest] `(if ~x ~body (my-cond ~@rest))))");
    let info = parse_defmacro(&sexp).unwrap();
    assert_eq!(info.clauses.len(), 2);
    assert_eq!(info.clauses[1].fixed_params.len(), 2);
    assert!(info.clauses[1].rest_param.is_some());
    assert_eq!(
        info.clauses[1].rest_param.as_ref().unwrap().as_ref(),
        "rest"
    );
}
...
fn synthesize_simple_clause() {
    let clause = MacroClause {
        fixed_params: vec![MacroParam::Name("a".into()), MacroParam::Name("b".into())],
        rest_param: None,
