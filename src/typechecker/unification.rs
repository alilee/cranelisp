        ...
        match texpr {
            ...
            TypeExpr::Named(name) => match name.as_str() {
                ...
                _ => {
                    ...
                    // For qualified names like "macros/Sexp", use the bare name
                    // in the resulting Type::ADT (canonical form).
        ...
        match texpr {
            TypeExpr::Named(name) => {
                ...
                let concrete = match name.as_str() {
                    ...
                    _ if self.resolve_type_def_via_modules(name).is_some() => {
                        let bare = crate::names::bare_from_qualified(name);
                        Some(Type::ADT(bare.to_string(), vec![]))
                    }
                    _ => None,
                };
            ...
            TypeExpr::Applied(name, args) => {
                let resolved_args: Vec<Type> = args
                    ...
                    .collect::<Result<_, _>>()?;
                let bare = crate::names::bare_from_qualified(name);
                let ty = Type::ADT(bare.to_string(), resolved_args);
                self.unify(target_ty, &ty, span)
            }
            TypeExpr::FnType(params, ret) => {
    ...
    fn resolve_annotation_type(
        ...
        match texpr {
            TypeExpr::Named(name) => match name.as_str() {
                ...
                _ if self.resolve_type_def_via_modules(name).is_some() => {
                    let bare = crate::names::bare_from_qualified(name);
                    Ok(Type::ADT(bare.to_string(), vec![]))
                }
            ...
            TypeExpr::Applied(name, args) => {
                ...
                if self.resolve_type_def_via_modules(name).is_some() || name == "Vec" {
                    let bare = crate::names::bare_from_qualified(name);
                    Ok(Type::ADT(bare.to_string(), resolved_args))
                } else {
                    Err(CranelispError::TypeError {
                        message: format!("unknown type: {}", name),
                        span,
