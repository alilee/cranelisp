...
fn u4_case_discrimination_lowercase_mints_uppercase_errors() {
    ...
    assert!(matches!(
        resolve_via(&nested, &mut var_map, &r2, Some(&mint), Span::SYNTHETIC).unwrap_err(),
        ResolveError::TypeNotFound { .. }
    ));
}
...
fn u8_qualified_lowercase_name_does_not_mint() {
    ...
    assert!(
        var_map.is_empty(),
        "a qualified name must not be recorded as a var"
    );
...
fn ctx_self_type_none_errors() {
    ...
    let err =
        resolve_type_expr(&TypeExpr::SelfType, &mut var_map, &ctx, Span::SYNTHETIC).unwrap_err();
    assert!(matches!(err, ResolveError::TypeNotFound { .. }));
}
...
fn ctx_trait_type_param_aliases_self() {
    ...
    assert_eq!(
        got,
        Type::Var(9),
        "trait param `a` must alias self_type, not mint"
    );
    assert!(
        var_map.is_empty(),
        "an aliased trait param must not be minted"
    );
}
...
fn ctx_sig_free_var_mints_and_corefers() {
    ...
    assert_eq!(
        got,
        Type::Fn(
            vec![Type::Var(800), Type::Var(800)],
            Box::new(Type::Var(801))
        )
    );
}
...
fn ctx_hkt_decl_con_var_interception() {
    ...
    // bare f
    let bare = resolve_type_expr(
        &TypeExpr::TypeVar(Symbol::from("f")),
        &mut var_map,
        &ctx,
        Span::SYNTHETIC,
    )
...
fn ctx_hkt_decl_unknown_named_errors_neg() {
    ...
    let err =
        resolve_type_expr(&named("Ghosttype"), &mut var_map, &ctx, Span::SYNTHETIC).unwrap_err();
    match err {
...
fn ctx_intrinsic_scalar_fastpath_without_terminal() {
    ...
    for (n, ty) in [
        ...
        assert_eq!(
            resolve_type_expr(&named(n), &mut var_map, &ctx, Span::SYNTHETIC).unwrap(),
            ty
        );
    }
}
