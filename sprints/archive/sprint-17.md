...
### Wave 4: Showcase (Wave 3 green)

| Skill | Task | Status | Notes |
|-------|------|--------|-------|
| /repl | X5: ring4a.demo (S16 IO foundation) | done | `repl/demos/ring4a.demo` created |
| /repl | B5: ring4b.demo (IO sequencing) | done | `repl/demos/ring4b.demo` created |
| /port | B5: update exemplar with IO | partial | Pure exemplar runs; IO integration deferred |
| /examples | B4: IO examples (print sequence, echo, bind!) | done | `21-hello-io.cl`, `22-io-hello.cl`, `23-io-sequence.cl`, `24-io-echo.cl` |
| /docs | Update guide with IO sequencing section | partial | Deferred to Sprint 18 |
| /platform | Evaluate stderr `write` FIXME | done | Recommendation: Sprint 18+ scope |
| /qa | E2E + REPL experience test additions | done | 8 new failing tests added (uncommitted) documenting known defects — see Findings |

## Notes

- Wave 4 showcase completed with demos and examples. Some user-proxy items (docs update, exemplar IO) deferred.
- `/qa` added 8 new tests in uncommitted work that expose pre-existing defects (type annotations, prelude Option display, REPL experience regressions). These are intentionally left failing to document known gaps.

## Outcome

### Delivered
- **Export mechanism** (B0): `register_exports()` in typechecker + pipeline wiring. `(export ...)` forms fully functional.
- **Prelude remediation** (B1): 12 macros moved to domain modules. Prelude is now a pure `(export ...)` re-export shell with zero definitions.
- **IO-aware `do` macro** (B2): `do` expands to `bind` chains per spec §10.4. Pure sequencing migrated to `let [_ ...]`.
- **`read-line` verification** (B3): End-to-end confirmed working in batch and REPL.
- **Lambda RC fix** (X1): Root cause identified and fixed — `compile_lambda_body` now adds params to scope_stack and emits scope cleanup. 5 new tests.
- **Platform-aware test helper** (X2): `TestCapture` struct + `repl_session_with_test_capture()`. 4 IO tests un-ignored + 2 new.
- **Auto-curry tests** (X3): Confirmed genuinely not implemented. Ignore messages updated with spec refs.
- **R3 annotation gaps** (X4): 4 new ignored tests (3 HKT, 1 lazy seq). Genuine gaps documented.
- **IO demos**: `ring4a.demo` (IO foundation), `ring4b.demo` (IO sequencing)
- **IO examples**: 4 new examples (`21-hello-io.cl` through `24-io-echo.cl`)
- **Review**: 0 Blockers, 3 Important resolved (I1 deferred pre-existing, I3 duplicate `pure` removed, I6 documented)

### Deferred
- **Exemplar IO integration**: Pure exemplar works; IO integration requires more pipeline work. → Sprint 18+
- **Docs IO guide section**: Deferred to Sprint 18.
- **I1 (unused heap lambda params leak)**: Pre-existing, not S17 regression. → Sprint 18.
- **stderr `write`**: Platform FIXME evaluated; recommended for Sprint 18+.

### Findings
- 8 new failing tests added (uncommitted) exposing defects:
  - 3 type annotation expression tests (`:Int 42` not supported as expression — spec §2.3.8)
  - 3 prelude Option display tests (`(Some 42)` shows raw pointer with test prelude loaded)
  - 2 REPL experience tests (uncommitted additions)
- These are carried as debt items in Sprint 18.
