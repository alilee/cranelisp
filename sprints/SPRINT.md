...
- 2026-06-10: **W-MacroTrait RT6 (FIXME 0300) DONE — §7.6 trait-method-as-value, 5-crate cascade, 32/32 green.** User-approved /arch change (`Expr::Var.resolved_call` + `Expr::var` ctor) → frontend/typecheck/backend/int slices. Typecheck `resolve_value_position_trait_methods` deferred pass resolves a value-position trait-method Var via `try_resolve_trait_method` over its `inferred_type` (String→`str-eq`, Float→`add-f64` — Symptom B fixed, not just the missing-wrapper Symptom A); backend `compile_trait_method_as_value` emits a zero-capture dispatch-wrapper, replacing the hard-coded-Int operator-as-value path; Decision 43 held (no trait knowledge in backend). /review PASS-WITH-FINDINGS (1 Suggestion → FIXME 0306 /dev: local-shadow annotation false-positive, neutralized by backend locals-first ordering). FIXME 0300 closed. Green: ~12 → ~8.
- 2026-06-09: **W-MacroTrait RT6 (FIXME 0300) — was BLOCKED on an /arch Decision (user-approved 2026-06-09).** Investigation found both symptoms (escaping-wrapper-not-emitted + wrong-impl-dispatch) share ONE root: `Expr::Var` has no `resolved_call` field, so a trait method in value position gets no resolution → backend fails (`undefined variable: show`) or falls back to a hard-coded Int primitive (String `=`→false, Float `+`→inf.0). Decision 43 forbids reintroducing trait-keyed dispatch in backend. Fix = 3-step cascade: **(1) /arch add `Expr::Var.resolved_call: Option<Box<ResolvedCall>>` to `cranelisp-types`** (public-API change — USER REVIEW), (2) /typecheck deferred value-position resolution pass, (3) /dev backend `compile_var` wrapper emission. 4 tests stay failing-not-ignored; 0300 re-targeted /arch with the contract. Backend untouched.

- 2026-06-10: **W-Repl + 0302 DONE — 3 /int display/REPL defects fixed.** 0301 bare-primitive docstring (new `src/builtin_docs.rs` table from §A.3 + `/doc` chain-follow); 0142 EOF-unclosed-form now errors per the user ruling (REPL read-loop flushes the pending unbalanced buffer → parse error to stdout; filed /spec 0307 to record §5.13.2); 0302 product single-ctor display reads fields from `TypeDef.constructor_scheme`. /review PASS-WITH-FINDINGS — 2 non-blocking Importants filed (0308 /arch relocate docstrings into cranelisp-primitives; 0309 /repl reconcile dash-form contradiction). repl_introspection+repl_negative 143/143; lifecycle+persist 50/50; lib 267/267. FIXMEs 0301/0302/0142 closed. Green: ~7 → ~4 (only R9 platform + R10 sharedstate remain).
