...
**Rationale.** The prototype had 3 ISA constructions and 9 duplicate primitive-name mappings. Each duplication is a latent divergence. Decisions 25, 26, 32, 33 — placing `code` directly on `ModuleEntry::Def`, scheduling class on the `PrimitiveKind::PlatformEffect` variant, structural decls as fields on `SymbolTable`, etc. — are all enforcements of this principle.
