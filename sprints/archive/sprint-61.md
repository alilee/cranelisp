...
**Status**: COMPLETE (closed 2026-04-22, user approval; archived to `sprints/archive/sprint-61.md`)
...
- **G (`/int`)**: Rename `register_dep_shim_publishes_before_caller_registers` test per S60 /review S2. **DONE** (Wave 5) — renamed to `register_dep_for_eval_publish_then_register_is_observable_to_downstream` in `src/session_v4.rs::persistent_worker_tests` (test + surrounding doc comment at :4627). Rewrite-to-call-actual-fn option (b) deferred: the shim blocks on `wait_module_inmem_complete_blocking` and requires a worker fixture — non-trivial; future work. Test passes; `cargo check` + `cargo clippy` clean.
...
- **L (`/backend`) — Wave 2 /review I-2 (added 2026-04-22, first-time deferral)**: `design/backend/ring2-rc.md §5.5` 3-rule expansion lacks a "Sketch comparison" addendum for the new `borrowed_vars` rule. Append one-paragraph section per `CLAUDE.md §"Sketch Oracle"`. See `design/review/sprint-61-wave-2.md §Importants I-2`. **DONE (Wave 5)** — §5.5.1 addendum landed (~175 words): sketch had an equivalent `borrowed_vars` set with an orthogonal gating strategy (auto-upgrade in `emit_consuming_caller_rc` rather than explicit `is_last_use` gate); reimplementation's approach is clearer. BONUS: §5.6.1 addendum also landed (~200 words) for the newer Wave 4 capture-return inc rule — finding: sketch had the same latent bug (no inc emitted when a lambda body returns a heap capture; `pop_scope_for_value` only scans `scope_stack`, which explicitly excludes captures at `sketch/src/codegen/closures.rs:194–198`). Both addenda cite sketch files and line ranges and justify reimplementation divergence per `CLAUDE.md §"Sketch Oracle"`.
...
| Skill | Task | Status |
|---|---|---|
| /qa | Implement E-1 — convert ~10 tests + `ReplSessionBuilder` helper to `tempfile::TempDir` per `tests/plan/tempdir-audit.md`; insert rule text into `tests/CLAUDE.md` | DONE 2026-04-22 — 10/10 CONVERT rows addressed (1 helper + 9 tests); rule text inserted in `tests/CLAUDE.md §"Fresh Temp Directory per Test"`. `examples_run` row deferred (cache-path indirection out of scope). See `tests/plan/tempdir-audit.md §"Wave 5 implementation status"`. |
| /qa | Implement K — consolidate inline E2E helpers in `tests/sprint61_bare_primitive.rs` into `tests/helpers/mod.rs` (Wave 2 /review I-1) | DONE 2026-04-22 — `project_root`, `binary_path`, `runs_dir`, `run_repl_with_stdlib`, `tempdir_project_from_fixture` added to `tests/helpers/mod.rs`. `sprint61_bare_primitive.rs` now imports them. 5/5 consecutive test runs green. |
| /qa | Implement H — 3 `[Tested+Neg]` promotions from shortlist; commit spec-annotation updates | DONE 2026-04-22 — 3/3 promotions landed, passing. Annotations updated at `repl/spec.md` §5.1 (816), §5.2 (825), §3.4 (502) and `/imports` row at line 392. See `tests/plan/neg-coverage-candidates.md §"Wave 5 landed"`. |
| /qa | Implement I — repro-handoff migration, tests/ side (`memory/feedback_repro_handoff.md`) | DONE 2026-04-22 — `tests/exemplar_solver_correctness.rs` rewritten: both tests inline repro sources as Rust string literals + use fresh TempDirs; T-S2-1 copies `grid.cl`/`solver.cl` subset from exemplar into TempDir. No subprocess-exec of `exemplar/*.cl`. Exemplar `.cl` files ready for /port deletion follow-on. |
| /port | Implement I (exemplar side) — delete `exemplar/repro-slice2.cl` + `exemplar/test-eliminate-contract.cl`; refresh `exemplar/solver.cl:370+` FIXME block to reflect 3-layer closure + point at `tests/exemplar_solver_correctness.rs` | DONE 2026-04-22 — both `.cl` repro files deleted (now `D` in `git status`); `solver.cl` FIXME block rewritten (~34 LOC, down from ~36) covering Layer 1 algorithmic fix (Wave 2), Layer 2 ensure_module_exists atomicity fix (Wave 3), Layer 3 `emit_capture_return_inc` codegen fix (Wave 4); pre-existing 81-cell stack overflow preserved as Known Issue carry. `cargo run -- --run exemplar/solver.cl` prints board through IO then hits the pre-existing overflow (expected). |
| /qa | Implement J — Phase 3a plan-gap retrospective + Ring 1/2 plan additions | DONE 2026-04-22 — `tests/plan/sprint-61-plan-gap-retro.md` authored; Ring 1 + Ring 2 plans gained inline-ADT-arg class rows. Plan expansion only; tests not authored (reserved for S62+). |
| /qa | Implement M — stale FIXME cleanup at `tests/exemplar_solver_correctness.rs:150` (Wave 2 /review S-3) | DONE 2026-04-22 — stale `FIXME(/backend)` removed as part of the I rewrite; the Layer 3 fix narrative is preserved as retrospective context in the file header comment. |
...
| /repl | Author `repl/demos/ring4s.demo` showcasing Slices 1, 3, 4 visible deliverables | DONE 2026-04-22 — `repl/demos/ring4s.demo` authored (74 LOC); showcases Slice 1 bare-primitive name echo (add-i64 / mul-i64 / not / str-concat echo + /sig + call convergence), Slice 4 `(fn [_] b)` bind-closure stability via the 7-line `then`-combinator repro (returns `:primitives/Int 50`), Slice 0 scheduler + IO trace env-var interface (sample trace excerpt inline from race-evidence logs), Slice 3 race-closure narrative (H4/H5/H6). `repl/demos/CLAUDE.md` script library table extended with ring4q/ring4r/ring4s entries. |
| /repl | Replay all 26+ prior demos — no regressions | DONE 2026-04-22 — 28/28 demos exit=0 under `DEMO_FAST=1` via `./repl/showcase`. No new panics or signals introduced. Pre-existing error output in ring4a (Pure/bind unimported, carry from S17), ring4h (`;#!` shell-escape file-creation flake, carry from S23), ring4k (Color.Red constructor path + `fst` accessor synthesis pending FQTypeName, carry from S51) unchanged. Intentional error demos (first-session/ring4d/ring4i) produce their expected error lines. Batch log: `repl/demos/runs/batch-S61-ring4s-replay/`. |
| /port | Refresh `exemplar-progress.demo` if solver behaviour improved | DONE 2026-04-22 — `repl/demos/exemplar-progress.demo` refreshed: header narrates Sprint 61 3-layer closure (Layer 1 algorithmic, Layer 2 ensure_module_exists, Layer 3 inline-ADT-arg capture); added unsolvable-detection stanza demonstrating the Layer 1 narrative on the self-contained 4x4 solver. +17 LOC delta (39 → 56). Self-contained 4x4 solver continues to work unchanged; no regression. |
| /stdlib | Refresh `stdlib-progress.demo` only if Slice 1 changed user-visible stdlib behaviour | DONE 2026-04-22 — small-stanza refresh: added 3-line "Sprint 61: bare-name echo at the prompt" section demonstrating `add-i64`, `str-concat`, `not` now echoing `:(Fn …) primitives/<name> ; primitive - <doc>` at the prompt (Slice 1 outcome, spec §8.9). No `stdlib/*.cl` changes. Demo replay (`DEMO_FAST=1 ./repl/showcase stdlib-progress`) clean end-to-end. +7 LOC (66 → 73). |
| /examples | 27-example sweep verification | DONE 2026-04-22 — 28/28 examples pass under `cargo run -- --run` with examples-local prelude. Automated `tests/examples_run.rs::every_example_file_runs_under_examples_prelude` green (3.3s); test cross-checks on-disk `.cl` file set against expected-exit table (`assert_eq!(on_disk, tabled, ...)` line 152), so sweep coverage is exhaustive — no manual re-run needed. Sprint 61 Slices 1/3/4 introduced no example regressions; `21-hello-io.cl` still at deterministic exit 243 post-Slice-4 capture-return-inc fix (previous [101, 133, 141] tolerance tightened). No `examples/CLAUDE.md` updates warranted — no new example pattern this sprint. |
| /docs | User-doc refresh if any user-visible behaviour changed | DONE 2026-04-22 — **no-op disposition**. Audited `user/getting-started.md` + `user/caching.md` + `user/CLAUDE.md` against Sprint 61 Slices 0/1/2/3/4. Slice 1 bare-primitive echo at REPL (e.g. typing `add-i64` alone) is not demonstrated anywhere in the guide — all `add-i64` references are calls like `(add-i64 3 4)`, so no stale prompt transcript to update. Slice 0 observability env vars (`CRANELISP_SCHEDULER_TRACE`, `CRANELISP_IO_TRACE`) are developer diagnostics; no troubleshooting section exists in `user/` and the S60 `CRANELISP_CODEGEN_DUMP=*` precedent says these stay developer-only until a dedicated diagnostics section is commissioned. Slices 2/3/4 (solver correctness, scheduler race closures, closure double-free) are internal stability with no user-visible surface. No `user/*.md` edits. No FIXMEs filed. |
| /review | Per-wave /review pass for Wave 5 — `design/review/sprint-61-wave-5.md` | **DONE 2026-04-22 — PASS** (0 Blockers / 0 Importants / 2 Suggestions). All four read-only audits clean: no subprocess-exec of deleted exemplar repros in `tests/`; zero new `#[ignore]` from Wave 5; `ls repl/demos/*.demo \| wc -l` = 28 (matches claim); `git diff e20a7fa -- src/` = G rename only. All 5 methodology items (E-1/H/I/J/K/M/L/G) closed with artefacts. S-1: `tempdir_with_exemplar_modules` still read-only-copies `exemplar/grid.cl`+`solver.cl` (partial I migration, exemplar-coupled test deferred to future exemplar-rewrite sprint). S-2: stale `tests/sprint23.rs:6` "#[ignore] stubs" comment predates Sprint 52 re-enable. Wave 5 commit readiness: **GO**. |
| /review | Final /review report — PASS required — `design/review/sprint-61-final.md` | **DONE 2026-04-22 — PASS — ready for /sprint close**. Sprint-final sign-off authored (~180 LOC). Revised close-criteria audit passed: methodology pivot recorded in SPRINT.md §Scope; S62 primary workstream named (concurrency audit + `loom` + structured interleaving tests); FQTypeName displaced to S63+; 7 baseline ledger entries all honestly dispositioned (zero undocumented, zero `flaky`/`timing-sensitive`/`pre-existing`); all 5 per-wave reports published; design-doc trajectory complete across /int + /backend. Finding counts: 0 Blockers + 4 Importants + 17 Suggestions across 5 waves; all 4 Importants folded (Wave 1 I-1 → S62, Wave 2 I-1/I-2 → Wave 5 K/L landed, Wave 3 I-1 → S62). /sprint authorised to proceed with Phase 6 close work (commit + outcome + archive + ROADMAP + S62 draft + `.claude/commands/sprint.md` methodology-pivot update). |
