...
**redefine invariants (load-bearing).** A BROKEN entry's `code` field holds the trap stub's `Code` handle — a `code: None` + `ast: Some` entry would be silently RECOMPILED against the new-world callee by `derive_codegen_batch`'s synth-def sweep (the unsoundness the trap closes). The retention pool (`SharedState.retained_code`) is append-only to session end and pairs each trap stub with the provenance buffer its baked address reads. **Impl redefinition is `defn` redefinition** (spec §5.4.5; `design/int/impl-redefinition-hot-reload.md`): a same-type re-`impl` hot-reloads through the SAME `commit_staging_to_live` → `commit_slotted_def` GOT-patch path — there is no impl-specific commit or recompile path. What makes it work is that `derive_codegen_batch`'s `TopLevel::TraitImpl` arm enrolls the **mangled** method Defs (`Trait.method$<fq-type>`, homed in the impl WRITER's module) into the **forced** first loop, which ignores `already_compiled`; the callable is never the unmangled method name, and the `already_compiled`-gated sweep would skip the re-impl because `AbiPreserving` carries the prior code over. The enrollment set is keyed on the **TRAIT alone** — `{trait}.` prefix + a `$` in the remainder — never on `impl_.methods` (FIXME 0791): a method whose source changes explicit → **default** is not in `impl_.methods`, and its re-staged default `Defn` rides `finalize_cluster`'s WORKING program, never the `program` slice `derive_codegen_batch` sees, so a per-method narrowing silently keeps the stale override alive (spec §7.1.5). Over-enrolling a sibling impl of the same trait for another type costs a recompile and changes nothing observable — that is the accepted tradeoff.

**Every forced enrollment must resolve** (`forced_enrollment_resolves` + the `force_enroll` `debug_assert!`). `try_push` returns `bool` and every call site discards it, so a name that resolves to no live `Def` enrolls nothing, silently — the shape that let the pre-S115 arm push the unmangled `size` while the function's own rustdoc claimed trait-impl mangled names were an enrolled category. The instrument is **debug-tier only**: in a `debug_assertions`-off build the predicate is not evaluated and `force_enroll` degenerates to the bare `try_push`, byte-for-byte the prior behaviour. It is a detector, never a gate — there is no release fallback and therefore no polarity to invert. Do not "optimise" `force_enroll` back down to `try_push` at any forced call site. `__expr`/`__macro_*` are gate-exempt (fresh-slot churn every turn would exhaust the 1024-slot GOT). A slot-less staged Def displacing a slotted prior with compiled code must route the prior `Code` through the pool at the commit gate (`RetainedCode::frozen`) — dropping it is a UAF through the still-embedded GOT slot (FIXME 0479).
...
## Program-result ownership — `src/result_owner.rs` (S118, FIXME 0745)

Every **successful** execution result crosses from generated typed code into exactly ONE `OwnedProgramResult`, which carries `(value, ty)` from the driver through the result's final observation and then releases it exactly once through backend's canonical per-concrete drop glue. Design: `design/int/result-owner.md` (§8 = the slice order, §9 = the acceptance set).

- **The order is binding and identical in all three modes**: *observe, then release*. REPL formats the whole `StyledDoc` first (`main.rs`'s turn / EOF-flush / `agent/pull.rs` submit each call `EvalResult::release_program_result()` after the text is complete); `--run` computes the exit code and releases **before** `wait_object_complete`/`shutdown`/trace flush/`exit`; the linked stub converts, calls the relocated glue, then `exit`s. A mode that diverges here is a `mode-divergence` defect.
- **`result_is_exit_code` is one predicate.** An `Int` result narrows to `i32`; every other type exits 0. `--run` reads it through `OwnedProgramResult::exit_code`, the linked stub bakes it through `startup_result_exit`. Before S118 the stub `ireduce`d unconditionally and a non-`Int` linked `main` exited with the low 32 bits of a heap pointer.
- **Classification is backend's public `HeapCategory::classify`, asked BEFORE any keyed lookup.** `request_if_owning` emits no artifact row for `NeverHeap | Value`, so absence from the projection is ambiguous on its face — int must ask the same question first, and then a keyed miss is unambiguously a hard error. **Never grow an int-side "is this a heap type" list** (that is the resolver-mirror class). `Mixed` is an OWNING result; the glue body's `guard_nullary` handles the bare-tag case and int does not replicate it.
- **The release KEY is the producer's key, not a re-derivation.** It comes from the result-producing entry's `codegen_view` body `ConcreteType` — the same value backend computed its `result_roots` from — with the `IO` head stripped by the same rule (`strip_io_head`). `ConcreteType::from_type` over the observed `Type` is only the fallback for an entry with no codegen view, and a failure THERE is the hard error. This matters because the observed type is **not** always concrete: `[]` displays as `(Vec t1)` and a bare polymorphic `None` as `(Option t2)` (both spec-required, `repl/spec.md` §1.5/§4.1), and backend resolves those through `lenient_from_expr`'s `ConcreteType::Int` placeholder. Int must reach the verdict the producer reached. (FIXME 0892 carries this back to `/design`.)
- **A release target is never a bare address.** `GlueTarget` pairs the address with the retention `Code` of the code that produced the result (Principle 22). The adapter is selected by that `Code`: `Code::Jit` reads the `{artifact, owner}` pair from `SharedState.fresh_jit_drop_glues` and clones the **pair** (so an armed owner survives a pair-atomic row replacement); `Code::Linker` derives the canonical symbol with `drop_glue_symbol_name` and resolves it once through that entry's own linker. A `Linker` result never consults a `Jit` row. *As-built note*: `try_cache_hit_load` only ever restores **dependency** modules, never the CLI target, so `main`/`__expr` are always fresh-JIT today — the cache adapter is design-mandated (§3.2) and unit-guarded, not currently reached in production.
- **`IO a` is unwrapped exactly once**, at the driver boundary (`pipeline::program_outcome_to_result`'s clean arm, `trampoline`'s clean arm). The owner constructor REFUSES an `IO a` type, which is why `repl/format.rs`'s old defensive `ty.is_io()` branch (a second IO owner that re-drove `cranelisp_run_io` on the displayed word) is deleted rather than merely unreachable.
- **One finalization chokepoint**, shared by `release`/`release_in_place` and the `Drop` backstop, so the glue cannot run twice; error/trap outcomes carry no successful result and invoke no glue.
- **Do not wire result ownership into `worker::inline_jit_codegen_for_names`** — it has no production caller and discards `CompilationArtifacts.drop_glues`. And do not add a third writer to `fresh_jit_drop_glues`: the two publish gates (`publish_prepared_turn`, `PreparedMacroTurn::publish`) each insert `{artifact, owner}` as ONE value, and a writer that updates them separately would pair an old address with a new JIT.
...
- **The quote shield — int's expander is quote-blind by shield, not by accident** (`design/int/quote-shield.md`). `expand_scoped`'s non-empty `Sexp::List` arm opens with two guard clauses (Rule Q / Rule QQ) that hold the reader-quote family out of Pass-1 expansion, so quoted DATA reaches the frontend fold (`build_forms`/`build_form`) intact and is desugared there, never macro-expanded first (the corruption `(defn f [] '(m x))` would otherwise suffer). `(quote X)` is held fully verbatim (no descent); a `quasiquote` body is walked by `shield_qq`, which holds everything verbatim EXCEPT the body of a LIVE `unquote`/`unquote-splicing` (re-entered through `expand_scoped` — ordinary expression position, §9.4.2), tracking quasiquote nesting depth that mirrors the frontend `expand_qq_template`/`expand_qq_list` math EXACTLY. The family is recognized STRUCTURALLY (bare-symbol head + `len() == 2`) — the SAME test the fold uses (`quasiquote.rs::is_quote`/`is_quasiquote`), consulting neither `shadows` nor the resolver — so shield and fold stay in lockstep; if the two tests diverge a subtree gets double-desugared or expanded-then-desugared. **Both int-side scope-aware walks carry this shield, from ONE classifier** (S115, FIXME 0718 / `expansion-qualification-scope.md` §2.4): `expander::quote_head` is the single structural test, called by `expand_scoped`/`shield_qq` AND by the qualify walk's twin (`process_form::macro_resolution::qualify_scoped`/`qualify_shield_qq`) — a symbol inside quoted data is not a reference, so qualifying `'(name)` to `'(dm/name)` would change a runtime VALUE. Do not add a second private copy of the test in either walk. (The frontend fold keeps its own crate-private copy — collapsing all three is FIXME 0789, `target: /arch`.)

- **The qualify walk's binder model is the expander's, seeded identically.** `qualify_scoped` and `expand_scoped` share ONE binder enumeration (`expander::is_binding_form`/`params_scope`/`pattern_binders`/`is_annotation_symbol`), and both seed the **defn NAME** into its own body scope (§2.5) so a first-definition self-call `(defn f [x] (f x))` is never qualified to a defining module's `f`. `defmacro`/`defmacro-` stays OUT of `is_binding_form` (that predicate gates dispatchers whose arms do not cover it); the qualify walk shields it explicitly via `expander::is_defmacro_head`, routing to `qualify_defn` — same slots (head, name, docstring, param bracket(s) verbatim; clause bodies qualified under the param scope), no second shape (§2.6).
