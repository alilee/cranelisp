...
pub enum Expr {
    ...
    Match {
        ...
        arms: Vec<MatchArm>,
        span: Span,
        /// Compiler-generated matches (e.g. macro param destructuring) skip exhaustiveness checking.
    ...
    /// Pure execution tracing: evaluates body while recording function call boundaries.
    /// Returns a Trace ADT. Module list is empty to trace the current module.
    Trace {
        modules: Vec<String>,
        body: Box<Expr>,
        span: Span,
    },
    /// Fold over all test-* functions in loaded .test modules.
    /// Each test is traced (full GOT-swap). pass_fn is called for passing tests,
    /// fail_fn for failing tests. Returns the final accumulator.
    ///
    /// pass_fn :: (Fn [:a :String :Int] :a)       -- acc, test-name, nanos
    /// fail_fn :: (Fn [:a :String :Int :String :Trace] :a) -- acc, test-name, nanos, reason, trace
    RunTests {
        modules: Vec<String>,
        init: Box<Expr>,
        pass_fn: Box<Expr>,
...
pub struct TraitMethodSig {
    ...
    /// Named parameters for default body (empty if no default).
    pub default_param_names: Vec<String>,
    /// Raw Sexp body for default implementation (None if no default).
    ...
    pub fn format_tree(&self, indent: usize) -> String {
        ...
        match self {
            ...
            Expr::ParLet { bindings, body, .. } => {
                let mut lines = vec![format!("{}ParLet", prefix)];
                for (name, val) in bindings {
                    lines.push(format!("{}  {} =", prefix, name));
                    lines.push(val.format_tree(indent + 4));
                }
            ...
            Expr::ParBind { bindings, body, .. } => {
                let mut lines = vec![format!("{}ParBind", prefix)];
                for (name, val) in bindings {
                    lines.push(format!("{}  {} =", prefix, name));
                    lines.push(val.format_tree(indent + 4));
                }
            ...
            Expr::Trace { modules, body, .. } => {
                ...
                let mut lines = vec![format!("{}Trace [{}]", prefix, mods)];
                lines.push(body.format_tree(indent + 2));
                lines.join("\n")
            }
            Expr::RunTests { modules, init, pass_fn, fail_fn, .. } => {
                let mods = if modules.is_empty() {
                    "all".to_string()
                } else {
                    modules.join(", ")
                };
                let mut lines = vec![format!("{}RunTests [{}]", prefix, mods)];
                lines.push(init.format_tree(indent + 2));
                lines.push(pass_fn.format_tree(indent + 2));
                lines.push(fail_fn.format_tree(indent + 2));
                lines.join("\n")
            }
        }
    }
...
/// Mangle a trait impl method name: `Trait.method$target`.
/// e.g., `mangle_impl_method("Num", "+", "Int")` → `"Num.+$Int"`
pub fn mangle_impl_method(trait_name: &str, method_name: &str, target: &str) -> String {
    format!("{}.{}${}", trait_name, method_name, target)
}

/// Extract impl method Defns with mangled names (e.g., `Display.show$Int`).
    ...
    for ti in trait_impls(program) {
        ...
        for method in &ti.methods {
            result.push(Defn {
                ...
                name: mangle_impl_method(&ti.trait_name, &method.name, &target),
                docstring: None,
