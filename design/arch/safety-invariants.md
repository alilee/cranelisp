...
| R11 | **Concreteness at codegen** — no `Type::Var` reaches RC-classify / slot emission / mangle. **TARGET INVARIANT (S119 re-ruling, `/arch` 2026-07-28, user-directed — `design/arch/total-concreteness.md` §2): universal and kind-free.** I-CONC: for every entry, `callable_got_slot().is_some() ⇒ scheme.ty.is_concrete()` — whole-table, no kind licences (the S84 ⟺ restored as stated; reverse direction stays behaviourally enforced — a missed instance is a loud missing-slot failure, never a silent fallback). I-FRAME: every frame backend compiles and every site it emits is fully concrete; `defined_symbols()` admits no non-concrete scheme. I-ABI: the only surviving polymorphic callables are hand-written slot-less by-name runtime bodies on a closed pinned roster (`bind`, `race`, `select`, `catch-runtime-error`), each with declared representation dependencies. *(I-ABI re-ruled 2026-07-28 → **I-EMIT**, `design/arch/concreteness-types-first.md` §1: the emitted tree references NO polymorphic callable — `bind`/`race`/`select` re-kind inline, `catch-runtime-error` per-instantiation concrete facades; the roster demotes to the backend-interior realization contract. The witness-mint mechanism below is superseded-by-strengthening: the S120 form is the types-owned `CallableSlot` + `mint_callable_slot` + `CtorState`, ONE schema window with the ctor tranche, and the R6 cache re-check arm `CacheStale::NonConcreteSlot`.)* **Transitional state at HEAD (the 2026-07-27 kind-partition, demoted to record):** `UserFn` ⇒ `is_concrete()` MUST hold now (R-3's impossibility proof — concreteness is the ONLY licence for a type-directed-RC body); generic `Constructor` templates (incl. `IO.Bind`) and `vec-len` (the ONE polymorphic `Primitive{Extern}`) still hold slots under representation-contingent licences (I-CT′; declared contract) that retire at the S120 tranche (ctor monomorphisation + template-slot retirement; `vec-len` de-slot). **Factual correction of the 2026-07-27 text:** `bind`/`catch-runtime-error` are NOT slotted — slot-less `PrimitiveExtern`, by-name (FIXME 0360); the transitional slotted-polymorphic population is ctors + `vec-len` + the two hand-mints, census at `total-concreteness.md` §1. **Why the S84 phrasing did damage:** stated universally over a table with sanctioned exceptions it was unassertable and was asserted nowhere; the two unsanctioned `UserFn` mints hid in the shadow for 35 sprints. The re-ruling eliminates the exceptions instead of partitioning the invariant | **REGRADED `example-tested` (S119 Phase-3 gate)** — the prior `unconstructable` claim was a property of `finalize.rs`'s determination points that two hand-mint sites bypass: `adt.rs:618-637` (synthetic accessors — `Concrete { got_slot }` over `∀a. (Fn [(Bx a)] a)`) and `traits/impl_check.rs:1043,1078-1090` (`scheme::mono` over a residual `fn_type`). Measured: 2,497 release admissions + 5,499 category licences censused, two reproduced SIGSEGVs at the 1023/1024 boundary (`non-concrete-release-contract.md` §2). The class survived S84→S119 exactly because the invariant was one function's property, not a structural gate — and because its stated form was unassertable (see the invariant cell) | **P-1, the universal slot gate** (`non-concrete-producer-obligations.md` §2.1/§6.1 CS-1): ONE mint helper — the only way to obtain a slot in typecheck — restores the `UserFn` clause; the 0926 unit gate cell (polymorphic accessor ⇒ `Polymorphic`, slot-less) is the standing proof; NC-1 (`tests/plan/s119-test-plan.md` §3.7) reverts to the **universal** predicate with the transitional populations as intentional REDs against their staged retirements (FIXME 0930 routes the correction to `/qa`); backend's R17 census is the consequence-side detector until it reads zero. **S120 structural completion (`/arch`-ruled 2026-07-27; extended by the re-ruling — `total-concreteness.md` §5.2: ctor monomorphisation + template-slot retirement [FIXME 0931, forces the shared schema window], `vec-len` de-slot + I-ABI roster pin [FIXME 0932], platform-sig `Type::Var` refusal [FIXME 0933]; S121+: `Bind` payload-glue self-description [FIXME 0934]):** (i) a **types-owned witness mint** for `UserFnState::Concrete` — a fallible constructor checking `is_concrete()` at the crate boundary, so a fourth mint site outside typecheck's helper is also checked (payload privatisation assessed then, gated on a grep confirming reads are accessor-mediated per `callable_got_slot()` discipline; serde shape unchanged ⇒ NO schema bump); (ii) **R6 load-boundary validation**: a restored `Concrete{slot}` `UserFn` entry re-checks `is_concrete()` at the cache seam → diagnosed `CacheStale` — the durable warm-cache guard beyond the one-time schema window (serde bypasses any smart constructor; the trust boundary must re-check) | `/dev`(typecheck) CS-1; `/dev`(backend) census; `/arch` S120 witness mint |
