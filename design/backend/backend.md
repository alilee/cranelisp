...
#### 2.7.2 The typed-carrier consumer flip (S114 Track A — backend-local specifics)

The 0653 prong-3 carrier change (`Option<FQSymbol> resolved_target` → the two closed sums `VarRef::Local{binder,binding_span} | VarRef::Global(FQSymbol)` and `ApplyRef::Dispatch(FQSymbol) | ApplyRef::ViaCallee`) is a co-member of the ONE atomic Track-A flip wave (types + typecheck producer + backend consumer + `CACHE_SCHEMA_VERSION` 21→22). **The cross-crate design governs — `design/arch/typed-resolution-carrier.md` §4** (the pinned flip diff); this section records only the backend-local seam map so `/dev`(backend) has its checklist. `/arch` owns the types diff; `/design`(typecheck) owns the producer totality; the backend is a **consumer flip** — the `Option`-consults become exhaustive match arms.

**Backend-local flip specifics (the consumer half of §4):**

- **Exhaustive matches replace the `Option` + `variables`-consult convention** at the per-site inventory (`backend-keyed-consumer.md` §3): `compiler/apply.rs` (12 refs incl. S25), `literals.rs`, `fn_as_value.rs`, `match_codegen.rs`. Where a site today reads `resolved_target: Option<FQSymbol>` and falls back to a `variables`/scope consult on `None`, it becomes a match on `VarRef`/`ApplyRef`:
  - `VarRef::Local{binder, binding_span}` → the scope-stack read (the slot authority stays backend-side per §4(a) — the carrier ships binder IDENTITY, not a slot). **A scope-stack miss is now a HARD invariant failure carrying the binder identity in the message** (not a silent codegen error) — the `Local` constructor asserts the producer looked and found a local, so a backend miss is a producer/backend contract breach (Principle 18), not an unresolved reference.
  - `VarRef::Global(FQSymbol)` / `ApplyRef::Dispatch(FQSymbol)` → the existing `entry_at` keyed fetch (§2.7 end-state, unchanged — the value is `storage_fq()`).
  - `ApplyRef::ViaCallee` → the computed-callee / HOF path (the callee `Var`'s own `VarRef` governs, or a closure value) — a POSITIVE verdict, so its arm is a real branch, not a default.
- **`is_self_call` keys on `VarRef::Global`** (§2.7.1 fp1): the fp1 carrier-keyed shape becomes `callee VarRef::Global(fq) == FQSymbol{module: current_module, symbol: current_fn_name}` — the enclosing fn's storage FQ. The S25 TCO keyed read (`backend-keyed-consumer.md` §3, `compile_tail_self_call`) stays keyed; the flip only re-types the compared value from `Option<FQSymbol>` to the `VarRef::Global` arm. fp2 (SigDispatch mangled-name) is untouched (it reads `resolved_call`, not the `VarRef`).
- **No soft fallback** (Rev-2, `backend-keyed-consumer.md` §1.2): a `Local` whose binder is absent from the scope stack, or a `Global`/`Dispatch` whose `entry_at` fetches nothing, is a hard `CodegenError` — never a re-resolve. The closed sums make "unresolved" unconstructable at the boundary (the located `ViewBuildError::Unresolved` is raised at `from_expr` in typecheck, never reaching the backend).
- **Unit obligations** (`s114-test-plan.md` §3.4 items 3–4, `/dev`): `VarRef::Local.binding_span` = the binding FORM's span per binder kind (param/`let`/match-arm); a `VarRef::Local` scope-stack miss = hard invariant failure with the binder identity; `is_self_call` keys on `VarRef::Global == current fn storage FQ` (S25 stays keyed).
- **File-overlap coordination:** `match_codegen.rs` is touched by BOTH this consumer flip AND the binding-indirection consume contract's R3 (`binding-indirection-consume.md` §7) — serialize the two `/dev` change-sets (shared-tree race), no semantic dependency.
...
| Topic | File | Status |
|---|---|---|
| **The non-concrete release contract (S119 Spine 1)** | **`non-concrete-release-contract.md`** | **Live (NORMATIVE RULING, S119 Phase 3; measured before binding).** The class contract for releasing a value whose static type codegen cannot fully see. Three rules — **R-1** category before operation (a residual type variable has NO heap category; the `NULLARY_TAG_THRESHOLD` guard discriminates tags from pointers, never scalars from pointers, so no RC op is legal on such a word); **R-2** no fabricated concreteness, binding on producers in *both* crates (backend's `Err ⇒ HeapCategory::Mixed`, backend's type-keyed shallow-dec arm, typecheck's `lenient_from_expr` → `ConcreteType::Int`); **R-3** a non-concrete frame is not a legal codegen target, with the proof (§4.3) that counting crashes on scalars and not-counting UAFs on duplication, so monomorphisation is the only sound disposition. Carries the **five-face disposition table** (ctor template → canonical glue at the caller, pair deletes under I-CT′; accessor + trait instance → canonical glue after monomorphisation; IO existential `Bind` → runtime-directed teardown via the intrinsics tag-walker with backend owning only `Pure`'s payload; lenient-view root → canonical glue after the producer stops fabricating), the **S119 measurement record** (§2: two censuses, the +16 frame-key falsification re-run, and the new finding that the accessor family is **memory-unsafe** at the same 1023/1024 boundary as 0916), FIXME 0917's separate provenance ruling (§6, `NoReference` as the lattice's bottom), the staging table (§7), and the `/review` reject list (§8). **Supersedes `transitive-drop-glue.md` §4.1**, which defers here. |
...
| Binding-indirection consume contract (S114, 0668) | `binding-indirection-consume.md` | **Live (DESIGN, pre-implementation)**. The 0668 design-iteration anchor — ONE consume-position × operand-provenance contract (the structural alias-forwarding classifier + R1 alias-binding recognition / R2 consume-inc / R3 forwarding-suppresses-dec), the family matrix (A/E landed; G/F/B/C-off/I-1-capture RED), the F4 boundary (does NOT absorb B-2 analysis-ON — typecheck), the 5-item dev-wave map, and the sibling Track-B leak REDs F-R1/MS-P8 (fix-shape hypotheses + backend-vs-intrinsics discriminators). **§2 records the whole-match approximation (S115, FIXME 0697): `match_forwards_scrutinee` is a static ANY-var-arm predicate + merge-block R3 suppression → mixed ctor+var match leaks the temp scrutinee on the ctor path (leak-safe; /qa tripwire FIXME 0726).** **S118: the per-arm dec placement is no longer parked** — `transitive-drop-glue.md` §5/§5.1 + §7.3 own it (the release-gate use of `match_forwards_scrutinee` is deleted at `match_codegen.rs:149`; the predicate itself survives for `operand_live_binding_root`'s provenance trace). Cited from `ownership-codegen.md` §13.7 banner + §2.7.2 below (file-overlap coordination) |
| S115 carrier evidence + RC sweep (S115) | `s115-carrier-and-rc-sweep.md` | **Live (DESIGN, S115 Phase 3)**. The `/arch`-required GOT-slot carrier-state dumps (§1: 0705 = carrier CORRECT `ViaCallee`+`VarRef::Local` → backend totality; the `'='` fn-as-value face = carrier WRONG `Dispatch(prelude/=)` slot-less → typecheck producer — the pair does NOT collapse); the ONE RC-release sweep (§2: entry-`main` heap-payload leak both toggles `protect_return_value`; 0720 ADT-wrapped superseded loop-param `flush_superseded_heap_params_before_tail_jump`, isolated to the flush not the match-consume); the 0705 curry-the-local-closure totality arm (§3); the R4 mangle-family injectivity census (§4: `got_data_symbol_name` `.`→`_` flatten the ONE backend OWED-witness); the R6 persisted-index validation seam (§5, for /dev(backend, cache)); W-B5 confirmation (§6); 0696/0697 dispositions (§7) |
...
| Transitive drop glue + displacement (S116; **refreshed S118**) | `transitive-drop-glue.md` | **Live (DESIGN, implementation-ready)**. One named/per-concrete type-directed drop function, declaration-first finite construction for recursive types, compilation-local identity registry, identical JIT/object/cache behavior, and no depth cutoff or shallow fallback. Owns the common release mechanism used by match-owned scrutinees, explicit and compiler-synthesised capture environments, Vec/ADT fields, and the one TCO replacement/transfer predicate. **S118 refresh** adds: the §1.1 five-mechanism census (the migration must also absorb `vec_codegen`'s second named-glue identity home and `apply::emit_post_call_decs`); §3.4's as-built reconciliation D1–D9 (the registry holds `&mut M` and `finish()`es before body compilation, so it cannot reach a consumer as shaped — the reshape is the enabling slice); §5.1's 0782 single-owner ruling (the var-pattern binder borrows; the per-arm plan is the sole release); §6.1's four TCO fragments to fold; §7's serial slices S0–S6 with seams and flips; §8's exact atomic-deletion symbol list under S118 arch ruling 10; §9's armed-detector acceptance mapping. Includes the binding no-interim list and the refreshed submodule × complexity/edge/negative unit matrix. |
