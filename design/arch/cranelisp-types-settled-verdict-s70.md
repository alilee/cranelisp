...
| `CheckError`, `CheckResult`, `CheckState`, `SymbolTableAccess`, `SymbolTableRead`, `SymbolTableMut`, `TypeCheckEnv`, `ReplSnapshot` | typecheck facade + int facade consumed | No (in cranelisp-typecheck) | OK — typecheck-owned per typecheck facade §"Types originated here" + single-pair invariant (S72 W2 — `ClusterRead`/`ClusterWrite` renamed to `SymbolTableRead`/`SymbolTableMut`; parallel interior pair retired; post-S72 facade-coherence pass — `ClusterContext` renamed `SymbolTableAccess`) |
