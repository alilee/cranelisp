...
1. The `primitives` synthetic module's `SymbolTable` carries a `ModuleEntry::Def { kind: DefKind::Primitive, got_slot: Some(slot), code: Some(Code::Primitive), … }` entry per primitive (seeded by `cranelisp-primitives` at static-init via `PRIMITIVES_TABLE`; Arc-cloned into the session's `SymbolTables` map at session init; see `bounded-contexts.md` §4a + the `cranelisp-primitives` crate-root `//!` rustdoc; `facades/primitives.md` retired S74 Wave 3). `DefKind::Primitive` is the payload-free unit variant (S69 Submission 36 retired the prior `primitive_kind`/`jit_name` payload); primitive-ness is read from `kind`. `Code::Primitive` is the marker variant for process-static lifecycle per Decision 0048 (A2, revised S68 Phase 3). (FIXME 0244 reverses A2 — primitives entries become `code: None` and `Code::Primitive` retires; the `code`-side cascade across this facade is 0244's, tracked separately.)
