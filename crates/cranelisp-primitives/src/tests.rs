...
fn extern_shims() -> std::collections::HashMap<&'static str, *const u8> {
    ...
    declarations::harvest_shims(&rows)
}

#[test]
fn primitives_table_contains_every_user_declaration() {
    for row in declarations::declarations() {
        let name = match row {
            declarations::PrimitiveDecl::UserExtern { name, .. }
            | declarations::PrimitiveDecl::UserInline { name, .. } => name,
            declarations::PrimitiveDecl::HarvestExtern { .. } => continue,
        };
        assert!(
...
#[test]
fn content_harness_declarations_round_trip() {
    let mut count = 0usize;
    for row in declarations::declarations() {
        let (name, scheme, param_names) = match row {
            declarations::PrimitiveDecl::UserExtern {
                name,
                scheme,
                param_names,
                ..
            }
            | declarations::PrimitiveDecl::UserInline {
                name,
                scheme,
                param_names,
                ..
            } => (name, scheme, param_names),
            declarations::PrimitiveDecl::HarvestExtern { .. } => continue,
        };
        let params: Vec<&str> = param_names.iter().map(|p| p.as_ref()).collect();
        assert_content_row(name, &scheme.ty, &params);
        count += 1;
    }
    assert!(count >= 30, "expected >=30 declaration rows, got {count}");
}
...
fn static_slab_slots_populated_after_force() {
    ...
    assert_eq!(
        via_slab,
        *extern_shims().get("add-i64").unwrap(),
...
fn built_table_entries_carry_the_expected_declared_facts() {
    ...
    let neq_string = PRIMITIVES_TABLE
        .get("neq-string")
        .unwrap()
        .mode_summary()
        .unwrap();
