...
| `/review` | cranelisp-frontend | Verify one producer/enforcement seam, second-occurrence spans, pattern/definition mirrors, and no macro-specific fold | **IMPORTANT 2026-07-22 — checkpoint may not commit yet.** The prior preservation Blocker is resolved statically: annotation-internal comments are retained in source order immediately before the completed node at top level and in both list/bracket readers (including multiple comments), normal `parse` discards them, and dangling-subject negatives remain located at the introducer. Stale colon-atom/sibling-scanning documentation is removed. The previously passing structural checks remain clean: one recursive `Sexp::Annotated` producer; universal form/macro positions; malformed/dangling handling; quote/quasiquote preservation; one normalized constructor parser; second-occurrence constructor/field uniqueness; definition/pattern mirrors; one raw `UnresolvedTraitMethodSig` tail with no frontend classification; obsolete code paths removed; submodule complexity/edge/negative coverage. Remaining finding: unrelated formatting-only reflows persist in pre-existing test code (for example `reader/tests.rs`'s float assertion and several unchanged `ast_builder/tests.rs` call layouts), so the frozen diff does not yet satisfy the no-formatting-churn gate. No source tests run (static review only). |
