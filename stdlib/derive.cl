...
;;
;; Ported from sketch/lib/core/derive.cl. Uses primitives directly (no
;; prelude dependency) since this module is compiled outside the prelude graph.
;;
          ...
          param-sexps (smap (fn [p]
            (let [colon-p (str-concat ":" p)]
              (if (scontains? colon-p constrained)
                ...
                (slist (SexpSym p)))))
            params)]
      (SexpList (SCons (SexpSym name) (sfold sconcat SNil param-sexps))))))
  ...
  (match remaining
    [SNil acc
     ...
     (let [name-sym (SexpSym (ctor-name ctor))
       ...
       (build-ord-enum-lt-go all-names len rest (add-i64 idx 1) (sconcat acc arm-pair)))]))
    ...
    [SNil acc
     ...
     (if (ctor-nullary? ctor)
       (let [name-sym (SexpSym (ctor-name ctor))
             ...
             inner-arms (sconcat later (SCons (SexpSym "_") (SCons `false SNil)))
             arm-pair (SCons name-sym (SCons `(match b ~(SexpBracket inner-arms)) SNil))]
         (build-ord-sum-lt-go all-names len rest (add-i64 idx 1) (sconcat acc arm-pair)))
       (let [name (ctor-name ctor)
             n (ctor-field-count ctor)
             abinds (make-bindings "__da" n)
             bbinds (make-bindings "__db" n)
