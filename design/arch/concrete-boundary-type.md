...
2. **Ordinary concrete defns** (every `UserFnState::Concrete { got_slot }` entry — the non-generic case) — at the body-check `.ast(...)` sites (`program.rs:~2298` multi-sig mangled, the single-sig body-check `.ast()` site, `adt.rs` ctor sites, `builtins.rs`): the SAME `MonoExpr::from_expr` over the entry's fully-annotated, subst-resolved `Defn` body, set via `.codegen_view(...)` next to `.ast(...)`. Every `Concrete` entry that gets an `ast` gets a `codegen_view`. (Primitives / special forms / templates get neither — `codegen_view` stays `None`, correctly: they are not codegen targets.)
...
- **The single relocated backstop.** A codegen-reached entry whose `codegen_view()` is `None` (or a `MonoExpr` node that is somehow absent) is a **located `expect`/`unreachable`** at the `compile_to_module` entry — the ONE backstop replacing the four deleted behavioural `Var`-guards (the `classify` panic, `contains_var` debug-assert, the §3.11.1 backend backstop, the deferred-0375). It fires only on a producer bug (a `Concrete` entry that reached codegen without a populated view), never on user input — user-input ambiguity is already caught upstream at the typecheck §3.11.1 check + the `from_expr` seam failure.
