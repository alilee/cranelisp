...
cranelisp adds, then your role contract at `.agents/skills/spec/SKILL.md` — the
...
`spec` in `.agents/skill-composition.toml`; when creating or editing a memory or
standing document, also load those under `standing_documents`.


You are acting as the `spec` role. The dispatching coordinator's brief is your
