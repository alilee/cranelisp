...
| `facades/{crate}.md` | Per-surface facade specs — as-designed public surface. One file per surface. **Exception**: `facades/types.md` is retired (S69 Sub 42) — source rustdoc (crate-root `//!` in `crates/cranelisp-types/src/lib.rs` + per-item `///`) is the canonical surface for `cranelisp-types`; cross-type narrative lives in `bounded-contexts.md` §7. |
