...
fn into_owned_consuming_does_not_inc_on_wrap() {
    ...
    assert_eq!(
        read_rc(base),
        1,
        "starting rc = 1 (caller's transferred ref)"
    );
...
fn clio_effect_on_resource_with_capacity_additive_public_api_edge() {
    ...
    let _sibling: fn(i64, fn() -> CLInt) -> CLIO<CLInt> = CLIO::<CLInt>::effect_on_resource;
}
...
fn extract_layout_hash_reads_header() {
    ...
    assert_eq!(
        extract_layout_hash(";; layout-hash:   abc123  \n"),
        "abc123"
    );
}
...
fn t24_host_context_not_default_compile_fence() {
    ...
    let path = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("public-api.txt");
...
fn t25_null_alloc_with_tag_panic_message_contract() {
    ...
    let src =
        std::fs::read_to_string(&path).expect("can read crates/cranelisp-platform/src/lib.rs");
...
fn t27_host_callbacks_carries_new_fn_pointer_fields() {
    extern "C" fn dummy_alloc(_size: i64) -> i64 {
        0
    }
    let cb = HostCallbacks {
        alloc: dummy_alloc,
...
fn descriptor_scheduling_class(class_discriminant: u32) -> SchedulingClass {
    ...
    let func = PlatformFn {
        ...
        param_name_count: 0,
        concurrency: ConcurrencyDescriptor::from_scheduling_class(SchedulingClass::from_u32(
            class_discriminant,
        )),
    };
    let funcs = [func];
...
fn resource_serial_token_lands_on_effect_node() {
    ...
    let (tag, tok, default_tok) = unsafe {
        ...
        let default_tok = *((base0 + HEAP_HEADER_SIZE + IO_EFFECT_RESOURCE_OFFSET) as *const i64);
        (tag, tok, default_tok)
    };
...
fn manifest_to_descriptors_lifts_poll_shape_descriptor() {
    ...
    let (pname_s, pver_s, descs) = unsafe { manifest_to_descriptors(&manifest) }.expect("lift");
