...
The **backend** translates symbol-table entries into Cranelift IR and produces compilation artefacts: in-memory machine code for direct execution, object files for linking, and a cache pair (metadata plus object) for re-use across sessions. There is one compilation entry point regardless of mode; "in-memory vs object" is a property of the Cranelift module supplied to it, not a parameter on the entry point. The backend is a **pure keyed-lookup consumer** of typecheck's resolutions (Principle 24 "Resolve once"; BC §3 invariant 10): every name and type it consumes arrives as a resolved fully-qualified value on the mono view, read by direct keyed fetch with a hard error on miss — the backend performs no name resolution and no bare-type-name resolution of its own (landed S110 W0–W3 — the resolver seam is deleted; migration record: `design/arch/backend-keyed-consumer.md`).
...
- **Runtime cadence** is the running-program cadence — it executes as part of the running program, hosted across `cranelisp-primitives`, `cranelisp-intrinsics` (which hosts the `(trace ...)` execution-trace runtime per the S76 ruling — see `design/arch/tracing.md`), and (when IO-observation is enabled) the `src/io_trace.rs` consumer side. Atomic reference counting interleaved with normal execution; fork-join scopes during parallel evaluation; the IO trampoline reducing effect trees. Invisible outside the running program — it produces no handoffs to the other cadences.
