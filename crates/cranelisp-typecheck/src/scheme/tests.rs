    ...
    fn test_instantiate_leaves_unquantified_free_var_untouched() {
        ...
        let ty = instantiate(&s, &mut next_id);
        assert_eq!(ty, Type::Var(7), "unquantified free var must not be freshened");
    ...
    fn test_generalize_all_vars_in_env_is_mono() {
        ...
        let ty = Type::Fn(vec![Type::Var(0)], Box::new(Type::Var(1)));
        let scheme = generalize(&subst, &ty, &env_fv);
        assert!(scheme.type_vars.is_empty(), "no var may be quantified — all are in env");
        assert_eq!(scheme.ty, Type::Fn(vec![Type::Var(0)], Box::new(Type::Var(1))));
    }
    ...
    fn test_generalize_quantified_vars_are_sorted() {
        ...
        let ty = Type::Fn(
            vec![Type::Var(2), Type::Var(0)],
            Box::new(Type::Var(1)),
        );
        let scheme = generalize(&subst, &ty, &env_fv);
        assert_eq!(scheme.type_vars, vec![0, 1, 2], "quantified vars must be sorted");
    }
