...
fn infer_lambda_teardown_is_symmetric_on_body_error() {
    ...
    assert!(
        result.is_err(),
        "an undefined body var must make infer_lambda error"
    );
...
fn test_infer_apply_type_mismatch_int_add_float() {
    ...
    assert!(
        ...
        "add-i64 with float args should fail"
    );
}
...
fn test_infer_apply_auto_curry() {
    ...
    let ty = tc
        ...
        .expect("auto-curry should succeed");
...
fn test_infer_match_wildcard() {
    ...
    let mut expr = Expr::Match {
        ...
        arms: vec![
            ...
            MatchArm {
                pattern: Pattern::Wildcard { span: span(18, 19) },
                body: Expr::IntLit {
                    value: 0,
                    span: span(20, 21),
                    inferred_type: None,
...
fn test_resolve_primitive_non_primitive() {
    ...
    assert!(
        result.is_none(),
        "special forms should not resolve as primitives"
    );
}
...
fn test_resolve_primitive_qualified_non_primitive() {
    ...
    assert!(
        result.is_none(),
        "constructors should not resolve as primitives"
    );
}
...
fn test_resolve_primitive_extern_classifies_as_builtin() {
    ...
    assert_eq!(
        tc.resolve_primitive_jit_name_self("macros/sconcat")
            .as_deref(),
        Some("sconcat"),
...
fn register_constrained_fn(tc: &mut TestFixture) {
    ...
    let scheme = Scheme {
        ...
        constraints: {
            ...
            c.insert(
                a_id,
                vec![cranelisp_types::FQTraitName::new(
...
fn test_trait_method_plus_bool_error() {
    ...
    assert!(
        err.message()
            .contains("no impl of trait test/Num for type primitives/Bool"),
...
fn test_trait_method_plus_string_error() {
    ...
    assert!(
        err.message()
            .contains("no impl of trait test/Num for type primitives/String"),
...
fn test_trait_method_lt_bool_error() {
    ...
    assert!(
        err.message()
            .contains("no impl of trait test/Ord for type primitives/Bool"),
...
fn test_trait_method_lt_string_error() {
    ...
    assert!(
        err.message()
            .contains("no impl of trait test/Ord for type primitives/String"),
...
fn test_trait_method_plus_int_succeeds() {
    ...
    match resolution {
        ...
        _ => panic!(
            "expected BuiltinFn resolution (primitive trait-method short-circuit per FIXME 0185), got {resolution:?}"
        ),
    }
}
...
fn resolves_to_carrier_identity_discriminates_local_shadow() {
    ...
    tc.state
        .env
        .bind(Symbol::from("+"), crate::scheme::mono(Type::Int));
    assert!(
        !tc.state.resolves_to_carrier_identity("+"),
...
fn register_eq_and_display_traits(tc: &mut TestFixture) {
    ...
    let eq_decl = crate::traits::test_helpers::parse_trait_decl("(deftrait Eq (= [lhs rhs] Bool))");
    ...
    tc.register_trait_impl_self(&mk_eq_impl("Int", "eq-i64"))
        .unwrap();
    tc.register_trait_impl_self(&mk_eq_impl("String", "str-eq"))
        .unwrap();

    // Display trait: show :: (Fn [a] String)
    let display_decl =
        crate::traits::test_helpers::parse_trait_decl("(deftrait Display (show [x] String))");
...
fn let_bound_method_call(method: &str, binding_span: Span, args: Vec<Expr>) -> (Expr, Span) {
    ...
    let let_expr = Expr::Let {
        bindings: vec![(
            Symbol::from("f"),
            Expr::var(Symbol::from(method), binding_span),
        )],
        body: Box::new(f_call),
...
fn value_position_local_shadow_of_trait_method_does_not_miscompile() {
    ...
    let inner_let = Expr::Let {
        bindings: vec![(
            Symbol::from("g"),
            Expr::var(Symbol::from("show"), show_value_span),
        )],
...
fn trace_returns_trace_type_int_body() {
    ...
    assert_eq!(
        ty,
        trace_type(),
        "trace of an Int-bodied call must infer as Trace"
    );
}
...
fn par_bind_unwraps_io_bindings_body_uses_names_unwrapped() {
    ...
    let mut expr = par_bind(
        vec![
            (
                Symbol::from("a"),
                Expr::var(Symbol::from("act1"), span(10, 14)),
            ),
            (
                Symbol::from("b"),
                Expr::var(Symbol::from("act2"), span(15, 19)),
            ),
        ],
        body,
    );
...
fn par_bind_body_misusing_name_as_io_is_type_error() {
    ...
    tc.bind_local_self(
        ...
        mono(Type::Fn(
            vec![io_type(Type::Int)],
            Box::new(io_type(Type::Int)),
        )),
    );
