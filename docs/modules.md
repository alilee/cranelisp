...
> For project-level configuration of search paths — including local directories, environment variables, and remote HTTP repositories — see [Deployment Environment](deployment-environment.md). When a `cranelisp.toml` file is present, its `search-path` replaces the three-tier heuristic above for steps beyond the project root.
