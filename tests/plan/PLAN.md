...
| `e2e.rs` | 2701 | 309 | 70 | 0 | 30 | 5 | `spec_03_types.rs` (S64 W5 ✓), `spec_04_expressions.rs` (S64 W5 ✓), `spec_05_definitions.rs` (S64 W5 ✓), `spec_07_traits.rs` (S64 W5 ✓), `spec_appendix_a_builtins.rs` (S64 W5 ✓), `legacy/e2e.rs` (S64 W5 ✓ FIXME 0134) | /int (with /frontend, /typecheck, /backend) | S64 Wave 5 Batch 2 landed: language-conformance assertions consolidated into the 8 spec-section files via REPL-canonical authoring. Source preserved in legacy (heavy use of `compile_and_run_simple/with_macros/repl_session*` Rust-API helpers) for harvest under FIXME 0134 (consolidated with ring0/1/2). Spec-anchored re-authoring naturally deduplicates against ring0/1/2 + sketch_port. |
...
| `macros.rs` | 441 | 58 | 80 | 20 | 0 | 5 | `spec_09_macros.rs` (S64 W5 ✓), `legacy/macros.rs` (S64 W5 ✓ FIXME 0137) | /frontend (with /typecheck — macro pipeline internals) | S64 Wave 5 Batch 3 landed: 11 e2e tests in `tests/spec_09_macros.rs` (defmacro, multi-clause, quasiquote, begin, errors, persistence, REPL display). Macro-expansion-shape Rust-API tests + symbol-table inspection preserved in legacy for /frontend harvest under FIXME 0137. |
| `modules.rs` | 530 | 39 | 60 | 40 | 0 | 5 | `spec_08_modules.rs` (S64 W5 ✓), `legacy/modules.rs` (S64 W5 ✓ FIXME 0138) | /frontend (with /int — discover_module_graph internals) | S64 Wave 5 Batch 3 landed: 9 e2e tests in `tests/spec_08_modules.rs` (import specific + glob, qualified names, visibility, primitives synthetic module, non-existent name error, cycle detection, super-rejection, local shadowing). `discover_module_graph` Rust-API tests + multi-dot path edge cases preserved in legacy for /frontend harvest under FIXME 0138. |
...
| `ring0.rs` | 1135 | 216 | 60 | 0 | 40 | 5 | `spec_03_types.rs` (S64 W5 ✓), `spec_04_expressions.rs` (S64 W5 ✓), `spec_05_definitions.rs` (S64 W5 ✓), `spec_appendix_a_builtins.rs` (S64 W5 ✓), `legacy/ring0.rs` (S64 W5 ✓ FIXME 0134) | /typecheck (with /backend, /int — consolidated under FIXME 0134) | S64 Wave 5 Batch 2 landed: full quarantine. Ring 0 spec-anchored coverage absorbed into the 8 new spec-section e2e files via REPL-canonical authoring. Source preserved in legacy for unit-tier harvest. |
| `ring1.rs` | 2253 | 380 | 65 | 0 | 35 | 5 | `spec_05_definitions.rs` (S64 W5 ✓), `spec_06_pattern_matching.rs` (S64 W5 ✓), `spec_07_traits.rs` (S64 W5 ✓), `spec_08_modules.rs` (S64 W5 ✓), `spec_09_macros.rs` (S64 W5 ✓), `spec_appendix_a_builtins.rs` (S64 W5 ✓), `legacy/ring1.rs` (S64 W5 ✓ FIXME 0134) | /typecheck (with /backend, /int — consolidated under FIXME 0134) | S64 Wave 5 Batch 2 landed: full quarantine. Ring 1 spec-anchored coverage absorbed into spec-section e2e files. Source preserved in legacy for unit-tier harvest. |
| `ring2.rs` | 2484 | 405 | 70 | 0 | 30 | 5 | `spec_05_definitions.rs` (S64 W5 ✓), `spec_06_pattern_matching.rs` (S64 W5 ✓), `spec_07_traits.rs` (S64 W5 ✓), `spec_08_modules.rs` (S64 W5 ✓), `legacy/ring2.rs` (S64 W5 ✓ FIXME 0134) | /typecheck (with /backend, /int — consolidated under FIXME 0134) | S64 Wave 5 Batch 2 landed: full quarantine. Ring 2 ADT + trait coverage absorbed into spec-section e2e files. Source preserved in legacy for unit-tier harvest. |
...
| `sketch_port.rs` | 1886 | 296 | 50 | 0 | 50 | 30 | `spec_03_types.rs` thru `spec_appendix_a_builtins.rs` (distributed via Wave 5; previous waves W3/W4 absorbed io/runtime parts), `legacy/sketch_port.rs` (S64 W5 ✓ FIXME 0136) | /qa (test-shape harvest; mostly self-resolved) | S64 Wave 5 Batch 2 landed: full quarantine. Spec-anchored coverage carried forward across Waves 3/4/5; sketch-port duplicated ring0/1/2 coverage extensively. Pre-existing failure cluster historical at this point — recommended deletion at S65 alongside ReplSession removal. FIXME 0136 covers /qa-internal harvest verification. |
