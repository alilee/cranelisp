...
**Status: TARGET ARCHITECTURE (user-ratified direction 2026-06-16) — a dedicated arc run within S84.** **Reconciled to the tightened §3.11.1 (commit `2290aa9`, /spec; suite realigned `3fedb6b`, /qa): the check is FULL CONCRETENESS — no representation-based exemption.** `(Vec a)`/`(Fn a)`/`(Option a)`/`[]` are ALL §3.11.1 errors when unpinned at a codegen-reaching value position. The `ConcreteType` boundary type already encodes this (`from_type` fails on any `Var`, no `Vec`/`Fn` carve-out); the typecheck-side §3.11.1 verdict switches from the now-WRONG `is_representation_undetermined()` predicate to `!is_concrete()` (full concreteness) — they agree by construction (§1.4, §3.1). `is_representation_undetermined()` is RETIRED, gated on /dev flipping the §3.11.1 call site (FIXME 0386). Phase 1 (`ConcreteType` + the fallible conversion) LANDED in `crates/cranelisp-types/src/concrete.rs` (commit `5b3319c`). **Phase 2a LANDED (S84, /arch):** the §2.4 decision is settled (`MonoExpr` — parallel codegen AST) and the `MonoExpr`/`MonoDefnVariant` representation + `MonoExpr::from_expr` builder + unit tests landed in `crates/cranelisp-types/src/mono_expr.rs` (produces-but-unused; cache bumped 6 → 7). **Phase 2b LANDED (S84, /dev(typecheck)):** the mono-population seam built `MonoExpr` at `traits.rs:~1479` with an interim `allowed_vars` carve-out admitting scheme-quantified body vars. **Phase 4 DETAILED + RE-SEQUENCED before Phase 3 (S84, /arch, 2026-06-16):** §4 now splits Phase 4 into (A) mono-completeness (root-caused: a spurious partial instance is minted from the generic-caller recursion — `traits.rs::monomorphise_inner_parametric_hops` over-eagerly mints on the bare-var-result trigger with non-concrete args; the fix SUPPRESSES that mint, not "completes" it), (B) generic-body-codegen elimination (two filter sites — `defined_symbols()` + `derive_codegen_batch::try_push` — exclude `Polymorphic`), (C) the 0344 reconciliation. New sequence: Phase 1 → 2 → 4 → 3 → 5. Phases 3+5 follow. This document is the standing reference for that arc.

**Phase 3 THREADING SHAPE SETTLED + LANDED (S84, /arch, 2026-06-17):** the central P3 boundary decision — how the backend gets a concrete-typed body per codegen-bound entry — is ruled **option (a), additive field**: `ModuleEntry::Def` gains `codegen_view: Option<MonoDefnVariant>` ALONGSIDE the existing `ast: Option<DefnVariant>` (NOT a type-swap of `ast`, which would cascade a wide break; NOT a separate codegen structure that `compile_to_module` takes). The backend's Phase-3 read path consumes `MonoDefnVariant.body: MonoExpr` whose every node's `ty` is a `ConcreteType` — **no `Type`, no `Var`, by construction**. The `cranelisp-types` shape (`codegen_view` field + `ModuleEntry::codegen_view()` accessor + `DefBuilder::codegen_view()` setter), the `public-api.txt` regen (3 additive lines), and the `CACHE_SCHEMA_VERSION` 7 → 8 bump LANDED; the build stays green (the additive field defaults `None`, all literal-construction sites carry `codegen_view: None`). What POPULATES the view (the typecheck seam — mono instances + concrete defns) is /dev(typecheck)+/dev(backend) per §4 Phase 3 below + FIXMEs 0391/0392. See §4 Phase 3 for the landed shape and the ~13-site backend consumption map. **The view's totality scope + the backstop scope are SETTLED (S84 /arch, 2026-06-17, FIXME 0393 RESOLVED + §3.1.1):** `codegen_view` is total over `DefKind::UserFn { Concrete{slot} }` entries (defns + mono instances) ONLY — the body-AST-node-typed codegen targets; `DefKind::Constructor` ctor/accessor codegen is signature-driven (reads field `Type`s from the `scheme` → `from_type` → `classify`, never a body node's type) and legitimately carries `codegen_view: None`, so the relocated `compile_to_module` backstop is scoped to the `Concrete{slot}` `UserFn` arm and never trips on ctors/accessors/primitives.
...
### 1.4 The check is FULL CONCRETENESS — no representation exemption (reconciled to the tightened §3.11.1)

**The spec tightening (commit `2290aa9`, 2026-06-16) removed the representation-based exemption.** §3.11.1 now reads: *"The strictness is full concreteness — no type variable — NOT machine-shape determinacy. There is no representation-based exemption."* `(Vec a)`, `(Fn [a] a)`, `(Option a)`, and the empty vec-literal `[]` are **all type errors** when their variable is unpinned at a codegen-reaching value position — even when their machine shape is determinate. This is the same property `ConcreteType` already encodes: `from_type` fails on **any** `Var` (it has never had a `Vec`/`Fn` exemption), so the typecheck-side check and the boundary type **agree by construction — both are full concreteness.**

The two predicates after this arc lands:

- **`Type::is_concrete()` is the operative verdict — KEEPS, re-expressed via the conversion.** It becomes definitionally `ConcreteType::from_type(self).is_ok()`. It survives as (a) the GOT-slot-eligibility predicate at the typecheck slot gate (operating on `Type`, pre-conversion) AND (b) **the new §3.11.1 verdict**: a codegen-reaching value whose resolved type fails `is_concrete()` (equivalently `from_type(..).is_err()`) is the ambiguity error. The same predicate that gates the slot gates the §3.11.1 check — one full-concreteness verdict (Principle 7).

- **`Type::is_representation_undetermined()` is the WRONG predicate — RETIRED (gated on /dev).** It embodies the now-rejected representation-determinacy notion: it returns `false` for `(Vec a)`/`(Fn a)` ("uniformly heap, admit the unpinned var"), which the tightened spec now **rejects**. It must NOT be the §3.11.1 verdict. The §3.11.1 check must reject ANY residual free var at a codegen-reaching value position — i.e. `!is_concrete()` / `from_type(..).is_err()`, not `is_representation_undetermined()`. The predicate's retirement from `cranelisp-types` is **gated on /dev switching the §3.11.1 call site** (`program.rs:1595`, `is_codegen_ambiguous_type`) from `is_representation_undetermined()` to the full-concreteness verdict; until then the predicate is **kept-but-deprecated** (its rustdoc carries the retirement note; removing it now breaks the typecheck build, which still calls it). See §3 and FIXME 0386. The whole FIXME-0379 representation-undetermined backend backstop is *unnecessary at the boundary* regardless: `classify` takes `ConcreteType`, which has no `Var`.

- **`Type::contains_var()`'s `debug_assert!` callers are subsumed** — the boundary type no longer admits a `Var` to assert against.

**Why full-concreteness and the boundary type agree by construction.** `ConcreteType::from_type` and `is_concrete()` are the same verdict over `Type` (the unit test `from_type_agrees_with_is_concrete` pins this). Neither has a `Vec`/`Fn` carve-out. So the typecheck-side §3.11.1 check (on `Type`, via `is_concrete()`) and the backend boundary (`ConcreteType`, no `Var`) reject *exactly the same set* — there is no gap where a value admitted by the check would fail at the boundary, nor vice versa. This is the reconciliation the tightening forces: under the old representation-determinacy framing the two could disagree (the check admitted `(Vec a)`; the boundary type rejected it). Full concreteness collapses them to one verdict.
...
| **§3.11.1 position-complete typecheck check** (Wave 2, FIXME 0379, commit `9569536`) | **VERDICT CHANGES to full-concreteness, THEN subsumed by the conversion.** Under the old (now-rejected) framing the scan called `is_representation_undetermined()` — which admits `(Vec a)`/`(Fn a)`. The tightened §3.11.1 (commit `2290aa9`) requires rejecting ANY residual free var, so the check's verdict **must switch to `!is_concrete()` / `ConcreteType::from_type(..).is_err()`** (the full-concreteness verdict — see §3.11.1 check spec below). This is /dev's change (FIXME 0386), not a removal of the check: the position-complete scan *machinery* (`find_ambiguous_value_position` walking every codegen-reaching value position via `for_each_child_expr`) stays; only its per-node verdict flips from the representation-undetermined predicate to the concreteness predicate. The same change **removes the `(is-some None)` direct-constructor skip** (`expr_is_direct_constructor_value`) — see below. Once Phase 3 lands, the standalone scan is *subsumed* by `MonoExpr::from_expr` (the conversion is the choke point); until then it is the load-bearing interim guard. |
| **`Type::is_representation_undetermined()`** (Wave 2 predicate) | **WRONG predicate under the tightened spec — RETIRED, gated on /dev.** It returns `false` for `(Vec a)`/`(Fn a)`, admitting them — but the tightened §3.11.1 REJECTS them. It is NOT the §3.11.1 verdict (that is `!is_concrete()`). Retirement from `cranelisp-types` is gated on /dev switching the §3.11.1 call site off it (FIXME 0386); KEPT-but-deprecated until then (its rustdoc carries the retirement note; the typecheck build still calls it, so removing it now breaks the build). No other use — keep it for nothing; it retires the moment /dev flips the one call site. |
...
**Net:** the arc KEEPS the slot gate, `is_concrete()`, `Polymorphic`, and Wave 1b; **changes the §3.11.1 check's verdict to full-concreteness** (`!is_concrete()`, dropping the representation-determinacy framing) and removes the `(is-some None)` direct-constructor skip; **RETIRES `is_representation_undetermined()`** (gated on /dev flipping the §3.11.1 call site); SUBSUMES the §3.11.1 check into the conversion at Phase 3; SUPERSEDES/DROPS the 0375 backstop and the FIXME-0381 deferred state by making the illegal input unconstructable; folds 0375 in (it does not re-arm — it deletes).

### 3.1 The §3.11.1 check's new full-concreteness verdict (spec for /dev — FIXME 0386)

The position-complete scan (`crates/cranelisp-typecheck/src/program.rs`, `find_ambiguous_value_position` / `is_codegen_ambiguous_type` at `:1594`) currently rejects a codegen-reaching value iff `is_representation_undetermined()` AND the ADT is `Mixed`-shaped. Under the tightened §3.11.1 it must reject a codegen-reaching value whose **resolved type is not fully concrete** — ANY `Var`, no representation exemption, no `Mixed`-shape gate. The change for /dev:

1. **Replace the verdict.** `is_codegen_ambiguous_type(state, ty)` returns `true` iff `!ty.is_concrete()` (equivalently `ConcreteType::from_type(ty).is_err()`). This rejects `(Vec a)`, `(Fn [a] a)`, `(Option a)`, a bare `Type::Var`, a `TyConApp` head — any residual free var — at a codegen-reaching value position. The `adt_type_is_mixed_shape` helper and its `Mixed`-gate become **dead** (delete them — they were the representation-determinacy refinement the tightening rejects). The bare-`Var` / `TyConApp` `_ => false` arm (which previously *excluded* bare vars as "transient/local") is removed — under full concreteness a bare var at a genuinely codegen-reaching value position is the ambiguity error too. (The callee-position carve-out — `callee_span`, the dispatch position that legitimately carries a transient var — STAYS: it is the §4.2 "dispatch not value" exclusion, not a representation exemption.)

2. **Remove the `(is-some None)` direct-constructor skip.** The `&& !self.expr_is_direct_constructor_value(state, child)` guard (`program.rs:1755`) was a FIXME-0382 carve-out: it admitted a direct-constructor value (`None`, `(Some x)`) because its *constructor* (hence tag-vs-pointer representation) was statically known even with a free var. That is a **representation-determinacy** argument — exactly what the tightening rejects. Under full concreteness, `(is-some None)` with `None : (Option a)` unpinned **is** the ambiguity error (the spec's own worked example, §3.11.1: *"`(is-some None)` … is ambiguous → error. Fix by annotating the constructor concrete: `(is-some :(Option Int) None)`."*). **/dev removes the `expr_is_direct_constructor_value` guard + the method** so the direct-constructor `None` is rejected with the clean §3.11.1 ambiguity error instead of slipping past to the downstream "undefined function: is-some" codegen error the ledger records. The acceptance guard `mono_is_some_unannotated_none_rejected_neg` flips green on this removal.

3. **Predicate retirement coupling.** Once step 1 lands, the `is_representation_undetermined()` call at `:1595` is gone — /dev then files (or /arch actions) the predicate's removal from `cranelisp-types` + the `public-api.txt` removal line. The §3.11.1-check change and the predicate retirement are one logical change-set on /dev's side (FIXME 0386); the `cranelisp-types` removal is /arch's to action once the call site is dead.

**Confirm-by-construction: the new verdict matches the boundary type.** Both are full concreteness. `is_codegen_ambiguous_type(ty) == !ty.is_concrete()`; the boundary admits `ty` iff `ConcreteType::from_type(ty).is_ok() == ty.is_concrete()`. They are the same verdict — the check rejects exactly what the boundary type cannot represent, no gap.

**Acceptance set (the 5 failing-first guards /qa committed in `3fedb6b`, `tests/regression.rs`):**
- `mono_vec_free_var_value_rejected_neg` — unpinned `(Vec a)` value rejected (premise INVERTED by the tightening; was `..._admitted_pos`). Green when the verdict drops the `Vec` exemption.
- `mono_fn_free_var_value_rejected_neg` — unpinned `(Fn [a] a)` polymorphic-function value rejected. Green when the verdict drops the `Fn` exemption.
- `mono_is_some_unannotated_none_rejected_neg` — `(is-some None)` unannotated rejected with a clean ambiguity error. Green when the direct-constructor skip is removed (step 2).
- `mono_vec_empty_annotation_pins_and_compiles_pos` — `:(Vec Int) []` annotation pins-and-compiles (the **escape** — gated on FIXME 0385, see §5).
- `mono_bare_annotated_value_pins_and_compiles_pos` — bare `:(Vec Int)`-annotated value pins-and-compiles (the Vec leg is gated on FIXME 0385).

**Definition-admit + REPL-display dispositions preserved.** The check fires ONLY at codegen-reaching value positions (the `for_each_child_expr` value-producing children, excluding the callee dispatch position). It does NOT fire for: a named polymorphic *definition* with result-only free vars (§3.11.3 disposition 1 — slot-less, sound, dead-for-codegen until a concrete use mints an instance — `(defn empty [] [])` is admitted); a *template* (the `Polymorphic`/`Constrained` arm is a mono source, never type-checked-as-a-value at its definition); a *bare polymorphic value at the REPL* (§3.11.2 disposition 3 — introspection display, never reaches the codegen-value check). The full-concreteness verdict narrows nothing about *where* the check fires — only *what verdict* it returns at the positions it already visits. Dispositions 1 and 3 stay admitted because they are not codegen-reaching value positions, exactly as before the tightening.
...
### Phase 3 — backend consumes `MonoExpr`/`ConcreteType`; `classify` loses the `Var` arm

#### 3.0 The threading shape — SETTLED + LANDED (`cranelisp-types`, /arch, 2026-06-17)

**Ruled: option (a) — an ADDITIVE `codegen_view: Option<MonoDefnVariant>` field on `ModuleEntry::Def`, alongside the existing `ast: Option<DefnVariant>`.** This is the central P3 boundary decision: the backend must read a concrete view (`MonoExpr` carrying `ConcreteType`) for every codegen-bound entry, with NO `Type`/`inferred_type` on its read path (the §2.4 structural commitment). The candidates and the ruling:

- **(a) `ModuleEntry::Def` gains `codegen_view: Option<MonoDefnVariant>` (CHOSEN).** The mono pass / body-check populates it; the backend reads `MonoDefnVariant.body: MonoExpr`, whose every node's `ty` is a `ConcreteType`. Carried **alongside** `ast` (not replacing it) so the transitional read path is intact: the backend keeps reading `ast` until /dev(backend) flips `compile_to_module` to `codegen_view`, then `ast`'s codegen role retires. **Why additive, not a type-swap of `ast`:** swapping `ast: Option<DefnVariant>` → `Option<MonoDefnVariant>` would break ~26 literal-construction sites + the backend's `Defn`-reconstruction path simultaneously (a wide, non-green change); the additive field defaults `None`, keeps the build green, and lets /dev migrate the read path incrementally (the explicit brief instruction when a type-swap cascades).
- **(b) A separate codegen-bound structure `compile_to_module` takes (REJECTED).** Threading a parallel `Vec<MonoDefnVariant>` argument through `compile_to_module` + the scheduler + the cache would re-introduce the P2b transitional parallel-`Vec` shape at the crate boundary (the very thing this move retires). The view belongs ON the entry (single source of truth, Principle 7) — the symbol table is already the per-symbol codegen-input carrier (`ast`, `kind`, `got_slot`); `codegen_view` is its concrete-boundary sibling.

**The landed `cranelisp-types` surface** (`crates/cranelisp-types/src/module.rs`):
- `ModuleEntry::Def.codegen_view: Option<MonoDefnVariant>` — `#[serde(default)]`, participates in the cached `.meta.json` shape (no pointer/`C` state).
- `ModuleEntry::codegen_view(&self) -> Option<&MonoDefnVariant>` — the backend's read-through accessor (mirrors the direct `ast` destructure; `None` for non-`Def` and non-codegen entries).
- `DefBuilder::codegen_view(self, MonoDefnVariant) -> Self` — the populate setter for the typecheck seam.
- `into_concrete` carries the field forward; `build()` defaults it `None`.
- `public-api.txt`: 3 additive lines. `CACHE_SCHEMA_VERSION` **7 → 8** (the serialized `ModuleEntry::Def` surface changed). Both LANDED.

**What populates `codegen_view` — BOTH cases (spec for /dev; see §4 Phase 3 below):**
1. **Monomorphised instances** (`name$Args`) — at the mono-population seam (`traits.rs::register_mono_entry`, ~`:1609`): the `MonoDefnVariant` already built at `:1511` (currently pushed to the transitional `CheckState.mono_variants` parallel `Vec`) is instead handed to the entry via `builder.codegen_view(mono_variant)`. The parallel `Vec` retires.
2. **Ordinary concrete defns** (every `DefKind::UserFn { fn_state: UserFnState::Concrete { got_slot } }` entry — the non-generic case) — at the body-check `.ast(...)` sites (`program.rs:~2298` multi-sig mangled, the single-sig body-check `.ast()` site, `builtins.rs`): the SAME `MonoExpr::from_expr` over the entry's fully-annotated, subst-resolved `Defn` body, set via `.codegen_view(...)` next to `.ast(...)`. Every `Concrete{slot}` `UserFn` entry that gets an `ast` gets a `codegen_view`. **The view is total over `Concrete{slot}` `UserFn` entries ONLY (defns + mono instances), NOT over the whole `defined_symbols()` set** (FIXME 0393 resolution, §3.1.1): `DefKind::Constructor` ctor/accessor bodies are `inferred_type: None` by design (backend reads field types from the signature, not node types — `from_expr` cannot and need not build a view; `codegen_view` stays `None`, correctly); primitives / platform-effects / special forms / templates likewise get `None` (not body-AST codegen targets). The §3.1 backstop is scoped to the `Concrete{slot}` `UserFn` arm accordingly — see §3.1.1 for the partition predicate, the ctor/accessor signature-path `from_type` conversion, and the `$Var`-variant disposition.

#### 3.1 The backend consumption map (design for /dev(backend) — FIXME 0391)

- **Crates:** `cranelisp-backend` (the read sites below), `cranelisp-runtime`/`-intrinsics` only if a signature crosses (it does not — `classify` is backend-internal).
- **`HeapCategory::classify` signature change.** `crates/cranelisp-backend/src/heap.rs:438`: `classify<C, L>(ty: &Type, …)` → `classify<C, L>(ty: &ConcreteType, …)`. The two non-total arms become **inexpressible and are DELETED** (not converted): `Type::Var(_) => Mixed` (`heap.rs:~490`) and `Type::TyConApp(_, _) => Mixed` (`heap.rs:~492`) — a `ConcreteType` has no `Var`/`TyConApp` variant, so the match is exhaustive over `{Int,Bool,String,Float,Fn,ADT}` with NO catch-all and NO panic case. `classify` is now **total over `ConcreteType`**. The `classify_adt` delegate likewise takes the `ConcreteType::ADT` args. **The already-deleted backend backstop (0375) is NOT re-added** — the deferred-`Mixed`-then-panic comment block (`heap.rs:456–479`) is removed with the arms.
- **The ~13 `inferred_type` read sites → read the concrete view.** Each backend site that today reads `Expr.inferred_type` / `.inferred_type()` reads the corresponding `MonoExpr` node's `ty: &ConcreteType` instead (the codegen walk is now over `MonoExpr`, not `Expr`). The per-site list (file:line, fn, what it reads — the migration is mechanical: `expr.inferred_type()` → `mono_node.ty()`, then the `classify` call takes the `&ConcreteType` directly, no `&Type` deref):

  | File:line | Fn | Reads the type to… |
  |---|---|---|
  | `compiler/apply.rs:877` | `trace_accessor_intrinsic` | check arg is `primitives/Trace` ADT (route to trace ABI) |
  | `compiler/control_flow.rs:78` | `compile_parallel_spawn_body` | record sparkable binding type for RC |
  | `compiler/control_flow.rs:165` | `compile_parallel_bind_continuation` | record IVar binding type for RC |
  | `compiler/control_flow.rs:525` | `compile_effect_continuation` | unwrap `IO(T)` inner for RC scope |
  | `compiler/control_flow.rs:1611` | `compile_auto_curry` | classify each curried arg for RC guards |
  | `compiler/match_codegen.rs:82` | `compile_match` | scrutinee type → `MatchContext` (field extraction) |
  | `compiler/match_codegen.rs:132` | `bind_as_pattern` | record pattern-bound var type for RC cleanup |
  | `compiler/match_codegen.rs:172` | `dec_temporary_scrutinee` | classify scrutinee heap category (temp RC dec) |
  | `compiler/mod.rs:1611` | `protect_return_value` | classify body type (return-value RC inc guard) |
  | `compiler/trace_codegen.rs:664` | `emit_body_discard` | classify trace body type (selective RC dec) |
  | `compiler/vec_codegen.rs:485` | `vec_elem_type` | extract `Vec` element type from the `ADT` args |

  (Plus the ~24 `HeapCategory::classify` CALL sites across `apply.rs`/`control_flow.rs`/`match_codegen.rs`/`mod.rs`/`trace_codegen.rs`/`vec_codegen.rs` — each now passes a `&ConcreteType` it reads off a `MonoExpr` node or derives from a parent `ConcreteType`, never a `&Type`.) **7 backend codegen files** carry the `inferred_type` reads; all migrate to `MonoExpr.ty()`.

- **`compile_to_module` consumes `MonoExpr`.** Today (`lib.rs:660–686`) it destructures `ModuleEntry::Def { ast, .. }`, fails on `ast: None` (`lib.rs:668`), and reconstructs a single-variant `Defn` to walk as `Expr`. Phase 3 flips this to read `entry.codegen_view()` and walk the `MonoDefnVariant.body: MonoExpr`. The codegen dispatch (`compiler/mod.rs::FnCompiler::compile_expr`, the `Expr`-variant match ~`:1100`) becomes a `MonoExpr`-variant match (one-for-one — `MonoExpr` mirrors `Expr`'s 14 non-`Annotate` variants; the `Annotate` arm is gone, erased at build).
- **The single relocated backstop — SCOPED to body-AST-node-typed codegen targets (FIXME 0393 resolution; see §3.1.1).** A codegen-reached entry whose `codegen_view()` is `None` is a **located `expect`/`unreachable`** at the `compile_to_module` entry — the ONE backstop replacing the four deleted behavioural `Var`-guards (the `classify` panic, `contains_var` debug-assert, the §3.11.1 backend backstop, the deferred-0375) — **BUT it applies only to the entries the backend codegen's from a body AST that reads node `inferred_type`/`ty`: `DefKind::UserFn { fn_state: Concrete{slot} }` defns + mono instances.** Ctors/accessors (`DefKind::Constructor`), primitives, and platform-effects are codegen'd by *signature/extern-driven* mechanisms that never read a body node's type — they MUST NOT trip the `expect`. §3.1.1 states the exact partition predicate, the ctor/accessor signature-path `Type → ConcreteType` conversion, and the `$Var` multi-sig disposition. The backstop fires only on a producer bug (a body-AST-codegen target that reached codegen without a populated view), never on user input — user-input ambiguity is already caught upstream at the typecheck §3.11.1 check + the `from_expr` seam failure.
- **The §3.11.1 typecheck check STAYS.** The user-facing position-complete diagnostic (§3.1 of this doc, /typecheck-side) is NOT removed by Phase 3 — it is the user's clean ambiguity error. Phase 3 retires only the *standalone backend scan machinery* (now subsumed by `MonoExpr::from_expr` being the choke point). `is_representation_undetermined()` is already retired before Phase 3 (gated on /dev flipping the §3.11.1 verdict, FIXME 0386).
- **Public-API / BC / cache:** `classify` is backend-internal (no baseline move). `codegen_view` + its accessor + setter are the additive `cranelisp-types` baseline move (LANDED, 3 lines). `CACHE_SCHEMA_VERSION` 7 → 8 (LANDED). BC §3 invariant 9 rewritten (below) — the belt-and-braces two-predicate framing collapses to "the boundary type has no `Var`; `classify` is total over `ConcreteType`."
- **Risk:** MEDIUM — the breadth of read sites; each mechanically moves to `MonoExpr.ty()`. The `#[should_panic]` backstop tests retire.
- **Size:** MEDIUM-LARGE.

#### 3.1.1 Backstop scope + the signature-driven codegen targets (FIXME 0393 resolution — /arch, 2026-06-17)

FIXME 0392 (population, LANDED) surfaced that `codegen_view` is **NOT total** over the `defined_symbols()` codegen-target set: `DefKind::Constructor` ctor/accessor bodies, `$Var` multi-sig variants, and primitives get `codegen_view: None` (their `inferred_type` is `None` by design, so `MonoExpr::from_expr` cannot build a view). Yet all are legitimate `compile_to_module` targets. §3.1's single relocated backstop ("a `None` view at a codegen-reached entry is an `expect`") would therefore fire on a user `deftype`'s ctor the moment Phase 3 flips the read. The resolution scopes the backstop and gives the signature-driven targets their own concrete-type source.

**The partitioning fact (survey-grounded, `crates/cranelisp-backend/src/`).** `compile_to_module` walks a body AST for EVERY `Def` entry uniformly (`lib.rs:660–686` reconstructs a `Defn` from `ast` with no per-`DefKind` dispatch; the body is an `Expr` the `FnCompiler` walks). But *what the walk reads to classify a heap category* splits cleanly into two disjoint mechanisms:

| Codegen mechanism | What it reads for `classify` | `codegen_view` relevance |
|---|---|---|
| **Body-AST node types** — `UserFn { Concrete{slot} }` defns + mono instances | each `Expr`/`MonoExpr` node's `inferred_type`/`ty` (the ~13 read sites, §3.1) | **REQUIRED** — these are the entries the backstop guards |
| **Signature / function params** — ALL `Def` bodies, incl. ctor/accessor | the entry's `scheme` (`Type::Fn(params,_)`) → `variable_types` (`mod.rs:1066–1098`) → `compile_consuming_arg_list`; the match-path via `extract_constructor` reading `scheme` (`mod.rs:756–769`) → `bind_data_pattern_fields` (`match_codegen.rs:452`) | **IRRELEVANT** — never reads a body node's type |

A ctor body (`Expr::ConstrADT` of `Var` fields — `compile_constr_adt`, `apply.rs:784`) and an accessor body (`Expr::Match` over a `Var` scrutinee) are walked, but `compile_constr_adt`/`emit_adt_construct` read **no node `inferred_type`** — they inc field *variables* via `compile_consuming_arg_list` (which reads `variable_types`, sourced from the `scheme` at param binding), and `emit_adt_construct` reads no type at all. So a ctor/accessor never reaches a `MonoExpr.ty()` read; its `MonoExpr` body, if it had one, would carry no load-bearing `ConcreteType`.

**1. The backstop predicate.** The `codegen_view`-must-be-`Some` `expect` applies to an entry iff it is a **body-AST-node-typed codegen target**:

> `matches!(entry.kind(), DefKind::UserFn { fn_state: UserFnState::Concrete { .. } })`
> — i.e. the entries that previously read `.ast`/`Defn` node `inferred_type`s. Mono instances are registered as exactly this kind (`register_mono_entry` builds a `Concrete{got_slot}` `UserFn`), so the predicate covers both the ordinary-concrete-defn case and the mono-instance case (they are the same `DefKind` discriminator — Principle 20: the kind *is* the determinant).
...
**2. Ctor/accessor codegen sources its field types from the signature via `from_type` (no body view needed).** `HeapCategory::classify` under Phase 3 takes `&ConcreteType` (no `Var` arm). At the ctor/accessor sites the backend converts each signature field `Type` → `ConcreteType` via `ConcreteType::from_type`, which **must succeed**: a codegen-reached ctor instance has fully concrete field types — an unpinned ctor field at a value position is a §3.11.1 type error caught upstream (§1.4 / §3.1), so §3.11.1 guarantees concreteness on this path. A `from_type` failure here is therefore a compiler bug — the **relocated `expect`/compiler-bug** (the same family as the body-AST backstop, applied at the signature-conversion site). The sites that take the `Type → ConcreteType` conversion (the signature-read classify calls — these are the ctor/accessor codegen's only type reads):

| File:line | Fn | Reads the field `Type` from… |
|---|---|---|
| `compiler/mod.rs:1066–1098` | function-entry param binding | the entry's `scheme` (`Type::Fn(params,_)`) → `variable_types` (used by `compile_consuming_arg_list`'s `classify`, `apply.rs:~484`) |
| `compiler/mod.rs:756–769` | `extract_constructor` | the `DefKind::Constructor` entry's `scheme` → `CtorField.ty` (the accessor/match field-type source) |
| `compiler/match_codegen.rs:452` | `bind_data_pattern_fields` | the `extract_constructor` field types → `classify(ft, …)` |

Under Phase 3 these `classify` calls take a `&ConcreteType` obtained by `ConcreteType::from_type(field_ty)?`-with-`expect` (the conversion is the choke point; its `Err` is the compiler-bug `expect`, NOT a user error — user-facing ambiguity was rejected upstream). **This keeps "no `Var` reaches `classify`" total across BOTH paths:** the body-AST path (each `MonoExpr.ty()` is already a `ConcreteType`) AND the signature path (each field `Type` converts via `from_type` before reaching `classify`). `classify` is total over `ConcreteType` on every call, from either source.

> **Note for /dev(backend):** the `extract_constructor`/`CtorField` and the param-binding `variable_types` currently carry `Type` (with a `Var` variant). The minimal Phase-3 move is to convert *at the `classify` call site* (`from_type(ty).expect("ctor/accessor field type concrete by §3.11.1 — compiler bug if not")`), not to retype `CtorField`/`variable_types` to `ConcreteType` wholesale (that wider retype is optional cleanup, out of the FIXME-0391 minimum). The `expect` message names §3.11.1 so the located failure is self-explaining.

**3. The `$Var` multi-sig variant — never a legitimate codegen target; a reached one is a bug, not a `None`-tolerated case.** A multi-sig clause with an unconstrained param (`(defn f ([x] x) …)`, mangling to `f$Var`) whose body genuinely carries a `Type::Var` param is **effectively polymorphic** — it is the same shape as a `Polymorphic` template, and under Phase-4 part B (generic-body-codegen elimination, which lands BEFORE Phase 3 in the re-sequenced order) such a slot-less/effectively-generic variant is **excluded from the `defined_symbols()` codegen batch** at both filter sites. So by the time Phase 3 reads `codegen_view`, a free-var `$Var` variant is **not enumerated as a codegen target at all**. The disposition:

- If the `$Var` variant is genuinely polymorphic (unconstrained free param), it is a mono *source*, never a codegen target — Phase-4 part B's filter excludes it (it must be `Polymorphic`/`Constrained`-classed, slot-less, not `Concrete{slot}`). If it is still mis-classed as `Concrete{slot}` after Phase 4, that is a **Phase-4 mono-completeness / slot-gate gap** (the slot gate `is_concrete()` should have denied it a slot — §3 "Slot gate" row), surfaced by the body-AST backstop (1) firing on it. The backstop firing is then *correct* — it has caught a producer bug (a non-concrete entry that wrongly got a `Concrete{slot}`), exactly the §3.11.1/slot-gate violation the structural model forbids.
- It is therefore NOT a `None`-tolerated case: a `Concrete{slot}`-classed entry MUST have a `codegen_view`. The only entries legitimately carrying `None` are the signature-driven kinds (2) + templates/primitives. A `$Var` variant that reaches Phase-3 codegen with a free var is a §3.11.1 gap to fix upstream (slot gate / mono completeness), not a backstop to soften.

**Net resolution.** `codegen_view` is total over **`DefKind::UserFn { Concrete{slot} }`** entries (defns + mono instances) — NOT over the whole `defined_symbols()` set. §3.0's "EVERY `Concrete{slot}` codegen-bound entry ends with `Some`" is exact as written; the gap 0393 named is that `defined_symbols()` *also* yields `Constructor` entries, which are body-AST-walked but signature-typed, so they need no view. The backstop scopes to the `Concrete{slot}` `UserFn` arm; ctors/accessors take the signature-path `from_type` conversion; `$Var` free-var variants are excluded by Phase-4 part B and, if reached, are a slot-gate bug the backstop correctly catches.
...
**Cache-schemes-without-codegen (the P4B generic-only-prelude case) — design for /dev(int)/(backend).** Part B has a corollary the cache layer must absorb: **a prelude/stdlib module whose only def is an uninstantiated generic produces NO codegen artifact** (its `Polymorphic` body is excluded from the `defined_symbols()` batch at both filter sites), **yet its SCHEME must still be cached** so a downstream module can monomorphise it on a later cold-load. Today the cache writes a module's `.meta.json` (schemes, symbol-table shape) **alongside** the `.o` codegen artifact, and the e2e guard `cache_prelude_modules_cached` asserts the prelude's modules round-trip from cache. After part B, a generic-only module has a `.meta.json` to write but **no `.o`** — the cache must not gate `.meta.json` emission on the presence of a codegen object.

**Disposition: this is a cache-layer change, NOT "the prelude always has something to emit."** The empty-`.o` case is real (a module exporting only `(defn id [x] x)` codegens nothing under part B). The cache's `.meta.json` (schemes / `ModuleEntry` shape — the *typecheck output*) is **independent of codegen output** (the `.o`) and must be persisted regardless. Design for /dev:
- `.meta.json` persistence is driven by the **typecheck result** (the module's schemes + symbol-table entries), not by whether `compile_to_module` produced any object. A module that type-checks but codegens nothing still writes its `.meta.json`.
- The `.o` (codegen object) is written iff the module's `defined_symbols()` batch is non-empty (a generic-only module's batch is empty → no `.o`, which is correct — there is nothing concrete to compile).
- On cold-load, a module with a `.meta.json` but no `.o` rehydrates its schemes (so its generics are available as mono *sources*) and emits no code — exactly its session-init behaviour after part B. The downstream module that reaches a concrete instantiation mints + codegens the concrete instance into **its own** `.o`, as it already does for every other on-demand mono instance.
- **`cache_prelude_modules_cached` stays green** because the assertion is about the *schemes/meta* round-tripping, which still happens; the `.o` absence for a generic-only module is the correct new state, not a cache miss. /dev(backend) confirms the cache writer (`crates/cranelisp-backend/src/cache/`) decouples `.meta.json` emission from `.o` emission; /dev(int) confirms the loader tolerates a `.meta.json`-only module (no `.o` to mmap). If the current cache writer *requires* an `.o` to write the meta (a coupling), that coupling is the part-B cache change — split it.

This is filed as a FIXME `target: /dev` (cache decoupling, part of the Phase-4-part-B change-set) — see §6.
...
## 6. The 0385 annotation-resolution coordination (the §3.11.1 rejection's escape)

**The §3.11.1 rejection and the 0385 annotation-resolution fix MUST land together.** The tightened §3.11.1 rejects an unpinned `(Vec a)` value and directs the user to disambiguate with `:(Vec Int) []` (the spec's own worked example). But /qa found (FIXME 0385, `target: /dev`) that **`:(Vec Int)` does not resolve** — the type-annotation type-expression resolver reports `unknown type 'Vec' (from module '')` even when `Vec` is imported. The builtin `Vec` type constructor resolves fine for *inference* of vec literals but is not found by the *annotation type-expr resolver* (the path that resolves `:(T args...)`), which the user-declared parameterised ADTs `Option`/`Box` reach.

**The coupling.** Rejecting `(id [])` without a working `:(Vec Int) []` annotation would leave the user with **no escape** — the ambiguity error fires, the directed remedy fails to compile, and a valid program is unfixable. So part of landing the §3.11.1 verdict change (§3.1) is that the `Vec`-in-annotation gap (0385) must be fixed in the **same wave**. Concretely: the two positive acceptance guards (`mono_vec_empty_annotation_pins_and_compiles_pos`, the Vec leg of `mono_bare_annotated_value_pins_and_compiles_pos`) are RED on the 0385 gap and only flip green when `Vec` resolves in annotation position — they are the join condition. /sprint must schedule the §3.11.1 verdict change (FIXME 0386) and the `Vec`-annotation fix (FIXME 0385) in one wave, not sequentially.

**It is /dev's (type-expr resolution), NOT an arch boundary change — confirmed.** The gap is in the type-annotation type-expression resolver (the same `resolve.rs` seam that handles `:(Option Int)` via `type_def_view_of`, per `crates/cranelisp-typecheck/CLAUDE.md` §"Product-ctor dual facet"). The builtin `Vec` is registered as a `primitives`-module built-in type; the annotation resolver must qualify a bare `Vec` to its `primitives` registration the way the inference path does (the `(from module '')` empty-module signature is the tell — the resolver is not qualifying `Vec`). **No `cranelisp-types`/interface change is required:** `Type::ADT(FQTypeName{module: primitives, name: Vec}, [args])` is the existing boundary representation of an applied `Vec`; the resolver just needs to *produce* it for the annotation `:(Vec Int)`. No new boundary type, no facade/BC shape change. If /dev finds the builtin `Vec` lacks a resolvable `TypeDefInfo`-equivalent that the annotation resolver can reach (a registration gap rather than a resolver gap), that is still a typecheck-internal registration fix — escalate via FIXME `target: /arch` only if it turns out to need a *new cross-crate type*, which the `Type::ADT(Vec, …)` precedent says it does not. FIXME 0385 stays `target: /dev`.
