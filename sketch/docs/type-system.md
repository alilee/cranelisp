...
When a trait declares a method with a default body and an `impl` block does not override it, the typechecker synthesizes a `Defn` from the stored default S-expression. The synthesized defn has a mangled name (e.g. `<=$MyType`), is type-checked with the dispatch parameter pre-unified to the impl target type, and is compiled identically to explicit impl methods. Default methods may call other methods of the same trait. See `docs/spec/07-traits.md` section 7.1.5 for details.

### Derive: Automatic Trait Implementation

The `derive` macro generates trait implementations by structural recursion over a type's constructors. It is implemented as pure library macros in `lib/core/derive.cl` — no compiler support is needed beyond the existing macro system.

Supported traits:

| Trait | Method(s) | Strategy |
|---|---|---|
| `Eq` | `=` | Field-wise equality; nullary constructors compare by tag, data constructors compare fields with `(= a b)` |
| `Ord` | `<`, `>` | Constructor ordering follows declaration order; data constructor fields use lexicographic comparison; `>` is derived as `(< b a)` via default methods |
| `Display` | `show` | Nullary constructors produce their name as a string; data constructors produce `Name(field1 field2 ...)` |

Derive works with all type shapes:

- **Enum types** — all-nullary constructors (e.g. `Color`): tag-based comparison
- **Product types** — single data constructor (e.g. `Point`): field-wise operations
- **Sum types** — mixed nullary and data constructors (e.g. `Option`): constructor discrimination then field-wise

For polymorphic types, derive automatically propagates trait constraints to type parameters that appear in constructor fields. The `dt-constraints-for-trait` function inspects field type annotations and adds the derived trait as a constraint on any type parameter used as a field type:

```clojure
(derive [Eq] (deftype (Option a) None (Some [:a val])))
;; generates: (impl Eq (Option :Eq a) ...)
;; because field :a in Some requires Eq for the generated (= a b) calls
```

This means derived implementations are constrained polymorphic — they monomorphise at call sites just like hand-written constrained functions.
