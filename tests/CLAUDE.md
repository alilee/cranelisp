...
| `plan/s103-test-plan.md` | S103 test plan (Phase 3): increment-II QA-first drafting set (reuse-token L-C3 fence + R5 witness/negative + differential-oracle write-path extension + typecheck-drain e2e envelope + T1 full-cure §18.1.1 acceptance); II-G1–G4 gate plan (F2v witness fixture, measurement lanes, mechanism→gate map); the h3 owed-signal flip criterion (L-D5); FIXME-0499 lane-refactor (L-S1 lands, L-M1 grows with B3); harness gaps needing /design-backend/typecheck/src + /arch. |
| `plan/0488-isolation.md` | 0488 per-signature seam attribution (S102 Wave 2): all three signatures = typecheck-side "never minted" (none backend/B3.1); (a)+(b) mono-derivation reference-shape misses, (c) fold-bodied template scheme over-generalization (root-cause guard landed); unit-test shapes + owner recommendation for the fix dispatch. |
| `plan/s104-utilization-measurement.md` | S104 utilization-thesis measurement plan + acceptance instrument: the core-count-controlled harness (`tests/perf/s104_utilization.py` + `s104_spot.py`), the M-static/M-dynamic config matrix + interaction reads, the `[SPARK_STATS]`/`[SPARK_SITE_STATS]` counters, the discrimination experiment, F5/F6 adequacy, the U-G1–U-G6 gates, and the single-shot order-of-magnitude doctrine (wall stats-OFF, idle out-of-band). |
| `plan/s105-attribution-results.md` | S105 residual-attribution RESULTS + decision-gate verdict (Phase-5 Wave-1): the per-fixture attribution vectors (F4-hard/F3/F7/F8), the per-lever direct-oracle net-recovery table, and the verdict — **F4-hard residual = unavailable-parallelism → accept-done → `--release`** (no memory lever built); F3 carries a large residual-atomic-RC term (NONATOMIC_RC ~76%, sound-cure = 0526/0528) but serial already wins 8×; the F8 gate-5 sub-verdict (stack lever fires only in-frame, never on the parallel/recursive shape); INVALID/UNAVAILABLE rows (I4 HITM UNAVAILABLE — host PMU; walls order-of-magnitude). |
| `plan/s106-test-plan.md` | S106 test plan (Phase 3): the FIXME-burn-down QA-first drafting set — per-item e2e deliverables for the confirmed-defect repros (0541/0542/0546/0548/0551) + behaviour-change guards (0539/0540/0543/0545/0549/0550/0538/0365/0416) + GREEN robustness (0544 non-TTY byte-identical, 0499 L-S1 preamble-grid generalization); the WS-A/read-loop test sequencing (0538/0548/0549 on save.rs; 0544/0551 on the main.rs read loop); the 0496/0498 unit-tier "no /qa e2e owed" dispositions + named /dev obligations; harness gaps (PTY-e2e G-1, agent-lane build-break G-2, /repl+/spec ruling deps G-4/G-5). |
| `plan/s105-residual-attribution.md` | S105 residual-attribution plan (Phase 3): the upgraded instrument (`tests/perf/s105_attribution.py`) + per-fixture attribution vector over the six fidelity instruments; the 2×2 allocator-swap×ownership-off factorial + explicit interaction term `I`; oracle granularity discipline (coarse `NO_OWNERSHIP` ceiling vs fine per-mechanism probes); direct-oracle net-recovery lever selection; the two new fixtures (F7 (a)-isolating, F8 parallel stack-alloc witness with the 0525 gate-5 divergence property); the decision gate + accept-done→`--release` branch; the `STACK_SLOT_HITS` backend-side-read caveat; and the NEW `/dev(cranelisp-backend)` instrumentation list (N1 alloc-bytes, N2 per-branch alloc, N3 per-site residual-atomic RC, N4 fine stack-oracle env gate). |
