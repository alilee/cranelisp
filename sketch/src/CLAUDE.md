...
| File | Responsibility |
|---|---|
| `main.rs` | Entry point: parses `--run`/`--exe`/`--cwd` flags. `cranelisp` = bare REPL, `cranelisp foo.cl` = REPL with file, `cranelisp --run foo.cl` = batch, `cranelisp --run` = batch user.cl, `cranelisp --exe <output> [file.cl]` = standalone executable |
| `lib.rs` | Re-exports all modules |
| `batch.rs` | Batch-mode pipeline: parse, typecheck, compile, run. Deferred per-module cache writes via parallel `CacheWritePacket` processing (rayon `par_iter`) after entire compile loop completes (mono specializations may land in earlier modules). Exports `try_load_cached_module()` shared by batch and REPL |
| `repl/` | Interactive REPL (submodules: `mod.rs` `ReplSession` struct + `run()`/`run_with_file()`/`run_repl_loop()` + `project_root` field + prelude loading + module graph compilation + background cache writes via `CacheWriter` + cascade reload (`find_direct_dependents`/`find_transitive_dependents`) + file watching, `commands.rs` slash command enum + parser, `handlers.rs` `/sig` `/doc` `/type` `/info` and other command handlers, `input.rs` per-`ReplInput` variant handlers + `check_redefn_safety()` type-breaking redefinition guard, `format.rs` type/value/symbol/signature formatting + special form docstrings, `util.rs` string reading + paren balancing) |
...
| `intrinsics.rs` | Language-internal `extern "C"` implementations (`alloc`, `panic`), IO trampoline (`IoTask`, `Continuation` newtypes with `run()` method) |
...
| `cache.rs` | Compiled module cache: SHA-256 source hashing, `.cranelisp-cache/` directory management, `CompiledModule` serialization, `compile_module_to_object()` for ObjectModule `.o` emission, cache manifest read/write, `CacheWritePacket` (Send-safe snapshot), `build_cache_packet()` / `process_cache_packet()` for background/parallel writes, `extract_cache_inputs()` for pre-extracting module data, atomic file writes |
| `cache_writer.rs` | Background cache writer for REPL responsiveness: mpsc channel + background thread, processes `CacheWritePacket`s, writes accumulated manifests on shutdown |
...
- **IO as deferred Task tree**: `IO` is a compiler-seeded ADT with `Pure` (tag=0), `Effect` (tag=1, opaque Rust closure), `Bind` (tag=2, internal) in the `primitives` module. `pure` wraps in `Pure` (`lib/core/io.cl`); `bind` is an inline primitive constructing `Bind` nodes; `do` and `bind!` are prelude macros. Effects are deferred — platform DLLs return `Effect` nodes via `CLIO::effect()`. The trampoline (`IoTask::run()` in `intrinsics.rs`) forces the tree with O(1) stack depth
- **Prelude macros**: `list`, `do`, `bind!`, `vec` are `defmacro`s in the prelude (not parser sugar). `list` expands to nested `Cons`/`Nil`, `bind!` expands to nested `bind`/`fn`, `do` expands to nested `bind` calls (IO-specific)
