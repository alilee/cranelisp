...
### 3.3.4 Rank-1 polymorphic function values are returnable; only rank-2 use is unsupported [S109]

A function may freely **return or contain** another function whose type is **rank-1 polymorphic** — every `∀` sits at the enclosing definition's own boundary (prenex; see [§3.10](#310-rank-1-hindley-milner)). `(defn mkid [] (fn [y] y))`, `const` / `(fn [y] x)`, `compose`, and their **written** twins `(defn mk [] (fn [:b y] y))` and `(defn weird [x] (fn [:b y] x))` are **all accepted**: a written `:b` on a returned lambda's parameter is **not** special — a written free type variable is identical to an inference-generated one (written ≡ unwritten, see [§3.3.1](#331-a-bare-type-variable-is-an-inference-variable-with-a-name)). `(defn mk [] (fn [:b y] y))` has type `∀a. (Fn [] (Fn [a] a))` and is **accepted**; each call `(mk)` instantiates `a` fresh, yielding a single monomorphic function instance (row 10). The MUSTs below are consolidated, alongside what the language does *not* support, in the boundary statement at [§3.5.5](#355-the-polymorphism-boundary).

What Cranelisp does **not** support is **rank-2 / first-class polymorphism** — a `∀` that survives *nested inside* a function type and is then used at two different types. This is rejected **not at the definition** but at the point of misuse, by three mechanisms that already exist in the type system:

1. **Value restriction (a single instance used at two types).** The result of an application is a single monomorphic instance, not generalized. Binding it and using it at more than one type is a unification conflict: `(let [f (mkid)] (pair (f "x") (f 5)))` fails because the one instance `f` cannot be both `(Fn [String] String)` and `(Fn [Int] Int)` (row 18). More generally, `let` bindings are monomorphic (no generalization at `let`; see [§3.4](#34-type-schemes)): a `let`-bound value — application result, variable, or lambda alike — carries a monotype within the binding form, so a single polymorphic instance used at two types is rejected however it is bound. Generalization happens only at the enclosing `defn` boundary — that is why `(defn mk3 [] (let [g (fn [:b y] y)] g))` is accepted: `g` is used once, flows to the `defn` boundary, and generalizes there, not at the `let`.
...
> **MUST (i) — a single polymorphic instance used at two types is rejected (value restriction).** The result of an application bound by `let` MUST NOT be generalized; using that one instance at two distinct types MUST be rejected as a type (unification) conflict. `(let [f (mkid)] (pair (f "x") (f 5)))` MUST be rejected. (Row 18.) [S109]

> **MUST (j) — a polymorphic value applied at two types inside a callee is rejected (rank-2 argument).** A function parameter MUST carry a single monotype for the body-check; a definition whose body applies one parameter at two distinct types MUST be rejected by unification. `(defn apply2 [f] (pair (f "x") (f 5)))` MUST be rejected. This is the [§3.10](#310-rank-1-hindley-milner) "no rank-2 polymorphism" restriction. (Row 19.) [S109]

> **MUST (k) — a result-only variable left unresolved at a codegen-reaching use is the §3.11 ambiguity error, not a rank-1 rejection.** A named definition that returns a value whose free type variable appears only in its result MUST be admitted (sound, code-less until instantiated — [§3.11.3](#3113-a-named-polymorphic-definition-with-result-only-free-variables-is-sound-not-ambiguous), disposition 1). A **use** of such a value that reaches code generation without pinning the variable to a concrete type MUST be rejected as the [§3.11](#311-ambiguous-types) ambiguous-type error, exactly as for an unpinned `[]`/`None` and for return-type dispatch — never as a rejection of the returning definition. [S109]

### 3.3.5 Worked examples [S109]

The following table is normative (EBNF `[:a x]` annotation order; `zed : ∀a. Zeroable a => (Fn [] a)` with `Int` and `Float` impls of `Zeroable`; `nadd : (Fn [a a] a)` — numeric add returns `self`; `mkid = (defn mkid [] (fn [y] y)) : ∀a. (Fn [] (Fn [a] a))`; `pair : (Fn [a b] (Pair a b))`). Each row's verdict is a requirement.
...
| 10 | `(defn mk [] (fn [:b y] y))` (returned) | `∀a. (Fn [] (Fn [a] a))`, **accepted** | rank-1 poly return; written `:b` ≡ unwritten; each `(mk)` instantiates fresh |
...
| 18 | `(let [f (mkid)] (pair (f "x") (f 5)))` | **error** | value restriction: one application-result instance used at two types → unification conflict (MUST i) |
| 19 | `(defn apply2 [f] (pair (f "x") (f 5)))` | **error** | rank-2 argument: param `f` is one monotype, applied at two types (MUST j) |
...
A type scheme is introduced only at a definition boundary (generalization at `defn`, [§3.5.2](#352-two-pass-checking)); `let` bindings carry monotypes. For the full statement of where the language does and does not admit polymorphism, see [§3.5.5](#355-the-polymorphism-boundary).
...
Note: `let` bindings use **monomorphic** schemes — there is **no** generalization at `let`. Generalization occurs only at the enclosing `defn` boundary (and the other generalization boundaries per [§3.5](#35-type-inference-algorithm-w)). This is the **language definition**, not an implementation limitation: monomorphic-`let` is a deliberate — and potentially-movable — boundary of the type system, stated in full at [§3.5.5](#355-the-polymorphism-boundary).
...
### 3.5.5 The polymorphism boundary [S110]

Cranelisp draws a deliberate line through the space of polymorphism: it supports **rank-1 (prenex) polymorphism with generalization only at definition boundaries**, and it does **not** support let-polymorphism or rank-2 / first-class polymorphism. This subsection states that line in full and consolidates requirements that also appear at [§3.3.4](#334-rank-1-polymorphic-function-values-are-returnable-only-rank-2-use-is-unsupported), [§3.4](#34-type-schemes), [§3.10](#310-rank-1-hindley-milner), and [§3.11](#311-ambiguous-types). It **changes no behaviour** — it names the existing boundary as one thing and states it unambiguously.

**The boundary is a language definition, not an implementation limitation.** Everything below is a property of *the language*, decided deliberately. The line is drawn where it is because rank-1 + monomorphic-`let` is the precondition under which every reachable function instance has fully concrete parameter and result types after monomorphisation-from-roots ([§3.10](#310-rank-1-hindley-milner)) — the property that lets value representation stay a backend-internal detail ([§12.1](12-runtime.md#121-value-representation)). The boundary **MAY be shifted in a future language revision** — for example by adopting let-generalisation, which would relax the monomorphic-`let` line below — **if** a concrete scenario shows the boundary limiting real programs. Until such a scenario arises the capability question is deliberately **parked**: the boundary is movable, but it is not currently scheduled to move. The point of stating it here is that the spec says it **one way** — where §3.5.3's `let`-rule note previously hedged monomorphic-`let` as "the current implementation," it is here the normative language definition.

**Supported under the spec (the current boundary).**

- **Generalization at definition boundaries.** A top-level `defn` (and the other generalization boundaries per [§3.5](#35-type-inference-algorithm-w)) generalizes its inferred type into a scheme (see [§3.5.2](#352-two-pass-checking)). This is the sole site at which a type scheme is introduced.
- **Instantiation at every use.** Each reference to a polymorphic name instantiates its scheme with fresh unification variables (the Variable Reference rule in [§3.5.3](#353-inference-rules); [§3.10](#310-rank-1-hindley-milner)); distinct uses receive independent instantiations. This is the sole source of polymorphism in the language.
- **Rank-1 (prenex) polymorphism.** Universal quantification appears only at the outermost level of a scheme ([§3.4](#34-type-schemes), [§3.10](#310-rank-1-hindley-milner)) — never nested inside a function parameter, an ADT field, or any other interior position of a type.
- **Returning or containing a rank-1 polymorphic function value.** A definition may **return or contain** a function whose type is rank-1 polymorphic — every `∀` at the returning definition's own boundary — and a *written* parameter annotation on the returned lambda is identical to an inference-generated variable (written ≡ unwritten). `(defn mk [] (fn [:b y] y))` of scheme `∀a. (Fn [] (Fn [a] a))` is accepted; each call `(mk)` instantiates `a` fresh ([§3.3.4](#334-rank-1-polymorphic-function-values-are-returnable-only-rank-2-use-is-unsupported) MUST (f), row 10).
- **Concrete-type ascription / context resolves return-type-polymorphic dispatch.** A concrete type annotation, or a disambiguating surrounding context, in value position resolves an otherwise-ambiguous return-type-polymorphic form ([§3.3.3](#333-a-value-position-annotation-is-a-check-or-a-resolution-not-an-abstraction) MUST (d), rows 13–15).
- **A result-only-variable definition is sound, code-less until instantiated.** A named definition whose free type variables appear **only in its result** is a legitimate polymorphic scheme — sound, and dead for code generation until a concrete use pins it ([§3.11.3](#3113-a-named-polymorphic-definition-with-result-only-free-variables-is-sound-not-ambiguous); [§3.3.4](#334-rank-1-polymorphic-function-values-are-returnable-only-rank-2-use-is-unsupported) MUST (k); disposition 1 of [§3.11.4](#3114-the-three-dispositions-are-exhaustive-and-consistent)).

**Decisively NOT supported (the boundary line).**

- **Let-polymorphism / let-generalisation.** `let` bindings use **monomorphic** schemes; there is **no** generalization at `let`. A `let`-bound value — application result, variable, or lambda alike — carries a monotype within the binding form, so a single polymorphic instance used at two types is rejected however it is bound. Generalization happens only at the enclosing `defn` boundary; that is why `(defn mk3 [] (let [g (fn [:b y] y)] g))` is accepted (`g` is used once, flows to the `defn` boundary, and generalizes there — not at the `let`) while `(let [f (mkid)] (pair (f "x") (f 5)))` is rejected ([§3.3.4](#334-rank-1-polymorphic-function-values-are-returnable-only-rank-2-use-is-unsupported) MUST (i), row 18). This is the line the former §3.5.3 "current implementation" note stated as tentative and this subsection states as normative.
- **Rank-2 / first-class polymorphism.** A `∀` that survives nested inside a function type and is then used at two different types is unsupported, by two mechanisms already in the type system: a polymorphic *instance* bound as a value and used at two types (value restriction — [§3.3.4](#334-rank-1-polymorphic-function-values-are-returnable-only-rank-2-use-is-unsupported) MUST (i), row 18), and a polymorphic value passed as an argument and applied at two types inside the callee (rank-2 argument — [§3.3.4](#334-rank-1-polymorphic-function-values-are-returnable-only-rank-2-use-is-unsupported) MUST (j), row 19; [§3.10](#310-rank-1-hindley-milner)).
- **A result-only variable at a codegen-reaching use, left unpinned.** Admitting the *definition* (above) does not admit an unpinned *use*: a use that forces code generation with the result variable unpinned is the [§3.11](#311-ambiguous-types) ambiguity error ([§3.3.4](#334-rank-1-polymorphic-function-values-are-returnable-only-rank-2-use-is-unsupported) MUST (k); disposition 2 of [§3.11.4](#3114-the-three-dispositions-are-exhaustive-and-consistent)). Full let-generalisation would lift some such cases; under this boundary that is explicitly the road **not taken**, not a deferred obligation.
- **First-class constrained values / constrained closures.** A constrained function has no concrete monotype until its type variables are pinned at a call site, so it cannot occupy value position ([§3.6.6](#366-restrictions); the constrained-polymorphism corollary of [§3.10](#310-rank-1-hindley-milner)).
...
This admitting-the-definition / rejecting-the-unpinned-use pair sits on the polymorphism boundary consolidated at [§3.5.5](#355-the-polymorphism-boundary): the result-only-variable definition is supported; a result-only variable at a codegen-reaching use, left unpinned, is decisively not.
