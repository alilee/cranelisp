...
| 0494 | → **/backend** (`apply.rs::compile_poll_effect`) | open (site CONFIRMED, fix owed) | **A** — **ROOT CAUSE CONFIRMED** (`349ddc0`, layout-neutral stale-*dec* oracle `CRANELISP_RC_DEC_CHECK` + async-signal-safe backtrace). `conn` (tag-0 size-32 Connection ADT) is **RC-dec'd after free** on the launched-strand teardown; both free + stale-dec JIT-emitted. Site: `compile_poll_effect` (~1049-1055) bakes `send-conn` leaf args **"no inc — consuming conv"** while `build_poll_state_drop_glue` **unconditionally decs each** — balanced for a temporary (`make-resp req`), a **double-free for a borrowed var owned elsewhere** (`conn`, captured by the enclosing `(fn [req]…)`; `sleep`'s nested `(fn [_]…)` re-captures it). "Keep-alive shifted a UAF into a double-free." **Fix = borrowed-vs-owned arg RC discipline: inc borrowed leaf args at bake (drop-glue dec then balances).** Verify with the oracle → flip both guards + un-ignore `exemplar_web`. Refuted en route: untracked-buffer write, vec-COW store, marshaling, Rust-path stale-dec. |
