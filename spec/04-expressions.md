...
Each parameter name is a **local binder** and MUST be a **bare (unqualified) symbol** (§5, *Binder positions*); a qualified **or dotted** spelling (`(fn [m/x] …)`, `(fn [a.x] …)`) is a compile-time error, span at the parameter. [S115] The same rule holds for `defn`/`defmacro` parameters (§5.1, §5.5). [S113]
