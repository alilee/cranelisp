...
fn u3_constraint_abstract_unification_asymmetry() {
    ...
    assert_eq!(
        apply(&subst, &Type::Var(20)),
        Type::Var(10),
        "flexible 20 must acquire (bind to) rigid 10"
    );
    assert_eq!(
        apply(&subst, &Type::Var(10)),
        Type::Var(10),
    ...
    for (l, r) in [
        ...
        assert!(
            ...
            "skolem-escape must be a plain type error, never `unknown type`: {}",
            err.message()
        );
        assert_eq!(
            apply(&subst, &Type::Var(10)),
