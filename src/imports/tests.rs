...
fn session_env_reregisters_import_alias() {
    ...
    assert!(
        entry.is_some(),
        "alias `<module>.<alias>` (m.u) must be registered"
    );
    assert_eq!(
...
fn check_terminal_closure_permits_name_in_declared_exports() {
    ...
    assert!(
        res.is_ok(),
        "a declared re-export (Int ∈ D(prelude)) must pass: {res:?}"
    );
}
...
fn check_terminal_closure_permits_when_declared_exports_unknown() {
    ...
    assert!(
        res.is_ok(),
        "unknown D(M) must permit (never false-fire): {res:?}"
    );
}
...
fn check_terminal_closure_generalizes_beyond_prelude() {
    ...
    assert!(
        res.is_err(),
        "the gate generalizes to any module, not just prelude"
    );
}
...
fn check_terminal_closure_permits_legitimate_reexport() {
    ...
    assert!(
        res.is_ok(),
        "a declared re-export (add-i64 ∈ D) must pass: {res:?}"
    );
}
...
fn check_terminal_closure_noop_for_private_write() {
    ...
    assert!(
        res.is_ok(),
        "a private import edge is not a public phantom: {res:?}"
    );
}
