...
| Skill | Crate | Task | Depends on | Status |
|---|---|---|---|---|
| /dev | cranelisp-intrinsics | **0254** — host the 12 trace bodies + pure descriptor-driven `trace_format` + `TRACE_STACK`/guard + catalog entries (15→27) + nested-trace runtime guard | Wave 1 (catalog exists) | pending |
| /dev | cranelisp-backend | **0255** — display-descriptor baking (JIT + object modes) + swap-all discovery-in-codegen + `trace_codegen` rework + exe-bundle force-link restore | 0254 (bodies/layout) | pending |
| /dev | cranelisp-frontend | **0259** — reserved-name enforcement (`trace` rejected in binder/definition positions) + stale `--link` comment cleanup | none | pending |
| /spec | spec/ | **0257** — §4.12.9 all-modes; §4.12.5 nested-runtime-error; §4.12.3 swap-all visibility; §2 reserved-word entry; §4.12.4/App-A "root special form" terminology; §3.2.4/§11.1 form/ADT asymmetry split | none (parallel) | pending |
