...
**The typed successor — `VarRef` / `ApplyRef` (S114, FIXME 0653 prong 3; `design/arch/typed-resolution-carrier.md`; corollary at `principles/24-resolve-once.md`).** The `Option<FQSymbol>` shape above is scheduled for retirement: `None` conflates "local by design" with "unresolved by producer bug" (the S113 check-gate-leak class), and the backend disambiguates by convention. The user-directed close is a pair of CLOSED sums constructed only by typecheck — `VarRef::Local { binder, binding_span } | VarRef::Global(FQSymbol)` on the Var side (binder identity carried; slot mapping stays backend-side) and `ApplyRef::Dispatch(FQSymbol) | ApplyRef::ViaCallee` on the Apply side (the Apply's third legal state gets its own constructor — no shared shape re-smuggling an ambiguous `None`). **"Unresolved" has no constructor**; the phase-boundary completeness gate is the constructor itself, and a dropped carrier fails as a located typecheck error at view-build, never a codegen-time miss. Both enums are deliberately NOT `#[non_exhaustive]` (closed sum = the contract; the ownership-mode-vocabulary exception class). **Landed dormant S114 Phase 3** (vocabulary only, additive `public-api.txt`); the field flip (`resolved_target` → `resolution: VarRef` / `dispatch: ApplyRef`), the `MethodResolutions` split into total typed `var_refs` + `apply_refs` maps, and the `CACHE_SCHEMA_VERSION` 21→22 bump land as ONE coordinated carrier wave — the pinned diff, producer totality contract, and sequencing are in `typed-resolution-carrier.md` §3–§5.
