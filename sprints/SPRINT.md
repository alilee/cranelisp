...
**Live smoke #2 (user, 2026-06-22) → wire-path DEFECT found + fixed.** With the primer fixed, Build mode **fired** (model issued a `submit` tool_use), but hit an Anthropic **400** — `tool_use` without a paired `tool_result`. **This is the realization of the Wave-2 2R Suggestion S1 that `/sprint` DEFERRED** (`/review` rated it "fidelity not correctness"; live it's a hard 400 on the first repair iteration). **Root cause:** the inner validator-repair loop (`pull.rs:497`) recorded the compiler-error feedback as a bare `record_user` while the outer loop had already recorded the `submit` as an assistant tool_use → un-paired tool_use in the assembled request. **`/dev` (`ca685ff`) found + fixed TWO seams:** (1) repair feedback now a `record_tool_result` paired to the submit id (threaded `&call.id` into `validate_and_repair`, recorded BEFORE `assemble_request`); (2) post-submit continuation paired against the *original* broken id not the *repair* id (a 2nd latent 400) — `validate_and_repair` now returns the final tool_use id. **rig-trait `MockModel` test** captures the real `CompletionRequest` + asserts tool_use↔tool_result pairing; **both fixes revert-verified** (each goes RED on revert — the stub can't, it sits above rig). Green: agent lane 41/41, lib 61/61, default 1519/1519. **→ user re-smoke live.**

**Finding (methodology — extends the S88 lesson, TWICE this sprint):** the deterministic stub sits ABOVE rig and cannot enforce Anthropic's tool_use↔tool_result pairing, so **every transcript-construction site needs a rig-trait `MockModel` test** that builds a real `CompletionRequest` — not just stub coverage. S88 learned this on the pull path; S89 hit it again on the **validator-repair path** (a NEW transcript site the pull-path test didn't cover). **And the primer (model-instruction) is part of the feature.** Both classes were green in CI and broken live. `/sprint` deferral of 2R-S1 ("fidelity") was a miss — a transcript-pairing finding is a correctness Blocker against the real API, not a Suggestion. Durable rule for the agentic track: **a transcript-construction change is not done until a rig-trait `MockModel` test pins its pairing, revert-verified.**
