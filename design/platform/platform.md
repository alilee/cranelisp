...
| `adt-marker-binding.md` | **Current (S118 Phase 3, design-only; selection provisional pending `/arch`)** | **Keep**. The ADT marker-binding mechanism decision (FIXME 0873 / audit R4): how a DLL binds a Rust marker type to a cranelisp FQ type name. Compares explicit impls / derive / macro-emitted binding; recommends an optional schema-checked `adts:` arm on `declare_platform!` that makes name agreement a build error. Describes only the current ABI-v9 contract. |
