...
- **Where the gap is caught.** Two surfacing sites, both inside `process_module_forms`:
  ...
  2. *FQ function / type* (`(mac/helper …)`): typecheck maps `QualifiedModuleUnknown` → `ResolutionGap::SymbolTypechecked` (checker.rs); `check_program_compat` now returns `Ok(Some(gap))` instead of swallowing it; `finalize_module` calls `handle_fq_autoload_gap`. (In practice an FQ function head also contains `/`, so it usually blocks via the Pass-2 macro-recogniser path first — both routes converge on `load_fq_dep_module`.)
- **Load + retry.** `load_fq_dep_module` resolves the module file with the **same rules as `import`** (`pipeline::resolve_module_file`; no new search semantics), `register_dep` + `register_module(dep, true)` + `block_for_typecheck`, and returns `ProcessResult::Blocked { form_index, … }`. The worker loop (`handle_typecheck_work_shared`) and the REPL retry loop (`process_single_form`) already drive block→resume. On resume the referencing form re-dispatches against the now-loaded module. Already-loaded / cache-hit deps use `block_for_typecheck` + `scheduler.unblock_module` (immediate re-queue — no future `notify_typecheck_done` sweep would fire).
- **Macro-vs-fn discrimination is orchestrator-owned and implicit in the resume:** only the dependency typecheck-and-compile is forced (synchronous via `block_for_typecheck` in the worker loop); functions are NOT speculatively JIT-pushed. A dep macro's clause code is JIT'd by the dep's own Pass-2 codegen, so the resumed expand finds it in memory. **Consequently the `priority_boost_jit` / `wait_for_inmem` priority-codegen subsystem was never needed and is deleted** (B5 disposition — see `src/scheduler.rs`).
...
**Known limitation (pre-existing, NOT introduced by 0268):** a **cross-module macro loaded from the disk cache** (`.cranelisp-cache`) fails with "clause N is not in memory" — the cache loader restores macro type info but does not JIT the clause code, and the on-demand recompile can't find the macro sexp for a cache-loaded module. This reproduces identically for a plain `(import [mac [twice]])` of a cached macro, so it is a cache-loader gap, not an FQ-autoload gap. The S76 acceptance suite uses fresh (`PreludeVariant::None`) runs and is unaffected.
