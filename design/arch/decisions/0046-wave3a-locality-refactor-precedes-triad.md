...
Decision 0044's "Sequencing" section is amended to reference the α-then-β split. The original sequencing item 3 ("Wave 3a triad re-fires") becomes item 3' ("Wave 3a-α — locality refactor; precondition") and 3'' ("Wave 3a-β — triad re-fires atop locality-correct typecheck"). The semantic content of Decision 0044 (the cluster-atomic shape, ClusterContext, View, the two-pass surface) does not change.
