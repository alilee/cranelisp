...
impl TypeChecker {
        ...
        for defn in program_defns {
            if let Some(scheme) = self.lookup(&defn.name) {
                if scheme.is_constrained() {
                    let mut fn_deferred: Vec<(Span, String, String, Type)> = deferred
                        .iter()
                        .filter(|(span, _, _, _)| span.0 >= defn.span.0 && span.1 <= defn.span.1)
                        .cloned()
                        .collect();
    ...
    fn collect_mono_requests(&mut self) -> Vec<(Span, String, Vec<Type>)> {
        ...
        for (span, fn_name, arg_tys) in pending {
            ...
            if concrete.iter().all(|t| !has_type_var(t)) {
                // Always include the request — monomorphise_all needs it to create
                // SigDispatch entries even for already-generated specializations.
    ...
    fn monomorphise_fn(
        ...
        for (span, trait_name, method_name, arg_type) in &cf.deferred_resolutions {
            let concrete = apply_with(&self.subst, &local_subst, arg_type);
            let type_name = type_to_name(&concrete).ok_or_else(|| {
            ...
            let mangled_method =
                crate::ast::mangle_impl_method(trait_name, method_name, &type_name);
            mr.insert(
                *span,
    ...
    pub fn register_constrained_fn(&mut self, defn: &Defn, scheme: &Scheme) {
        let deferred = std::mem::take(&mut self.deferred_resolutions);
        let fn_deferred: Vec<(Span, String, String, Type)> = deferred
            .iter()
            .filter(|(span, _, _, _)| span.0 >= defn.span.0 && span.1 <= defn.span.1)
            .map(|(span, trait_name, method_name, arg_type)| {
                // Resolve arg_type through current subst so it matches
                // the scheme's quantified vars (which were resolved via finalize_defn_type).
                (*span, trait_name.clone(), method_name.clone(), apply(&self.subst, arg_type))
            })
            .collect();
        // Put back any deferred resolutions that don't belong to this function
        let remaining: Vec<(Span, String, String, Type)> = deferred
            .into_iter()
            .filter(|(span, _, _, _)| !(span.0 >= defn.span.0 && span.1 <= defn.span.1))
            .collect();
