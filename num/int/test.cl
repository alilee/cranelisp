(import
  [super
   [rem abs sign negate even? odd? min-int max-int clamp]])
(import
  [testing.assertions [assert-true assert-false assert-eq]])
(import [primitives [Option String]])
(defn test-rem [] : (Option String) (assert-eq 1 (rem 7 3)))
...
(defn test-even [] : (Option String) (assert-true (even? 4)))
(defn test-odd [] : (Option String) (assert-true (odd? 7)))
