...
**S119 scoping of the S84 ⟺ (`/arch`, 2026-07-27).** The biconditional above is a statement about **`DefKind::UserFn`** — inference-derived bodies whose emitted RC is type-directed. The other slot carriers (`Constructor`; `PrimitiveBody::Extern`; `PlatformEffect`) hold slots over non-concrete schemes under **different licences** (ctor representation-parametricity per I-CT′; the hand-written body's declared contract), and phrasing the invariant universally left it unassertable — the shadow in which the S119-censused `adt.rs`/`impl_check.rs` hand-mints hid. Canonical kind-partitioned statement: BC §7 "S119 step-back ruling"; checkable form + mechanism: `safety-invariants.md` §4 R11.
