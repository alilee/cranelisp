...
| `repl.rs` | Slash-command dispatch (`process_commands` / `dispatch_command` / `parse_slash_command` / all `handle_*`), prompt/banner/line-editor (`print_banner`, `prompt_string`/`continuation_prompt_string`, `pretty_print`, `parens_balanced`), and the introspection-display helpers (`describe_symbol`/`collect_related[_for]`, `format_eval_result`, `format_def_entry`, `resolve_entry_for_display`, the `format_*` free fns, `ReplCommand`/`ImportClass`). |
...
- **REPL input abstraction — `src/repl_input.rs` (S106, FIXMEs 0544 + 0551, `repl/spec.md` §10.8).** The `main.rs` read loop reads through one `ReplInput`, gated on `IsTerminal` for stdin. **TTY** = a `rustyline` editor (history recall + inline editing + per-project `<project_root>/.cranelisp_history`, cap 1000, graceful-degrade). **Non-TTY** (piped/redirected — the e2e harness) = `rustyline` is NEVER instantiated; the loop reads **fd 0 directly, one byte up to the newline, no read-ahead**, and the prompt is written verbatim so output stays **byte-identical** (the `non_tty_repl_line_editor_off` golden is the load-bearing guard). The no-read-ahead reader is load-bearing for **0551**: the poll-shape `read-line` platform leaf (`platforms/stdio`) shares fd 0 with the host, so a read-ahead `BufReader` (the old `stdin.lock().lines()`) would swallow the line a later `read-line` turn should consume, leaking it back as an undefined-variable error. Both sides now read exactly one line and leave the remainder on the fd. `WouldBlock`/`EINTR` are **retried, not treated as EOF** (0551 (B); the old `Err(_) => break` ended the session on any read error). The agent write-consent read (§15.2) goes through the SAME `ReplInput` (`read_consent_line`) — never a second reader. Companion platform fix: `set_stdin_nonblocking` now records fd-0's original flags and `restore_stdin_flags` returns them on the poll terminal (0551 (A)); the leaf reads byte-wise so it never over-reads into `STDIN_BUF`.
...
- **`src/imports.rs`** hosts the int-side import/export installer (`install_imports` / `install_exports`) — replaces typecheck's struck `register_imports`/`register_exports` (BC §2 invariants 2+8). Writes per-symbol `ModuleEntry::Import { source, visibility }` bindings into the current module's table (visibility `Private` for `(import …)`, `Public` for `(export …)` re-export edges; the `Reexport` variant is retired) + module-path aliases into `SharedState.module_aliases` (keyed `<owner>.<alias>`). Resolution semantics (glob/specific/member-glob, visibility checks, ambiguity detection via `insert_detecting_ambiguity`) mirror the deleted typecheck bodies. typecheck reads `module_aliases` read-only; `ModuleCompiler` carries `module_aliases: &ModuleAliases` and threads it into `check_forms`.
  - **`export` brings the name into the exporting module's OWN bare scope (§8.4.0).** Both `install_imports` and `install_exports` produce `ModuleEntry::Import` inner-scope bindings differing ONLY in the visibility flag (`Private` vs `Public`); resolution (§8.6.2 chain-follow) treats both identically, so a module can USE a name it exports with no separate import. `import`/`export` is the same bring-into-scope operation, visibility apart — nothing extra was needed for the export-into-scope conformance.
  - **§8.6.4 definition-over-(import|export|prelude) rejection (FIXME 0514 — the no-exception ruling, 2026-07-04).** A definition whose bare name is already in scope via an explicit `import`, an `export` (§8.4.0 — a Public inner Import edge), OR the implicit prelude (§8.8.1 — no exception) is a compile-time error, resolved by the fully-qualified reference. The rejection lives at the **shared typecheck seam** `cranelisp_typecheck::check_forms` Pass-1 registration (`TypeCheckEnv::reject_def_over_binding`, checker.rs) — the single mode-uniform chokepoint both REPL/Additive and batch/Replace traverse, and the only place that also sees the prelude OUTER scope. The predicate probes the defined name through the two-scope resolver `resolve_current_or_prelude` and rejects when it resolves to a `home != current_module` (an import/export inner entry or a prelude PUBLIC terminal); a resolve to the module's OWN prior `Def` (`home == current`) is ordinary redefinition, allowed; a miss is a free fresh definition. Both directions collapse (def-over-import AND import-over-def) because Pass-0 installs imports/exports before `check_forms` registers defs. The former int-side machinery (Additive-gated `reject_def_over_import` commit-gate pre-scan in `worker::commit_staging_to_live`, the symmetric `reject_over_local_def` in `imports::insert_detecting_ambiguity` via `install_imports_gated`/`install_exports_gated`, and `imports::def_over_binding_error`) is DELETED. The int-side Pass-0 installer keeps only §8.6.5 ambiguity detection, now including the **distinct-terminal prelude-overlap poison** (`insert_detecting_ambiguity` takes `prelude_fallback` and poisons an incoming import/export whose bare name resolves to a DISTINCT terminal in the prelude outer scope; same-terminal overlaps dedup, FIXME 0514/0515). Redundant `(import [m [X]]) (export [m [X]])` is import-over-import (same terminal → dedup), never reaching the def seam. NOTE: a pure-REPL import entered over an *existing* local def in a SEPARATE later cluster is not caught (no def is registered that cluster; the installer skips the import) — the only residual companion gap, not covered by the current matrix.
