...
- `ABI_VERSION: u32` — bump on breaking changes (currently 3)
- `STRING_HEADER_BYTES: usize` — string layout constant (8 bytes for i64 length prefix)
- `SchedulingClass` — scheduling annotation per platform function (see Section 11)
- `PlatformFn` (`#[repr(C)]`) — function descriptor with name, jit_name, ptr, param_count, type_sig, docstring, param_names (all as raw pointer + length pairs), and `scheduling: SchedulingClass`
...
pub enum SchedulingClass {
    Sequential     = 0, // must execute in order relative to other calls to this function
    Commutative    = 1, // reorderable; no shared mutable state between calls
    ResourceSerial = 2, // parallel unless runtime resource tokens conflict
}
```
...
| Scheduling class | `SchedulingClass` enum in `PlatformFn` descriptor — used by compiler for automatic IO parallelism analysis |

Platform functions construct `Effect` nodes using CLIO helpers:

```rust
CLIO::effect(move || { ... })
// Creates an Effect node with resource_token=0 (unrestricted).
// The scheduling class is declared in declare_platform! and stored in
// the platform manifest — the runtime uses it for auto-scheduling decisions.

CLIO::effect_on_resource(token, move || { ... })
// Creates an Effect node with a non-zero resource_token.
// Use for ResourceSerial functions where the token identifies the shared resource.
// token = &MY_SINGLETON as *const _ as i64   (for global singleton resources)
// token = heap_ptr_for_resource as i64        (for per-instance resources)
```

The scheduling class declared in `declare_platform!` tells the compiler (at compile time) whether calls can be parallelised. The resource token set by the platform function (at runtime) tells the trampoline whether two `Par` branches in the same group conflict and must be serialised.
...
Each `PlatformFn` in the manifest carries a `type_sig` field — an S-expression string representing the cranelisp type — and a `scheduling` field declaring its scheduling class.

```
"(Fn [String] (IO Int))"   scheduling: Sequential  ← print
"(Fn [] (IO String))"      scheduling: Sequential  ← read-line
"(Fn [String] (IO String))" scheduling: Commutative ← http-get (hypothetical)
```
...
5. Records the `SchedulingClass` alongside the type in the typechecker's environment
6. Registers the function in the typechecker's environment

The scheduling class is stored in the `CompiledModule` entry for the platform function. The compiler reads it during `bind!` independence analysis (see `docs/concurrency.md`) to determine whether a pair of calls can be automatically parallelised.
...
## 11. Scheduling Guidance for Platform Authors

Every platform function must declare a `SchedulingClass`. Choosing the right class is important — it directly controls whether the compiler will automatically parallelise calls to this function in user `bind!` chains.

### When to use `Sequential`

Use `Sequential` when calls to this function share mutable state with other calls to the same function, so that execution order is observable:

| Effect | Reason |
|---|---|
| `read-line` | Advances the stdin read position; order determines which line each call returns |
| `print` | Writes to stdout; order determines the sequence of output lines |
| `file-read-line` | Advances a file cursor; order matters |
| `file-write` | Appends to a file; order determines file content |
| `recv` on a channel | Consuming messages; order determines which message each call gets |

Two `Sequential` calls from the same platform in a `bind!` chain will always execute in source order, regardless of data independence.

### When to use `Commutative`

Use `Commutative` when calls to this function do not share mutable state with each other, so that execution order does not affect observable results:

| Effect | Reason |
|---|---|
| `http-get url` | Each URL is independent; results are determined solely by the URL |
| `time-now` | Clock reads do not affect each other |
| `random` (from a thread-safe RNG) | Independent samples |
| `file-read-all path` | Reading a complete file; no shared cursor state between calls to different paths |
| `dns-lookup host` | Each hostname is independent |

Two `Commutative` calls that are also data-independent in a `bind!` chain will be automatically parallelised by the compiler.

### When to use `ResourceSerial`

Use `ResourceSerial` when calls to this function may conflict *with calls to the same function on the same resource*, but are safe to run in parallel when the resources differ:

| Effect | Reason |
|---|---|
| `file-read(fd)` | Calls with different fds are independent; calls with the same fd must be serial |
| `log(handle, msg)` | Calls with different log handles are independent; calls on the same handle must be serial |
| `db-query(conn, sql)` | Calls on different connections are independent; calls on the same connection must be serial |

**Important**: resource tokens must be globally unique across all loaded platforms. Recommended patterns:
- **Singleton resource**: `&MY_STATIC_FIELD as *const _ as i64` (address is unique per binary)
- **Per-instance resource**: `Box::into_raw(Box::new(state)) as i64` (store as the resource handle)

Do NOT use raw OS fd integers (e.g. `3`) — two platforms using `fd=3` for different purposes would collide.

Two `ResourceSerial` calls in a `bind!` chain may be placed in a `Par` node by the compiler. The trampoline then checks their runtime tokens: different tokens run in parallel; same token runs sequentially.

### Conservative default

When in doubt, use `Sequential`. The compiler will not parallelise calls declared as `Sequential` — correctness is preserved. Incorrectly declaring a `Sequential` effect as `Commutative` or `ResourceSerial` can result in observable non-determinism (e.g., garbled stdout interleaving, incorrect read sequences).

## 13. No Built-in Default
...
Currently, effects execute eagerly (`docs/io.md`). The planned evolution (see `docs/io.md` "Evolution" section and `docs/concurrency.md`) switches to a **Task + trampoline** model where `main` returns a Task description that the platform interprets.

The Task type is a transparent ADT:

```clojure
(deftype (Task a)
  (Pure [:a val])
  (Effect [:EffectDesc desc])
  (Bind [:(Task b) inner :(fn [b] (Task a)) cont])
  (Par [:(Task b) left :(Task c) right :(fn [b c] (Task a)) cont]))
```

Platform functions would change from executing effects to constructing Task nodes:

```clojure
;; Current (eager): print executes immediately, returns IOVal(0)
;; Task model: print returns Effect(PrintDesc("hello"))
;;
;; The user code is unchanged:
(defn main []
  (bind! [_ (print "hello")
          line (read-line)]
    (pure line)))
...
The platform provides a **trampoline runner** — a flat iterative loop that interprets the Task tree with O(1) stack depth:

1. `Bind` nodes push their continuation onto an explicit stack and loop
2. `Effect` nodes execute the actual IO (calling into the platform DLL)
3. `Pure` nodes pop the continuation stack and loop
4. `Par` nodes dispatch both branches to a thread pool, join, then continue

The `Par` node is the concurrency payoff — the compiler inserts `Par` nodes automatically for data-independent `Commutative` effects in `bind!` chains (see `docs/concurrency.md`). The platform runner dispatches them concurrently. This enables concurrent IO transparently:

```clojure
;; Compiler inserts Par node automatically — both requests in flight simultaneously
(bind! [r1 (http-get "url1")
        r2 (http-get "url2")]
  (pure (process r1 r2)))
```

The DLL mechanism is fully compatible — the platform exports both individual effect functions (for `Effect` node interpretation) and the `run_task` trampoline. The `EffectDesc` ADT would need a constructor per platform function, or a generic tagged representation.
