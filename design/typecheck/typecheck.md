...
**The non-concrete producer obligations (S119 Spine 1) — `non-concrete-producer-obligations.md`.** `design/backend/non-concrete-release-contract.md` rules that a frame whose parameter or result types are not fully concrete is **not a legal codegen target** (R-3) and that no producer may present a downstream gate with a type more concrete than what it knows (R-2 — Principle 25 on the type channel). Three sites in this crate violate that, all three by bypassing the S84 slot gate above rather than by lacking a mechanism: `adt.rs::synthesise_one_accessor` (`:618-637`) mints `Concrete { got_slot }` over a scheme whose type is not `is_concrete()`; `traits/impl_check.rs` (`:1043`) calls `scheme::mono` on a `fn_type` that still carries `Type::Var` and mints the same pairing; `MonoExpr::lenient_from_expr`'s `node_ty` replaces any non-concrete node type with `ConcreteType::Int`. The subordinate doc rules all three: **P-1** (no site may construct `Concrete { got_slot }` for a non-concrete scheme — the §9.3 gate made site-independent and converged onto one helper); **P-2** (a monomorphised accessor / trait-method instance is named by the ONE canonical `build_mangled_name`; the FIXME's proposed `mangle_trait_method` key widening is **rejected as lossy** on the argument-supplied type parameter and would re-open the 0483/0508/0519 collision class); **A-MINT** (an accessor instance is re-synthesis at concrete type arguments, never a body re-check — its body is `Span::SYNTHETIC` and therefore outside span-keyed carrier transport); and the **defaulting step** for the lenient view (L-1/L-2/L-3 — default residual *parameter positions* under a preserved type constructor, never substitute the node's type; licensed only where the residual cannot occur in a declared parameter type; refused as a located error over a constrained variable; and self-checking because the strict builder must then accept the body). Closes FIXMEs **0924** and **0913**; gates **0916** and rider **0867** (which unblocks on P-1 alone, not on the whole obligation). Zero `cranelisp-types` delta and zero public-API delta; **one cache-schema question stands for `/arch`** — both halves are serde-visible meaning changes and a stale sidecar re-introduces the memory-unsafety and the leak.
...
| **The non-concrete producer obligations — typecheck's half of the S119 release contract (FIXMEs 0924 + 0913; gates 0916 + rider 0867)** | **`non-concrete-producer-obligations.md`** | **RULING** (S119 Phase 3 round 2; pre-implementation). Governed by `design/backend/non-concrete-release-contract.md` (R-2/R-3, §4 faces 2/3/5, §5.2/§5.4) — where they disagree, that ruling wins. Pins: **P-1** the universal slot gate (no site constructs `Concrete { got_slot }` over a non-concrete scheme — the §9.3 S84 invariant made site-independent, converging `finalize.rs`'s determination points with `adt.rs:618-637` and `impl_check.rs:1078-1090` onto ONE helper); **P-2** one mangler (`build_mangled_name`), with the FIXME's `mangle_trait_method` `$Type$Arg` widening **rejected as lossy** on the argument-supplied parameter; **A-MINT** (accessor instantiation is re-synthesis, never a body re-check — `Span::SYNTHETIC` bodies are outside span-keyed transport); the F2 scheme-truth fix (`scheme::mono` asserts absence of quantification, not concreteness); and 0913's **defaulting step** at `build_concrete_codegen_view`'s `NotConcrete` arm — L-1 (licensed only where the residual cannot occur in a declared parameter type; multi-sig `f$Var` variants are the fence), L-2 (constructor-rooted only; a residual **root** is not defaultable and keeps a counted placeholder whose census gates its removal), L-3 (a constrained variable is a located error), self-checked by re-running the strict builder. Cost stated against the measured census (281 release admissions + 1,073 category licences owed to zero). **Rider 0867 unblocks on P-1 alone**, and §4.2 recommends landing them in one change-set. Zero `cranelisp-types` / public-API delta; **one open `/arch` item — the cache-schema window** (§5). Cites Principles 5/6/7/18/20/24/25/26. |
