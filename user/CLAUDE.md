...
| `errors/` | Error-message catalogue, written as each error is confirmed | started — `trait-impl-diagnostics.md` (binder-position rejects incl. value-level binders; dangling-qualifier empty-module/local-half rejects; `:`-must-bind-a-type; type-parameter-must-be-lowercase; `deftrait`/`impl` declaration diagnostics; no-impl + return-poly `:Type` dispatch diagnostics) |
