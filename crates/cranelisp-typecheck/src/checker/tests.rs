...
fn test_bare_module_has_root_contents_only() {
    ...
    assert!(
        root_table.get("let").is_some(),
        "let should be at root \"\""
    );
    assert!(
        root_table.get("defn").is_some(),
        "defn should be at root \"\""
    );
    assert!(root_table.get("fn").is_some(), "fn should be at root \"\"");
    assert!(
        root_table.get("match").is_some(),
        "match should be at root \"\""
    );
    assert!(
        root_table.get("deftype").is_some(),
        "deftype should be at root \"\""
    );
    assert!(
        root_table.get("deftrait").is_some(),
        "deftrait should be at root \"\""
    );
    assert!(
        root_table.get("impl").is_some(),
        "impl should be at root \"\""
    );
    assert!(
        root_table.get("defmacro").is_some(),
        "defmacro should be at root \"\""
    );
    drop(root_table);

    // --- Bare module is empty (no special forms seeded — FIXME 0193) ---
    assert!(
        tf.symbol_table().get("if").is_none(),
        "if not seeded into bare modules"
    );
    assert!(
        tf.symbol_table().get("let").is_none(),
        "let not seeded into bare modules"
    );
    ...
    assert!(
        tf.symbol_table().get("Float").is_none(),
        "Float needs import"
    );
    assert!(
        tf.symbol_table().get("String").is_none(),
        "String needs import"
    );
    assert!(
        tf.symbol_table().get("add-i64").is_none(),
        "add-i64 needs import"
    );
    assert!(
        tf.symbol_table().get("str-concat").is_none(),
        "str-concat needs import"
    );
    assert!(tf.symbol_table().get("bind").is_none(), "bind needs import");
    assert!(tf.symbol_table().get("Pure").is_none(), "Pure needs import");
    assert!(
        tf.symbol_table().get("SexpSym").is_none(),
        "SexpSym needs import"
    );
    assert!(tf.symbol_table().get("+").is_none(), "+ needs prelude");
    assert!(
        tf.symbol_table().get("TestResult").is_none(),
        "TestResult needs import"
    );
    assert!(
        tf.symbol_table().get("discover-tests").is_none(),
        "discover-tests needs import"
    );
    assert!(
        tf.symbol_table().get("run-test").is_none(),
        "run-test needs import"
    );
    ...
    assert!(
        prims_table.get("add-i64").is_some(),
        "add-i64 in primitives"
    );
...
fn test_set_current_module_creates_new() {
    ...
    assert!(
        tf.symbol_table().get("if").is_none(),
        "special forms at root \"\", not seeded"
    );
...
fn test_switch_back_to_user_preserves_builtins() {
    ...
    assert!(
        tf.symbol_table().get("if").is_none(),
        "user not architecturally privileged"
    );
    assert!(tf.symbol_table().get("add-i64").is_none());
}
...
fn test_resolve_qualified_uses_alias() {
    ...
    assert!(
        result.is_some(),
        "resolve_qualified should resolve 'opt/Some' via alias"
    );
}
...
fn qualified_lookup_unknown_absolute_module_still_surfaces_gap() {
    ...
    assert!(
        matches!(
            gap,
            Some(cranelisp_types::ResolutionGap::SymbolTypechecked(_))
        ),
...
fn test_trait_method_dispatch_falls_back_to_prelude_outer_scope() {
    ...
    match resolved {
        ...
        other => panic!("expected BuiltinFn(add-i64) for (Num,+,Int); got {other:?}"),
    }
}
...
fn prelude_fallback_qualified_never_falls_back() {
    ...
    seed_module(
        &mut tf,
        "other",
        vec![("something_else", Visibility::Public)],
    );
    ...
    let result = tf
        .resolve_qualified(&ModuleFullPath::from("other"), "helper")
        .unwrap();
    assert!(
        result.is_none(),
...
fn prelude_module_does_not_fall_back_onto_itself() {
    ...
    tf.prelude_fallback
        .insert(ModuleFullPath::from("prelude"), true);
...
fn prelude_private_type_not_reachable_via_resolve_family() {
    ...
    assert!(
        tf.env()
            .resolve_type(&state, &TypeName::from("Hidden"), Span::SYNTHETIC)
            .is_err(),
...
fn prelude_private_trait_method_not_reachable() {
    ...
    assert!(
        env.method_to_trait_with_state(&state, &Symbol::from("priv-op"))
            .is_none(),
...
fn prelude_fallback_resolves_pattern_ctor_when_bit_on() {
    ...
    assert!(
        tf.env()
            .lookup_constructor_type_with_state(&state, "Just2")
            .is_none(),
...
fn prelude_fallback_internal_ctor_gate_rejects_bind() {
    ...
    assert!(
        !tf.env()
            .is_internal_constructor_check_with_state(&state, "Bind"),
    ...
    assert!(
        !tf.env()
            .is_internal_constructor_check_with_state(&state, "Pure"),
...
fn prelude_fallback_ctor_chokepoints_off_when_bit_off() {
    ...
    assert!(
        tf.env()
            .lookup_constructor_type_with_state(&state, "Solo")
            .is_none(),
...
fn prelude_private_ctor_not_reachable_via_fallback() {
    ...
    assert!(
        tf.env()
            .lookup_constructor_type_with_state(&state, "HiddenC")
            .is_none(),
