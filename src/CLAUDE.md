...
- **`src/imports.rs`** hosts the int-side import/export installer (`install_imports` / `install_exports`) — replaces typecheck's struck `register_imports`/`register_exports` (BC §2 invariants 2+8). Writes per-symbol `ModuleEntry::Import { source, visibility }` bindings into the current module's table (visibility `Private` for `(import …)`, `Public` for `(export …)` re-export edges; the `Reexport` variant is retired) + module-path aliases into `SharedState.module_aliases` (keyed `<owner>.<alias>`). Resolution semantics (glob/specific/member-glob, visibility checks, ambiguity detection via `insert_detecting_ambiguity`) mirror the deleted typecheck bodies. typecheck reads `module_aliases` read-only; `ModuleCompiler` carries `module_aliases: &ModuleAliases` and threads it into `check_forms`.
  ...
  - **§8.6.4 definition-over-(import|export) rejection (FIXME 0484).** A definition whose name is already bound by an explicit import OR export is a compile-time error, resolved by the fully-qualified reference. Two symmetric seams, BOTH gated on the incremental REPL (`ModuleStrategy::Additive`) path so a whole-module (`Replace`) load — whose own defs and own glob/specific imports/exports co-arrive as one cluster — lets the module's own definition legitimately win (same-cluster; this is what keeps a re-exporting-and-redefining prelude fixture loading): (1) **def-over-import/export** is a PRE-SCAN in `worker::commit_staging_to_live` (`reject_def_over_import`) ahead of any live mutation, so a rejected commit leaves live byte-identical (the import/export stays the binding, introspection unchanged); (2) **import/export-over-local-def** (the symmetric direction) is in `imports::insert_detecting_ambiguity` (`reject_over_local_def`, threaded via `install_imports_gated`/`install_exports_gated` from the Additive Pass-0 `handle_import`/`handle_export`). The check is UNIFORM across glob/specific (it never consults the import shape — the gate selects the PATH, not the shape) and reads the inner scope uniformly for import and export. Redundant `(import [m [X]]) (export [m [X]])` is NOT a collision — both are indirect `Import` edges with the same terminal, so they dedup+upgrade in the `both_indirect` arm, never reaching the reject. Prelude-PROVIDED names are an OUTER scope (never `Import` entries here) so they stay shadowable. The single diagnostic source is `imports::def_over_binding_error` (names the source module + the `module/name` FQ remedy).
