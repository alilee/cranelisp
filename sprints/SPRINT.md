...
|---|---|---|---|
| /design (platform) | `poll-support.md §3.1/3.5/3.6` | **DONE** — ctx-vtable poll-fn skeleton; `Connection`=`(deftype Connection [:primitives/Int fd])` opaque 1-field (platform reads `r` back, not user-destructurable); deleted `desc_out`/header-slot/`desc_of`/`set_desc`; 4 roles + commutative/one-shot + full-duplex per-direction tokens; §2.2 `Reactor.{acquire,retire}` wrappers. Wave-2 overrun "cannot arise". CARRIED intact: web.cl/serve.cl split (0469), load-order §3.6.3, singleton stdin token (0471). | DONE |
| /design (backend) | `io-trampoline.md §17` | **DONE** — pure subtraction: only delta is DELETING `inject_poll_leading_pair` (+~14 sites) + the `arg_vals[0..1]` peel; poll node stays v8-uniform (no header slot/role/desc_out/growth); `Connection [fd]` ordinary 1-field (`FIELDS_START=24`, normal `CLAdt::construct` → Wave-2 overrun dissolved); `PollFn`/`Poll`/`cranelisp-types`-codegen untouched; backend `public-api.txt` UNCHANGED; dead §17.5 offset contract retired (nothing crosses the seam by baked offset). `ring2-rc.md §3.5.10` (0474) model-independent, STANDS. `CACHE_SCHEMA_VERSION` bump noted. | DONE |
| /design (int) | `reactor.md §7` | **DONE** — §7 rewritten to the host ctx-vtable: `acquire(token,cap,waker)→Acquired|Parked` over the §8.1 semaphore-per-token map + a **per-effect held-permit ledger keyed by effect identity** (the §2.16 `RegId` ≡ waker data ptr; token==0 fast-path, idempotent-per-effect, Parked enqueues), `retire(token)` idempotent (full-duplex close retires both per-direction tokens). **Release tramp-owned** (not a vtable entry): `EffectPoll` release-guard fires `release_all(effect)` on Ready, `release_all+deregister` on drop=cancel — unifying permit+interest+parked-waiter cleanup under one identity key; cancel never re-enters the poll-fn. `await_poll_node` scheduling-blind (deleted stamp/read/role/offsets). **§8.2 ordering→inference pinned §7.7** (+ `bind-chain-analysis` G2 banner; /qa watch: same-token timing pair must use the same explicit handle). §8 (0479) + §9 (0475) + `bind-chain-analysis §3.7` (0478) STAND (model-independent). `ABI_VERSION=9` loader (`io-integration.md §I3`). Survivor invariant §7.5: handle still marshals `arg(0)`=`state+8`, but the **platform** reads `r` now (tramp introspects nothing) — Phase-5 unit pin owed. | DONE |
