...
/// Extract param types from a function Type.
fn extract_param_types(ty: &Type) -> Vec<Type> {
    match ty {
...
// Thread-local pointer to the active TypeChecker, used by `cranelisp_trace_format`.
// Set by `set_trace_tc` before evaluating any expression that may contain `(trace ...)`.
// The pointer is valid for the duration of the evaluation (TypeChecker doesn't move).
thread_local! {
    static TRACE_TC_PTR: std::cell::Cell<*const TypeChecker> =
        std::cell::Cell::new(std::ptr::null());
}

/// Set the TypeChecker pointer used by `cranelisp_trace_format`.
/// Call this before evaluating any expression, and the pointer will be valid
/// for the entire execution (TypeChecker lives in a stable location for the session).
pub fn set_trace_tc(tc: *const TypeChecker) {
    TRACE_TC_PTR.with(|c| c.set(tc));
}

// JIT-callable function: format a runtime value using the full REPL display machinery.
// - `val`: the i64 runtime value to format
// - `type_ptr`: pointer to a leaked `Box<Type>` describing the value's type
// Returns a cranelisp heap String pointer (RC=1).
// Falls back to `"?"` if the TypeChecker pointer has not been set.
#[allow(dead_code)]
extern "C" fn cranelisp_trace_format(val: i64, type_ptr: i64) -> i64 {
    TRACE_TC_PTR.with(|c| {
        let tc_ptr = c.get();
        let s = if tc_ptr.is_null() {
            "?".to_string()
        } else {
            let tc = unsafe { &*tc_ptr };
            let ty = unsafe { &*(type_ptr as *const crate::types::Type) };
            crate::repl::format::format_result_value(val, ty, tc)
        };
        cranelisp_runtime::primitives::alloc_string(s.as_bytes())
    })
}
...
fn register_non_platform_symbols(builder: &mut JITBuilder) {
    ...
    builder.symbol("cranelisp_par_eval", intrinsics::par_eval as *const u8);
    builder.symbol(
        "cranelisp_ivar_create",
        intrinsics::ivar_create as *const u8,
    );
    builder.symbol(
        "cranelisp_ivar_spark",
        intrinsics::ivar_spark as *const u8,
    );
    builder.symbol(
        "cranelisp_ivar_force",
        intrinsics::ivar_force as *const u8,
    );
    builder.symbol(
        "cranelisp_dec_guarded",
        intrinsics::dec_guarded as *const u8,
    );
    builder.symbol(
        "cranelisp_dec_closure_guarded",
        intrinsics::dec_closure_guarded as *const u8,
    );
    builder.symbol(
        "cranelisp_dec_mixed_guarded",
        intrinsics::dec_mixed_guarded as *const u8,
    );
    builder.symbol(
        "cranelisp_rc_underflow_check",
        intrinsics::rc_underflow_check as *const u8,
    );

    // Trace runtime symbols
    builder.symbol(
        "cranelisp_trace_enter",
        cranelisp_runtime::trace::cranelisp_trace_enter as *const u8,
    );
    builder.symbol(
        "cranelisp_trace_exit",
        cranelisp_runtime::trace::cranelisp_trace_exit as *const u8,
    );
    builder.symbol(
        "cranelisp_trace_swap_got",
        cranelisp_runtime::trace::cranelisp_trace_swap_got as *const u8,
    );
    builder.symbol(
        "cranelisp_trace_restore_got",
        cranelisp_runtime::trace::cranelisp_trace_restore_got as *const u8,
    );
    builder.symbol(
        "cranelisp_collect_trace",
        cranelisp_runtime::trace::cranelisp_collect_trace as *const u8,
    );
    builder.symbol(
        "cranelisp_trace_first_child_nanos",
        cranelisp_runtime::trace::cranelisp_trace_first_child_nanos as *const u8,
    );
    builder.symbol(
        "cranelisp_trace_format",
        cranelisp_trace_format as *const u8,
    );
    ...
    builder.symbol("str-eq", primitives::str_eq as *const u8);
    builder.symbol("quote-sexp", crate::marshal::quote_sexp as *const u8);
    builder.symbol("sconcat", crate::marshal::sconcat as *const u8);
    ...
    builder.symbol("vec-set-rc", primitives::vec_set_rc as *const u8);
    builder.symbol("vec-push-rc", primitives::vec_push_rc as *const u8);
    builder.symbol("vec-push-cow-grow", primitives::vec_push_cow_grow as *const u8);
...
/// Returns (alloc, panic, free, par_eval, ivar_create, ivar_spark, ivar_force, builtin_methods, builtin_method_info).
...
> {
    ...
    // Declare par_eval intrinsic: (i64, i64) -> i64
    let mut par_eval_sig = module.make_signature();
    par_eval_sig.params.push(AbiParam::new(types::I64));
    par_eval_sig.params.push(AbiParam::new(types::I64));
    par_eval_sig.returns.push(AbiParam::new(types::I64));
    let par_eval_func_id = module
        .declare_function("cranelisp_par_eval", Linkage::Import, &par_eval_sig)
        .map_err(|e| CranelispError::CodegenError {
            message: format!("failed to declare par_eval intrinsic: {}", e),
            span: (0, 0),
        })?;

    // Declare IVar intrinsics: all (i64) -> i64
    let mut ivar_sig = module.make_signature();
    ivar_sig.params.push(AbiParam::new(types::I64));
    ivar_sig.returns.push(AbiParam::new(types::I64));

    let ivar_create_func_id = module
        .declare_function("cranelisp_ivar_create", Linkage::Import, &ivar_sig)
        .map_err(|e| CranelispError::CodegenError {
            message: format!("failed to declare ivar_create intrinsic: {}", e),
            span: (0, 0),
        })?;
    let ivar_spark_func_id = module
        .declare_function("cranelisp_ivar_spark", Linkage::Import, &ivar_sig)
        .map_err(|e| CranelispError::CodegenError {
            message: format!("failed to declare ivar_spark intrinsic: {}", e),
            span: (0, 0),
        })?;
    let ivar_force_func_id = module
        .declare_function("cranelisp_ivar_force", Linkage::Import, &ivar_sig)
        .map_err(|e| CranelispError::CodegenError {
            message: format!("failed to declare ivar_force intrinsic: {}", e),
            span: (0, 0),
        })?;
    ...
    // Declare two-arg primitives: (i64, i64) -> i64
    for name in &["str-concat", "str-eq", "sconcat"] {
        let mut sig = module.make_signature();
        sig.params.push(AbiParam::new(types::I64));
        sig.params.push(AbiParam::new(types::I64));
        sig.returns.push(AbiParam::new(types::I64));
        ...
        builtin_methods.insert(name.to_string(), fid);
        builtin_method_info.insert(name.to_string(), (name.to_string(), 2));
    }
    ...
    {
        // vec-set-rc: (i64, i64, i64, i64) -> i64
        let mut sig = module.make_signature();
        sig.params.push(AbiParam::new(types::I64));
        sig.params.push(AbiParam::new(types::I64));
        sig.params.push(AbiParam::new(types::I64));
        ...
        sig.returns.push(AbiParam::new(types::I64));
        let fid = module
            .declare_function("vec-set-rc", Linkage::Import, &sig)
            .map_err(|e| CranelispError::CodegenError {
                message: format!("failed to declare vec-set-rc: {}", e),
                span: (0, 0),
            })?;
        builtin_methods.insert("vec-set-rc".to_string(), fid);
    }
    {
        // vec-push-rc: (i64, i64, i64) -> i64
        let mut sig = module.make_signature();
        sig.params.push(AbiParam::new(types::I64));
        sig.params.push(AbiParam::new(types::I64));
        sig.params.push(AbiParam::new(types::I64));
        sig.returns.push(AbiParam::new(types::I64));
        let fid = module
            .declare_function("vec-push-rc", Linkage::Import, &sig)
            .map_err(|e| CranelispError::CodegenError {
                message: format!("failed to declare vec-push-rc: {}", e),
                span: (0, 0),
            })?;
        builtin_methods.insert("vec-push-rc".to_string(), fid);
    }
    {
        // vec-push-cow-grow: (i64, i64) -> i64
        let mut sig = module.make_signature();
        sig.params.push(AbiParam::new(types::I64));
        sig.params.push(AbiParam::new(types::I64));
        sig.returns.push(AbiParam::new(types::I64));
        let fid = module
            .declare_function("vec-push-cow-grow", Linkage::Import, &sig)
            .map_err(|e| CranelispError::CodegenError {
                message: format!("failed to declare vec-push-cow-grow: {}", e),
                span: (0, 0),
            })?;
        builtin_methods.insert("vec-push-cow-grow".to_string(), fid);
    }
    {
        // vec-len: (i64) -> i64
        let mut sig = module.make_signature();
    ...
    // Declare guarded dec intrinsics: (i64, i64, i64) -> i64
    for name in &["cranelisp_dec_guarded", "cranelisp_dec_mixed_guarded"] {
        let mut sig = module.make_signature();
        sig.params.push(AbiParam::new(types::I64));
        sig.params.push(AbiParam::new(types::I64));
        sig.params.push(AbiParam::new(types::I64));
        ...
        builtin_methods.insert(name.to_string(), fid);
        builtin_method_info.insert(name.to_string(), (name.to_string(), 3));
    }
    // Declare closure guarded dec: (i64, i64) -> i64
    {
        let name = "cranelisp_dec_closure_guarded";
        let mut sig = module.make_signature();
        sig.params.push(AbiParam::new(types::I64));
        sig.params.push(AbiParam::new(types::I64));
        sig.returns.push(AbiParam::new(types::I64));
...
impl Jit {
    ...
    pub fn new() -> Result<Self, CranelispError> {
        ...
        let (alloc_func_id, panic_func_id, free_func_id, par_eval_func_id, ivar_create_func_id, ivar_spark_func_id, ivar_force_func_id, builtin_methods, builtin_method_info) =
    ...
    pub fn register_type_defs(&mut self, tc: &TypeChecker) {
        ...
        for (ctor_name, type_name) in &constructor_to_type_cg {
            self.trait_method_names.insert(ctor_name.clone());
            // Register dotted form (e.g., "Option.None") for qualified constructor refs
            self.trait_method_names
                .insert(format!("{}.{}", type_name, ctor_name));
        }
    ...
    pub fn register_runtime_symbols(&self, linker: &mut crate::linker::Linker) {
        ...
        linker.register_symbol("cranelisp_par_eval", intrinsics::par_eval as *const u8);
        linker.register_symbol(
            "cranelisp_ivar_create",
            intrinsics::ivar_create as *const u8,
        );
        linker.register_symbol(
        ...
        linker.register_symbol("str-eq", primitives::str_eq as *const u8);
        linker.register_symbol("quote-sexp", crate::marshal::quote_sexp as *const u8);
        linker.register_symbol("sconcat", crate::marshal::sconcat as *const u8);
        ...
        linker.register_symbol("vec-set-rc", primitives::vec_set_rc as *const u8);
        linker.register_symbol("vec-push-rc", primitives::vec_push_rc as *const u8);
        linker.register_symbol("vec-push-cow-grow", primitives::vec_push_cow_grow as *const u8);
    ...
    pub fn compile_defn_with_resolutions(
        ...
        let par_eval_fid = self.par_eval_func_id;
        let ivar_create_fid = self.ivar_create_func_id;
        let ivar_spark_fid = self.ivar_spark_func_id;
        let ivar_force_fid = self.ivar_force_func_id;
        let trait_names = self.trait_method_names.clone();
        let param_types = extract_param_types(&_scheme.ty);
        ...
        for cm in modules.values() {
            ...
            }) = cm.get("main")
            {
                if !ptr.is_null() {
                    ...
                    let result = main_fn();
                    // Only trampoline if main returns IO
                    let returns_io = match &scheme.ty {
                        crate::types::Type::Fn(_, ret) => {
                            matches!(ret.as_ref(), crate::types::Type::ADT(name, _) if name == "IO")
                        }
                        _ => false,
                    };
                    if returns_io {
    ...
    pub fn compile_expr(
        ...
        fn_slots: &HashMap<String, FnSlot>,
        modules: &HashMap<crate::module::ModuleFullPath, crate::module::CompiledModule>,
    ) -> Result<extern "C" fn() -> i64, CranelispError> {
        ...
        compile_function_indirect(
            ...
            Some(panic_fid),
            expr_types,
            &[], // compile_expr wrapper has no params
        )?;
        ...
        Ok(unsafe { mem::transmute::<*const u8, extern "C" fn() -> i64>(code) })
    }
    ...
    pub fn eval_expr(
        ...
        let f = self.compile_expr(expr, method_resolutions, expr_types, fn_slots, modules)?;
        Ok(f())
    }
    ...
    pub fn compile_and_run(
        ...
        for d in &result.default_method_defns {
            all_defns.push(d);
        }
        ...
        // Build combined extra_defns: multi_defns + default method defns + mono defns
        ...
        for (defn, &func_id) in all_defns.iter().zip(func_ids.iter()) {
            ...
            // Extract param types from the function's scheme for RC tracking
            let param_types = tc
                .lookup(&defn.name)
                .map(|s| extract_param_types(&s.ty))
                .unwrap_or_default();
            compile_function(
                defn,
                program,
                ...
                self.alloc_func_id,
                self.free_func_id,
                self.par_eval_func_id,
                self.ivar_create_func_id,
                self.ivar_spark_func_id,
                self.ivar_force_func_id,
                method_resolutions,
                fn_specific,
                &self.builtin_methods,
                &tc.modules,
