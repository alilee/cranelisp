...
**Sprint 61 Wave 5**: Audited `user/*.md` against Sprint 61 Slices 0/1/2/3/4 — **no-op disposition**. Slice 1 (bare-primitive name echo at the REPL, e.g. typing `add-i64` alone yields `:(Fn [Int Int] Int) primitives/add-i64 ; primitive - ...`) did not require a doc edit because the guide never demonstrates typing a primitive without calling it — every `add-i64` reference in `getting-started.md` is already a call form like `(add-i64 3 4)`. Slice 0 observability env vars (`CRANELISP_SCHEDULER_TRACE`, `CRANELISP_IO_TRACE`) are developer diagnostics; no troubleshooting section exists in `user/` and the S60 `CRANELISP_CODEGEN_DUMP=*` precedent keeps these developer-only until a dedicated diagnostics section is commissioned. Slices 2 (sudoku correctness), 3 (scheduler race closures), 4 (closure double-free) are internal stability with no user-visible surface. No `user/*.md` edits; no FIXMEs filed.

**Sprint 60 Wave 4**: Audited `user/*.md` (4 files: `getting-started.md`, `caching.md`, `tutorial/curriculum.md`, `plan-docs.md`) against Sprint 60 observable-behavior changes. One targeted edit: updated the `examples/21-hello-io.cl` reference in `getting-started.md` to note that bundled examples now run directly via `cargo run -- --run examples/FOO.cl` without `CRANELISP_LIB=`, thanks to `examples/Cranelisp.toml` + `examples/lib/prelude.cl` (Sprint 60 Wave 2). Showed the concrete invocation. No other user-facing touch needed: (a) `/sig` docstring format fix (Wave 1) — no `/sig` examples exist in `user/`; (b) `CRANELISP_CODEGEN_DUMP=*` — not yet a user-documented feature per brief guidance (no existing diagnostics/CLIF section in Developer Tools); (c) cache build marker, dual-GOT/RC/persistence/race fixes — invisible stability improvements, no doc implication. No defects surfaced; no FIXMEs filed.
