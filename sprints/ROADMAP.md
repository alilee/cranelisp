...
| 51 | Stateless TC | Fully stateless TypeChecker: FQTypeName migration (TypeName → FQTypeName), global registry elimination (TypeDefRegistry/TraitRegistry/ImplRegistry deleted, resolution through module system), all TC state extracted to SharedState/stack-local, CompilerSession.tc deleted, cache fix (11 tests). | `/arch`, `/typecheck`, `/backend`, `/frontend`, `/int`, `/qa` |
| 52 | Failure triage + FIXME clearance | Remaining 21 failures (sketch-port triage, IO/platform, checked_div, ring2, E2E). Remaining FIXMEs. Sprint 23 test triage (3x deferred — user approval required). Post-restructure architecture doc. Prior-ring spec traceability. | all skills |
| 53+ | TBD | Ring 4 gate review, exemplar E2E, performance baseline, Ring 4 acceptance. Planned after Sprint 52. | TBD |
