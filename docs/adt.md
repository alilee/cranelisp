...
A `match` on a concrete ADT type must cover all constructors or include a wildcard/variable catch-all arm. Non-exhaustive matches are a compile-time error. The runtime panic path ("match failed") remains in generated code as a safety net.
