...
pub extern "C" fn add_f64(a: i64, b: i64) -> i64 {
    let result = f64::from_bits(a as u64) + f64::from_bits(b as u64);
    result.to_bits() as i64
}

#[unsafe(export_name = "sub-f64")]
pub extern "C" fn sub_f64(a: i64, b: i64) -> i64 {
    let result = f64::from_bits(a as u64) - f64::from_bits(b as u64);
    result.to_bits() as i64
}

#[unsafe(export_name = "mul-f64")]
pub extern "C" fn mul_f64(a: i64, b: i64) -> i64 {
    let result = f64::from_bits(a as u64) * f64::from_bits(b as u64);
    result.to_bits() as i64
}

#[unsafe(export_name = "div-f64")]
pub extern "C" fn div_f64(a: i64, b: i64) -> i64 {
    let result = f64::from_bits(a as u64) / f64::from_bits(b as u64);
    result.to_bits() as i64
}
...
pub extern "C" fn eq_i64(a: i64, b: i64) -> i64 {
    (a == b) as i64
}

#[unsafe(export_name = "lt-i64")]
pub extern "C" fn lt_i64(a: i64, b: i64) -> i64 {
    (a < b) as i64
}

#[unsafe(export_name = "gt-i64")]
pub extern "C" fn gt_i64(a: i64, b: i64) -> i64 {
    (a > b) as i64
}

#[unsafe(export_name = "le-i64")]
pub extern "C" fn le_i64(a: i64, b: i64) -> i64 {
    (a <= b) as i64
}

#[unsafe(export_name = "ge-i64")]
pub extern "C" fn ge_i64(a: i64, b: i64) -> i64 {
    (a >= b) as i64
}

#[unsafe(export_name = "neq-i64")]
pub extern "C" fn neq_i64(a: i64, b: i64) -> i64 {
    (a != b) as i64
}
...
pub extern "C" fn eq_f64(a: i64, b: i64) -> i64 {
    (f64::from_bits(a as u64) == f64::from_bits(b as u64)) as i64
}

#[unsafe(export_name = "lt-f64")]
pub extern "C" fn lt_f64(a: i64, b: i64) -> i64 {
    (f64::from_bits(a as u64) < f64::from_bits(b as u64)) as i64
}

#[unsafe(export_name = "gt-f64")]
pub extern "C" fn gt_f64(a: i64, b: i64) -> i64 {
    (f64::from_bits(a as u64) > f64::from_bits(b as u64)) as i64
}

#[unsafe(export_name = "le-f64")]
pub extern "C" fn le_f64(a: i64, b: i64) -> i64 {
    (f64::from_bits(a as u64) <= f64::from_bits(b as u64)) as i64
}

#[unsafe(export_name = "ge-f64")]
pub extern "C" fn ge_f64(a: i64, b: i64) -> i64 {
    (f64::from_bits(a as u64) >= f64::from_bits(b as u64)) as i64
}

#[unsafe(export_name = "neq-f64")]
pub extern "C" fn neq_f64(a: i64, b: i64) -> i64 {
    (f64::from_bits(a as u64) != f64::from_bits(b as u64)) as i64
}
...
pub extern "C" fn not(b: i64) -> i64 {
    (b ^ 1) & 1
}

#[unsafe(export_name = "eq-bool")]
pub extern "C" fn eq_bool(a: i64, b: i64) -> i64 {
    (a == b) as i64
}

#[unsafe(export_name = "neq-bool")]
pub extern "C" fn neq_bool(a: i64, b: i64) -> i64 {
    (a != b) as i64
}

// =============================================================================
// JIT-registration table
// =============================================================================

/// The 23 Ring 0 primitive JIT symbols paired with their Rust shim addresses.
///
/// (FIXME 0174 quoted "19" as the canonical count; the actual table includes
/// the negation operators `neq-i64`/`neq-f64`/`neq-bool` and `eq-bool` that
/// `primitives_inline.rs` recognises but `ring0_primitives()` does not yet
/// surface as `PrimitiveDef` rows. Backend's inline matcher must keep pace
/// with this table; conversely `ring0_primitives()` is the typecheck-side
/// table that drives `register_primitives` allocation of `got_slot`s — that
/// table covers 20 names today. The handful of `neq-*` shims live here for
/// `primitive_for_trait_method`'s extended set: when `Eq.!=` is referenced
/// as a value, dispatch lands on `neq-i64` / `neq-f64` / `neq-bool`.)
///
/// Consumed at JIT setup time by `cranelisp-backend::jit::intrinsic_symbols()`
/// (so backend's `JITBuilder::symbol()` resolves `Linkage::Import` calls to
/// these shims for the GOT-indirect path) AND by the integration crate's
/// post-`register_builtins` pass that stores each shim's address into the
/// primitives module's GOT slot at `got_slot` (so call-by-symbol — the
/// mappable path — resolves to the shim too).
///
/// Single source of truth; everything else derives from this list.
pub fn ring0_jit_symbols() -> Vec<(&'static str, *const u8)> {
    vec![
        ("add-i64", add_i64 as *const u8),
        ("sub-i64", sub_i64 as *const u8),
        ("mul-i64", mul_i64 as *const u8),
        ("div-i64", div_i64 as *const u8),
        ("add-f64", add_f64 as *const u8),
        ("sub-f64", sub_f64 as *const u8),
        ("mul-f64", mul_f64 as *const u8),
        ("div-f64", div_f64 as *const u8),
        ("eq-i64", eq_i64 as *const u8),
        ("lt-i64", lt_i64 as *const u8),
        ("gt-i64", gt_i64 as *const u8),
        ("le-i64", le_i64 as *const u8),
        ("ge-i64", ge_i64 as *const u8),
        ("neq-i64", neq_i64 as *const u8),
        ("eq-f64", eq_f64 as *const u8),
        ("lt-f64", lt_f64 as *const u8),
        ("gt-f64", gt_f64 as *const u8),
        ("le-f64", le_f64 as *const u8),
        ("ge-f64", ge_f64 as *const u8),
        ("neq-f64", neq_f64 as *const u8),
        ("not", not as *const u8),
        ("eq-bool", eq_bool as *const u8),
        ("neq-bool", neq_bool as *const u8),
    ]
}
...
mod tests {
    ...
    #[test]
    fn ring0_jit_symbols_count() {
        // 22 = 4 int-arith + 4 float-arith + 6 int-cmp (eq, lt, gt, le, ge, neq)
        //    + 6 float-cmp + not + eq-bool + neq-bool
        let table = ring0_jit_symbols();
        assert_eq!(table.len(), 23);
    }

    #[test]
    fn ring0_jit_symbols_unique_names() {
        let table = ring0_jit_symbols();
        ...
        assert_eq!(
            names.len(),
            original_len,
            "ring0_jit_symbols must have unique names"
        );
    }
}
