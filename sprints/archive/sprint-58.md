...
**Wave 6 /review Importants** (commit `53d8339`; per user direction, carried to Sprint 59 opening rather than addressed in Sprint 58 close):
- **I-1 (cosmetic, /sprint or /qa)**: commit-hash typo `a9e174e` → `a9e194c` was identified at 4 occurrences (sprints/SPRINT.md:725 + tests/plan/ring4.md:1093,1102,1140); `/sprint` corrected all 4 inline before final close. **RESOLVED in close.**
- **I-2 (verification hygiene, /stdlib)**: stdlib audit count drift — `/stdlib`'s commit `98bf4ef` says 32 `(import [prelude []])` files; actual count is 35. Conclusion unchanged (`seq/lazy.cl` was the only at-risk file per /review's independent spot-checks of 5 files). The 3 missed files (likely `derive.cl`, `defs.cl`, `default.cl`) need /stdlib spot-check verification in Sprint 59 opening to lock in the audit.
- **I-3 (structural debt artefact, /int + /sprint)**: dual-path persistence (compile_dep_inline vs handle_* paths) is named in §Findings as the next structural debt but disposition is "tracked for a future sprint" without a concrete artefact (target sprint name or stub design doc). `/review` recommends commissioning a stub design doc at `design/int/dual-path-persistence-collapse.md` so the next stabilisation sprint picks it up cold. Sprint 59 opening should commission this stub OR explicitly assign the work to a named future sprint.
...
- **The race shape (Defect 1) lived at 5 sites, not 1**. `/int`'s diagnosis (`compile_dep_inline` registered with scheduler before publishing sexps) was correct but localised; the actual fix in `a9e194c` had to apply the publish-before-register reordering at 5 sites in `src/session_v4.rs` + `src/worker.rs`. The race window was structural in the persistent-worker model, not incidental — supports the broader Decision-37 cache-hit-integration discipline that `register_module`'s recursion is the single correct shape.
