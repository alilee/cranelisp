...
//! 2. `parse_defmacro(sexp)` — extract name, clauses (params + body per clause)
//! 3. `synthesize_macro_clause_defn(...)` — build a Defn per clause with arg destructuring
//! 4. `compile_macro(tc, jit, ...)` — type-check + compile each clause → extract code_ptrs
...
/// A macro parameter: either a simple name or a bracket destructuring pattern.
#[derive(Clone, Debug, serde::Serialize, serde::Deserialize)]
pub enum MacroParam {
    /// Simple named parameter: binds one Sexp arg.
    Name(String),
    /// Bracket destructuring: `[a b & rest]` — unwraps SexpBracket, binds contents.
    Bracket {
        fixed: Vec<String>,
        rest: Option<String>,
    },
...
impl MacroParam {
    /// Get the display name for this parameter (for introspection).
    pub fn display_name(&self) -> String {
        match self {
            MacroParam::Name(n) => n.clone(),
            MacroParam::Bracket { fixed, rest } => {
                let mut parts: Vec<String> = fixed.clone();
                if let Some(r) = rest {
...
/// A single macro clause: params + body.
#[derive(Clone)]
...
/// A compiled macro clause stored in MacroEnv.
...
/// Macro clause info for module system storage (no func_ptr).
#[derive(Clone, Debug, serde::Serialize, serde::Deserialize)]
...
pub struct MacroEntry {
    ...
    pub clauses: Vec<MacroClauseEntry>,
}

impl MacroEntry {
    /// Check if any clause is zero-arg (for bare-symbol expansion).
    pub fn has_zero_arg_clause(&self) -> bool {
        self.clauses
            .iter()
            .any(|c| c.fixed_params.is_empty() && c.rest_param.is_none())
    }
}

/// Build MacroClauseEntry values from compiled pointers and parsed clauses.
pub fn build_clause_entries(
    info: &DefmacroInfo,
    ptrs: Vec<*const u8>,
) -> Vec<MacroClauseEntry> {
    info.clauses
        .iter()
        .zip(ptrs)
        .map(|(clause, ptr)| MacroClauseEntry {
            func_ptr: ptr,
            fixed_params: clause.fixed_params.clone(),
            rest_param: clause.rest_param.clone(),
            body_sexp: Some(clause.body_sexp.clone()),
        })
        .collect()
}

/// Build MacroClauseInfo values from parsed clauses (for module system).
pub fn build_clause_infos(info: &DefmacroInfo) -> Vec<MacroClauseInfo> {
    info.clauses
        .iter()
        .map(|clause| MacroClauseInfo {
            fixed_params: clause.fixed_params.clone(),
            rest_param: clause.rest_param.clone(),
        })
        .collect()
...
/// Parse macro parameters from bracket items. Supports simple names and bracket destructuring.
///
/// Examples: `[a b]`, `[a & rest]`, `[[x y] body]`, `[[& items] body]`
fn parse_macro_params(
    items: &[Sexp],
) -> Result<(Vec<MacroParam>, Option<String>), CranelispError> {
    ...
    while i < items.len() {
        match &items[i] {
            ...
            Sexp::Symbol(pname, _) => {
                fixed_params.push(MacroParam::Name(pname.clone()));
                i += 1;
            }
            Sexp::Bracket(inner, _) => {
                // Bracket destructuring: [a b & rest]
                let mut bracket_fixed = Vec::new();
                ...
                while j < inner.len() {
                    match &inner[j] {
                        Sexp::Symbol(s, _) if s == "&" => {
                            if j + 1 >= inner.len() {
                                return Err(CranelispError::ParseError {
                                    message: "& must be followed by rest parameter name"
                                        .to_string(),
                                    offset: inner[j].span().0,
                                });
                            }
                            match &inner[j + 1] {
                                Sexp::Symbol(rname, _) => bracket_rest = Some(rname.clone()),
                                _ => {
                                    return Err(CranelispError::ParseError {
                                        message: "rest parameter must be a symbol".to_string(),
                                        offset: inner[j + 1].span().0,
                                    })
                                }
                            }
                            j += 2;
                        ...
                        Sexp::Symbol(pname, _) => {
                            bracket_fixed.push(pname.clone());
                            j += 1;
                        }
                        _ => {
                            return Err(CranelispError::ParseError {
                                message: "bracket destructuring param must be a symbol"
                                    .to_string(),
                                offset: inner[j].span().0,
                            })
                        }
...
fn parse_macro_clause(sexp: &Sexp) -> Result<MacroClause, CranelispError> {
    ...
    let (fixed_params, rest_param) = parse_macro_params(bracket_items)?;
    Ok(MacroClause {
        fixed_params,
        rest_param,
...
/// Parse `(defmacro name [a b & rest] body)` or multi-clause
/// `(defmacro name ([a] body1) ([a b] body2))`.
pub fn parse_defmacro(sexp: &Sexp) -> Result<DefmacroInfo, CranelispError> {
    ...
    let clauses = match &children[next_idx] {
        Sexp::Bracket(items, _) => {
            ...
            let (fixed_params, rest_param) = parse_macro_params(items)?;
            vec![MacroClause {
                fixed_params,
                rest_param,
...
/// Synthesize a `Defn` for a single macro clause.
///
/// Given clause `[a b & rest] body`, generates nested match:
/// ```text
/// (defn __macro_foo__$0__ [:(SList Sexp) __args__]
///   (match __args__
///     [(SCons a __t1__)
///      (match __t1__
///        [(SCons b rest) body])]))
/// ```
///
/// For bracket destructuring `[[x y] body]`, generates additional match levels
/// to unwrap SexpBracket and destructure inner SList.
///
...
pub fn synthesize_macro_clause_defn(
    ...
    let fn_name = format!("__macro_{}__${}__", macro_name, clause_idx);
    let args_name = "__args__";

    // Expand quasiquotes in the body sexp before building AST
    let body_sexp = expand_quasiquotes(&clause.body_sexp)?;
    // Rewrite SYN=(0,0) spans to unique synthetic spans so that
    // resolution maps don't collide when the body has multiple calls.
    let body_sexp = rewrite_spans(body_sexp, span);
    ...
    // Offset spans by clause_idx to avoid cross-clause collisions
    let clause_span = (span.0 + clause_idx * 100, span.1 + clause_idx * 100);

    let has_params = !clause.fixed_params.is_empty() || clause.rest_param.is_some();
    let wrapped_body = if !has_params {
        body_expr
    } else {
    ...
    // Parameter annotation: :(macros/SList macros/Sexp)
    let param_annotation = TypeExpr::Applied(
        "macros/SList".to_string(),
        vec![TypeExpr::Named("macros/Sexp".to_string())],
    );
...
/// Build nested match expressions for macro params that may include bracket destructuring.
fn build_macro_param_chain(
    ...
    match param {
        MacroParam::Name(pname) => {
            // Simple name: peel one SCons, like existing build_match_chain
            ...
            let inner_body = if params.len() == 1 && rest_param.is_some() {
                ...
                build_macro_param_chain(&tail_binding, &params[1..], rest_param, body, span)
            };
            ...
            Expr::Match {
                ...
                arms: vec![MatchArm {
                    pattern: Pattern::Constructor {
                        name: "macros/SCons".to_string(),
                        bindings: vec![pname.clone(), tail_binding],
                        span: pat_span,
                    },
                    body: inner_body,
                    span: pat_span,
        ...
        MacroParam::Bracket { fixed, rest } => {
            // Bracket destructuring: extract positional arg, match against macros/SexpBracket,
            // then destructure inner SList.
            ...
            // Build inner body: first destructure bracket contents, then continue with remaining params
            let continuation = if params.len() == 1 && rest_param.is_some() {
                body
            } else {
                build_macro_param_chain(&tail_binding, &params[1..], rest_param, body, span)
            };

            // Build bracket destructuring: match __bracket_N__ against macros/SexpBracket
            let bracket_inner = build_bracket_destructure(
                &bracket_temp,
                fixed,
                rest,
                continuation,
            ...
            // Outer SCons match to extract the positional arg
            Expr::Match {
                scrutinee: Box::new(Expr::Var {
                    name: scrutinee_name.to_string(),
                    span: scrut_span,
                ...
                arms: vec![MatchArm {
                    pattern: Pattern::Constructor {
                        name: "macros/SCons".to_string(),
                        bindings: vec![bracket_temp, tail_binding],
                        span: pat_span,
                    },
                    body: bracket_inner,
                    span: pat_span,
...
/// Build match expression to destructure a SexpBracket into its inner SList elements.
///
/// Generates: `(match scrutinee [(SexpBracket __inner__) (match __inner__ [SCons chains...])])`
fn build_bracket_destructure(
    scrutinee_name: &str,
    fixed: &[String],
    rest: &Option<String>,
    body: Expr,
    ...
    let inner_name = format!("{}_items__", scrutinee_name);
    let bracket_match_span = (
        span.0 + (depth_offset + 10) * 3 + 1,
        span.1 + (depth_offset + 10) * 3 + 1,
    );
    let bracket_scrut_span = (
        span.0 + (depth_offset + 10) * 3 + 2,
        span.1 + (depth_offset + 10) * 3 + 2,
    );
    let bracket_pat_span = (
        span.0 + (depth_offset + 10) * 3 + 3,
        span.1 + (depth_offset + 10) * 3 + 3,
    );

    // Inner: destructure the SList items using existing chain logic
    let inner_span = (span.0 + (depth_offset + 20) * 3, span.1 + (depth_offset + 20) * 3);
    let bracket_tail_prefix = format!("{}__", inner_name);
    let inner_body = build_match_chain(&inner_name, fixed, rest, body, inner_span, &bracket_tail_prefix);

    Expr::Match {
        scrutinee: Box::new(Expr::Var {
            name: scrutinee_name.to_string(),
            span: bracket_scrut_span,
        }),
        arms: vec![MatchArm {
            pattern: Pattern::Constructor {
                name: "macros/SexpBracket".to_string(),
                bindings: vec![inner_name],
                span: bracket_pat_span,
            },
            body: inner_body,
            span: bracket_pat_span,
        }],
        span: bracket_match_span,
        compiler_generated: true,
    }
}
    ...
    if params.is_empty() {
        ...
        let tail_binding = if params.len() == 1 {
            if let Some(rest_name) = rest_param {
                ...
                format!("{}__t{}__", tail_prefix, 0)
            }
        } else {
            format!("{}__t{}__", tail_prefix, params.len() - 1)
        };
        ...
        let inner_body = if params.len() == 1 && rest_param.is_some() {
            ...
            build_match_chain(&tail_binding, &params[1..], rest_param, body, span, tail_prefix)
        };
...
/// Compile a defmacro form: synthesize + compile each clause independently.
///
/// Returns one function pointer per clause. If `macro_env` is provided, macros
/// in the body sexps (e.g. `(list ...)`) are expanded before the body AST is built.
pub fn compile_macro(
    tc: &mut TypeChecker,
    jit: &mut Jit,
    info: &DefmacroInfo,
    macro_env: Option<&MacroEnv>,
) -> Result<Vec<*const u8>, CranelispError> {
    let mut ptrs = Vec::new();
    for (i, clause) in info.clauses.iter().enumerate() {
        // Expand macros in the body sexp before synthesis
        let expanded_clause = if let Some(env) = macro_env {
            if !env.is_empty() {
                let expanded_body = expand_sexp(clause.body_sexp.clone(), env, 0)?;
                MacroClause {
                    fixed_params: clause.fixed_params.clone(),
                    rest_param: clause.rest_param.clone(),
        ...
        let defn = synthesize_macro_clause_defn(&info.name, i, &expanded_clause, info.span)?;
        ...
        if let Type::Fn(_, ref ret) = scheme.ty {
            ...
            if resolved_ret != Type::ADT("Sexp".to_string(), vec![]) {
                return Err(CranelispError::TypeError {
                    message: format!(
                        "macro '{}' clause {} body has type {}, expected Sexp",
                        info.name, i, resolved_ret
                    ),
                    span: info.span,
                });
            }
...
/// Call a compiled clause function with the given runtime args list.
fn call_clause_fn(func_ptr: *const u8, args: &[Sexp]) -> Sexp {
...
/// Check if a clause matches the given args (arity + structural patterns).
fn clause_matches(clause: &MacroClauseEntry, args: &[Sexp]) -> bool {
    let n = args.len();
    let fixed_count = clause.fixed_params.len();
    ...
    } else if n != fixed_count {
        return false;
    }

    // Structural check: bracket params must receive SexpBracket with compatible element count
    for (i, param) in clause.fixed_params.iter().enumerate() {
            ...
            if i < n {
                match &args[i] {
                    Sexp::Bracket(items, _) => {
                        ...
                        if rest.is_some() {
                            if item_count < fixed.len() {
                                return false;
                            }
                        } else if item_count != fixed.len() {
                            return false;
                        }
                    }
                    _ => return false,
...
/// Dispatch a macro call: try clauses in order, call the first match.
fn dispatch_macro(entry: &MacroEntry, args: &[Sexp]) -> Result<Sexp, CranelispError> {
    for clause in &entry.clauses {
        if clause_matches(clause, args) {
            return Ok(call_clause_fn(clause.func_ptr, args));
        }
    }
    Err(CranelispError::ParseError {
        message: format!(
            "macro '{}' has no clause matching {} argument{}",
            entry.name,
            args.len(),
...
/// Check if children represent `(quote expr)`.
...
fn make_sexp_sym(name: &str) -> Sexp {
    Sexp::List(
        vec![
            Sexp::Symbol("macros/SexpSym".to_string(), SYN),
...
fn make_sexp_int(val: i64) -> Sexp {
    Sexp::List(
        vec![
            Sexp::Symbol("macros/SexpInt".to_string(), SYN),
            Sexp::Int(val, SYN),
        ],
        SYN,
    )
...
fn make_sexp_float(val: f64) -> Sexp {
    Sexp::List(
        vec![
            Sexp::Symbol("macros/SexpFloat".to_string(), SYN),
            Sexp::Float(val, SYN),
        ],
        SYN,
    )
...
fn make_sexp_bool(val: bool) -> Sexp {
    Sexp::List(
        vec![
            Sexp::Symbol("macros/SexpBool".to_string(), SYN),
            Sexp::Bool(val, SYN),
        ],
        SYN,
    )
...
fn make_sexp_str(val: &str) -> Sexp {
    Sexp::List(
        vec![
            Sexp::Symbol("macros/SexpStr".to_string(), SYN),
...
/// Build `(CtorName items_sexp)` where CtorName is macros/SexpList or macros/SexpBracket.
...
fn make_list_call(elements: Vec<Sexp>) -> Sexp {
    let nil = Sexp::Symbol("macros/SNil".to_string(), SYN);
    elements.into_iter().rev().fold(nil, |acc, elem| {
        Sexp::List(
            vec![Sexp::Symbol("macros/SCons".to_string(), SYN), elem, acc],
            SYN,
        )
    })
}

/// Build `(macros/__sconcat__ a b)`.
fn make_concat(a: Sexp, b: Sexp) -> Sexp {
    Sexp::List(
        vec![
            Sexp::Symbol("macros/__sconcat__".to_string(), SYN),
            a,
            b,
        ],
        SYN,
...
/// Every node is converted to its Sexp constructor form.
fn expand_quote_template(template: &Sexp) -> Sexp {
    match template {
        ...
        Sexp::List(children, _) => {
            ...
            make_sexp_container("macros/SexpList", make_list_call(expanded))
        }
        Sexp::Bracket(children, _) => {
            let expanded: Vec<Sexp> = children.iter().map(expand_quote_template).collect();
            make_sexp_container("macros/SexpBracket", make_list_call(expanded))
        }
    }
}
...
fn make_gensym_name(base: &str) -> String {
    ...
    // Strip the trailing '#' to get the base name
    let stem = &base[..base.len() - 1];
    format!("{}__auto_{}", stem, counter)
}
...
///
/// The `gensyms` map tracks auto-gensym symbols (`x#`) within this quasiquote.
...
fn expand_qq_template(
    ...
    match template {
        ...
        Sexp::Symbol(s, _) => {
            // Auto-gensym: at level 0, symbols ending in '#' get unique names
            if level == 0 && s.ends_with('#') && s.len() > 1 {
        ...
        Sexp::List(children, span) => {
            if children.is_empty() {
                return Ok(make_sexp_container("macros/SexpList", make_list_call(vec![])));
            }
            ...
            if is_unquote(children) {
                if level == 0 {
                    ...
                    let inner = expand_qq_template(&children[1], level - 1, gensyms)?;
            ...
            expand_qq_list_children(children, level, "macros/SexpList", gensyms)
        }

        // Brackets — same as lists but with macros/SexpBracket constructor
        Sexp::Bracket(children, _) => {
            if children.is_empty() {
                return Ok(make_sexp_container("macros/SexpBracket", make_list_call(vec![])));
            }
            expand_qq_list_children(children, level, "macros/SexpBracket", gensyms)
        }
    }
}
...
fn expand_qq_list_children(
    ...
    if !has_splice {
        ...
        let expanded: Vec<Sexp> = children
            .iter()
            .map(|c| expand_qq_template(c, level, gensyms))
            .collect::<Result<Vec<_>, _>>()?;
        ...
        for child in children {
            ...
            current_group.push(expand_qq_template(child, level, gensyms)?);
        }
...
///
/// This is used both in macro bodies (via `synthesize_macro_clause_defn`) and in
...
pub fn expand_sexp(sexp: Sexp, env: &MacroEnv, depth: usize) -> Result<Sexp, CranelispError> {
    ...
    match sexp {
        Sexp::List(children, span) if !children.is_empty() => {
            // Expand quote and quasiquote forms before checking macros
            ...
            if let Sexp::Symbol(ref head, _) = children[0] {
                if let Some(entry) = env.get(head) {
                    let args = &children[1..];
                    let expanded = dispatch_macro(entry, args)?;
                    let expanded = rewrite_spans(expanded, span);
                    // If macro produced a begin form, expand each child individually
                    // (expand_sexp skips begin forms to support defmacro-in-results,
                    // but children from macro results may contain macro calls)
                    if is_begin(&expanded) {
                        if let Sexp::List(begin_children, begin_span) = expanded {
                            let expanded_children: Vec<Sexp> = begin_children
                                .into_iter()
                                .map(|c| {
                                    if is_defmacro(&c) || is_begin(&c) {
                                        Ok(c)
                                    } else {
                                        expand_sexp(c, env, depth + 1)
                                    }
                                })
                                .collect::<Result<Vec<_>, _>>()?;
                            return Ok(Sexp::List(expanded_children, begin_span));
                        }
                    }
        ...
        Sexp::Symbol(ref name, _) => {
            if let Some(entry) = env.get(name) {
                if entry.has_zero_arg_clause() {
                    let expanded = dispatch_macro(entry, &[])?;
...
mod tests {
    ...
    fn test_parse_defmacro_no_params() {
        ...
        assert!(info.clauses[0].fixed_params.is_empty());
    ...
    fn test_parse_defmacro_fixed_params() {
        ...
        assert!(matches!(&info.clauses[0].fixed_params[0], MacroParam::Name(n) if n == "a"));
        assert!(matches!(&info.clauses[0].fixed_params[1], MacroParam::Name(n) if n == "b"));
        assert!(info.clauses[0].rest_param.is_none());
    }
    ...
    fn test_parse_defmacro_with_rest() {
        ...
        assert!(matches!(&info.clauses[0].fixed_params[0], MacroParam::Name(n) if n == "a"));
        assert_eq!(info.clauses[0].rest_param, Some("rest".to_string()));
    }
    ...
    fn test_parse_defmacro_rest_only() {
        ...
        assert_eq!(info.clauses.len(), 1);
        assert!(info.clauses[0].fixed_params.is_empty());
        assert_eq!(info.clauses[0].rest_param, Some("args".to_string()));
    }

    #[test]
    fn test_parse_defmacro_multi_clause() {
        let sexp = parse_sexp("(defmacro foo ([x] x) ([x y] y))").unwrap();
        let info = parse_defmacro(&sexp).unwrap();
        assert_eq!(info.name, "foo");
        ...
        assert!(info.clauses[1].rest_param.is_none());
    }

    #[test]
    fn test_parse_defmacro_multi_clause_with_rest() {
        let sexp = parse_sexp("(defmacro foo ([x] x) ([x & rest] rest))").unwrap();
        let info = parse_defmacro(&sexp).unwrap();
        assert_eq!(info.clauses.len(), 2);
        assert!(info.clauses[0].rest_param.is_none());
        assert_eq!(info.clauses[1].rest_param, Some("rest".to_string()));
    }

    #[test]
    fn test_parse_defmacro_bracket_destructure() {
        let sexp = parse_sexp("(defmacro foo [[a b] body] body)").unwrap();
        let info = parse_defmacro(&sexp).unwrap();
        assert_eq!(info.clauses.len(), 1);
        assert_eq!(info.clauses[0].fixed_params.len(), 2);
        match &info.clauses[0].fixed_params[0] {
            MacroParam::Bracket { fixed, rest } => {
                assert_eq!(fixed, &vec!["a", "b"]);
                assert!(rest.is_none());
            }
            _ => panic!("expected Bracket param"),
        }
        assert!(matches!(&info.clauses[0].fixed_params[1], MacroParam::Name(n) if n == "body"));
    }

    #[test]
    fn test_parse_defmacro_multi_clause_with_docstring() {
        let sexp =
            parse_sexp("(defmacro foo \"A macro\" ([x] x) ([x y] y))").unwrap();
        let info = parse_defmacro(&sexp).unwrap();
        assert_eq!(info.docstring, Some("A macro".to_string()));
        assert_eq!(info.clauses.len(), 2);
    }
    ...
    fn test_synthesize_no_params() {
        ...
        let info = parse_defmacro(&sexp).unwrap();
        let defn =
            synthesize_macro_clause_defn(&info.name, 0, &info.clauses[0], info.span).unwrap();
        assert_eq!(defn.name, "__macro_always-42__$0__");
        assert_eq!(defn.params, vec!["__args__"]);
        // Body should be the raw expression (no match wrapper for zero params)
    ...
    fn test_synthesize_fixed_params() {
        ...
        let defn =
            synthesize_macro_clause_defn(&info.name, 0, &info.clauses[0], info.span).unwrap();
        assert_eq!(defn.name, "__macro_foo__$0__");
        // Should have nested Match for destructuring
        match &defn.body {
            Expr::Match { arms, .. } => {
                assert_eq!(arms.len(), 1);
                match &arms[0].pattern {
                    Pattern::Constructor { name, bindings, .. } => {
                        assert_eq!(name, "macros/SCons");
                        assert_eq!(bindings[0], "a");
                    }
                    _ => panic!("expected macros/SCons pattern"),
                }
                // Inner match for b
                match &arms[0].body {
                    Expr::Match {
                        ...
                        _ => panic!("expected inner macros/SCons pattern"),
                    },
    ...
    fn test_synthesize_with_rest() {
        ...
        let defn =
            synthesize_macro_clause_defn(&info.name, 0, &info.clauses[0], info.span).unwrap();
        // match __args__ [(SCons a rest) body]
        match &defn.body {
            Expr::Match { arms, .. } => {
                assert_eq!(arms.len(), 1);
                match &arms[0].pattern {
    ...
    #[test]
    fn test_synthesize_bracket_destructure() {
        let sexp = parse_sexp("(defmacro foo [[a b] body] body)").unwrap();
        let info = parse_defmacro(&sexp).unwrap();
        let defn =
            synthesize_macro_clause_defn(&info.name, 0, &info.clauses[0], info.span).unwrap();
        // Outer: match __args__ [(SCons __bracket_2__ __t1__) ...]
        match &defn.body {
            Expr::Match { arms, .. } => {
                assert_eq!(arms.len(), 1);
                match &arms[0].pattern {
                    Pattern::Constructor { name, bindings, .. } => {
                        assert_eq!(name, "macros/SCons");
                        assert!(bindings[0].starts_with("__bracket_"));
                    }
                    _ => panic!("expected outer macros/SCons pattern"),
                }
                // Inner: match __bracket_N__ [(macros/SexpBracket __inner__) ...]
                match &arms[0].body {
                    Expr::Match { arms: inner, .. } => {
                        match &inner[0].pattern {
                            Pattern::Constructor { name, .. } => {
                                assert_eq!(name, "macros/SexpBracket");
                            }
                            _ => panic!("expected macros/SexpBracket pattern"),
                        }
                    }
                    _ => panic!("expected inner Match for bracket destructure"),
                }
            }
            _ => panic!("expected outer Match"),
        }
    }
    ...
    fn test_qq_symbol() {
        ...
        let result = expand_qq_template(&template, 0, &mut HashMap::new()).unwrap();
        // Should produce (macros/SexpSym "foo")
        let s = format!("{}", result);
        assert_eq!(s, r#"(macros/SexpSym "foo")"#);
    }
    ...
    fn test_qq_float() {
        ...
        let result = expand_qq_template(&template, 0, &mut HashMap::new()).unwrap();
        let s = format!("{}", result);
        assert_eq!(s, "(macros/SexpFloat 3.14)");
    }
    ...
    fn test_qq_bool() {
        ...
        let result = expand_qq_template(&template, 0, &mut HashMap::new()).unwrap();
        let s = format!("{}", result);
        assert_eq!(s, "(macros/SexpBool true)");
    }
    ...
    fn test_qq_str() {
        ...
        let result = expand_qq_template(&template, 0, &mut HashMap::new()).unwrap();
        let s = format!("{}", result);
        assert_eq!(s, r#"(macros/SexpStr "hello")"#);
    }
    ...
    fn test_qq_list_no_unquotes() {
        ...
        let result = expand_qq_template(&template, 0, &mut HashMap::new()).unwrap();
        let s = format!("{}", result);
        assert!(s.contains("macros/SexpList"));
        assert!(s.contains(r#"(macros/SexpSym "if")"#));
        assert!(s.contains("(macros/SexpBool true)"));
        assert!(s.contains("(macros/SexpInt 42)"));
    }
    ...
    fn test_qq_list_with_unquote() {
        ...
        let result = expand_qq_template(&template, 0, &mut HashMap::new()).unwrap();
        let s = format!("{}", result);
        // Should have (macros/SexpList (macros/SCons (macros/SexpSym "if") (macros/SCons c ...)))
        assert!(s.contains("macros/SexpList"));
        assert!(s.contains(r#"(macros/SexpSym "if")"#));
        assert!(s.contains("macros/SCons c"));
        assert!(s.contains("macros/SCons t"));
    }
    ...
    fn test_qq_bracket_with_unquote() {
        ...
        let result = expand_qq_template(&template, 0, &mut HashMap::new()).unwrap();
        let s = format!("{}", result);
        assert!(s.contains("macros/SexpBracket"));
        assert!(s.contains(" v "));
    ...
    fn test_qq_nested_quasiquote() {
        ...
        assert!(s.contains(r#"(macros/SexpSym "quasiquote")"#));
        // The inner unquote should also be preserved as a literal (not evaluated)
        assert!(s.contains(r#"(macros/SexpSym "unquote")"#));
    }
    ...
    fn test_qq_splicing_at_top_level_errors() {
        ...
        let result = expand_qq_template(&template, 0, &mut HashMap::new());
    ...
    fn test_expand_quasiquotes_finds_nested_qq() {
        ...
        let s = format!("{}", result);
        assert!(s.contains("macros/SexpSym"));
