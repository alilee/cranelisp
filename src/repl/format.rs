...
pub fn format_symbol_info(name: &str, info: &SymbolInfo) -> String {
    match info {
        ...
        SymbolInfo::Macro {
            ...
            if clauses.len() == 1 {
                // Single clause: backward-compatible display
                ...
                // Multi-clause: show each clause signature
                let clause_strs: Vec<String> = clauses
                    ...
                    .collect();
                format!("{} :: macro: {}", name, clause_strs.join(" | "))
            }
        }
...
/// Sig display for macros.
/// Single clause: `(defmacro my-when "doc" [:Sexp cond :Sexp body] Sexp)`
/// Multi clause: `(defmacro cond "doc" ([x] Sexp) ([x body & rest] Sexp))`
pub fn format_macro_defn_sig(entry: &MacroEntry) -> String {
    let doc_part = entry
        ...
        .map(|d| format!(" \"{}\"", d))
        .unwrap_or_default();
    if entry.clauses.len() == 1 {
        let clause = &entry.clauses[0];
        let mut parts = Vec::new();
        for p in &clause.fixed_params {
            parts.push(format!(":Sexp {}", p.display_name()));
        }
        ...
        let clause_strs: Vec<String> = entry
            .clauses
            .iter()
            .map(|clause| {
                let mut parts = Vec::new();
