...
> **S115 Phase-2 re-audit (2026-07-20, `/arch`).** Rows R1, R7, R8, R13, R14 updated in
> place against source at HEAD; R4/R6 marked SCHEDULED S115. This re-audited table is the
> baseline the S115 `/qa` instrumentation-completeness matrix consumes.
...
| R13 | **Fork-join error-slot ferry** — worker panic reaches the join; no silent swallow (spec §12.4.3) | **`unasserted`** known pre-existing (test-discovery.md owed item) — **deferral USER-SANCTIONED at S115 Phase 1**: stays parked, gated on the test-discovery implementation wave (not a stabilisation lever) | tier-3 ferry obligation at both fork-join boundaries when actioned | `/design`(intrinsics/backend) — parked, named |
