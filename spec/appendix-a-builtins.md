...
- **`shr` right-shift semantics.** A single `shr` is provided; its behaviour is **defined by the operand type**, not baked into the operator name. On the current signed `Int`, `shr` is **arithmetic** (sign-extending; lowers to `sshr`). Per-type right-shift semantics are defined when other integer types are introduced. `shl` left-shift fills zeros regardless of sign.
  - **No logical (zero-filling) right shift is provided as a primitive.** Because `Int` is the only integer type today and it is signed, its right shift is arithmetic (matching Clojure's `bit-shift-right`). A bit-pattern / logical right shift (CLIF `ushr`, matching Clojure's `unsigned-bit-shift-right`) is **deliberately not** part of the Ring-0 primitive surface. It is a future extension that arrives either with an unsigned integer type (whose type-directed `shr` would be logical) or as an explicit dedicated primitive when a bit-pattern domain demands it — not before there is a use requiring it (optional-minimal-surface principle). This single-`shr`-is-arithmetic choice is the recorded normative decision (scribed + shipped S91, reaffirmed S106 `/spec` review).
