...
- **Failing-not-ignored defect repros (S100 close)**: the canonical `cargo nextest run` carries **4 intentional failing tests** — the `tests/vec_query_value_use.rs` narrow repros for one defect: `vec-get`/`vec-set`/`vec-push` used as *values* (passed to a HOF) SIGSEGV through NULL GOT slots (owner `/backend`; fix sequenced before the ownership-design increment-I sibling work — see `design/backend/ownership-codegen.md` §12.7). The green control `vec_len_as_value_through_hof_returns_length_control` in the same file pins the working path. These are **known-defect guards, not regressions** — un-ignored per `memory/feedback_failing_not_ignored.md` so they flip green when the owning skill fixes the defect. Durable record: `tests/plan/ledger.md`. (The S81-era guards — 14 e2e + 2 unit-tier for `0337`–`0344` — all flipped green when S82 fixed those defects.) A genuine regression is any RED **beyond** these named guards. Verified at S100 close: 1812 passed / 4 failed / 1 skipped.
