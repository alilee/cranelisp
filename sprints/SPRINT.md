...
| P5 | /arch | vec-assoc ownership + 0611 ratify (design-only) | (shim §II.3) | (shim) | — `5d7970fe`: vec-assoc = 3-layer ownership root (MayAliasOf + dead-in-prod declared facts) **→ CARRY S111** (§3.7, schema 19→20); 0611 carrier (A) RATIFIED (`CheckResult.unresolved_dispatch`, no types edit); FIXMEs 0623/0624 |
| W-TC | /dev | 0590 four-resolver convergence (typecheck) | (shim §II.3) | (shim) | — `5ed07d60`: 4 mirrors → one `TypeExprCtx`; TX-5 GREEN, FV-13/14 fence holds; TX-1 test was ill-formed (capital `Self`) → FIXME 0625. Baseline 11→10. Zero pub-API |
| P5 | /testing | vec-assoc sibling repros + 0625 | (shim §II.3) | (shim) | — `2c038960`: 4 sibling UAF guards (let/match × REPL/`--link`) RED-for-right-reason (carry trigger, → §3.7); TX-1 `Self`→`self` GREEN, 0625 deleted. Baseline 10→13 |
| W-RD | /dev | R16/R17 + 0585 die-leg (typecheck+int) | (shim §II.3) | (shim) | — `91b8b12f`: one §3.11 finalize gate, 2 consumers (tc body + int entry/eval) + top-level-poly die-leg; **5 flips (R16/R17 + VP-3/4/5), baseline 13→8**; RD-3 fence + rows 13–15 hold; golden 5/5; no schema bump. 0624→/design |
| 0604 | /dev (int) | index-feed write-race | (shim §II.3) | (shim) | — `c39e3db0`: **RE-SCOPED** — index feed PROVEN inert under recipe (0× --run vs 39× REPL); writer is FOREGROUND compile path, quiet in this env → carry S111 (/qa re-attrib). In-mem isolation hardening landed; §25.5 severance → FIXME 0626. NOT closed |
| C-4 | /dev (int) | multi-arity-main investigation | (shim §II.3) | (shim) | — `94038b09`: **RE-ATTRIBUTED** — int seam correct; root is typecheck finalize pass-ordering (main spuriously Polymorphic). No int change |
| C-4 fix | /dev (typecheck) | pass-ordering fix | (shim §II.3) | (shim) | — `303df28a`: scoped `regeneralize_only_polymorphic` after overload drain (collapses spurious-poly main, no mono-root demotion); mode-uniform exit 7. Baseline 8→7 |
| tc-rev | /review | typecheck chain (0590+W-RD+C-4) | (shim §II.3) | (shim) | — `939bda93`: **2 Blockers** — finalize.rs gate/drain ordering: B1 wrong-reject (overload-in-let) + B2 wrong-accept (spec-MUST bypass); duty-split fix direction; I2 = 0349 3rd instance→/design |
| P5 | /testing | B1/B2 repros + C-4 retag | (shim §II.3) | (shim) | — `4840d8c7`: 2 Blocker guards RED-for-right-reason (both mode-divergent); C-4 retagged wrong-reject. Baseline 7→9 |
